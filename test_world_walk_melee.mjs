...
now+=400;a=c.bridge.beginWalkMelee({angle:0});now+=a.contactWindow[1]+251;assert.equal(c.bridge.resolveWalkMelee({seq:a.seq,contact:hit,contactAge:a.contactWindow[1]}).reason,'expired-or-locked');
// Slow delivery is bounded independently from the actual sampled strike time.
for(const contactAge of [NaN,-1,341,590]){now+=700;a=c.bridge.beginWalkMelee({angle:0});now+=500;assert.equal(c.bridge.resolveWalkMelee({seq:a.seq,contact:hit,contactAge}).reason,'contact-time');assert.equal(c.bridge.resolveWalkMelee({seq:a.seq,contact:hit,contactAge:200}).reason,'sequence','rejected contact cannot replay');}
now+=700;a=c.bridge.beginWalkMelee({angle:0});now+=500;assert.equal(c.bridge.resolveWalkMelee({seq:a.seq,contact:hit}).reason,'contact-time','late pose without sampled time cannot claim grace');
now+=700;a=c.bridge.beginWalkMelee({angle:0});now+=200;assert.equal(c.bridge.resolveWalkMelee({seq:a.seq,contact:hit,contactAge:250}).reason,'contact-time','future time is invalid even inside attack window');
now+=700;a=c.bridge.beginWalkMelee({angle:0});now+=a.contactWindow[1]+250;assert(c.bridge.resolveWalkMelee({seq:a.seq,contact:hit,contactAge:a.contactWindow[1]}).confirmed,'last valid delivery boundary');assert.equal(c.bridge.resolveWalkMelee({seq:a.seq,contact:hit,contactAge:200}).reason,'sequence');
