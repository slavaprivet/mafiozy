...
const source=fs.readFileSync('world.html','utf8').replace(/\r\n/g,'\n'),realPick=sourceFunction(source,'pickNpcWaypoint');
const baselinePick=historicalEagerWander19(realPick.replace(/\r\n/g,'\n')),candidatePick=realPick;
...
const report={scenario:'Current eager native DFS versus lazy native DFS with parent-direction continuation; identical visited/parent/minDepth8/net6/maxDepth18/520-node/shuffled-direction constraints. 288 physically valid origins spread across all source map; 60simsec at20Hz and10Hz; unchanged actual route queue/4ms/collision/arrival/speed.',reverseOrder,results,limits:f.limits+' CPU candidate only. Movement uses actual source arrival radius and full execution footprint/sweep; no LIVE or FPS claim.'};
fs.writeFileSync('outputs/npc_wander_lazy19'+(process.argv.includes('--20hz')?'_20hz':'_10hz')+(reverseOrder?'_reverse':'')+'.json',JSON.stringify(report,null,2));
fs.writeFileSync('outputs/pickNpcWaypoint_lazy_staged19.js',candidatePick+'\n');
