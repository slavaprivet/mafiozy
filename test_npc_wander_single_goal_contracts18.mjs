...
const candidate=applied?realPick:singleGoalWanderCandidate18(realPick);
...
for(let pass=0;pass<2;pass++)for(let i=0;i<256;i++){
 ...
 for(let frame=0;frame<80&&!ready;frame++){f.frame();ready=f.b.pickNpcWaypoint(n);}assert(ready);assert(n._route.length>=8);
