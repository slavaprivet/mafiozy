...
const leftA=_businessRaidRecoveryFlow(cacheBi,left),leftB=_businessRaidRecoveryFlow(cacheBi,left),rightA=_businessRaidRecoveryFlow(cacheBi,right),leftC=_businessRaidRecoveryFlow(cacheBi,left),rightB=_businessRaidRecoveryFlow(cacheBi,right);
...
const beerKey=leftA.key,beerBlocked=leftA.blocked.reduce((sum,value)=>sum+value,0);operation='print_shop';const changed=_businessRaidRecoveryFlow(cacheBi,left),operationSwap={{builds:cacheBi._businessRaidRecoveryFlowBuilds,coverBuilds:cacheBi._businessRaidCoverBuilds,u8Allocs,i16Allocs,keyChanged:changed.key!==beerKey,blockedChanged:changed.blocked.reduce((sum,value)=>sum+value,0)!==beerBlocked}};
const otherBi={{r:2,c:2,H:12,W:16}};_businessRaidRecoveryFlow(otherBi,left);const separate={{builds:otherBi._businessRaidRecoveryFlowBuilds,coverBuilds:otherBi._businessRaidCoverBuilds,u8Allocs,i16Allocs}};
...
console.log(JSON.stringify({{boundary,operationSwap,separate,results,earlyRecoveries,maxStep,maxMs}}));
"""
