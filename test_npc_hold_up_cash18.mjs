...
assert.match(hold,/else if\(!cashAvailable\)ref\._npcCashOfferUntil=0/,'an empty or pending victim clears the cash bundle');
...
assert.match(server,/VALUES\(\?,\?,\?,\?,\?,\?,'unreported'/);
...
let monoNow=1000,wallNow=1_800_000_000_000,cash=40;
const cooldowns=new Map(),actions=[];
...
const sandbox={
  ...
  _districtRepAdd:()=>{},_npcActionFloat:()=>{},triggerWitnessChain:()=>{},
  _npcFleeIntimidatedWitness:()=>true,_npcActionSay:()=>{},showToast:()=>{},
  _npcQueueWitnessCall:()=>true,
};
vm.createContext(sandbox);
for(const source of [hold,take,complete])vm.runInContext(source,sandbox);
