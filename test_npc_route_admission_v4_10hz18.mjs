...
const f=await createCivilianNativeFixture(),b=f.box,allOrigins=[];const hz=10,dt=1/hz,frames=60*hz;
...
for(let r=3;r<b.MAP_ROWS-4;r+=2)for(let c=3;c<b.MAP_COLS-4;c+=2){const n={r:r+.5,c:c+.5};if(allOrigins.length%10===0)n.c+=.11;if(b.npcWaypointOk(n,n.r,n.c)&&b._npcBodyPassable(n.r,n.c,b.npcPassable))allOrigins.push(n);}
const origins=Array.from({length:288},(_,i)=>allOrigins[Math.floor(i*allOrigins.length/288)]);assert(allOrigins.length>=288);
...
function actual(cap){
 ...
 for(let frame=0;frame<frames;frame++){
  f.nextFrame(dt);let cost=0,anyRequest=false;
  for(const n of b.NPCS){
   ...
   const t=performance.now(),to=n._route[n._routeIndex||0],dr=to.r-n.r,dc=to.c-n.c,d=Math.hypot(dr,dc),step=Math.min(d,1.8/4.1*dt),r=n.r+dr/Math.max(d,.0000001)*step,c=n.c+dc/Math.max(d,.0000001)*step;
...
const report={appliedCap:8,synthetic,actual:{baseline,candidate},reverseOrder:{candidate:reverseCandidate,baseline:reverseBaseline},limits:f.limits+' Applied max8 compared with reconstructed historical max2 in VM; unchanged actual 4ms CPU budget and fixed NPC iteration. 60 simulated seconds at10Hz across entire map/1.8m/s per run with physical waypoint following. Reverse-order repeat guards warm-up ordering. Not FPS or whole update measurement.'};
fs.writeFileSync('outputs/npc_route_admission_v4_10hz18.json',JSON.stringify(report,null,2));console.log(JSON.stringify({actual:report.actual,reverseOrder:report.reverseOrder,limits:report.limits},null,2));
