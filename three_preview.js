// 3D NPC animation v332: idle NPCs plant both feet and gait follows measured displacement only.
...
if ((rendererParams.get('force3d') === '1' || rendererParams.get('render') !== 'canvas') && rendererConfig.threeEnabled !== false) {
  (async () => {
    ...
    try {
      ...
      if(rendererParams.has('previewcoast'))bridge?.previewApproachCoast?.();
...
#include <colorspace_fragment>
        ...
        const coastStaticBuckets=new Map(),coastStaticMatrix=new THREE.Matrix4(),coastStaticPosition=new THREE.Vector3(),coastStaticQuaternion=new THREE.Quaternion(),coastStaticScale=new THREE.Vector3(1,1,1),coastStaticColor=new THREE.Color();let coastStaticSources=0,coastStaticBatches=0;
        const stageCoastStatic=(chunk,key,geometry,material,pose,color=null,castShadow=false,receiveShadow=false)=>{const bucketKey=`${chunk}:${key}`;let bucket=coastStaticBuckets.get(bucketKey);if(!bucket){bucket={geometry,material,items:[],castShadow,receiveShadow};coastStaticBuckets.set(bucketKey,bucket);}bucket.items.push({pose,color});coastStaticSources++;};
        const flushCoastStatic=()=>{for(const bucket of coastStaticBuckets.values()){const mesh=new THREE.InstancedMesh(bucket.geometry,bucket.material,bucket.items.length);bucket.items.forEach(({pose,color},i)=>{const [x,y,z]=pose;coastStaticMatrix.compose(coastStaticPosition.set(x,y,z),coastStaticQuaternion.identity(),coastStaticScale);mesh.setMatrixAt(i,coastStaticMatrix);if(color!==null)mesh.setColorAt(i,coastStaticColor.setHex(color));});mesh.instanceMatrix.needsUpdate=true;if(mesh.instanceColor)mesh.instanceColor.needsUpdate=true;mesh.castShadow=bucket.castShadow;mesh.receiveShadow=bucket.receiveShadow;mesh.computeBoundingSphere?.();scene.add(mesh);coastStaticBatches++;}renderer.domElement.dataset.coastStaticBatch=`${coastStaticSources}:${coastStaticBatches}:spatial-20-tile-v333`;};
        const benchSeatGeo=new THREE.BoxGeometry(4,.3,.72),benchLegGeo=new THREE.BoxGeometry(.22,1,.5),promenadeLampPostGeo=new THREE.CylinderGeometry(.12,.18,5.2,8),promenadeLampBulbGeo=new THREE.SphereGeometry(.34,10,7),promenadeLampBulbMat=new THREE.MeshBasicMaterial({color:0xffce75});
        for(let c=8;c<74;c+=11){const chunk=Math.floor(c/20),x=toX(c),z=toZ(153);stageCoastStatic(chunk,'bench-seat',benchSeatGeo,benchMat,[x,1.05,z],null,true,true);for(const dx of [-1.45,1.45])stageCoastStatic(chunk,'bench-leg',benchLegGeo,metalMat,[x+dx,.5,z],null,true,true);stageCoastStatic(chunk,'lamp-post',promenadeLampPostGeo,metalMat,[x+3.4,2.6,z]);stageCoastStatic(chunk,'lamp-bulb',promenadeLampBulbGeo,promenadeLampBulbMat,[x+3.4,5.3,z]);}
        const umbrellaColors=[0xff5d62,0x36b8d4,0xffc74e,0x64c887],umbrellaPoleGeo=new THREE.CylinderGeometry(.07,.09,2.5,7),umbrellaShadeGeo=new THREE.ConeGeometry(2.2,.9,14),umbrellaShadeMat=new THREE.MeshStandardMaterial({color:0xffffff,roughness:.7});for(let i=0;i<12;i++){const r=158+(i%3)*2.1,c=8+Math.floor(i/3)*19+(i%2)*3,chunk=Math.floor(c/20),x=toX(c),z=toZ(r);stageCoastStatic(chunk,'umbrella-pole',umbrellaPoleGeo,metalMat,[x,1.25,z]);stageCoastStatic(chunk,'umbrella-shade',umbrellaShadeGeo,umbrellaShadeMat,[x,2.65,z],umbrellaColors[i%4],true,false);}flushCoastStatic();
      ...
      const npcPartEntries=Object.entries(npcParts),npcPartMeshes=npcPartEntries.map(([,mesh])=>mesh),remotePartMeshes=Object.values(remoteParts);
      ...
      const instanceSlotVisibility=new WeakMap(),dirtyActorMatrixMeshes=new Set();
      const actorSlotVisibility=mesh=>{let slots=instanceSlotVisibility.get(mesh);if(!slots){slots=[];instanceSlotVisibility.set(mesh,slots);}return slots;};
      const setPart=(mesh,index,root,px,py,pz,rx=0,scale=unitScale,ry=0,rz=0)=>{instanceQuat.setFromEuler(instanceEuler.set(rx,ry,rz));localMatrix.compose(instancePosition.set(px,py,pz),instanceQuat,scale);instanceMatrix.multiplyMatrices(root,localMatrix);mesh.setMatrixAt(index,instanceMatrix);actorSlotVisibility(mesh)[index]=true;dirtyActorMatrixMeshes.add(mesh);};
      ...
      const npcLifeAnimationPose=(src,i,t)=>{
        ...
        const role=String(src.role||'').toLowerCase(),visualRole=String(src.visualRole||'').toLowerCase(),policeActor=visualRole==='police'||!!src.police||role.includes('police')||role.includes('cop'),guardActor=visualRole==='guard'||role.includes('guard'),gangActor=visualRole==='gang'||!!src.gang||role.includes('gang')||role.includes('boss')||role.includes('district_')||role.includes('occupier'),combatActor=policeActor||guardActor||gangActor;
        ...
        const neutralIdle=!dead&&!walking&&!crawling&&!limping&&!phoneCalling&&!cowering&&!surrendering&&!helping&&!panicking&&!talking&&!alerted&&!ambientKind&&!firing,idleStance=policeActor?'police':guardActor?'guard':gangActor?'gang':'civilian';
        // A deterministic, brief look-around window keeps a crowd alive without
        // making every head oscillate continuously. Weight transfer is confined
        // to the upper body, so stationary feet remain fully planted.
        const idleLookClock=(t*.001+ambientSeed*.37)%17,idleLookActive=neutralIdle&&idleLookClock>11.8&&idleLookClock<14.4,idleLookPulse=idleLookActive?Math.sin((idleLookClock-11.8)/2.6*Math.PI):0,idleLookSide=(ambientSeed&1)?1:-1,idleHeadTurn=idleLookPulse*idleLookSide*(policeActor?.25:guardActor?.21:gangActor?.18:.14),idleWeightShift=neutralIdle?Math.sin(t*.00058+ambientSeed*.31)*(policeActor?.016:guardActor?.02:gangActor?.032:.025):0,idleBodyX=idleWeightShift,idleHeadX=idleWeightShift*1.18;
        ...
        const strideScale=walking?(.9+pace*.1)*(panicking?1.14:1):1,swing=walking?step*(crawling?.28:limping?.42:.72)*gait*strideScale:0;
        ...
        const torsoTwist=walking&&!crawling?-step*(.022+pace*.012)*gait:talking?Math.sin(t*.004+i)*.075:neutralIdle?idleWeightShift*.22:0;
        ...
        if(neutralIdle&&policeActor){armY=2.12;armZ=.08;leftArmPitch=-.2+idle*.014;rightArmPitch=-.28-idle*.012;leftArmRoll=-.07;rightArmRoll=.08;leftArmX=-.73;rightArmX=.73;}
        else if(neutralIdle&&guardActor){armY=2.14;armZ=.12;leftArmPitch=-.3+idle*.016;rightArmPitch=-.4-idle*.014;leftArmRoll=-.1;rightArmRoll=.1;leftArmX=-.74;rightArmX=.72;}
        else if(neutralIdle&&gangActor){armY=2.03;armZ=.1;leftArmPitch=-.09+idle*.025;rightArmPitch=-.35-idle*.018;leftArmRoll=-.05;rightArmRoll=.11;leftArmX=-.8;rightArmX=.72;}
        else if(neutralIdle){leftArmPitch=idle*.025;rightArmPitch=-idle*.02;leftArmRoll=-.018;rightArmRoll=.018;}
      ...
      const hidePart=(mesh,index)=>{const slots=actorSlotVisibility(mesh);if(slots[index]===false)return;instanceMatrix.compose(instancePosition.set(0,-1000,0),instanceQuat.identity(),hiddenScale);mesh.setMatrixAt(index,instanceMatrix);slots[index]=false;dirtyActorMatrixMeshes.add(mesh);};
      const hideNpcVisual=i=>{for(const [key,mesh] of npcPartEntries){if(key==='eyeWhite'||key==='pupil'||key==='shoe'){hidePart(mesh,i*2);hidePart(mesh,i*2+1);}else hidePart(mesh,i);}};
      ...
      const animate = t => {
        ...
        if(dynamic){
          ...
          for(const [key,mesh] of npcPartEntries)mesh.count=(key==='eyeWhite'||key==='pupil'||key==='shoe')?activeNpcInstances*2:activeNpcInstances;
          ...
          for(let i=0;i<Math.min(NPC_CAP,dynamic.npcs.length);i++){
            ...
            const measuredSpeed=Math.hypot(motion.velocityX,motion.velocityZ),recentMeasuredMove=t-motion.lastMoveAt<440&&measuredSpeed>.018,poseLocksGait=!!(animBits&(NPC_ANIM_COWER|NPC_ANIM_SURRENDER|NPC_ANIM_HELP)),panicMoving=!!(animBits&NPC_ANIM_PANIC),actuallyMoving=!dead&&!poseLocksGait&&recentMeasuredMove,motionFade=Math.max(0,1-dt*(actuallyMoving?1.8:7.5));
          ...
          let crawlingNpcCount=0,limpingNpcCount=0,reactingNpcCount=0,bleedingNpcCount=0,panickingNpcCount=0,coweringNpcCount=0,surrenderingNpcCount=0,helpingNpcCount=0,socialNpcCount=0,alertedNpcCount=0,ambientNpcCount=0,firingPoliceCount=0,firingGuardCount=0,walkingArmNpcCount=0,idleNpcCount=0,idleNpcLegMotionMax=0,idleLookNpcCount=0;const idleStanceCounts={civilian:0,gang:0,police:0,guard:0};
          citizenPool.forEach((npc,i)=>{
            ...
            setPart(npcParts.body,i,rootMatrix,pose.idleBodyX,2.05-pose.crouch-pose.hit*.08-medicCrouch,0,-pose.hit*.09,npcBodyScale,pose.torsoTwist,pose.shoulderSway);
            ...
            setPart(npcParts.head,i,rootMatrix,pose.idleHeadX+pose.hit*pose.hitSide*.08,3.3-pose.crouch+(pose.walking?0:pose.idle*.025)-medicCrouch,0,pose.hit*.12*pose.hitSide,faceScale,pose.headCounter,pose.headTilt);
            setPart(npcParts.hair,i,rootMatrix,pose.idleHeadX,3.34-pose.crouch-medicCrouch,0);
            ...
            setPart(npcParts.leftArm,i,rootMatrix,(lifeArmPose?pose.leftArmX:-.78)+pose.idleBodyX,medicArmY,medicArmZ,lifeArmPose?pose.leftArmPitch-pose.hit*.42*pose.hitSide:medicArmPitch,unitScale,lifeArmPose?pose.leftArmYaw:0,lifeArmPose?pose.leftArmRoll:0);
            setPart(npcParts.rightArm,i,rootMatrix,(lifeArmPose?pose.rightArmX:.78)+pose.idleBodyX,medicArmY,medicArmZ,lifeArmPose?pose.rightArmPitch+pose.hit*.36*pose.hitSide:medicArmPitch,unitScale,lifeArmPose?pose.rightArmYaw:0,lifeArmPose?pose.rightArmRoll:0);
            ...
            for(const [eyeIndex,sx] of [[i*2,-.17],[i*2+1,.17]]){setPart(npcParts.eyeWhite,eyeIndex,rootMatrix,sx+pose.idleHeadX,3.37-pose.crouch,.405,0,eyeScale,pose.headCounter,pose.headTilt);setPart(npcParts.pupil,eyeIndex,rootMatrix,sx+pose.idleHeadX,3.37-pose.crouch,.455,0,pupilScale,pose.headCounter,pose.headTilt);}
          ...
          for(let i=Math.min(NPC_CAP,dynamic.npcs.length);i<NPC_CAP;i++)npcAppearanceSlotSignatures[i]='';if(npcColorBuffersDirty)for(const mesh of npcPartMeshes)if(mesh.instanceColor)mesh.instanceColor.needsUpdate=true;
