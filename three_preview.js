...
#include <colorspace_fragment>
      ...
      if(worldSnapshot?.landmarks){
        ...
        const npcBusinessControlGroup=new THREE.Group();scene.add(npcBusinessControlGroup);let npcBusinessControlSig='';
      ...
      const updateNpcLabel=(entry,src,x,y,z)=>{
        if(src.said){const sig=`said-v252:${src.hired?1:0}:${src.speech||''}`;if(sig!==entry.sig){entry.sig=sig;const c=entry.canvas.getContext('2d');c.clearRect(0,0,768,192);c.fillStyle='rgba(12,10,7,.98)';c.fillRect(36,8,696,176);c.strokeStyle='#e5bd55';c.lineWidth=11;c.strokeRect(42,14,684,164);fitOutlinedLabelText(c,'САИД · ВАЖНЫЙ NPC',384,57,620,54,'#ffd76b');fitOutlinedLabelText(c,src.hired?'Правая рука · нанят':'Поговорить · можно нанять',384,135,620,45,'#ffffff');entry.sprite.scale.set(17.2,4.3,1);entry.texture.needsUpdate=true;}entry.sprite.visible=true;entry.sprite.position.set(x,y+.35,z);return;}
        const role=String(src.role||'civilian').toLowerCase(),behavior=String(src.behavior||''),prisonStaff=behavior==='prison_patrol'||role.startsWith('prison_'),gang=!!src.gang||role.includes('gang')||role.includes('boss')||role.includes('district_')||role.includes('occupier'),police=!!src.police||role.includes('police')||role.includes('cop'),guard=role.includes('guard'),owner=src.visualRole==='owner',medic=role.includes('medic'),civilian=!police&&!gang&&!guard&&!owner;
        ...
        const accent=police?'#58b9ff':gang?'#ff5367':guard?'#ffc857':owner?'#ffe08a':medic?'#64e7dd':'#7ee7a5',name=String(src.name||title).slice(0,28),hp=Math.max(0,Math.round(+src.hp||0)),maxHp=Math.max(1,Math.round(+src.maxHp||60)),level=gang?Math.max(1,Math.round(+src.level||1)):null,rankText=gang?` · ${level} ур.`:'',familyText=behavior==='hostile_zone'?'':(family?` · ${family}`:''),subline=owner?name:prisonStaff?`${police?'ПАТРУЛЬ':'КАРАУЛ'} · ${hp}/${maxHp}`:`${name} · HP ${hp}/${maxHp}`,sig=`v327:${title}:${subline}:${rankText}:${familyText}`;
        if(sig!==entry.sig){entry.sig=sig;const c=entry.canvas.getContext('2d');c.clearRect(0,0,768,192);if(civilian){c.fillStyle='rgba(5,10,17,.97)';c.fillRect(84,8,600,176);c.strokeStyle=accent;c.lineWidth=10;c.strokeRect(90,14,588,164);fitOutlinedLabelText(c,title,384,57,520,56,accent);fitOutlinedLabelText(c,subline,384,135,540,51,'#ffffff');entry.sprite.scale.set(15.5,3.88,1);}else{c.fillStyle=owner?'rgba(22,16,8,.98)':'rgba(3,8,14,.98)';c.fillRect(8,8,752,176);c.strokeStyle=accent;c.lineWidth=11;c.strokeRect(14,14,740,164);c.fillStyle=accent;c.fillRect(30,88,708,5);fitOutlinedLabelText(c,`${title}${rankText}${familyText}`,384,55,690,owner?49:prisonStaff?62:54,accent);fitOutlinedLabelText(c,subline,384,137,680,owner?56:prisonStaff?60:51,'#ffffff');entry.sprite.scale.set(owner?18.2:prisonStaff?17.8:17.2,owner?4.55:prisonStaff?4.75:4.3,1);}entry.texture.needsUpdate=true;}entry.sprite.visible=true;entry.sprite.position.set(x,y,z);
      };
      ...
      // Embedded Chromium can report buttons===1 here although accepted RMB
      // aim is still held. The local aim state is authoritative for the chord.
      ...
      const animate = t => {
        if(t-customGangHqAt>2000){customGangHqAt=t;refreshCustomGangHqs();refreshNpcBusinessControls();}
        ...
        if(dynamic){
          ...
          let crawlingNpcCount=0,limpingNpcCount=0,reactingNpcCount=0,bleedingNpcCount=0,panickingNpcCount=0,coweringNpcCount=0,surrenderingNpcCount=0,helpingNpcCount=0,socialNpcCount=0,alertedNpcCount=0,ambientNpcCount=0,firingPoliceCount=0,firingGuardCount=0,walkingArmNpcCount=0;
          citizenPool.forEach((npc,i)=>{
            ...
            armed?setPart(npcParts.hat,i,rootMatrix,0,3.77-pose.crouch,0):hidePart(npcParts.hat,i);const lifeGestureDisarms=pose.phoneCalling||pose.surrendering||pose.cowering||pose.helping||pose.panicking||pose.talking||!!pose.ambientKind;armed&&!lifeGestureDisarms&&!(severMask&2)?setPart(npcParts.gun,i,rootMatrix,pose.firing?.5:.72,pose.firing?2.28-pose.recoil*.08:2-pose.crouch*.35,pose.firing?.78:.52,pose.firing?-1.16-pose.recoil*.3:pose.hit*.18):hidePart(npcParts.gun,i);
          ...
          for(let i=0;i<Math.min(NPC_CAP,dynamic.npcs.length);i++){
            ...
            const pick=(arr,salt=0)=>arr[(((seed>>>salt)^Math.imul(seed,31+salt))>>>0)%arr.length],role=String(src.role||'').toLowerCase(),said=!!src.said||role==='said',police=src.visualRole==='police'||!!src.police||role.includes('police')||role.includes('cop'),gang=!!src.gang||role.includes('gang')||role.includes('boss')||role.includes('district_')||role.includes('occupier'),businessOwner=src.visualRole==='owner',ownerStyle=Math.abs(Number.isFinite(+src.ownerStyle)?+src.ownerStyle:+src.look?.ownerStyle||0)%10,prisonStaff=!!src.prisonStaff||String(src.behavior||'')==='prison_patrol'||role.startsWith('prison_'),securityGuard=!prisonStaff&&(src.visualRole==='guard'||role.includes('guard')),guardMajor=securityGuard&&String(src.guardClass||'')==='major',guardStyle=String(src.guardStyle||'generic'),securityUniform=SECURITY_UNIFORMS[guardMajor?guardStyle:'business']||SECURITY_UNIFORMS.generic,responseTier=Math.max(0,Math.min(3,+src.responseTier||0)),tactical=responseTier>0,prisonGuard=prisonStaff&&role.includes('guard'),staffVariant=Math.abs(+src.staffVariant||0)%3,prisonGear=String(src.prisonGear||''),armed=gang||securityGuard||role.includes('guard')||police,look=src.look||{};
            let authoredSuit=0,authoredTrousers=0,authoredHair=0,ownerAccent=0;try{if(look.suit||src.outfit)authoredSuit=instanceColor.set(look.suit||src.outfit).getHex();if(look.trousers)authoredTrousers=instanceColor.set(look.trousers).getHex();if(look.hairColor)authoredHair=instanceColor.set(look.hairColor).getHex();if(look.accent)ownerAccent=instanceColor.set(look.accent).getHex();}catch(_){authoredSuit=authoredTrousers=authoredHair=ownerAccent=0;}
            ...
            const faction=String(src.faction||src.family||'').toLowerCase(),bodyColor=said?0xf4f2e9:prisonStaff?(prisonGuard?[0x35483c,0x3e4937,0x2b4039][staffVariant]:[0x245f9b,0x31527d,0x1f6777][staffVariant]):police?tacticalBody:securityGuard?securityUniform[0]:gang?(faction.includes('yellow')?0xf1e4c4:faction.includes('purple')?0x7447aa:pick([0xb84a5d,0x5f6f85,0xb48a49],3)):authoredSuit||pick(npcShirts,1),legColor=said?0x17191c:prisonStaff?(prisonGuard?[0x1e2922,0x242a20,0x18251f][staffVariant]:[0x172536,0x202c3c,0x16313a][staffVariant]):police?tacticalLeg:securityGuard?securityUniform[1]:gang?(faction.includes('yellow')?0x5e523c:faction.includes('purple')?0x342a48:0x3b3740):businessOwner?(authoredTrousers||0x111318):pick(npcTrousers,5),skinColor=Number.isFinite(+look.skin)?npcSkins[Math.abs(+look.skin|0)%npcSkins.length]:pick(npcSkins,7),hairColor=said?0x17191c:businessOwner?(authoredHair||pick(npcHairs,9)):pick(npcHairs,9),shoeColor=said||police||prisonStaff||securityGuard||businessOwner?0x090b0e:pick(npcShoes,11),hatColor=police?tacticalHat:securityGuard?securityUniform[1]:gang?bodyColor:pick(npcHats,13),hairStyle=businessOwner?ownerStyle:Number.isFinite(+look.hair)?Math.abs(+look.hair|0)%6:(seed>>>16)%6,accessory=(seed>>>20)%12,capOn=businessOwner?false:said?false:prisonStaff?prisonGear==='cap':tactical?false:police||armed||(Number.isFinite(+look.hat)&&+look.hat>0)||hairStyle===4||hairStyle===5,helmetOn=!said&&(prisonStaff||tactical)&&prisonGear.includes('helmet'),wearHat=capOn||helmetOn,glassesOn=!said&&!armed&&!businessOwner&&(accessory===0||accessory===4||role.includes('staff')),neckOn=businessOwner||said||armed||accessory%3===1,bagOn=!said&&!armed&&!businessOwner&&accessory%4===2,moustacheOn=!said&&!armed&&+look.gender!==1&&(businessOwner?ownerStyle===2||ownerStyle===5:accessory===3||accessory===9),variantSig=`${bodyColor}:${legColor}:${skinColor}:${hairColor}:${shoeColor}:${hatColor}:${hairStyle}:${prisonGear||'city'}:${responseTier}:${securityGuard?guardStyle:'none'}:${businessOwner?ownerStyle:'none'}:${glassesOn?1:0}:${neckOn?1:0}:${bagOn?1:0}:${moustacheOn?1:0}`,slotSig=`${key}:${variantSig}`;
            if(npcAppearanceSlotSignatures[i]!==slotSig){npcAppearanceSlotSignatures[i]=slotSig;npcColorBuffersDirty=true;
              ...
              npcParts.head.setColorAt(i,instanceColor.setHex(skinColor));npcParts.hair.setColorAt(i,instanceColor.setHex(hairColor));npcParts.hairBun.setColorAt(i,instanceColor.setHex(hairColor));npcParts.hairMohawk.setColorAt(i,instanceColor.setHex(hairColor));npcParts.ownerHairSide.setColorAt(i,instanceColor.setHex(hairColor));npcParts.ownerHairBack.setColorAt(i,instanceColor.setHex(hairColor));npcParts.ownerHairQuiff.setColorAt(i,instanceColor.setHex(hairColor));npcParts.hat.setColorAt(i,instanceColor.setHex(hatColor));npcParts.hatBrim.setColorAt(i,instanceColor.setHex(hatColor));npcParts.shoe.setColorAt(i*2,instanceColor.setHex(shoeColor));npcParts.shoe.setColorAt(i*2+1,instanceColor.setHex(shoeColor));npcParts.glasses.setColorAt(i,instanceColor.setHex(accessory%2?0x26343e:0x4b3028));npcParts.neckAccent.setColorAt(i,instanceColor.setHex(businessOwner?(ownerAccent||0xd6b45e):said?0x090a0c:pick(npcAccents,15)));npcParts.ownerLapel.setColorAt(i*2,instanceColor.setHex(businessOwner?0xe7dfcf:bodyColor));npcParts.ownerLapel.setColorAt(i*2+1,instanceColor.setHex(businessOwner?0xe7dfcf:bodyColor));npcParts.ownerPocket.setColorAt(i,instanceColor.setHex(ownerAccent||0xd6b45e));npcParts.bag.setColorAt(i,instanceColor.setHex(pick(npcBags,17)));npcParts.moustache.setColorAt(i,instanceColor.setHex(hairColor));
            ...
            hidePart(npcParts.hair,i);hidePart(npcParts.hairBun,i);hidePart(npcParts.hairMohawk,i);hidePart(npcParts.ownerHairSide,i);hidePart(npcParts.ownerHairBack,i);hidePart(npcParts.ownerHairQuiff,i);hidePart(npcParts.ownerLapel,i*2);hidePart(npcParts.ownerLapel,i*2+1);hidePart(npcParts.ownerPocket,i);hidePart(npcParts.hat,i);hidePart(npcParts.hatBrim,i);hidePart(npcParts.glasses,i);hidePart(npcParts.neckAccent,i);hidePart(npcParts.bag,i);hidePart(npcParts.moustache,i);hidePart(npcParts.prisonHelmet,i);hidePart(npcParts.prisonHelmetBand,i);hidePart(npcParts.prisonVisor,i);hidePart(npcParts.prisonVest,i);hidePart(npcParts.securityBadge,i);hidePart(npcParts.riotShield,i);hidePart(npcParts.shieldViewport,i);
            if(!businessOwner&&!wearHat&&hairStyle!==0)setPart(npcParts.hair,i,rootMatrix,0,3.34-pose.crouch,0,0,unitScale,pose.headCounter,pose.headTilt);
            ...
            if(!businessOwner&&!wearHat&&hairStyle===3){hidePart(npcParts.hair,i);setPart(npcParts.hairMohawk,i,rootMatrix,0,3.72-pose.crouch,-.02,0,unitScale,pose.headCounter,pose.headTilt);}
            if(businessOwner){
            ...
            if(prisonStaff||tactical||securityGuard)setPart(npcParts.prisonVest,i,rootMatrix,0,2.02-pose.crouch,.02);
            if(securityGuard)setPart(npcParts.securityBadge,i,rootMatrix,.35,2.35-pose.crouch,.455);
        ...
        if(bridge){
          ...
          renderer.domElement.dataset.prisonAlarmBeaconProfile='fixed-lightbar-double-pulse-soft-glow-v327';renderer.domElement.dataset.prisonAlarmVisual=prisonAlarmActive?`active:red-${prisonAlarmRedPower.toFixed(2)}:blue-${prisonAlarmBluePower.toFixed(2)}:${prisonAlarmBeacons.length}`:'quiet';renderer.domElement.dataset.prisonVehicleGateLock=prisonAlarmActive?'closed':'open';
