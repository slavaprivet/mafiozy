...
#include <colorspace_fragment>
      ...
      for(const mesh of [npcParts.uniqueGunRail,npcParts.uniqueGunCharm,npcParts.uniqueGunStock,npcParts.uniqueGunMuzzle])mesh.onBeforeRender=()=>{if(mesh.instanceColor)mesh.instanceColor.needsUpdate=true;};
      ...
      const animate = t => {
        ...
        if(dynamic){
          ...
          citizenPool.forEach((npc,i)=>{
            ...
            armed?setPart(npcParts.hat,i,rootMatrix,0,3.77-pose.crouch,0):hidePart(npcParts.hat,i);const lifeGestureDisarms=pose.phoneCalling||pose.surrendering||pose.cowering||pose.helping||pose.panicking||(!empireBoss&&pose.talking)||!!pose.ambientKind,showGun=armed&&!lifeGestureDisarms&&!(severMask&2),gunX=pose.firing?.5:.72,gunY=pose.firing?2.28-pose.recoil*.08:empireBoss&&pose.talking?1.68:2-pose.crouch*.35,gunZ=pose.firing?.78:empireBoss&&pose.talking?.28:.52,gunPitch=pose.firing?-1.16-pose.recoil*.3:empireBoss&&pose.talking?.42:pose.hit*.18;showGun?setPart(npcParts.gun,i,rootMatrix,gunX,gunY,gunZ,gunPitch):hidePart(npcParts.gun,i);const uniqueWeaponId=String(src.uniqueWeaponId||''),uniqueWeaponColor=npcEmpireWeaponColors[uniqueWeaponId];if(showGun&&uniqueWeaponColor){const variant=Math.max(0,Object.keys(npcEmpireWeaponColors).indexOf(uniqueWeaponId)),weaponFamily=String(src.weapon||''),longWeapon=weaponFamily.includes('rifle')||weaponFamily.includes('sniper')||weaponFamily.includes('shotgun')||weaponFamily.includes('tommy'),railScale=instanceScale.set((longWeapon?1.55:1.02)+(variant%3)*.2,.62+(variant%4)*.13,.78+(variant%5)*.1);setPart(npcParts.uniqueGunRail,i,rootMatrix,gunX,gunY+.16,gunZ+.08,gunPitch,railScale);setPart(npcParts.uniqueGunStock,i,rootMatrix,gunX,gunY-.03,gunZ-(longWeapon?.52:.31),gunPitch,instanceScale.set(.72+(variant%4)*.13,.72+(variant%3)*.16,longWeapon?1.15:.7));setPart(npcParts.uniqueGunMuzzle,i,rootMatrix,gunX,gunY+.03,gunZ+(longWeapon?1.12:.72),gunPitch+Math.PI/2,instanceScale.set(.72+(variant%2)*.25,1+(variant%5)*.15,.72+(variant%2)*.25));setPart(npcParts.uniqueGunCharm,i,rootMatrix,gunX+(variant%2?-.18:.18),gunY-.19,gunZ+.2,gunPitch+t*.0015,instanceScale.setScalar(.82+(variant%4)*.14));for(const mesh of [npcParts.uniqueGunRail,npcParts.uniqueGunCharm,npcParts.uniqueGunStock,npcParts.uniqueGunMuzzle])mesh.setColorAt(i,instanceColor.setHex(uniqueWeaponColor));}else{hidePart(npcParts.uniqueGunRail,i);hidePart(npcParts.uniqueGunCharm,i);hidePart(npcParts.uniqueGunStock,i);hidePart(npcParts.uniqueGunMuzzle,i);}
