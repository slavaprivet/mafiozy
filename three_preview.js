...
#include <colorspace_fragment>
      ...
      const EMPIRE_FLAG_CAP=64,empireHqRingGeometry=new THREE.RingGeometry(3.5,4.15,40),empireHqPoleGeometry=new THREE.CylinderGeometry(.07,.1,6,8),empireHqFlagGeometry=new THREE.PlaneGeometry(3.5,1.75),empireHqMarkers=[];
      const EMPIRE_HOSPITAL_CAP=6,empireHospitalMarkers=[];
      const redrawEmpireHqLabel=(marker,src)=>{const g=marker.context,c=marker.canvas,color=src.color||'#872f3b',accent=src.accent||'#e8bd67',name=String(src.gangName||'Криминальная семья').toUpperCase();g.clearRect(0,0,c.width,c.height);g.fillStyle='rgba(7,9,13,.97)';g.strokeStyle=color;g.lineWidth=8;g.fillRect(8,8,c.width-16,c.height-16);g.strokeRect(8,8,c.width-16,c.height-16);g.textAlign='center';g.textBaseline='middle';if(src.isHq){g.fillStyle=accent;g.font='900 31px Arial';g.fillText(`🚩 ШТАБ · ${name}`,c.width/2,c.height/2,470);}else{g.fillStyle='#fff1c8';g.font='900 20px Arial';g.fillText('ВЛАДЕНИЕ СЕМЬИ',c.width/2,28,470);g.fillStyle=accent;g.font='900 35px Arial';g.fillText(name,c.width/2,65,470);}marker.texture.needsUpdate=true;};
