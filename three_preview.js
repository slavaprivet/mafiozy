...
#include <colorspace_fragment>
      ...
      const projectileColorSignatures=new Array(BULLET_CAP).fill(''),bulletHoleMatrixSignatures=new Array(32).fill(''),bloodMatrixSignatures=new Array(48).fill(''),goreColorSignatures=new Array(12).fill('');
      ...
      const animate = t => {
        ...
        if(dynamic){
          ...
          for(let i=0;i<activeShellInstances;i++){const src=dynamic.shellFx[i],rot=+src.rot||0;instanceQuat.setFromEuler(instanceEuler.set(rot,rot*.7,rot*1.3));instanceMatrix.compose(instancePosition.set((src.c-originC)*WORLD_SCALE,.18+(+src.z||0)*WORLD_SCALE,(src.r-originR)*WORLD_SCALE),instanceQuat,unitScale);shellPool.setMatrixAt(i,instanceMatrix);}if(activeShellInstances)shellPool.instanceMatrix.needsUpdate=true;
          ...
          let bulletHoleMatricesDirty=false;for(let i=0;i<activeBulletHoleInstances;i++){const src=dynamic.bulletHoleFx[i],ang=+src.ang||0,size=(src.heavy?1.12:.68)*(+src.scale||1),fade=Math.max(.48,1-Math.max(0,+src.age||0)/120000),height=1.82+((+src.id||i)*.37%1)*.72,matrixSig=`${src.id??i}:${src.r}:${src.c}:${ang}:${src.rot||0}:${size}`;if(bulletHoleMatrixSignatures[i]!==matrixSig){bulletHoleMatrixSignatures[i]=matrixSig;instanceQuat.setFromEuler(instanceEuler.set(0,-Math.PI/2-ang,+src.rot||0));instanceMatrix.compose(instancePosition.set((src.c-originC)*WORLD_SCALE-Math.cos(ang)*.055,height,(src.r-originR)*WORLD_SCALE-Math.sin(ang)*.055),instanceQuat,instanceScale.set(size,size,1));bulletHoleDecals.setMatrixAt(i,instanceMatrix);bulletHoleMatricesDirty=true;}bulletHoleDecals.setColorAt(i,instanceColor.setRGB(.52*fade,.46*fade,.39*fade));}if(bulletHoleMatricesDirty)bulletHoleDecals.instanceMatrix.needsUpdate=true;if(activeBulletHoleInstances&&bulletHoleDecals.instanceColor)bulletHoleDecals.instanceColor.needsUpdate=true;
          let bloodMatricesDirty=false;for(let i=0;i<activeBloodInstances;i++){const src=dynamic.bloodFx[i],fade=Math.max(.12,Math.min(1,(+src.life||0)/Math.max(1,+src.max||1))),scale=Math.max(.28,Math.min(1.65,(+src.radius||5)*.105)),sx=src.trail?scale*.72:scale,sy=src.trail?scale*1.42:scale,matrixSig=`${src.id??i}:${src.r}:${src.c}:${src.rot||0}:${sx}:${sy}`;if(bloodMatrixSignatures[i]!==matrixSig){bloodMatrixSignatures[i]=matrixSig;instanceQuat.setFromEuler(instanceEuler.set(-Math.PI/2,0,+src.rot||0));instanceMatrix.compose(instancePosition.set((src.c-originC)*WORLD_SCALE,.045,(src.r-originR)*WORLD_SCALE),instanceQuat,instanceScale.set(sx,sy,1));bloodDecals.setMatrixAt(i,instanceMatrix);bloodMatricesDirty=true;}src.soot?bloodDecals.setColorAt(i,instanceColor.setRGB(src.crater?.025:.045,src.crater?.022:.038,src.crater?.02:.03)):bloodDecals.setColorAt(i,instanceColor.setRGB(.28+.22*fade,.008,.014));}if(bloodMatricesDirty)bloodDecals.instanceMatrix.needsUpdate=true;if(activeBloodInstances&&bloodDecals.instanceColor)bloodDecals.instanceColor.needsUpdate=true;if(telemetryDue)renderer.domElement.dataset.bloodDecals=String(activeBloodInstances);
