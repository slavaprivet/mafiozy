...
#include <colorspace_fragment>
      ...
      for(const [partName,mesh] of Object.entries(npcParts))mesh.name=`npc-${partName}`;
      ...
      compileLoadedScene=()=>{
        ...
        try{
          ...
          if(prisonLightRoots.length){const prisonWarmStartedAt=performance.now();prisonLightRoots.forEach(root=>{root.visible=false;});renderer.compile(scene,camera);if(realTimeShadows)renderer.shadowMap.needsUpdate=true;renderer.render(scene,camera);prisonLightRoots.forEach((root,index)=>{root.visible=prisonLightVisibility[index];});renderer.domElement.dataset.prisonCombatWarmup=`ready:${prisonLightRoots.length}:${(performance.now()-prisonWarmStartedAt).toFixed(1)}ms`;}
      ...
      const animate = t => {
        ...
        if(dynamic){
          ...
          renderer.domElement.dataset.ambientNpcAnimations=String(ambientNpcCount);renderer.domElement.dataset.npcAmbientProfile='phone-smoke-stretch-deterministic-lod-v251';renderer.domElement.dataset.firingPolice=String(firingPoliceCount);renderer.domElement.dataset.policeFireAnimation='two-hand-aim-recoil-all-weapons-v1';
