...
#include <colorspace_fragment>
      ...
      const defsFromSnapshot=snapshot=>(snapshot?.blocks||[]).flatMap(b=>(b.buildingParts?.length?b.buildingParts:(b.building?[b.building]:[])).flatMap((q,partIndex)=>{
        ...
        return [[(q.c-originC)*WORLD_SCALE,(q.r-originR)*WORLD_SCALE,q.w*WORLD_SCALE,q.d*WORLD_SCALE,q.height,styleIndexes[b.styleId]??0,(nearPoi?.name||nearPoi?.label||'').toString().slice(0,14).toUpperCase(),b.styleId||'downtown',{r:q.r,c:q.c,w:q.w,d:q.d,minR:q.minR,maxR:q.maxR,minC:q.minC,maxC:q.maxC,tiles:q.tiles,primary:partIndex===0,architecturalKind,empireBuildingKey:String(q.empireBuildingKey||'')}]];
      }));
      ...
      const occluders=[],buildingPickables=[],facadeMaterials=[],shopMaterials=[],buildingCurbDefs=[],businessExteriorById=new Map(),empireBuildingObjects=new Map();
      ...
      const evictFarStreamedSectors=(playerR,playerC)=>{
        ...
        for(const [sectorKey,group] of streamedSectorGroups){
          ...
          for(const [key,object] of empireBuildingObjects)if(objects.has(object))empireBuildingObjects.delete(key);
      ...
      const createBuilding=(definition,bi)=>{
        ...
        const mainBuilding=buildingBox(x,z,w,d,lowerH,wall,localRoof);mainBuilding.userData.fadeMaterials=[wall,localRoof];mainBuilding.userData.building=buildingMeta;mainBuilding.userData.mainBuilding=true;occluders.push(mainBuilding);buildingPickables.push(mainBuilding);if(buildingMeta.primary!==false&&buildingMeta.empireBuildingKey)empireBuildingObjects.set(String(buildingMeta.empireBuildingKey),mainBuilding);if(detailed)outline(mainBuilding);
      ...
      const EMPIRE_BUILDING_CAP=24,empireBuildingPoleGeometry=new THREE.CylinderGeometry(.055,.08,4.4,8),empireBuildingFlagGeometry=new THREE.PlaneGeometry(2.7,1.3),empireBuildingBandGeometry=new THREE.BoxGeometry(5.4,.5,.34),empireBuildingMarkers=[];
      const empireOperationThemes={
        beer_bar:['🍺',0xd79336],pawnshop:['💎',0x49a9bf],bookmaker:['🎟️',0xd2b247],strip_club:['💃',0xd63b83],
        gun_shop:['🔫',0x9ea8ae],chop_shop:['🔧',0xd97532],poker_club:['♠',0x3ba66b],print_shop:['▤',0x8b63c7]
      };
      ...
      const restoreEmpireBuildingTint=marker=>{const object=marker.buildingObject;if(!object)return;const materials=object.userData?.fadeMaterials||[];for(let i=0;i<materials.length;i++)if(materials[i]?.color&&object.userData.empireBaseColors?.[i])materials[i].color.copy(object.userData.empireBaseColors[i]);marker.buildingObject=null;};
      const redrawEmpireBuildingMarker=(marker,src)=>{
        const theme=empireOperationThemes[src.operationType]||empireOperationThemes.beer_bar,color=src.color||'#872f3b',accent=src.accent||'#e8bd67';
        const fg=marker.flagContext,fc=marker.flagCanvas;fg.clearRect(0,0,fc.width,fc.height);fg.fillStyle=color;fg.fillRect(0,0,fc.width,fc.height);fg.fillStyle=accent;fg.fillRect(0,0,fc.width,10);fg.fillRect(0,fc.height-10,fc.width,10);fg.font='900 48px Arial';fg.textAlign='center';fg.textBaseline='middle';fg.fillStyle='#fff';fg.fillText(theme[0],112,48);marker.flagTexture.needsUpdate=true;
        const g=marker.context,c=marker.canvas;g.clearRect(0,0,c.width,c.height);g.fillStyle='rgba(5,8,12,.96)';g.strokeStyle=color;g.lineWidth=9;g.fillRect(7,7,c.width-14,c.height-14);g.strokeRect(7,7,c.width-14,c.height-14);g.fillStyle=`#${new THREE.Color(theme[1]).getHexString()}`;g.fillRect(18,18,76,c.height-36);g.fillStyle='#fff';g.font='900 38px Arial';g.textAlign='center';g.textBaseline='middle';g.fillText(theme[0],56,c.height/2);g.textAlign='left';g.fillStyle='#fff4d2';g.font='900 24px Arial';g.fillText(String(src.operationName||'БИЗНЕС').toUpperCase().slice(0,27),112,43,385);g.fillStyle=accent;g.font='800 19px Arial';g.fillText(`${String(src.gangName||'СЕМЬЯ').toUpperCase().slice(0,24)} · $${Math.min(200,+src.incomePerMinute||0)}/МИН`,112,80,385);marker.texture.needsUpdate=true;marker.bandMaterial.color.setHex(theme[1]);marker.bandMaterial.emissive.setHex(theme[1]);
      };
      for(let i=0;i<EMPIRE_BUILDING_CAP;i++){
        const root=new THREE.Group(),poleMaterial=new THREE.MeshStandardMaterial({color:0xc9b78c,roughness:.45,metalness:.65}),pole=new THREE.Mesh(empireBuildingPoleGeometry,poleMaterial);pole.position.x=-2.1;root.add(pole);
        const flagCanvas=document.createElement('canvas');flagCanvas.width=224;flagCanvas.height=96;const flagContext=flagCanvas.getContext('2d'),flagTexture=new THREE.CanvasTexture(flagCanvas);flagTexture.colorSpace=THREE.SRGBColorSpace;flagTexture.generateMipmaps=false;flagTexture.minFilter=THREE.LinearFilter;const flagMaterial=new THREE.MeshBasicMaterial({map:flagTexture,side:THREE.DoubleSide,toneMapped:false}),flag=new THREE.Mesh(empireBuildingFlagGeometry,flagMaterial),flagPivot=new THREE.Group();flag.position.x=1.36;flagPivot.position.x=-2.1;flagPivot.add(flag);root.add(flagPivot);
        const bandMaterial=new THREE.MeshStandardMaterial({color:0xd79336,emissive:0xd79336,emissiveIntensity:.55,roughness:.32,metalness:.28}),band=new THREE.Mesh(empireBuildingBandGeometry,bandMaterial);root.add(band);
        const canvas=document.createElement('canvas');canvas.width=512;canvas.height=104;const context=canvas.getContext('2d'),texture=new THREE.CanvasTexture(canvas);texture.colorSpace=THREE.SRGBColorSpace;texture.generateMipmaps=false;texture.minFilter=THREE.LinearFilter;const label=new THREE.Sprite(new THREE.SpriteMaterial({map:texture,transparent:true,depthWrite:false,toneMapped:false}));label.scale.set(11.5,2.34,1);label.renderOrder=25;root.add(label);root.visible=false;scene.add(root);empireBuildingMarkers.push({root,pole,flagPivot,flag,flagCanvas,flagContext,flagTexture,band,bandMaterial,canvas,context,texture,label,signature:'',buildingObject:null});
      }
      ...
      const animate = t => {
        ...
        if(dynamic){
          ...
          let visibleEmpireBuildings=0;for(let i=0;i<EMPIRE_BUILDING_CAP;i++){const marker=empireBuildingMarkers[i],src=dynamic.empireBuildings?.[i];marker.root.visible=!!src;if(!src){restoreEmpireBuildingTint(marker);marker.signature='';continue;}visibleEmpireBuildings++;const object=empireBuildingObjects.get(String(src.buildingKey||''))||null,meta=object?.userData?.building||null,rootX=object?.position?.x??((+src.c-originC)*WORLD_SCALE),rootZ=object?.position?.z??((+src.r-originR)*WORLD_SCALE);marker.root.position.set(rootX,0,rootZ);const sig=`${src.buildingKey}:${src.leaderId}:${src.operationType}:${src.incomePerMinute}:${src.color}:${src.accent}`;if(marker.signature!==sig||marker.buildingObject!==object){restoreEmpireBuildingTint(marker);marker.signature=sig;marker.buildingObject=object;const roofY=object?empireHqRoofYAt(+src.r||0,+src.c||0):.2,span=Math.max(4,Math.min(9,(+(meta?.w)||2)*WORLD_SCALE*.7));marker.pole.position.y=roofY+2.2;marker.flagPivot.position.y=roofY+3.55;marker.band.position.y=roofY+.38;marker.band.scale.x=span/5.4;marker.label.position.y=roofY+5.25;if(object){const materials=object.userData?.fadeMaterials||[];if(!object.userData.empireBaseColors)object.userData.empireBaseColors=materials.map(material=>material?.color?.clone?.()||null);const familyColor=new THREE.Color(src.color||'#872f3b'),themeColor=new THREE.Color((empireOperationThemes[src.operationType]||empireOperationThemes.beer_bar)[1]);for(let mi=0;mi<materials.length;mi++)if(materials[mi]?.color&&object.userData.empireBaseColors[mi])materials[mi].color.copy(object.userData.empireBaseColors[mi]).lerp(mi===0?familyColor:themeColor,(mi===0 ? .42 : .24));}redrawEmpireBuildingMarker(marker,src);}marker.flagPivot.rotation.y=Math.atan2(camera.position.x-marker.root.position.x,camera.position.z-marker.root.position.z);marker.flag.scale.x=.96+Math.sin(t*.006+i*.83)*.07;marker.bandMaterial.emissiveIntensity=.42+(Math.sin(t*.003+i)+1)*.12;}if(telemetryDue){renderer.domElement.dataset.visibleEmpireBusinessBuildings=String(visibleEmpireBuildings);renderer.domElement.dataset.empireBusinessVisualProfile='bounded-roof-sign-family-tint-flag-v363';}
