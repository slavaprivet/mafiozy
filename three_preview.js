...
#include <colorspace_fragment>
      ...
      const nearestNpcInteraction=()=>{const state=bridge?.getPlayerState?.();if(npcActionMenuOpen()||!dynamicState?.npcs?.length||state?.driving||state?.dead||state?.vehicleEntry)return null;const pr=originR+player.position.z/WORLD_SCALE,pc=originC+player.position.x/WORLD_SCALE;let best=null,bestD=3.05;for(let i=0;i<dynamicState.npcs.length&&i<NPC_CAP;i++){const src=dynamicState.npcs[i];if(!src||src.dead)continue;const r=+src.r,c=+src.c;if(!Number.isFinite(r)||!Number.isFinite(c))continue;const d=Math.hypot(r-pr,c-pc),key=String(src.id||src.sourceId||'');if(key&&d<bestD){bestD=d;best={src,index:i,key,distance:d};}}return best;};
