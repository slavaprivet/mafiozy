// 3D character/weapons v357: overlapped shoulder, elbow, hip and ankle joints keep NPC limbs attached through motion.
...
#include <colorspace_fragment>
      ...
      const npcParts={
        ...
        leftLeg:makeInstances(new THREE.CapsuleGeometry(.21,.96,5,10),new THREE.MeshStandardMaterial({color:0xffffff,vertexColors:true,roughness:.8,emissive:0x15181d,emissiveIntensity:.42}),NPC_CAP),
        rightLeg:makeInstances(new THREE.CapsuleGeometry(.21,.96,5,10),new THREE.MeshStandardMaterial({color:0xffffff,vertexColors:true,roughness:.8,emissive:0x15181d,emissiveIntensity:.42}),NPC_CAP),
        leftArm:makeInstances(new THREE.CapsuleGeometry(.18,.64,5,10),new THREE.MeshStandardMaterial({color:0xffffff,vertexColors:true,roughness:.76,emissive:0x15181d,emissiveIntensity:.42}),NPC_CAP),
        rightArm:makeInstances(new THREE.CapsuleGeometry(.18,.64,5,10),new THREE.MeshStandardMaterial({color:0xffffff,vertexColors:true,roughness:.76,emissive:0x15181d,emissiveIntensity:.42}),NPC_CAP),
        forearm:makeInstances(new THREE.CapsuleGeometry(.16,.52,5,10),new THREE.MeshStandardMaterial({color:0xffffff,vertexColors:true,roughness:.76,emissive:0x15181d,emissiveIntensity:.42}),NPC_CAP*2),
        hand:makeInstances(new THREE.SphereGeometry(.19,10,8),new THREE.MeshStandardMaterial({color:0xffffff,vertexColors:true,roughness:.72,emissive:0x211611,emissiveIntensity:.3}),NPC_CAP*2),
      ...
      const EMPIRE_HQ_CAP=19,empireHqRingGeometry=new THREE.RingGeometry(3.5,4.15,40),empireHqPoleGeometry=new THREE.CylinderGeometry(.07,.1,6,8),empireHqFlagGeometry=new THREE.PlaneGeometry(2.7,1.35),empireHqMarkers=[];
      ...
      const animate = t => {
        ...
        if(dynamic){
          ...
          citizenPool.forEach((npc,i)=>{
            ...
            const bodyIndex=Math.abs(+look.body||0)%NPC_BODY_PROFILES.length,bodyProfile=NPC_BODY_PROFILES[bodyIndex],female=+look.gender===1,faceStyle=Math.abs(+look.face||0)%10,shoulderScale=female?.82:1,hipScale=female?1.08:1,depthScale=female?.94:1,shoulderX=.7*bodyProfile.shoulder*shoulderScale,armScale=bodyProfile.arm*(female?.9:1),legScale=bodyProfile.leg*(female?.92:1),hipWidth=bodyProfile.hip*hipScale;
            ...
            const tailored=empireBoss||src.visualRole==='owner'||(!police&&!medic&&(bodyIndex+faceStyle)%2===0);tailored?setPart(npcParts.collar,i,rootMatrix,0,2.55-pose.crouch-medicCrouch,.54*bodyProfile.bodyZ*depthScale,0,instanceScale.set(bodyProfile.bodyX*shoulderScale,1,1),pose.torsoTwist,pose.shoulderSway):hidePart(npcParts.collar,i);
            const roleHemOn=empireBoss||police||medic||src.visualRole==='owner'||role.includes('guard')||tailored;roleHemOn?setPart(npcParts.roleHem,i,rootMatrix,0,1.35-pose.crouch*.42-medicCrouch*.3,0,0,instanceScale.set(hipWidth*(female?1.08:.96),female?1.08:.9,bodyProfile.bodyZ*depthScale*.92),pose.torsoTwist,pose.shoulderSway):hidePart(npcParts.roleHem,i);
            // Reuse the existing hip pool for every NPC. This shared pelvis
            // overlaps both rotating leg capsules, so a stride cannot expose a
            // dark gap between trousers and torso; it costs no additional pool.
            setPart(npcParts.femaleHips,i,rootMatrix,0,1.35-pose.crouch*.45-medicCrouch*.35,0,0,instanceScale.set(hipWidth*(female?1.14:1.02),female?.72:.58,bodyProfile.bodyZ*depthScale*.94));
            const faceX=(female?.94:1)*(faceStyle===7?1.08:faceStyle===5?.94:1),faceYScale=faceStyle===1?1.08:faceStyle===6?.94:1,faceZ=female?.95:1,eyeSpread=.16+(faceStyle%3)*.012+(faceStyle===7?.015:0),faceDepth=.455+(faceStyle%4)*.008;
            ...
            setPart(npcParts.nose,i,rootMatrix,0,faceY-.08,faceDepth+.01,Math.PI/2,instanceScale.set(1+(faceStyle%3)*.09,faceStyle===7?1.12:1,1),pose.headCounter,pose.headTilt);
            setPart(npcParts.mouth,i,rootMatrix,0,faceY-.27,faceDepth,0,instanceScale.set(faceStyle===3?1.35:faceStyle===7?1.18:faceStyle===0?.9:1,1,1),pose.headCounter,faceStyle===5?-.18:faceStyle===6?.12:pose.headTilt);
            const browTilt=[0,.28,-.08,.05,.12,-.28,-.18,.1,.02,.2][faceStyle];for(const [browIndex,sx] of [[i*2,-eyeSpread],[i*2+1,eyeSpread]])setPart(npcParts.brow,browIndex,rootMatrix,sx,faceY+.2,faceDepth-.01,0,unitScale,pose.headCounter,sx>0?-browTilt:browTilt);
            const earSpread=.42+faceX*.02;for(const [earIndex,sx] of [[i*2,-earSpread],[i*2+1,earSpread]])setPart(npcParts.ear,earIndex,rootMatrix,sx,faceY-.01,0,0,instanceScale.set(.55,faceStyle===8?1.12:1,.55),pose.headCounter,pose.headTilt);
            ...
            const legX=.34*hipWidth;setPart(npcParts.leftLeg,i,rootMatrix,-legX,.76+pose.leftLift-pose.crouch*.16-medicCrouch*.16,0,pose.leftSwing+pose.legBend+medicCrouch*.52,instanceScale.set(legScale,1,legScale));
            setPart(npcParts.rightLeg,i,rootMatrix,legX,.76+pose.rightLift-pose.crouch*.16-medicCrouch*.16,0,pose.rightSwing-pose.legBend-medicCrouch*.52,instanceScale.set(legScale,1,legScale));
            ...
            setPart(npcParts.leftArm,i,rootMatrix,leftArmX,medicArmY+.08,medicArmZ,leftPitch,armPartScale,leftYaw,leftRoll);setPart(npcParts.rightArm,i,rootMatrix,rightArmX,medicArmY+.08,medicArmZ,rightPitch,armPartScale,rightYaw,rightRoll);
            ...
            for(const [eyeIndex,sx] of [[i*2,-eyeSpread],[i*2+1,eyeSpread]]){setPart(npcParts.eyeWhite,eyeIndex,rootMatrix,sx,3.37-pose.crouch,faceDepth-.05,0,instanceScale.set(1+(faceStyle===1?.08:0),.78*blink*ageEye,.42),pose.headCounter,pose.headTilt);setPart(npcParts.pupil,eyeIndex,rootMatrix,sx+gazeX,3.37-pose.crouch+gazeY,faceDepth,0,instanceScale.set(1,.78*blink*ageEye,.3),pose.headCounter,pose.headTilt);}
          ...
          let visibleEmpireHqs=0;for(let i=0;i<EMPIRE_HQ_CAP;i++){const marker=empireHqMarkers[i],src=dynamic.empireHqs?.[i];marker.root.visible=!!src;if(!src){marker.signature='';continue;}visibleEmpireHqs++;marker.root.position.set((+src.c-originC)*WORLD_SCALE,0,(+src.r-originR)*WORLD_SCALE);const sig=`${src.leaderId}:${src.gangName}:${src.color}:${src.accent}:${src.r}:${src.c}`;if(marker.signature!==sig){marker.signature=sig;const roof=roofAnchorAt(+src.r||0,+src.c||0,9),roofY=Math.max(.15,+roof?.y||0);marker.pole.position.y=roofY+3.05;marker.flag.position.y=roofY+5.05;marker.label.position.y=roofY+7.25;try{marker.ringMaterial.color.set(src.color||'#872f3b');marker.flagMaterial.color.set(src.color||'#872f3b');}catch(_){marker.ringMaterial.color.setHex(0x872f3b);marker.flagMaterial.color.setHex(0x872f3b);}redrawEmpireHqLabel(marker,src);}const pulse=1+Math.sin(t*.003+i*.67)*.055;marker.ring.scale.setScalar(pulse);marker.flag.rotation.y=camera.rotation.y;marker.flag.scale.x=.96+Math.sin(t*.006+i)*.08;}if(telemetryDue){renderer.domElement.dataset.visibleEmpireHqs=String(visibleEmpireHqs);renderer.domElement.dataset.empireHqProfile='server-owned-ring-flag-gang-label-v357';}
          ...
          if(telemetryDue){renderer.domElement.dataset.npcAppearanceVariants=String(npcAppearanceSignatures.size);renderer.domElement.dataset.npcAppearanceSystem='creator-grade-shared-instanced-rig-v357';renderer.domElement.dataset.npcDetailProfile='profiled-torso-jaw-beard-tailored-jackets-blink-gaze-overlapped-limbs';renderer.domElement.dataset.npcJointProfile='shared-overlap-shoulder-elbow-pelvis-ankle-v357';renderer.domElement.dataset.npcMicroExpressions='deterministic-blink-and-eye-saccade-no-allocation';renderer.domElement.dataset.jailStaffGear='guards:military-helmet-riot-shield;police:cap-tactical-helmet-bare';renderer.domElement.dataset.policeResponseTiers=String(dynamic.npcs.reduce((m,n)=>Math.max(m,+n.responseTier||0),0));renderer.domElement.dataset.jailStaffMaxHp='200';}
