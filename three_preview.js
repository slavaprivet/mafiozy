...
#include <colorspace_fragment>
      ...
      const eyeScale=new THREE.Vector3(1,.78,.42),pupilScale=new THREE.Vector3(1,.78,.3),npcBodyScale=new THREE.Vector3(1,1,1),gangAuraScale=new THREE.Vector3(1.3,1.3,1.3),npcLimbOffset=new THREE.Vector3(),npcElbowOffset=new THREE.Vector3(),npcForearmOffset=new THREE.Vector3(),npcLimbQuat=new THREE.Quaternion(),npcThighOffset=new THREE.Vector3(),npcKneeOffset=new THREE.Vector3(),npcShinOffset=new THREE.Vector3(),npcAnkleOffset=new THREE.Vector3(),npcLegQuat=new THREE.Quaternion();
      ...
      const npcParts={
        ...
        leftLeg:makeInstances(new THREE.CapsuleGeometry(.19,.38,5,10),new THREE.MeshStandardMaterial({color:0xffffff,vertexColors:true,roughness:.8,emissive:0x15181d,emissiveIntensity:.42}),NPC_CAP),
        rightLeg:makeInstances(new THREE.CapsuleGeometry(.19,.38,5,10),new THREE.MeshStandardMaterial({color:0xffffff,vertexColors:true,roughness:.8,emissive:0x15181d,emissiveIntensity:.42}),NPC_CAP),
        ...
        neck:makeInstances(new THREE.CylinderGeometry(.2,.24,.38,10),new THREE.MeshStandardMaterial({color:0xffffff,vertexColors:true,roughness:.82,emissive:0x24160f,emissiveIntensity:.3}),NPC_CAP,false),
        coatSeam:makeInstances(new THREE.BoxGeometry(.045,.92,.035),new THREE.MeshStandardMaterial({color:0xffffff,vertexColors:true,roughness:.68,emissive:0x111318,emissiveIntensity:.38}),NPC_CAP,false),
        shin:makeInstances(new THREE.CapsuleGeometry(.17,.3,5,10),new THREE.MeshStandardMaterial({color:0xffffff,vertexColors:true,roughness:.8,emissive:0x15181d,emissiveIntensity:.42}),NPC_CAP*2),
        cuff:makeInstances(new THREE.CylinderGeometry(.19,.2,.12,10),new THREE.MeshStandardMaterial({color:0xffffff,vertexColors:true,roughness:.64,metalness:.06,emissive:0x111318,emissiveIntensity:.34}),NPC_CAP*2,false),
        shoeSole:makeInstances(new THREE.BoxGeometry(.38,.075,.62),new THREE.MeshStandardMaterial({color:0xffffff,vertexColors:true,roughness:.58,metalness:.05,emissive:0x08090b,emissiveIntensity:.38}),NPC_CAP*2,false)
      };
      ...
      const npcPairedPartKeys=new Set(['eyeWhite','pupil','shoe','shoeSole','shin','forearm','hand','cuff','brow','ear','uniqueGunLimb','ownerLapel','policePatch']),npcQuadPartKeys=new Set(['uniqueGunSpike','hairCurls']);
      ...
      const EMPIRE_FLAG_CAP=64,empireHqRingGeometry=new THREE.RingGeometry(3.5,4.15,40),empireHqPoleGeometry=new THREE.CylinderGeometry(.07,.1,6,8),empireHqFlagGeometry=new THREE.PlaneGeometry(3.5,1.75),empireHqMarkers=[],empireHqRoofBox=new THREE.Box3();
      const EMPIRE_HOSPITAL_CAP=6,empireHospitalMarkers=[];
      const redrawEmpireHqLabel=(marker,src)=>{const g=marker.context,c=marker.canvas,color=src.color||'#872f3b',accent=src.accent||'#e8bd67',name=String(src.gangName||'Криминальная семья').toUpperCase(),operation=String(src.operationName||'').toUpperCase(),income=Math.min(200,Math.max(0,+src.incomePerMinute||0)),title=src.isHq?`🚩 ШТАБ · ${name}`:(operation?`${src.operationIcon||'🏴'} ${operation} · $${income}/МИН`:`🚩 ВЛАДЕНИЕ · ${name}`);g.clearRect(0,0,c.width,c.height);g.fillStyle='rgba(7,9,13,.96)';g.strokeStyle=color;g.lineWidth=8;g.fillRect(8,8,c.width-16,c.height-16);g.strokeRect(8,8,c.width-16,c.height-16);g.fillStyle=accent;g.font=`900 ${src.isHq?31:27}px Arial`;g.textAlign='center';g.textBaseline='middle';g.fillText(title,c.width/2,c.height/2,470);marker.texture.needsUpdate=true;};
      ...
      const empireHqRoofYAt=(r,c)=>{let bestY=.15,bestD=14;for(const object of buildingPickables){const m=object.userData?.building;if(!m)continue;const inside=Number.isFinite(+m.minR)?r>=+m.minR-.7&&r<=+m.maxR+1.2&&c>=+m.minC-.7&&c<=+m.maxC+1.2:false,d=inside?0:Math.hypot((+m.r||0)-r,(+m.c||0)-c);if(d>bestD)continue;object.updateWorldMatrix(true,false);empireHqRoofBox.setFromObject(object);const top=Number.isFinite(empireHqRoofBox.max.y)?empireHqRoofBox.max.y:.15;if(inside||d<bestD){bestD=d;bestY=Math.max(.15,top);}}return bestY;};
      for(let i=0;i<EMPIRE_FLAG_CAP;i++){
        ...
        const pole=new THREE.Mesh(empireHqPoleGeometry,new THREE.MeshStandardMaterial({color:0xc9b78c,roughness:.45,metalness:.68}));pole.position.y=3.05;pole.renderOrder=17;root.add(pole);const flagCanvas=document.createElement('canvas');flagCanvas.width=256;flagCanvas.height=128;const flagContext=flagCanvas.getContext('2d'),flagTexture=new THREE.CanvasTexture(flagCanvas);flagTexture.colorSpace=THREE.SRGBColorSpace;flagTexture.generateMipmaps=false;flagTexture.minFilter=THREE.LinearFilter;const flagMaterial=new THREE.MeshBasicMaterial({color:0xffffff,map:flagTexture,side:THREE.DoubleSide,depthTest:false,depthWrite:false,toneMapped:false}),flag=new THREE.Mesh(empireHqFlagGeometry,flagMaterial),flagPivot=new THREE.Group();flag.position.x=1.78;flag.renderOrder=22;flagPivot.position.y=5.05;flagPivot.add(flag);root.add(flagPivot);
        const canvas=document.createElement('canvas');canvas.width=512;canvas.height=96;const context=canvas.getContext('2d'),texture=new THREE.CanvasTexture(canvas);texture.colorSpace=THREE.SRGBColorSpace;texture.generateMipmaps=false;const labelMaterial=new THREE.SpriteMaterial({map:texture,transparent:true,depthTest:false,depthWrite:false,toneMapped:false}),label=new THREE.Sprite(labelMaterial);label.position.y=7.25;label.scale.set(10.2,1.92,1);label.renderOrder=24;root.add(label);
        // Fixed, bounded roof facade kit. The operation changes visibility and
        // materials only when the server signature changes; no meshes are
        // allocated while the player moves through the city.
        const facade=new THREE.Group(),facadeMat=new THREE.MeshStandardMaterial({color:0x537a48,roughness:.38,metalness:.28,emissive:0x172619,emissiveIntensity:.35}),facadeDark=new THREE.MeshStandardMaterial({color:0x11151b,roughness:.5,metalness:.54}),facadeGlow=new THREE.MeshBasicMaterial({color:0x79e69b,toneMapped:false}),facadeBase=new THREE.Mesh(new THREE.BoxGeometry(5.8,.52,2.05),facadeDark),facadeBand=new THREE.Mesh(new THREE.BoxGeometry(5.2,.22,2.24),facadeGlow),facadeFront=new THREE.Mesh(new THREE.BoxGeometry(5.8,4.4,.22),facadeMat),facadeAwning=new THREE.Mesh(new THREE.BoxGeometry(6.45,.22,1.45),facadeGlow);facadeBase.position.y=.3;facadeBand.position.y=.68;facadeFront.position.set(0,-2.05,1.08);facadeAwning.position.set(0,-.48,1.62);facade.add(facadeBase,facadeBand,facadeFront,facadeAwning);for(const x of [-1.65,1.65]){const window=new THREE.Mesh(new THREE.PlaneGeometry(1.65,1.55),facadeDark);window.position.set(x,-2.15,1.205);facade.add(window);}
        const profileMeshes={};
        const beer=new THREE.Mesh(new THREE.CylinderGeometry(.78,.92,1.55,14),facadeMat);beer.position.y=1.48;profileMeshes.beer_bar=beer;
        const pawn=new THREE.Mesh(new THREE.OctahedronGeometry(1.05,0),facadeMat);pawn.position.y=1.62;profileMeshes.pawnshop=pawn;
        const book=new THREE.Mesh(new THREE.BoxGeometry(2.9,1.55,.62),facadeMat);book.position.y=1.48;profileMeshes.bookmaker=book;
        const strip=new THREE.Mesh(new THREE.TorusGeometry(1.08,.22,10,28),facadeGlow);strip.position.y=1.65;strip.rotation.x=Math.PI/2;profileMeshes.strip_club=strip;
        const gun=new THREE.Group(),gunBody=new THREE.Mesh(new THREE.BoxGeometry(2.6,.55,.55),facadeMat),gunBarrel=new THREE.Mesh(new THREE.BoxGeometry(2.2,.18,.18),facadeGlow);gunBody.position.y=1.48;gunBarrel.position.set(1.65,1.58,0);gun.add(gunBody,gunBarrel);profileMeshes.gun_shop=gun;
        const chop=new THREE.Mesh(new THREE.TorusGeometry(1.02,.34,8,16),facadeMat);chop.position.y=1.55;chop.rotation.x=Math.PI/2;profileMeshes.chop_shop=chop;
        const poker=new THREE.Mesh(new THREE.CylinderGeometry(1.1,1.1,.34,24),facadeMat);poker.position.y=1.45;poker.rotation.z=Math.PI/2;profileMeshes.poker_club=poker;
        const press=new THREE.Group(),pressTop=new THREE.Mesh(new THREE.BoxGeometry(2.7,.55,1.55),facadeMat),pressLeg=new THREE.Mesh(new THREE.BoxGeometry(.55,1.5,1.2),facadeDark);pressTop.position.y=2;pressLeg.position.y=1.15;press.add(pressTop,pressLeg);profileMeshes.print_shop=press;
        for(const mesh of Object.values(profileMeshes)){mesh.visible=false;facade.add(mesh);}const hqCrown=new THREE.Mesh(new THREE.ConeGeometry(1.45,1.8,5),facadeMat);hqCrown.position.y=1.65;hqCrown.visible=false;facade.add(hqCrown);facade.visible=false;root.add(facade);
        root.visible=false;scene.add(root);empireHqMarkers.push({root,ring,ringMaterial,pole,flagPivot,flag,flagCanvas,flagContext,flagTexture,flagMaterial,canvas,context,texture,label,facade,facadeMat,facadeGlow,profileMeshes,hqCrown,signature:''});
      }
      for(let i=0;i<EMPIRE_HOSPITAL_CAP;i++){
        const canvas=document.createElement('canvas');canvas.width=512;canvas.height=128;const context=canvas.getContext('2d'),texture=new THREE.CanvasTexture(canvas);texture.colorSpace=THREE.SRGBColorSpace;texture.generateMipmaps=false;texture.minFilter=THREE.LinearFilter;
        const sprite=new THREE.Sprite(new THREE.SpriteMaterial({map:texture,transparent:true,depthWrite:false,toneMapped:false}));sprite.scale.set(11.2,2.8,1);sprite.renderOrder=30;sprite.visible=false;scene.add(sprite);empireHospitalMarkers.push({sprite,canvas,context,texture,signature:''});
      }
      const redrawEmpireHospitalMarker=(marker,src)=>{const g=marker.context,w=marker.canvas.width,h=marker.canvas.height;g.clearRect(0,0,w,h);g.fillStyle='rgba(8,10,13,.94)';g.strokeStyle=src.accent||'#e8bd67';g.lineWidth=7;g.beginPath();g.roundRect(5,5,w-10,h-10,20);g.fill();g.stroke();g.fillStyle=src.color||'#872f3b';g.fillRect(18,18,90,h-36);g.fillStyle='#fff';g.font='700 43px sans-serif';g.textAlign='center';g.fillText(src.emblem||'♛',63,82);g.textAlign='left';g.fillStyle='#fff1c8';g.font='700 28px sans-serif';g.fillText(String(src.leaderName||'БОСС').toUpperCase().slice(0,22),125,49);g.fillStyle='#f2c64d';g.font='700 24px sans-serif';g.fillText(`🏥 ЛЕЧЕНИЕ · ${Math.max(0,+src.seconds||0)} С`,125,88);marker.texture.needsUpdate=true;};
      ...
      // Generic police, guard and gang weapons reuse the same semantic pools as
      // boss signature guns. This replaces the old single cuboid fallback
      // without adding meshes, draw calls or role-specific render passes.
      const npcGenericWeaponShapes=Object.freeze({
        pistol:{shape:'pistol',primary:0x232a31,accent:0xaeb8c0,body:[.66,.58,.72],barrel:.52,stock:0},
        heavy:{shape:'heavy-pistol',primary:0x2d3035,accent:0xd0a35d,body:[.78,.68,.86],barrel:.72,stock:.18,scope:.24},
        revolver:{shape:'revolver',primary:0x30343a,accent:0xd0ad69,body:[.7,.68,.62],barrel:.68,stock:.2,drum:.48},
        shotgun:{shape:'shotgun',primary:0x493426,accent:0xc0a36e,body:[.72,.64,1.22],barrel:1.72,stock:1.18,limbs:.34},
        smg:{shape:'smg',primary:0x242b32,accent:0x82919b,body:[.82,.72,.92],barrel:.9,stock:.65,drum:.38},
        tommy:{shape:'tommy',primary:0x4b3525,accent:0xc59b55,body:[.92,.78,1.08],barrel:1.2,stock:1.05,drum:.72},
        rifle:{shape:'rifle',primary:0x313b31,accent:0xa78a58,body:[.72,.66,1.22],barrel:1.8,stock:1.2,scope:.36},
        sniper:{shape:'sniper',primary:0x283529,accent:0x8fa96f,body:[.68,.62,1.34],barrel:2.15,stock:1.3,scope:.9},
        taser:{shape:'taser',primary:0x263c58,accent:0xe6c64f,body:[.7,.64,.75],barrel:.48,stock:.15,limbs:.26},
        rpg:{shape:'rpg',primary:0x394335,accent:0xb8a563,body:[.9,.82,1.35],barrel:1.75,stock:.72,scope:.42},
        melee:{shape:'bat',primary:0x6a442b,accent:0xc2ccd1,body:[.58,.58,1.65],barrel:0,stock:0,melee:true}
      });
      const npcGenericWeaponKey=value=>{const weapon=String(value||'').toLowerCase();if(/rpg|rocket/.test(weapon))return'rpg';if(/sniper/.test(weapon))return'sniper';if(/rifle/.test(weapon))return'rifle';if(/shotgun/.test(weapon))return'shotgun';if(/golden_tommy|tommy/.test(weapon))return'tommy';if(/smg|uzi|ump|mp5/.test(weapon))return'smg';if(/taser/.test(weapon))return'taser';if(/revolver|nagan/.test(weapon))return'revolver';if(/heavy|deagle|gold/.test(weapon))return'heavy';if(/bat|club|melee|fist/.test(weapon))return'melee';return'pistol';};
      ...
      const shellPool=makeInstances(new THREE.CylinderGeometry(.055,.055,.28,7),new THREE.MeshStandardMaterial({color:0xffffff,vertexColors:true,metalness:.9,roughness:.23}),30,false);
      ...
      const renderNpcEmpireWeapon=(src,i,root,pose,severMask,t)=>{
        ...
        const colorSig=`${visualKey}:${cfg.primary}:${cfg.accent}`;if(npcEmpireWeaponColorSignatures[i]===colorSig)return false;npcEmpireWeaponColorSignatures[i]=colorSig;
      ...
      const decorateVacantBuildingInterior=data=>{
        const property=data.apartment||{};if(!(data.kind==='building'&&data.type==='generic'&&!data.bizId&&!property.propertyKind))return false;
        const W=data.width*WORLD_SCALE,H=data.height*WORLD_SCALE,cx=(data.width/2-originC)*WORLD_SCALE,cz=(data.height/2-originR)*WORLD_SCALE,add=(geo,mat,x,y,z)=>{const mesh=new THREE.Mesh(geo,mat);mesh.position.set(x,y,z);mesh.castShadow=mesh.receiveShadow=true;mesh.layers.set(1);interiorGroup.add(mesh);return mesh;},concrete=new THREE.MeshStandardMaterial({color:0x575a59,roughness:.96}),plaster=new THREE.MeshStandardMaterial({color:0x8b877e,roughness:1}),dust=new THREE.MeshStandardMaterial({color:0x79674d,roughness:1});
        const technicalGrid=interiorGroup.children.find(o=>o.type==='GridHelper');if(technicalGrid){interiorGroup.remove(technicalGrid);technicalGrid.geometry?.dispose?.();technicalGrid.material?.dispose?.();}if(interiorFloor){const old=interiorFloor.material;interiorFloor.material=concrete;old?.dispose?.();}
        for(const [x,z,w,d] of [[cx,cz-H*.38,W*.84,.3],[cx-W*.42,cz,.3,H*.76],[cx+W*.42,cz,.3,H*.76]])add(new THREE.BoxGeometry(w,4.5,d),plaster,x,2.25,z);for(const [x,z,w,d] of [[cx-W*.28,cz-H*.24,4.2,2.8],[cx+W*.25,cz+H*.18,5.2,3.2]]){add(new THREE.BoxGeometry(w,.28,d),dust,x,.15,z);for(let i=0;i<3;i++){const crate=add(new THREE.BoxGeometry(1.25,.85,1.1),dust,x-w*.28+i*1.3,.58,z);crate.rotation.y=(i-1)*.13;}}
        const sign=apartmentLabelSprite('СВОБОДНОЕ ЗДАНИЕ','После покупки выбери назначение','#e5c66b');sign.position.set(cx,6.1,cz-H*.365+.25);sign.layers.set(1);interiorGroup.add(sign);renderer.domElement.dataset.vacantBuildingInterior='empty-shell-v1';return true;
      };
      const decorateConvertedBuildingInterior=data=>{
        const property=data.apartment||{},kind=String(property.propertyKind||''),operation=String(property.operationType||'');
        ...
        if(interiorFloor){const old=interiorFloor.material;interiorFloor.material=new THREE.MeshStandardMaterial({color:kind==='hq'?0x19131a:0x1a1e22,roughness:.72});old?.dispose?.();}
        floorAt(cx,cz,W*.9,H*.16,theme,.09);for(const x of [cx-W*.42,cx+W*.42])boxAt(x,cz,.25,H*.75,4.6,dark,2.3);boxAt(cx,cz-H*.38,W*.84,.25,4.6,dark,2.3);
        for(let i=-3;i<=3;i++)boxAt(cx+i*W*.105,cz-H*.375,W*.075,.12,.2,gold,.42);
        const title=kind==='hq'?'ШТАБ МАФИИ':String(property.operationName||'КРИМИНАЛЬНЫЙ БИЗНЕС').toUpperCase(),subtitle=kind==='hq'?(property.gangName||'ЦЕНТР УПРАВЛЕНИЯ'):`${property.operationIcon||'🏴'} НАЗНАЧЕНИЕ ЗДАНИЯ`,sign=apartmentLabelSprite(title,subtitle,color);sign.position.set(cx,6.5,cz-H*.365+.22);sign.layers.set(1);interiorGroup.add(sign);
        ...
        if(kind==='hq'){
          floorAt(cx,cz+H*.08,W*.5,H*.42,new THREE.MeshStandardMaterial({color:0x5a1822,roughness:.96}),.1);boxAt(cx,cz-H*.22,7.5,3.1,1.05,wood,.55);addChair(cx,cz-H*.17);for(const dx of [-W*.28,W*.28]){boxAt(cx+dx,cz+H*.18,5.8,2.5,.72,leather,.46);boxAt(cx+dx,cz+H*.18-1.05,5.8,.45,1.55,leather,1.08);}boxAt(cx,cz+H*.12,10,4.1,.72,wood,.48);for(const dx of [-4,-1.35,1.35,4]){addChair(cx+dx,cz+H*.17+2.6,Math.PI);addChair(cx+dx,cz+H*.17-2.6,0);}const safe=boxAt(cx+W*.34,cz-H*.29,3.2,2.5,3.6,steel,1.8);outline(safe);for(const x of [cx-W*.3,cx,cx+W*.3]){const banner=add(new THREE.PlaneGeometry(3.4,4.2),theme,x,3.1,cz-H*.365+.18);banner.castShadow=false;}for(let i=0;i<5;i++){const weapon=boxAt(cx-W*.35+i*1.55,cz-H*.3,1.25,.18,.18,steel,2.1+i%2*.7);weapon.rotation.z=(i-2)*.06;}
        }else if(operation==='beer_bar'){
          boxAt(cx,cz-H*.24,W*.62,2.7,1.25,wood,.68);for(let i=-4;i<=4;i++){add(new THREE.CylinderGeometry(.1,.14,.8,9),gold,cx+i*1.25,1.48,cz-H*.19);if(i%2===0)add(new THREE.CylinderGeometry(.18,.24,.65,10),glass,cx+i*1.25,1.85,cz-H*.19);}for(const x of [cx-W*.32,cx, cx+W*.32]){add(new THREE.CylinderGeometry(1.05,1.15,1.55,14),wood,x,.78,cz+H*.18);for(let a=0;a<4;a++){const ang=a*Math.PI/2;addChair(x+Math.cos(ang)*2.25,cz+H*.18+Math.sin(ang)*2.25,-ang);}}
        }else if(operation==='pawnshop'){
          for(const z of [cz-H*.18,cz+H*.12]){boxAt(cx,z,W*.62,2.2,1.1,dark,.58);const top=boxAt(cx,z,W*.59,1.8,.8,glass,1.4);top.castShadow=false;}for(let i=-4;i<=4;i++){const gem=add(new THREE.OctahedronGeometry(.38+(i%2)*.12,0),i%2?theme:gold,cx+i*1.45,1.75,cz-H*.18);gem.rotation.y=i*.5;}for(const x of [cx-W*.32,cx+W*.32])boxAt(x,cz-H*.3,3.2,1.7,4.2,steel,2.1);
        }else if(operation==='bookmaker'){
          for(let row=0;row<2;row++)for(let i=-3;i<=3;i++){const x=cx+i*3,z=cz-H*.25+row*3.4;boxAt(x,z,2.4,1.3,1.35,dark,.68);addScreen(x,1.75,z-.68,2.05,1.05);}for(const [x,z] of [[cx-W*.25,cz+H*.18],[cx,cz+H*.18],[cx+W*.25,cz+H*.18]]){boxAt(x,z,4.2,2.1,1.05,theme,.55);addScreen(x,2.15,z-1.08,3.4,1.65);}
        }else if(operation==='strip_club'){
          const stage=floorAt(cx,cz-H*.18,W*.48,H*.3,theme,.42);for(const x of [cx-W*.15,cx,cx+W*.15])add(new THREE.CylinderGeometry(.055,.055,5.7,10),steel,x,3.15,cz-H*.18);for(const x of [cx-W*.31,cx+W*.31]){boxAt(x,cz+H*.18,6.4,2.6,.72,leather,.48);boxAt(x,cz+H*.18-1.08,6.4,.4,1.5,leather,1.05);}for(let i=0;i<12;i++){const a=i*Math.PI/6,bulb=add(new THREE.SphereGeometry(.16,9,7),i%2?glow:gold,cx+Math.cos(a)*W*.25,2.1+Math.sin(a*2)*.35,cz-H*.18+Math.sin(a)*H*.2);bulb.castShadow=false;}
        }else if(operation==='gun_shop'){
          for(const z of [cz-H*.22,cz+H*.12]){boxAt(cx,z,W*.62,2.3,1.05,dark,.55);boxAt(cx,z,W*.58,1.8,.58,glass,1.28);}for(let row=0;row<3;row++)for(let i=-4;i<=4;i++){const weapon=boxAt(cx+i*1.55,cz-H*.355,1.2,.16,.16,row%2?gold:steel,1.65+row*.82);weapon.rotation.z=(i%3-1)*.08;}for(const x of [cx-W*.34,cx+W*.34])for(let k=0;k<3;k++)boxAt(x,cz+H*.26+k*1.45,2.2,1.15,.85,wood,.44);
        }else if(operation==='chop_shop'){
          for(const x of [cx-W*.22,cx+W*.22]){boxAt(x,cz,1.1,H*.55,.34,theme,.18);for(const z of [cz-H*.2,cz+H*.2])boxAt(x,z,2.4,1.2,1.35,steel,.68);}const car=boxAt(cx,cz,8.5,3.8,1.25,new THREE.MeshStandardMaterial({color:0x395f78,metalness:.55,roughness:.3}),2.15);for(const dx of [-3,3])for(const dz of [-1.75,1.75]){const tire=add(new THREE.TorusGeometry(.62,.25,9,18),black,cx+dx,1.82,cz+dz);tire.rotation.y=Math.PI/2;}boxAt(cx-W*.34,cz-H*.27,6,2.3,1.2,wood,.62);for(let i=0;i<7;i++)boxAt(cx-W*.41+i*.65,cz-H*.32,.18,.18,1.5,i%2?steel:gold,1.9);
        }else if(operation==='poker_club'){
          floorAt(cx,cz,W*.7,H*.62,new THREE.MeshStandardMaterial({color:0x173e2b,roughness:.92}),.1);for(const [x,z] of [[cx-W*.24,cz-H*.16],[cx+W*.24,cz-H*.16],[cx-W*.24,cz+H*.2],[cx+W*.24,cz+H*.2]]){add(new THREE.CylinderGeometry(2.55,2.55,.42,8),theme,x,.72,z);for(let a=0;a<6;a++){const ang=a*Math.PI/3;addChair(x+Math.cos(ang)*3.4,z+Math.sin(ang)*3.4,-ang+Math.PI/2);}for(let chip=0;chip<9;chip++)add(new THREE.CylinderGeometry(.18,.18,.12,12),chip%2?gold:glow,x+(chip%3-1)*.42,.99,z+(Math.floor(chip/3)-1)*.34);}
        }else{
        ...
        const warm=new THREE.PointLight(new THREE.Color(color),kind==='hq'?18:13,32,2);warm.position.set(cx,6.2,cz);warm.layers.set(1);interiorGroup.add(warm);renderer.domElement.dataset.convertedBuildingInterior=`purpose-v1:${kind}:${operation||'headquarters'}`;return true;
      };
      const decorateApartmentInterior=data=>{
        ...
        const ownerTitle=apt.owned?'МОЁ СВОБОДНОЕ ЗДАНИЕ':'СВОБОДНОЕ ЗДАНИЕ',ownerDetail=apt.owned?(apt.district||'Городской район'):apt.ownerName?`Владелец: ${apt.ownerName}`:(apt.district||'Городской район'),ownerColor=apt.owned?'#8dffae':apt.ownerName?'#7ed8ff':'#e5c66b',ownerSign=apartmentLabelSprite(ownerTitle,ownerDetail,ownerColor);ownerSign.position.set(cx,6.2,northZ+.68);ownerSign.layers.set(1);interiorGroup.add(ownerSign);
      ...
      const animate = t => {
        ...
        if(dynamic){
          ...
          let crawlingNpcCount=0,limpingNpcCount=0,reactingNpcCount=0,bleedingNpcCount=0,panickingNpcCount=0,coweringNpcCount=0,surrenderingNpcCount=0,helpingNpcCount=0,socialNpcCount=0,alertedNpcCount=0,ambientNpcCount=0,firingPoliceCount=0,firingGuardCount=0,walkingArmNpcCount=0,blinkingNpcCount=0,maxWalkingNpcBob=0,npcEmpireWeaponColorsDirty=false;
          citizenPool.forEach((npc,i)=>{
            ...
            if(pose.crawling)crawlingNpcCount++;else if(pose.limping)limpingNpcCount++;if(pose.hit>0)reactingNpcCount++;if(pose.walking){maxWalkingNpcBob=Math.max(maxWalkingNpcBob,pose.bob);if(!pose.firing)walkingArmNpcCount++;}if(pose.firing&&police)firingPoliceCount++;if(pose.firing&&!police&&(src.visualRole==='guard'||role.includes('guard')))firingGuardCount++;if(pose.panicking)panickingNpcCount++;else if(pose.cowering)coweringNpcCount++;else if(pose.surrendering)surrenderingNpcCount++;else if(pose.helping)helpingNpcCount++;else if(pose.talking)socialNpcCount++;else if(pose.alerted)alertedNpcCount++;else if(pose.ambientKind)ambientNpcCount++;
            ...
            setPart(npcParts.neck,i,rootMatrix,0,2.88-pose.crouch-medicCrouch,0,0,instanceScale.set(.9*faceX,1,.9*faceZ),pose.headCounter*.42,pose.headTilt*.36);
            setPart(npcParts.coatSeam,i,rootMatrix,0,2.08-pose.crouch-medicCrouch,.515*bodyProfile.bodyZ*depthScale,0,instanceScale.set(1,1,1),pose.torsoTwist,pose.shoulderSway);
            ...
            // Two-bone legs keep the feet planted while the knee absorbs the
            // stride. All offsets reuse frame scratch vectors: no per-NPC GC.
            const legX=.34*hipWidth,hipY=1.38-pose.crouch*.16-medicCrouch*.16;
            for(const side of [-1,1]){
              const left=side<0,partIndex=i*2+(left?0:1),lift=left?pose.leftLift:pose.rightLift,hipPitch=(left?pose.leftSwing+pose.legBend:pose.rightSwing-pose.legBend)+(left?1:-1)*medicCrouch*.52,kneeBend=left?pose.leftKneeBend:pose.rightKneeBend,shinPitch=hipPitch-kneeBend,footPitch=left?pose.leftFootPitch:pose.rightFootPitch,x=side*legX;
              npcLegQuat.setFromEuler(instanceEuler.set(hipPitch,0,0));npcThighOffset.set(0,-.31,0).applyQuaternion(npcLegQuat);npcKneeOffset.set(0,-.62,0).applyQuaternion(npcLegQuat);
              setPart(left?npcParts.leftLeg:npcParts.rightLeg,i,rootMatrix,x+npcThighOffset.x,hipY+lift+npcThighOffset.y,npcThighOffset.z,hipPitch,instanceScale.set(legScale,1,legScale));
              npcLegQuat.setFromEuler(instanceEuler.set(shinPitch,0,0));npcShinOffset.set(0,-.27,0).applyQuaternion(npcLegQuat);npcAnkleOffset.set(0,-.54,0).applyQuaternion(npcLegQuat);
              setPart(npcParts.shin,partIndex,rootMatrix,x+npcKneeOffset.x+npcShinOffset.x,hipY+lift+npcKneeOffset.y+npcShinOffset.y,npcKneeOffset.z+npcShinOffset.z,shinPitch,instanceScale.set(legScale,1,legScale));
              const footX=x+npcKneeOffset.x+npcAnkleOffset.x,footY=Math.max(.09,hipY+lift+npcKneeOffset.y+npcAnkleOffset.y),footZ=.18+npcKneeOffset.z+npcAnkleOffset.z;
              setPart(npcParts.shoe,partIndex,rootMatrix,footX,footY,footZ,footPitch,instanceScale.set(legScale,1,legScale));
              setPart(npcParts.shoeSole,partIndex,rootMatrix,footX,footY-.115,footZ+.025,footPitch,instanceScale.set(legScale,1,legScale));
            }
            ...
            for(const side of [-1,1]){const pitch=side<0?leftPitch:rightPitch,yaw=side<0?leftYaw:rightYaw,roll=side<0?leftRoll:rightRoll,ax=side<0?leftArmX:rightArmX,handIndex=i*2+(side>0?1:0),elbowBend=pose.firing?.38:pose.phoneCalling?.5:medicReach?.42:pose.cowering?.32:pose.walking?.1+Math.abs(pitch)*.08:.08;npcLimbQuat.setFromEuler(instanceEuler.set(pitch,yaw,roll));npcElbowOffset.set(0,-.4,0).applyQuaternion(npcLimbQuat);npcLimbQuat.setFromEuler(instanceEuler.set(pitch+elbowBend,yaw,roll));npcForearmOffset.set(0,-.34,0).applyQuaternion(npcLimbQuat);npcLimbOffset.set(0,-.72,0).applyQuaternion(npcLimbQuat);const forearmX=ax+npcElbowOffset.x+npcForearmOffset.x,forearmY=medicArmY+npcElbowOffset.y+npcForearmOffset.y,forearmZ=medicArmZ+npcElbowOffset.z+npcForearmOffset.z;setPart(npcParts.forearm,handIndex,rootMatrix,forearmX,forearmY,forearmZ,pitch+elbowBend,instanceScale.set(armScale*.94,1,armScale*(female?.9:1)),yaw,roll);setPart(npcParts.cuff,handIndex,rootMatrix,forearmX+npcForearmOffset.x*.72,forearmY+npcForearmOffset.y*.72,forearmZ+npcForearmOffset.z*.72,pitch+elbowBend,instanceScale.set(armScale,1,armScale),yaw,roll);setPart(npcParts.hand,handIndex,rootMatrix,ax+npcElbowOffset.x+npcLimbOffset.x,medicArmY+npcElbowOffset.y+npcLimbOffset.y,medicArmZ+npcElbowOffset.z+npcLimbOffset.z,0,instanceScale.set(armScale,armScale,armScale));}
            ...
            if(severMask&1){hidePart(npcParts.leftArm,i);hidePart(npcParts.forearm,i*2);hidePart(npcParts.cuff,i*2);hidePart(npcParts.hand,i*2);}
            if(severMask&2){hidePart(npcParts.rightArm,i);hidePart(npcParts.forearm,i*2+1);hidePart(npcParts.cuff,i*2+1);hidePart(npcParts.hand,i*2+1);}
            if(severMask&4){hidePart(npcParts.leftLeg,i);hidePart(npcParts.shin,i*2);hidePart(npcParts.shoe,i*2);hidePart(npcParts.shoeSole,i*2);}
            if(severMask&8){hidePart(npcParts.rightLeg,i);hidePart(npcParts.shin,i*2+1);hidePart(npcParts.shoe,i*2+1);hidePart(npcParts.shoeSole,i*2+1);}
          ...
          // Keep the upstream anatomy-aware gang pose, then apply the purpose facade pass.
          let visibleGangCount=0,gangColorsDirty=false;for(let i=0;i<NPC_CAP;i++){const src=dynamic.npcs[i];if(!src){gangColorSignatures[i]='';hidePart(npcParts.gangAura,i);hidePart(npcParts.gangBand,i);continue;}const role=String(src.role||'').toLowerCase(),gang=!!src.empireBoss||src.visualRole==='gang'||!!src.gang||role.includes('gang')||role.includes('boss')||role.includes('district_')||role.includes('occupier');if(!gang||src.dead){gangColorSignatures[i]='';hidePart(npcParts.gangAura,i);hidePart(npcParts.gangBand,i);continue;}visibleGangCount++;const faction=String(src.faction||src.family||'').toLowerCase();let auraColor=faction.includes('yellow')?0xffd83d:faction.includes('purple')?0xa668ff:faction.includes('moretti')?0xf5ead2:faction.includes('bellini')?0x596274:0xff4f68,bandColor=auraColor;try{if((src.empireBoss||src.empireCrew)&&src.bossColor)auraColor=instanceColor.set(src.bossColor).getHex();if((src.empireBoss||src.empireCrew)&&src.bossAccent)bandColor=instanceColor.set(src.bossAccent).getHex();}catch(_){}const x=npcVisualXs[i],z=npcVisualZs[i],pose=npcFramePoses[i]||npcAnimationPose(src,i,t),look=src.look||{},bodyProfile=NPC_BODY_PROFILES[Math.abs(+look.body||0)%NPC_BODY_PROFILES.length],shoulderScale=+look.gender===1?.82:1,depthScale=+look.gender===1?.94:1,colorSig=`${String(src.id??src.uid??i)}:${auraColor}:${bandColor}`;setNpcRoot(pose,i,x,z);setPart(npcParts.gangBand,i,rootMatrix,0,2.25-pose.crouch-pose.hit*.08,.405*bodyProfile.bodyZ*depthScale,-pose.hit*.09,instanceScale.set(bodyProfile.bodyX*shoulderScale*.92,1,bodyProfile.bodyZ*depthScale),pose.torsoTwist,pose.shoulderSway);rootMatrix.makeRotationY(0);rootMatrix.setPosition(x,.11,z);setPart(npcParts.gangAura,i,rootMatrix,0,0,0,-Math.PI/2,src.empireBoss?instanceScale.set(1.16,1.16,1.16):gangAuraScale);if(gangColorSignatures[i]!==colorSig){gangColorSignatures[i]=colorSig;npcParts.gangBand.setColorAt(i,instanceColor.setHex(bandColor));npcParts.gangAura.setColorAt(i,instanceColor.setHex(auraColor));gangColorsDirty=true;}}npcParts.gangAura.instanceMatrix.needsUpdate=true;npcParts.gangBand.instanceMatrix.needsUpdate=true;if(gangColorsDirty){npcParts.gangAura.instanceColor.needsUpdate=true;npcParts.gangBand.instanceColor.needsUpdate=true;}if(telemetryDue)renderer.domElement.dataset.visibleGangs=String(visibleGangCount);const selectedIndex=performance.now()<selectedNpcUntil?dynamic.npcs.findIndex(n=>!n.dead&&String(n.sourceId||'')===selectedNpcSourceId):-1;if(selectedIndex>=0){selectedNpcRing.visible=true;selectedNpcRing.position.set(npcVisualXs[selectedIndex],.15,npcVisualZs[selectedIndex]);const pulse=1+Math.sin(t*.009)*.09;selectedNpcRing.scale.setScalar(pulse);selectedNpcRing.rotation.y=t*.0017;selectedNpcOuter.material.opacity=.82+Math.sin(t*.011)*.16;if(telemetryDue)renderer.domElement.dataset.selectedGangNpc=selectedNpcSourceId;}else{selectedNpcRing.visible=false;if(selectedNpcSourceId&&performance.now()>=selectedNpcUntil){selectedNpcSourceId='';renderer.domElement.dataset.selectedGangNpc='none';}}
          let visibleEmpireHqs=0,visibleEmpireFlags=0;for(let i=0;i<EMPIRE_FLAG_CAP;i++){const marker=empireHqMarkers[i],src=dynamic.empireFlags?.[i]||dynamic.empireHqs?.[i];marker.root.visible=!!src;if(!src){marker.signature='';marker.facade.visible=false;continue;}const isHq=src.isHq!==false;visibleEmpireFlags++;if(isHq)visibleEmpireHqs++;marker.root.position.set((+src.c-originC)*WORLD_SCALE,0,(+src.r-originR)*WORLD_SCALE);const sig=`${src.leaderId}:${src.holdingId||''}:${src.kind||'hq'}:${src.gangName}:${src.color}:${src.accent}:${src.operationType||''}:${src.operationName||''}:${src.incomePerMinute||0}:${src.r}:${src.c}`;if(marker.signature!==sig){marker.signature=sig;const roofY=empireHqRoofYAt(+src.r||0,+src.c||0),operation=String(src.operationType||''),visualColors={beer_bar:'#d89a38',pawnshop:'#35c6b0',bookmaker:'#4ab46f',strip_club:'#f04fae',gun_shop:'#d9574f',chop_shop:'#e47c32',poker_club:'#8e56d9',print_shop:'#4ba6d8'},facadeColor=visualColors[operation]||src.color||'#872f3b';marker.ring.visible=isHq;marker.pole.scale.y=isHq?1:.62;marker.pole.position.y=roofY+(isHq?3.05:1.9);marker.flagPivot.position.y=roofY+(isHq?5.05:3.2);marker.flagPivot.scale.setScalar(isHq?1:.68);marker.label.position.y=roofY+(isHq?7.25:4.55);marker.label.scale.set(isHq?10.2:7.2,isHq?1.92:1.34,1);marker.facade.visible=isHq||!!operation;marker.facade.position.y=roofY+.12;marker.facade.rotation.y=Math.PI*.25;marker.hqCrown.visible=isHq;for(const [key,mesh] of Object.entries(marker.profileMeshes))mesh.visible=!isHq&&key===operation;try{marker.ringMaterial.color.set(src.color||'#872f3b');marker.facadeMat.color.set(facadeColor);marker.facadeMat.emissive.set(facadeColor).multiplyScalar(.18);marker.facadeGlow.color.set(src.accent||facadeColor);}catch(_){marker.ringMaterial.color.setHex(0x872f3b);}redrawEmpireHqFlag(marker,src);redrawEmpireHqLabel(marker,src);}const pulse=1+Math.sin(t*.003+i*.67)*.055;if(isHq)marker.ring.scale.setScalar(pulse);marker.flagPivot.rotation.y=Math.atan2(camera.position.x-marker.root.position.x,camera.position.z-marker.root.position.z);marker.flag.scale.x=.96+Math.sin(t*.006+i)*.08;}if(telemetryDue){renderer.domElement.dataset.empireFlagInput=String(dynamic.empireFlags?.length||dynamic.empireHqs?.length||0);renderer.domElement.dataset.visibleEmpireFlags=String(visibleEmpireFlags);renderer.domElement.dataset.visibleEmpireHqs=String(visibleEmpireHqs);renderer.domElement.dataset.empireHqProfile='world-roof-visible-flags-v368-purpose-facades-v369';}
          let visibleHospitalPatients=0;for(let i=0;i<EMPIRE_HOSPITAL_CAP;i++){const marker=empireHospitalMarkers[i],src=dynamic.hospitalPatients?.[i];marker.sprite.visible=!!src;if(!src){marker.signature='';continue;}visibleHospitalPatients++;const stack=dynamic.hospitalPatients.filter((p,j)=>j<i&&p.hospitalId===src.hospitalId).length,roofY=empireHqRoofYAt(+src.r||0,+src.c||0);marker.sprite.position.set((+src.c-originC)*WORLD_SCALE,roofY+5.3+stack*2.25,(+src.r-originR)*WORLD_SCALE);const sig=`${src.leaderId}:${src.hospitalId}:${src.seconds}:${src.color}:${src.accent}`;if(marker.signature!==sig){marker.signature=sig;redrawEmpireHospitalMarker(marker,src);}}if(telemetryDue){renderer.domElement.dataset.visibleEmpireHospitalPatients=String(visibleHospitalPatients);renderer.domElement.dataset.empireHospitalProfile='pooled-roof-icons-v363';}
          for(let i=0;i<Math.min(EMPIRE_FLAG_CAP,(dynamic.empireFlags||[]).length);i++){const src=dynamic.empireFlags[i],marker=empireHqMarkers[i];if(src&&src.isHq===false)marker.label.scale.set(9.7,1.92,1);}
          ...
          for(let i=0;i<Math.min(NPC_CAP,dynamic.npcs.length);i++){
            ...
            if(npcAppearanceSlotSignatures[i]!==slotSig){npcAppearanceSlotSignatures[i]=slotSig;npcColorBuffersDirty=true;
              for(const mesh of [npcParts.body,npcParts.leftArm,npcParts.rightArm,npcParts.roleHem])mesh.setColorAt(i,instanceColor.setHex(bodyColor));npcParts.coatSeam.setColorAt(i,instanceColor.setHex(bodyColor).multiplyScalar(.58));npcParts.forearm.setColorAt(i*2,instanceColor.setHex(bodyColor));npcParts.forearm.setColorAt(i*2+1,instanceColor.setHex(bodyColor));npcParts.cuff.setColorAt(i*2,instanceColor.setHex(bodyColor));npcParts.cuff.setColorAt(i*2+1,instanceColor.setHex(bodyColor));
              for(const mesh of [npcParts.femaleHips,npcParts.leftLeg,npcParts.rightLeg])mesh.setColorAt(i,instanceColor.setHex(legColor));npcParts.shin.setColorAt(i*2,instanceColor.setHex(legColor));npcParts.shin.setColorAt(i*2+1,instanceColor.setHex(legColor));
              for(const mesh of [npcParts.head,npcParts.jaw,npcParts.nose,npcParts.neck])mesh.setColorAt(i,instanceColor.setHex(skinColor));npcParts.hand.setColorAt(i*2,instanceColor.setHex(skinColor));npcParts.hand.setColorAt(i*2+1,instanceColor.setHex(skinColor));npcParts.ear.setColorAt(i*2,instanceColor.setHex(skinColor));npcParts.ear.setColorAt(i*2+1,instanceColor.setHex(skinColor));
            ...
            if(businessOwner){
              ...
              setPart(npcParts.ownerLapel,i*2,rootMatrix,-.23,2.27-pose.crouch-pose.hit*.08,.405,-pose.hit*.09,unitScale,pose.torsoTwist,pose.shoulderSway-.28);setPart(npcParts.ownerLapel,i*2+1,rootMatrix,.23,2.27-pose.crouch-pose.hit*.08,.405,-pose.hit*.09,unitScale,pose.torsoTwist,pose.shoulderSway+.28);setPart(npcParts.ownerPocket,i,rootMatrix,.34,2.18-pose.crouch-pose.hit*.08,.43,-pose.hit*.09,unitScale,pose.torsoTwist,pose.shoulderSway);setPart(npcParts.neckAccent,i,rootMatrix,0,2.42-pose.crouch-pose.hit*.08,.43,-pose.hit*.09,unitScale,pose.torsoTwist,pose.shoulderSway);
            }
            if(empireBoss){setPart(npcParts.ownerLapel,i*2,rootMatrix,-.27,2.3-pose.crouch-pose.hit*.08,.415,-pose.hit*.09,instanceScale.set(1.12,1.12,1.08),pose.torsoTwist,pose.shoulderSway-.32);setPart(npcParts.ownerLapel,i*2+1,rootMatrix,.27,2.3-pose.crouch-pose.hit*.08,.415,-pose.hit*.09,instanceScale.set(1.12,1.12,1.08),pose.torsoTwist,pose.shoulderSway+.32);setPart(npcParts.ownerPocket,i,rootMatrix,.35,2.16-pose.crouch-pose.hit*.08,.44,-pose.hit*.09,instanceScale.set(1+(bossStyle%3)*.12,1,1),pose.torsoTwist,pose.shoulderSway);}
            ...
            if(prisonStaff||police||securityGuard)setPart(npcParts.prisonVest,i,rootMatrix,0,2.02-pose.crouch-pose.hit*.08,.02,-pose.hit*.09,instanceScale.set(bodyProfile.bodyX*shoulderScale*.9,1,bodyProfile.bodyZ*depthScale*1.42),pose.torsoTwist,pose.shoulderSway);
            if(securityGuard||police)setPart(npcParts.securityBadge,i,rootMatrix,.31*bodyProfile.bodyX*shoulderScale,2.35-pose.crouch-pose.hit*.08,.62*bodyProfile.bodyZ*depthScale,-pose.hit*.09,unitScale,pose.torsoTwist,pose.shoulderSway);
            if(police){setPart(npcParts.policeBelt,i,rootMatrix,0,1.43-pose.crouch-pose.hit*.05,.01,-pose.hit*.06,instanceScale.set(bodyProfile.bodyX*shoulderScale*.9,1,bodyProfile.bodyZ*depthScale*1.1),pose.torsoTwist,pose.shoulderSway);setPart(npcParts.policeRadio,i,rootMatrix,-.39*bodyProfile.bodyX*shoulderScale,2.48-pose.crouch-pose.hit*.08,.62*bodyProfile.bodyZ*depthScale,-pose.hit*.09,unitScale,pose.torsoTwist,pose.shoulderSway);setPart(npcParts.policePatch,i*2,rootMatrix,-shoulderX*.87,2.55-pose.crouch,.02,0,unitScale,pose.torsoTwist,pose.shoulderSway-Math.PI/2);setPart(npcParts.policePatch,i*2+1,rootMatrix,shoulderX*.87,2.55-pose.crouch,.02,0,unitScale,pose.torsoTwist,pose.shoulderSway+Math.PI/2);}
            ...
            if(chainOn)setPart(npcParts.chain,i,rootMatrix,0,2.56-pose.crouch-pose.hit*.08,.54*bodyProfile.bodyZ*depthScale,-pose.hit*.09,instanceScale.set(Math.min(1.22,bodyProfile.shoulder*shoulderScale),1,1),pose.torsoTwist,pose.shoulderSway+Math.PI);
            if(neckOn&&!businessOwner)setPart(npcParts.neckAccent,i,rootMatrix,0,2.42-pose.crouch-pose.hit*.08,.42*bodyProfile.bodyZ*depthScale,-pose.hit*.09,unitScale,pose.torsoTwist,pose.shoulderSway);
            if(bagOn)setPart(npcParts.bag,i,rootMatrix,.62*bodyProfile.bodyX*shoulderScale,1.78-pose.crouch-pose.hit*.08,-.28*bodyProfile.bodyZ*depthScale,.06-pose.hit*.09,unitScale,pose.torsoTwist,pose.shoulderSway);
          ...
          if(telemetryDue){renderer.domElement.dataset.npcAppearanceVariants=String(npcAppearanceSignatures.size);renderer.domElement.dataset.npcAppearanceSystem='creator-grade-shared-instanced-rig-v370';renderer.domElement.dataset.npcDetailProfile='profiled-torso-neck-seam-cuffs-soles-jaw-beard-tailoring-blink-gaze';renderer.domElement.dataset.npcJointProfile='articulated-forearms-and-knees-v370';renderer.domElement.dataset.npcWeaponProfile='semantic-class-silhouettes-shared-pools-v370';renderer.domElement.dataset.npcMicroExpressions='deterministic-blink-and-eye-saccade-no-allocation';renderer.domElement.dataset.jailStaffGear='guards:military-helmet-riot-shield;police:cap-tactical-helmet-bare';renderer.domElement.dataset.policeResponseTiers=String(dynamic.npcs.reduce((m,n)=>Math.max(m,+n.responseTier||0),0));renderer.domElement.dataset.jailStaffMaxHp='200';}
          ...
          for(let i=0;i<Math.min(NPC_CAP,dynamic.npcs.length);i++){
            ...
            setPart(npcParts.coatSeam,i,rootMatrix,0,2.08,.51,0);
            setPart(npcParts.leftLeg,i,rootMatrix,-.34,1.03,0,-.18*fall);
            setPart(npcParts.rightLeg,i,rootMatrix,.34,1.03,0,.12*fall);
            setPart(npcParts.shin,i*2,rootMatrix,-.34,.45,.03,-.1*fall);
            setPart(npcParts.shin,i*2+1,rootMatrix,.34,.45,-.02,.08*fall);
            setPart(npcParts.shoe,i*2,rootMatrix,-.34,.09,.18,-.18*fall);
            setPart(npcParts.shoe,i*2+1,rootMatrix,.34,.09,.18,.12*fall);
            setPart(npcParts.shoeSole,i*2,rootMatrix,-.34,.035,.19,-.18*fall);
            setPart(npcParts.shoeSole,i*2+1,rootMatrix,.34,.035,.19,.12*fall);
            ...
            for(const part of ['shirtFront','collar','roleHem','jaw','beard','nose','mouth','femaleLashes','femaleLips','femaleHips'])hidePart(npcParts[part],i);for(const part of ['forearm','cuff','hand','brow','ear']){hidePart(npcParts[part],i*2);hidePart(npcParts[part],i*2+1);}hidePart(npcParts.hair,i);hidePart(npcParts.hairBun,i);hidePart(npcParts.hairMohawk,i);for(let curl=0;curl<4;curl++)hidePart(npcParts.hairCurls,i*4+curl);hidePart(npcParts.ownerHairSide,i);hidePart(npcParts.ownerHairBack,i);hidePart(npcParts.ownerHairQuiff,i);hidePart(npcParts.ownerLapel,i*2);hidePart(npcParts.ownerLapel,i*2+1);hidePart(npcParts.ownerPocket,i);hidePart(npcParts.hat,i);hidePart(npcParts.hatBrim,i);hidePart(npcParts.glasses,i);hidePart(npcParts.eyePatch,i);hidePart(npcParts.chain,i);hidePart(npcParts.neckAccent,i);hidePart(npcParts.bag,i);hidePart(npcParts.moustache,i);hidePart(npcParts.prisonHelmet,i);hidePart(npcParts.prisonHelmetBand,i);hidePart(npcParts.prisonVisor,i);hidePart(npcParts.prisonVest,i);hidePart(npcParts.securityBadge,i);hidePart(npcParts.policeBelt,i);hidePart(npcParts.policeRadio,i);hidePart(npcParts.policePatch,i*2);hidePart(npcParts.policePatch,i*2+1);hidePart(npcParts.riotShield,i);hidePart(npcParts.shieldViewport,i);
          ...
          let shellMatricesDirty=false,shellColorsDirty=false;for(let i=0;i<activeShellInstances;i++){const src=dynamic.shellFx[i],rot=+src.rot||0,weapon=npcGenericWeaponKey(src.weapon),shotgun=weapon==='shotgun',longCase=weapon==='rifle'||weapon==='sniper',wideCase=weapon==='heavy'||weapon==='revolver',shellScaleX=shotgun?1.34:wideCase?1.22:longCase?.86:1,shellScaleY=shotgun?1.18:longCase?1.62:wideCase?1.2:1,shellScaleZ=shellScaleX,shellColor=shotgun?0xb94332:longCase?0xd0a13d:wideCase?0xe0b64c:0xd6a638,matrixSig=`${src.id??i}:${src.r}:${src.c}:${src.z||0}:${rot}:${weapon}`;if(shellMatrixSignatures[i]!==matrixSig){shellMatrixSignatures[i]=matrixSig;instanceQuat.setFromEuler(instanceEuler.set(rot,rot*.7,rot*1.3));instanceMatrix.compose(instancePosition.set((src.c-originC)*WORLD_SCALE,.18+(+src.z||0)*WORLD_SCALE,(src.r-originR)*WORLD_SCALE),instanceQuat,instanceScale.set(shellScaleX,shellScaleY,shellScaleZ));shellPool.setMatrixAt(i,instanceMatrix);shellMatricesDirty=true;}const colorSig=`${weapon}:${shellColor}`;if(shellColorSignatures[i]!==colorSig){shellColorSignatures[i]=colorSig;shellPool.setColorAt(i,instanceColor.setHex(shellColor));shellColorsDirty=true;}}if(shellMatricesDirty)shellPool.instanceMatrix.needsUpdate=true;if(shellColorsDirty)shellPool.instanceColor.needsUpdate=true;if(telemetryDue){renderer.domElement.dataset.shellMatrixUpload=shellMatricesDirty?'dirty':'cached';renderer.domElement.dataset.shellMatrixProfile='weapon-class-brass-and-hulls-v370';renderer.domElement.dataset.shellVisualClasses='pistol-heavy-rifle-shotgun';}
        ...
        if(bridge){
          ...
          if(showInterior){
            ...
            const layoutSig=interiorData.layout?`${interiorData.layout.props?.map(p=>p.id).join(',')||''}:${interiorData.layout.safes?.map(s=>`${s.id}:${s.opened?1:0}`).join(',')||''}`:'',
              ...
              aptSig=interiorData.apartment?JSON.stringify({v:6,key:interiorData.apartment.key||'',owned:!!interiorData.apartment.owned,ownerUid:interiorData.apartment.ownerUid||'',ownerName:interiorData.apartment.ownerName||'',district:interiorData.apartment.district||'',propertyKind:interiorData.apartment.propertyKind||'',operationType:interiorData.apartment.operationType||'',operationName:interiorData.apartment.operationName||'',operationColor:interiorData.apartment.operationColor||'',gangName:interiorData.apartment.gangName||'',gangColor:interiorData.apartment.gangColor||'',gangAccent:interiorData.apartment.gangAccent||'',levels:interiorData.apartment.levels||{}}):'',
