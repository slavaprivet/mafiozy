...
#include <colorspace_fragment>
      ...
      for(const mesh of [npcParts.uniqueGunRail,npcParts.uniqueGunCharm])mesh.onBeforeRender=()=>{if(mesh.instanceColor)mesh.instanceColor.needsUpdate=true;};
      ...
      const animate = t => {
        ...
        if(dynamic){
          ...
          citizenPool.forEach((npc,i)=>{
            ...
            armed?setPart(npcParts.hat,i,rootMatrix,0,3.77-pose.crouch,0):hidePart(npcParts.hat,i);const lifeGestureDisarms=pose.phoneCalling||pose.surrendering||pose.cowering||pose.helping||pose.panicking||pose.talking||!!pose.ambientKind,showGun=armed&&!lifeGestureDisarms&&!(severMask&2),gunX=pose.firing?.5:.72,gunY=pose.firing?2.28-pose.recoil*.08:2-pose.crouch*.35,gunZ=pose.firing?.78:.52,gunPitch=pose.firing?-1.16-pose.recoil*.3:pose.hit*.18;showGun?setPart(npcParts.gun,i,rootMatrix,gunX,gunY,gunZ,gunPitch):hidePart(npcParts.gun,i);const uniqueWeaponId=String(src.uniqueWeaponId||''),uniqueWeaponColor=npcEmpireWeaponColors[uniqueWeaponId];if(showGun&&uniqueWeaponColor){const variant=Object.keys(npcEmpireWeaponColors).indexOf(uniqueWeaponId),railScale=instanceScale.set(1+(variant%3)*.22,.72+(variant%4)*.08,.88+(variant%5)*.08);setPart(npcParts.uniqueGunRail,i,rootMatrix,gunX,gunY+.16,gunZ+.08,gunPitch,railScale);setPart(npcParts.uniqueGunCharm,i,rootMatrix,gunX+(variant%2?-.18:.18),gunY-.14,gunZ+.2,gunPitch+t*.0015,instanceScale.setScalar(.78+(variant%4)*.1));npcParts.uniqueGunRail.setColorAt(i,instanceColor.setHex(uniqueWeaponColor));npcParts.uniqueGunCharm.setColorAt(i,instanceColor.setHex(uniqueWeaponColor));}else{hidePart(npcParts.uniqueGunRail,i);hidePart(npcParts.uniqueGunCharm,i);}
