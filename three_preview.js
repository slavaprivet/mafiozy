...
#include <colorspace_fragment>
      ...
      const playerName=new THREE.Sprite(new THREE.SpriteMaterial({map:playerNameTexture,transparent:true,depthTest:false}));playerName.position.y=7.45;playerName.scale.set(8.5,1.98,1);playerName.renderOrder=45;const playerHpBg=new THREE.Sprite(new THREE.SpriteMaterial({color:0x33151a,depthTest:false}));playerHpBg.position.y=5.92;playerHpBg.scale.set(4.15,.34,1);playerHpBg.renderOrder=44;const playerHpBar=new THREE.Sprite(new THREE.SpriteMaterial({color:0x55e778,depthTest:false}));playerHpBar.position.y=5.94;playerHpBar.scale.set(3.9,.21,1);playerHpBar.renderOrder=45;player.add(playerName,playerHpBg,playerHpBar);
      ...
      const makeNpcHealthTexture=(fill=false)=>{const canvas=document.createElement('canvas');canvas.width=fill?256:512;canvas.height=96;const ctx=canvas.getContext('2d');if(fill){ctx.fillStyle='#fff';ctx.beginPath();ctx.moveTo(12,8);ctx.lineTo(244,8);ctx.lineTo(254,48);ctx.lineTo(244,88);ctx.lineTo(12,88);ctx.lineTo(2,48);ctx.closePath();ctx.fill();}else{ctx.fillStyle='rgba(8,10,14,.94)';ctx.strokeStyle='#d9a62e';ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(18,5);ctx.lineTo(494,5);ctx.lineTo(508,48);ctx.lineTo(494,91);ctx.lineTo(18,91);ctx.lineTo(4,48);ctx.closePath();ctx.fill();ctx.stroke();ctx.fillStyle='#f3cc55';ctx.font='900 31px Arial';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText('HP',49,49);ctx.fillStyle='rgba(49,19,24,.92)';ctx.fillRect(91,25,390,46);ctx.strokeStyle='rgba(255,238,169,.55)';ctx.lineWidth=2;ctx.strokeRect(91,25,390,46);}const texture=new THREE.CanvasTexture(canvas);texture.colorSpace=THREE.SRGBColorSpace;texture.generateMipmaps=false;texture.minFilter=THREE.LinearFilter;texture.magFilter=THREE.LinearFilter;return texture;},npcHealthFrameTexture=makeNpcHealthTexture(),npcHealthFillTexture=makeNpcHealthTexture(true);
      const citizenPool=[];for(let i=0;i<NPC_CAP;i++){const hpGroup=new THREE.Group(),hpBg=new THREE.Sprite(new THREE.SpriteMaterial({map:npcHealthFrameTexture,transparent:true,depthTest:false,depthWrite:false,toneMapped:false})),hpBar=new THREE.Sprite(new THREE.SpriteMaterial({map:npcHealthFillTexture,color:0x58e67c,transparent:true,depthTest:false,depthWrite:false,toneMapped:false}));hpBg.position.y=4.55;hpBg.scale.set(4.4,.82,1);hpBar.position.set(.25,4.56,.01);hpBar.scale.set(3.25,.31,1);hpBar.renderOrder=42;hpBg.renderOrder=41;hpGroup.add(hpBg,hpBar);hpGroup.layers.enable(1);hpGroup.visible=false;scene.add(hpGroup);citizenPool.push({hpGroup,hpBar});}renderer.domElement.dataset.npcHpVisual='shared-framed-raised-left-fill-v349';
      ...
      const animate = t => {
        ...
        if(dynamic){
          ...
          citizenPool.forEach((npc,i)=>{
            ...
            const pct=Math.max(.03,pose.hpPct),identityFocused=nearbyNpcRing.visible&&nearbyNpcState?.index===i&&nearbyNpcState.distance<=3.05,damaged=pose.hpPct<.999||pose.hit>0||!!src.bleeding;npc.hpGroup.visible=!npcIsDead(src)&&(identityFocused||damaged);npc.hpGroup.position.set(x,pose.crawling?1.34:pose.bob+1.72-pose.crouch*.68,z);npc.hpBar.scale.x=3.25*pct;npc.hpBar.position.x=-1.375+1.625*pct;npc.hpBar.material.color.set(pct>.55?0x58e67c:pct>.25?0xffc94d:0xff5252);
          });
