...
let refreshCustomGangHqs=()=>{},refreshNpcBusinessControls=()=>{},refreshNpcBusinessOperations=()=>{},casinoExteriorAnimation=()=>{};
      ...
      if(worldSnapshot?.landmarks){
        ...
        refreshCustomGangHqs=()=>{const fresh=bridge?.getCustomGangHqs?.()||bridge?.getWorldSnapshot?.(90)?.landmarks?.customGangHqs||[],sig=JSON.stringify(fresh.map(h=>[h.id,h.name,h.r,h.c,h.flag]));if(sig===customGangHqSig)return;customGangHqSig=sig;while(customGangHqGroup.children.length)disposeTransientObjectTree(customGangHqGroup.children[customGangHqGroup.children.length-1]);for(const hq of fresh){const roof=roofAnchorAt(+hq.r+.5,+hq.c+.5,9),flag=hq.flag||{},primary=flag.primary||'#9b1f2d',g=new THREE.Group(),highlight=new THREE.Mesh(new THREE.RingGeometry(2.35,3.25,48),new THREE.MeshBasicMaterial({color:primary,transparent:true,opacity:.82,side:THREE.DoubleSide,depthTest:false,toneMapped:false})),pole=new THREE.Mesh(new THREE.CylinderGeometry(.09,.12,6,10),new THREE.MeshStandardMaterial({color:0xcaa86d,metalness:.65,roughness:.32}));highlight.rotation.x=-Math.PI/2;highlight.position.y=-2.78;highlight.renderOrder=42;g.add(highlight);pole.position.y=3;g.add(pole);const cloth=new THREE.Mesh(new THREE.PlaneGeometry(4.8,2.7,8,3),new THREE.MeshStandardMaterial({map:customGangFlagTexture(flag),side:THREE.DoubleSide,roughness:.78,emissive:new THREE.Color(primary),emissiveIntensity:.06}));cloth.position.set(2.45,4.65,0);cloth.rotation.y=.08;g.add(cloth);g.position.set(roof.x,roof.y,roof.z);customGangHqGroup.add(g);const label=labelSprite(String(hq.name||'БАНДА').toUpperCase(),primary);label.position.set(roof.x,roof.y+8.2,roof.z);label.scale.multiplyScalar(1.08);customGangHqGroup.add(label);}};refreshCustomGangHqs();
        const npcBusinessControlGroup=new THREE.Group();npcBusinessControlGroup.name='npc-business-control-flags';scene.add(npcBusinessControlGroup);let npcBusinessControlSig='';
        refreshNpcBusinessControls=()=>{const fresh=bridge?.getNpcBusinessControls?.()||[],sig=JSON.stringify(fresh.map(item=>[item.id,item.businessControl?.faction,item.businessControl?.guarded,item.businessControl?.guard_gid,item.businessControl?.defense_level]));if(sig===npcBusinessControlSig)return;npcBusinessControlSig=sig;while(npcBusinessControlGroup.children.length)disposeTransientObjectTree(npcBusinessControlGroup.children[npcBusinessControlGroup.children.length-1]);for(const item of fresh){const businessControl=item.businessControl||{},color=businessControl.color||(businessControl.faction==='yellow'?'#ffe34d':'#b887ff'),roof=roofAnchorAt(+item.r+.5,+item.c+.5,8),x=toX(+item.c+.5),z=toZ(+item.r+.5),ring=new THREE.Mesh(new THREE.RingGeometry(4.8,6.25,64),new THREE.MeshBasicMaterial({color,transparent:true,opacity:businessControl.guarded?.72:.4,side:THREE.DoubleSide,depthTest:false,toneMapped:false}));ring.rotation.x=-Math.PI/2;ring.position.set(x,.2,z);ring.renderOrder=40;npcBusinessControlGroup.add(ring);const flag=new THREE.Group(),pole=new THREE.Mesh(new THREE.CylinderGeometry(.075,.1,5.7,10),new THREE.MeshStandardMaterial({color:0xcab98d,metalness:.7,roughness:.28})),cloth=new THREE.Mesh(new THREE.PlaneGeometry(4.4,2.35,7,3),new THREE.MeshStandardMaterial({color,side:THREE.DoubleSide,roughness:.8,emissive:new THREE.Color(color),emissiveIntensity:.09}));pole.position.y=2.85;cloth.position.set(2.18,4.25,0);cloth.rotation.y=.08;flag.add(pole,cloth);flag.position.set(roof.x,roof.y,roof.z);flag.userData.businessControl=businessControl;npcBusinessControlGroup.add(flag);const defense=Math.max(0,Math.min(3,+businessControl.defense_level||0)),label=labelSprite(`${String(businessControl.gang_name||'БАНДА').toUpperCase()}${defense?'  '+'◆'.repeat(defense):''}`,color);label.position.set(roof.x,roof.y+7.1,roof.z);label.scale.multiplyScalar(.92);npcBusinessControlGroup.add(label);}renderer.domElement.dataset.npcBusinessControls=String(fresh.length);};refreshNpcBusinessControls();
        const npcBusinessOperationGroup=new THREE.Group();npcBusinessOperationGroup.name='npc-business-operations';scene.add(npcBusinessOperationGroup);let npcBusinessOperationSig='';
        refreshNpcBusinessOperations=()=>{const fresh=bridge?.getNpcBusinessOperations?.()||[],sig=JSON.stringify(fresh.map(item=>[item.gid,item.biz_id,item.phase,item.strength]));if(sig===npcBusinessOperationSig)return;npcBusinessOperationSig=sig;while(npcBusinessOperationGroup.children.length)disposeTransientObjectTree(npcBusinessOperationGroup.children[npcBusinessOperationGroup.children.length-1]);for(const item of fresh){const color=item.faction==='yellow'?'#ffe34d':'#b887ff',x=toX(+item.c+.5),z=toZ(+item.r+.5),ring=new THREE.Mesh(new THREE.RingGeometry(6.4,6.8,56),new THREE.MeshBasicMaterial({color,transparent:true,opacity:.82,side:THREE.DoubleSide,depthTest:false,toneMapped:false}));ring.rotation.x=-Math.PI/2;ring.position.set(x,.28,z);ring.renderOrder=42;npcBusinessOperationGroup.add(ring);const phases={travel:'МАРШ',assault:'ШТУРМ',capture:'ЗАХВАТ'},label=labelSprite(`⚔ ${phases[item.phase]||'ОПЕРАЦИЯ'} · ${Math.max(0,+item.strength||0)}`,color);label.position.set(x,6.4,z);label.scale.multiplyScalar(.86);npcBusinessOperationGroup.add(label);}renderer.domElement.dataset.npcBusinessOperations=String(fresh.length);};refreshNpcBusinessOperations();
        ...
        if(jail){
          ...
          for(const [rx,rz,title] of serviceRooms){pbox(rx,rz,6.2,4.05,.08,title==='МЕДПУНКТ'?paintedWall:concreteDark,.74);pbox(rx,rz+1.95,6.2,.22,3.15,paintedWall,.76);for(const sx of [-3,3])pbox(rx+sx,rz,.22,4.05,3.15,concrete,.76);const roomLabel=panel(title,rx,3.62,rz+2.08,5.45,.78,title==='МЕДПУНКТ'?'#8de9d8':'#e8c65b');roomLabel.rotation.y=Math.PI;}
          // Canteen and kitchen.
      ...
      const trunkMat=windTrunkMaterial(0x5b3826),leafMat=windLeafMaterial(0x39744b);
      ...
      const weaponExtraParts=[revolverCylinder,revolverBarrel,revolverHammer,heavySlide,heavyBarrel,heavyRearSight,goldTopRib,goldInlay,shotgunPump,tommyDrum,rifleForegrip,sniperLens,taserCoil,muzzleBrake,grenadeModel,molotovModel,c4Model];
      ...
      const limbUp=new THREE.Vector3(0,1,0),limbDir=new THREE.Vector3(),limbMid=new THREE.Vector3(),leftShoulder=new THREE.Vector3(),rightShoulder=new THREE.Vector3(),leftElbow=new THREE.Vector3(),rightElbow=new THREE.Vector3(),leftHandTarget=new THREE.Vector3(),rightHandTarget=new THREE.Vector3();
      ...
      const configureWeaponVisual=id=>{currentWeaponId=String(id||'').toLowerCase();currentWeaponFx=WEAPON_VISUALS[currentWeaponId]||WEAPON_VISUALS.pistol;const f=currentWeaponFx.family,unarmed=['','none','fists','unarmed'].includes(currentWeaponId),specialSidearm=['revolver','heavy-pistol'].includes(f);gun.visible=!unarmed;for(const part of [gunReceiver,gunSlide,gunBarrel,gunMuzzle,gunGrip])part.visible=!unarmed&&!['rpg','grenade','molotov','c4'].includes(f);for(const part of weaponExtraParts)part.visible=false;rpgTube.visible=f==='rpg';gunMagazine.visible=['smg','tommy','gold-tommy','rifle','sniper'].includes(f);gunStock.visible=['shotgun','tommy','gold-tommy','rifle','sniper'].includes(f);gunScope.visible=['rifle','sniper'].includes(f);if(specialSidearm){gunSlide.visible=gunBarrel.visible=gunMuzzle.visible=false;}revolverCylinder.visible=revolverBarrel.visible=revolverHammer.visible=f==='revolver';heavySlide.visible=heavyBarrel.visible=heavyRearSight.visible=f==='heavy-pistol';goldTopRib.visible=goldInlay.visible=f==='gold-pistol';shotgunPump.visible=f==='shotgun';tommyDrum.visible=['tommy','gold-tommy'].includes(f);rifleForegrip.visible=['rifle','sniper'].includes(f);sniperLens.visible=f==='sniper';taserCoil.visible=f==='taser';muzzleBrake.visible=['heavy-pistol','sniper','rifle'].includes(f);grenadeModel.visible=f==='grenade';molotovModel.visible=f==='molotov';c4Model.visible=f==='c4';const isLong=currentWeaponFx.grip==='two',isGold=f==='gold-pistol'||f==='gold-tommy',isNagan=currentWeaponId==='nagan';gunReceiver.material=isGold?weaponGold:gunMetal;gunSlide.material=isGold?weaponGold:gunMetal;gunGrip.material=f==='revolver'?gunWood:isGold?weaponGold:gunWood;tommyDrum.material=isGold?weaponGold:gunDark;revolverCylinder.material=revolverBarrel.material=isNagan?gunMetal:gunDark;revolverCylinder.scale.setScalar(isNagan?.86:1.14);revolverBarrel.scale.set(1,1,isNagan?.84:1.18);heavySlide.material=currentWeaponId==='pistol_heavy'?gunMetal:gunDark;gunReceiver.scale.set(f==='gold-pistol'?1.08:1,f==='heavy-pistol'?1.16:1,f==='revolver'?.86:1);gunSlide.scale.set(f==='gold-pistol'?1.13:1,f==='gold-pistol'?1.12:1,f==='gold-pistol'?.9:1);gunBarrel.scale.set(1,1,f==='gold-pistol'?.82:1);gunStock.position.z=currentWeaponFx.stockZ??-.58;gunStock.scale.set(1,1,currentWeaponFx.stockScaleZ??1);gun.scale.setScalar(f==='rpg'?1.08:isLong?1.03:['grenade','molotov'].includes(f)?1.35:f==='heavy-pistol'?.84:f==='revolver'?(isNagan?.78:.86):f==='gold-pistol'?.8:.76);gun.position.set(currentWeaponFx.grip==='one'?.76:.18,isLong?2.98:2.75,currentWeaponFx.holdZ??(isLong?.3:.62));gun.userData.baseY=gun.position.y;gun.userData.baseZ=gun.position.z;renderer.domElement.dataset.weaponModel=f;renderer.domElement.dataset.weaponVariant=currentWeaponId;renderer.domElement.dataset.weaponAnimationProfile=f==='revolver'?(isNagan?'nagan-cylinder-hammer':'magnum-cylinder-hammer'):f==='heavy-pistol'?'heavy-slide-kick':f==='gold-pistol'?'gold-quick-slide':'standard-slide';renderer.domElement.dataset.weaponGrip=currentWeaponFx.grip;renderer.domElement.dataset.weaponStockFit=`${gunStock.position.z.toFixed(2)}:${gunStock.scale.z.toFixed(2)}`;};
      ...
      const createVehicleSlot=({v,i})=>{
        ...
        car.userData={entityId:null,paint,wheelMeshes,wheelSpin:0,siren,cab,base,baseBaseY:base.position.y,hood,hoodBaseY:hood.position.y,driverDoorPivot,doorOpen:0,standardParts,damageBlend:0,wreckVisibilityApplied:false,taxiSign,spoiler,pickupBed,roofRack,cabrioSeats,stripe,fireEquipment,hoseCannon,towEquipment,towDeck,towEdges,towBoom:boom,towHookCable:hookCable,ambulanceRearDoors,ambulanceRearDoorPivots,medicalDoorOpen:0,helicopter,heliBody,tailBoom,rotor,newsCabin,newsStripe,newsLabels,newsLightPod,newsSearchLight,newsSearchTarget,newsBeamPivot,newsBeamCone,contactShadow,damageFx,damageSmoke,damageFlames,bodyHoles,bodyHoleLayers,glassCracks,vehicleLength:L,vehicleHalfWidth:utility?1.551:1.401,wreckGroup,wreckShell,wreckCab,wreckDoor,wreckWheels,wreckHeat,wreckGlow,wreckFlames,wreckSmoke,wreckDebris,wreckBreakaway,lastPosition:new THREE.Vector3(v[0],0,v[1]),hitMarkSignature:''};car.position.set(v[0], 0, v[1]);car.visible=false;car.frustumCulled=false;scene.add(car);cars.push(car);
      };
      ...
      const gangHirePrompt=document.createElement('div');gangHirePrompt.style.cssText='position:absolute;z-index:58;display:none;min-width:250px;max-width:330px;transform:translate(-50%,-100%);padding:11px 14px;border:1px solid #58f59a;border-radius:12px;background:linear-gradient(145deg,rgba(7,18,15,.97),rgba(10,12,20,.97));box-shadow:0 12px 34px #000d,0 0 22px #22e87944;color:#fff;pointer-events:none;font:700 12px system-ui;backdrop-filter:blur(8px)';gangHirePrompt.innerHTML='<div data-gang-hire-title style="font-size:14px;font-weight:950;color:#7effa8;letter-spacing:.3px">ВЗАИМОДЕЙСТВОВАТЬ</div><div data-gang-hire-name style="margin-top:3px;color:#fff"></div><div data-gang-hire-hint style="margin-top:6px;color:#ffe082;font-weight:900">E · НАНЯТЬ</div>';stage.appendChild(gangHirePrompt);const gangHireTitle=gangHirePrompt.querySelector('[data-gang-hire-title]'),gangHireName=gangHirePrompt.querySelector('[data-gang-hire-name]'),gangHireHint=gangHirePrompt.querySelector('[data-gang-hire-hint]');let selectedNpcInteraction=null;const showGangHirePrompt=info=>{selectedNpcInteraction=info||null;if(!info){gangHirePrompt.style.display='none';return;}gangHireTitle.textContent=info.eligible===false?'НАЙМ НЕДОСТУПЕН':'ВЗАИМОДЕЙСТВОВАТЬ';gangHireTitle.style.color=info.eligible===false?'#ff8b86':'#7effa8';gangHireName.textContent=`${info.name||'Боец банды'} · ${Math.max(1,+info.level||1)} ур.`;gangHireHint.textContent=info.eligible===false?String(info.message||'Сейчас нанять нельзя'):`E · НАНЯТЬ ЗА $${Math.max(0,+info.cost||0)}`;gangHirePrompt.style.display='block';};const gangHireProjection=new THREE.Vector3(),positionGangHirePrompt=(x,z)=>{if(gangHirePrompt.style.display==='none')return;const canvasRect=renderer.domElement.getBoundingClientRect(),stageRect=stage.getBoundingClientRect();gangHireProjection.set(x,5.5,z).project(camera);const px=canvasRect.left-stageRect.left+(gangHireProjection.x*.5+.5)*canvasRect.width,py=canvasRect.top-stageRect.top+(-gangHireProjection.y*.5+.5)*canvasRect.height;gangHirePrompt.style.left=`${Math.max(135,Math.min(stageRect.width-135,px))}px`;gangHirePrompt.style.top=`${Math.max(90,Math.min(stageRect.height-20,py))}px`;};renderer.domElement.dataset.gangHireInteraction='rmb-select-e-confirm';
      ...
      const crewBadges=[];for(let i=0;i<NPC_CAP;i++){const canvas=document.createElement('canvas');canvas.width=canvas.height=256;const texture=new THREE.CanvasTexture(canvas);texture.colorSpace=THREE.SRGBColorSpace;texture.generateMipmaps=false;texture.minFilter=THREE.LinearFilter;texture.magFilter=THREE.LinearFilter;const sprite=new THREE.Sprite(new THREE.SpriteMaterial({map:texture,transparent:true,depthTest:false,depthWrite:false,toneMapped:false}));sprite.scale.set(2.25,2.25,1);sprite.renderOrder=54;sprite.layers.enable(1);sprite.visible=false;scene.add(sprite);crewBadges.push({canvas,texture,sprite,sig:''});}const updateCrewBadge=(entry,src,x,z)=>{if(!src?.allied||src.dead){entry.sprite.visible=false;return;}const flag=src.gangFlag||{},primary=flag.primary||'#28d96f',secondary=flag.secondary||'#ffe07a',emblem=flag.emblem||'star',sig=`${primary}:${secondary}:${emblem}`;if(sig!==entry.sig){entry.sig=sig;const c=entry.canvas.getContext('2d'),symbol={crown:'♛',skull:'☠',diamond:'◆',wolf:'W',eagle:'♜',star:'★'}[emblem]||'★';c.clearRect(0,0,256,256);c.shadowColor='#000';c.shadowBlur=18;c.fillStyle=primary;c.strokeStyle='#92ffb6';c.lineWidth=10;c.beginPath();c.moveTo(128,18);c.lineTo(220,55);c.lineTo(204,167);c.quadraticCurveTo(128,240,52,167);c.lineTo(36,55);c.closePath();c.fill();c.stroke();c.shadowBlur=0;c.fillStyle=secondary;c.font='900 120px Georgia,serif';c.textAlign='center';c.textBaseline='middle';c.lineWidth=12;c.strokeStyle='rgba(0,0,0,.82)';c.strokeText(symbol,128,121);c.fillText(symbol,128,121);entry.texture.needsUpdate=true;}entry.sprite.visible=true;entry.sprite.position.set(x,6.3,z);};renderer.domElement.dataset.crewBadgeProfile='custom-gang-shield-overhead-v240';
      ...
      const npcAnimationBits=(src,t,dead=npcIsDead(src))=>{
        ...
        if(src.alerted||src.alert||src.sentry||src.lastStand||gesture==='lookout'||(stateActive&&(state==='alert'||state==='alerted'||state==='watching'||state==='last_stand')))bits|=NPC_ANIM_ALERT;
        return bits;
      };
      const npcLifeAnimationPose=(src,i,t)=>{
        ...
        const roll=(crawling?step*.016*gait:(limping?.105+step*.04*gait:step*.026*gait))+hit*hitSide*.24+(dodging?Math.sin(t*.012+i)*.18:0);
        const crouch=cowering?.86:helping?.42:gettingUp?.58:takingCover?.38:boarding?.24:0,statePitch=cowering?.28:helping?.38:gettingUp?.52:takingCover?.12:panicking?-.055:alerted?-.025:0,pitch=(crawling?1.22*crawlBlend:(walking?-(.025+pace*.035)*gait:0))+statePitch+hit*.2;
        ...
        if(shooting||aiming){const recoil=shooting?Math.max(0,1-Math.min(1,(+src.shotAge||0)/240)):.08;leftArmX=-.57;rightArmX=.57;armY=2.35;armZ=.42;leftArmPitch=-1.02-recoil*.18;rightArmPitch=-1.16-recoil*.26;leftArmRoll=-.3;rightArmRoll=.3;leftArmYaw=.12;rightArmYaw=-.1;}
        else if(reloading){const p=Math.max(0,Math.min(1,+src.reloadProgress||0)),snap=Math.sin(p*Math.PI);leftArmX=-.48;rightArmX=.48;armY=2.34;armZ=.38;leftArmPitch=-1.15+snap*.22;rightArmPitch=-.78-snap*.38;leftArmRoll=-.48;rightArmRoll=.38;leftArmYaw=.22;rightArmYaw=-.18;}
        else if(boarding){const reach=Math.sin(t*.01+i)*.08;leftArmX=-.62;rightArmX=.62;armY=2.34;armZ=.28;leftArmPitch=-.72+reach;rightArmPitch=-.82-reach;leftArmRoll=-.18;rightArmRoll=.18;}
      ...
      const keys=new Set(),bullets=[],reloadDebris=[],vehicleHoldMs=170;let aimPoint=new THREE.Vector3(0,0,-10),lastShot=0,muzzleLife=0,recoilKick=0,recoilSide=1,recoilShotSeq=0,reloadWasActive=false,activeReloadProgress=0,triggerHeld=false,laserAimHeld=false,laserAimAllowed=true,laserShotPulseUntil=0,throwAimHeld=false,throwAimStartedAt=0,throwAimKind='',throwAimAngle=0,throwAimRange=1.6,throwReleaseUntil=0,eHoldStarted=0,eHoldTimer=0,eHoldTriggered=false,eHoldVehicleId='',eHoldHasBuilding=false;
      ...
      addEventListener('keydown',e=>{keys.add(e.code);const typing=e.target?.matches?.('input,textarea,select,[contenteditable="true"]');if(e.code==='KeyE'&&!e.repeat&&!typing){const gangInteraction=selectedNpcInteraction&&performance.now()<selectedNpcUntil,vehicle=gangInteraction?null:bridge?.getNearbyVehicleInteraction?.(),building=gangInteraction?null:bridge?.getNearbyBuildingInteraction?.();eHoldStarted=performance.now();eHoldTriggered=false;eHoldVehicleId=String(vehicle?.id||'');eHoldHasBuilding=!!building;if(gangInteraction){renderer.domElement.dataset.interactionPriority='gang-hire-e';gangHirePrompt.style.transform='translate(-50%,-100%) scale(1.04)';}else if(vehicle){vehicleHoldPrompt.firstElementChild.textContent=eHoldHasBuilding?'🚪 Короткое E — здание · удержание — машина':'🚗 Удерживайте E — сесть в машину';vehicleHoldPrompt.style.display='block';vehicleHoldFill.style.width='0';renderer.domElement.dataset.vehicleHold=`waiting:${eHoldVehicleId}`;renderer.domElement.dataset.interactionPriority=eHoldHasBuilding?'tap-building-hold-vehicle':'hold-vehicle-only';eHoldTimer=setTimeout(()=>{if(keys.has('KeyE'))triggerVehicleEntry();},vehicleHoldMs);}else renderer.domElement.dataset.interactionPriority=building?'building-only':'none';e.preventDefault();e.stopImmediatePropagation();}else if(e.code==='Escape'&&!typing){showGangHirePrompt(null);selectedNpcUntil=0;bridge?.closeBuildingActions?.();}else if(e.code==='Space'&&!typing){e.preventDefault();e.stopImmediatePropagation();if(currentWeaponId==='c4')bridge?.plantC4?.();else if(!isThrowablePrimary())shoot(performance.now());}},true);
      addEventListener('keyup',e=>{keys.delete(e.code);if(e.code!=='KeyE')return;clearTimeout(eHoldTimer);vehicleHoldPrompt.style.display='none';gangHirePrompt.style.transform='translate(-50%,-100%)';if(selectedNpcInteraction&&performance.now()<selectedNpcUntil){const result=bridge?.hireSelectedGangNpc?.();renderer.domElement.dataset.gangHireAction=result?.ok?`pending:${result.sourceId||''}`:`rejected:${result?.reason||'none'}`;if(result?.ok){gangHireTitle.textContent='ПРЕДЛОЖЕНИЕ ОТПРАВЛЕНО';gangHireHint.textContent='Ожидаем ответ сервера…';selectedNpcUntil=performance.now()+900;}else{selectedNpcInteraction={...selectedNpcInteraction,eligible:false,message:result?.message||'Найм недоступен'};showGangHirePrompt(selectedNpcInteraction);}eHoldStarted=0;eHoldVehicleId='';eHoldHasBuilding=false;e.preventDefault();e.stopImmediatePropagation();return;}if(!eHoldTriggered){const bankResult=bridge?.interactBank?.(),result=bankResult?.ok?bankResult:bridge?.toggleNearbyBuildingActions?.(innerWidth/2,innerHeight*.58);renderer.domElement.dataset.buildingAction=result?.ok?(result.closed?'closed':`${result.kind}:${result.id||''}`):`rejected:${result?.reason||'none'}`;renderer.domElement.dataset.interactionPriority=eHoldHasBuilding?'building-tap-complete':eHoldVehicleId?'vehicle-short-ignored':'building-only-complete';}if(eHoldTriggered)renderer.domElement.dataset.vehicleHold='complete';else renderer.domElement.dataset.vehicleHold='short-building-action';eHoldStarted=0;eHoldVehicleId='';eHoldHasBuilding=false;},true);
      ...
      const isThrowablePrimary=()=>currentWeaponId==='grenade'||currentWeaponId==='molotov';
      const isSpecialPrimary=()=>isThrowablePrimary()||currentWeaponId==='c4';
      ...
      const selectNpcAtPointer=e=>{const src=gangNpcAtPointer(e,true);if(!src)return false;const result=bridge?.selectGangNpc?.(src.sourceId);if(!result?.ok)return false;selectedNpcSourceId=String(src.sourceId);selectedNpcUntil=performance.now()+10000;selectedNpcRing.visible=true;showGangHirePrompt(result);renderer.domElement.dataset.selectedGangNpc=selectedNpcSourceId;return true;};
...
let lastW=size.W,lastH=size.H,lastT=performance.now(),lastPresentedAt=0,walkPhase=0,lampAnchor='',fpsAt=performance.now(),fpsFrames=0,measuredFps=60,lastPixelRatioChangeAt=0,lastShadowAt=0,lastOcclusionAt=0,dynamicAt=0,dynamicState=null,nearbyActionAt=0,nearbyActionState=null,customGangHqAt=0,npcBusinessControlAt=0,npcBusinessOperationAt=0,cameraZoomMode='world',cameraZoomKey='',worldZoom=1,playerFloorElevation=0,playerVisualSig='',vehicleEntryState=null,fadedMaterials=[],firstFramePresented=false,fullMaterialsReady=false,materialCompileStarted=false,deferredWorldLoadStarted=false,playerHpPct=1,lastPlayerHp=-1,playerHitUntil=0,playerHitSide=1;
      ...
      const updateDayNight=t=>{
        ...
        bulbMat.color.setRGB(.38+night*.62,.4+night*.42,.42+night*.18);streetLights.forEach(({light},i)=>{light.intensity=night*(i%2?17:25);light.visible=light.userData.mfzBudgetVisible!==false&&night>.08;});
      ...
      const animate = t => {
        ...
        if(t-npcBusinessControlAt>1000){npcBusinessControlAt=t;refreshNpcBusinessControls();}
        if(t-npcBusinessOperationAt>1000){npcBusinessOperationAt=t;refreshNpcBusinessOperations();}
        ...
        if(dynamic){
          ...
          let crawlingNpcCount=0,limpingNpcCount=0,reactingNpcCount=0,bleedingNpcCount=0,panickingNpcCount=0,coweringNpcCount=0,surrenderingNpcCount=0,helpingNpcCount=0,socialNpcCount=0,alertedNpcCount=0,reloadingNpcCount=0,coveringNpcCount=0,vehicleNpcCount=0;
          citizenPool.forEach((npc,i)=>{
            ...
            armed?setPart(npcParts.hat,i,rootMatrix,0,3.77-pose.crouch,0):hidePart(npcParts.hat,i);const lifeGestureDisarms=pose.surrendering||pose.cowering||pose.helping||pose.panicking||pose.talking,combatPose=pose.aiming||pose.shooting,reloadPose=pose.reloading;armed&&!lifeGestureDisarms&&!(severMask&2)?setPart(npcParts.gun,i,rootMatrix,reloadPose?.42:combatPose?.58:.72,reloadPose?2.3:combatPose?2.34:2-pose.crouch*.35,reloadPose?.42:combatPose?.78:.52,reloadPose?-.82:pose.shooting?-.34:pose.aiming?-.2:pose.hit*.18):hidePart(npcParts.gun,i);
          ...
          renderer.domElement.dataset.squadAnimationStates=`reload:${reloadingNpcCount},cover:${coveringNpcCount},vehicle:${vehicleNpcCount}`;
          for(let i=0;i<NPC_CAP;i++){const src=dynamic.npcs[i],label=npcLabels[i];if(!src){label.sprite.visible=false;continue;}const x=npcVisualXs[i],z=npcVisualZs[i],role=String(src.role||'').toLowerCase(),family=String(src.family||src.faction||'').toLowerCase(),prisonStaff=String(src.behavior||'')==='prison_patrol'||role.startsWith('prison_'),gang=src.visualRole==='gang'||!!src.gang||role.includes('gang')||role.includes('boss')||role.includes('district_')||role.includes('occupier'),police=src.visualRole==='police'||!!src.police||role.includes('police')||role.includes('cop'),guard=src.visualRole==='guard'||role.includes('guard'),medic=role.includes('medic'),gangSuit=family.includes('yellow')?0xf1e8cf:family.includes('purple')?0x7043a5:family.includes('moretti')?0xe6dfd1:family.includes('bellini')?0x3f4652:Math.max(1,+src.level||1)>=4?0x8f3044:0xb54859,bright=medic?0xe8f2f4:police?0x328fe2:gang?gangSuit:guard?0xc48a28:[0x3e9bd1,0xdb5c68,0x79a84f,0xd39b42,0x8a6dbe][i%5];instanceColor.set(bright);npcParts.body.setColorAt(i,instanceColor);npcParts.leftArm.setColorAt(i,instanceColor);npcParts.rightArm.setColorAt(i,instanceColor);npcParts.hat.setColorAt(i,instanceColor);updateNpcSpeechLabel(label,src,x,8.05+(prisonStaff?(i%3)*.82:0),z);}for(const mesh of [npcParts.body,npcParts.leftArm,npcParts.rightArm,npcParts.hat])if(mesh.instanceColor)mesh.instanceColor.needsUpdate=true;
          for(let i=0;i<NPC_CAP;i++){const src=dynamic.npcs[i];if(!src){crewBadges[i].sprite.visible=false;continue;}updateCrewBadge(crewBadges[i],src,npcVisualXs[i],npcVisualZs[i]);}
          let visibleGangCount=0;
          for(let i=0;i<NPC_CAP;i++){
            const src=dynamic.npcs[i];
            if(!src){hidePart(npcParts.gangAura,i);hidePart(npcParts.gangBand,i);continue;}
            const role=String(src.role||'').toLowerCase(),gang=src.visualRole==='gang'||!!src.gang||role.includes('gang')||role.includes('boss')||role.includes('district_')||role.includes('occupier');
            if(!gang||src.dead){hidePart(npcParts.gangAura,i);hidePart(npcParts.gangBand,i);continue;}
            visibleGangCount++;
            const faction=String(src.faction||src.family||'').toLowerCase(),flag=src.gangFlag||{},auraColor=src.allied?0x35ff78:(faction.includes('yellow')?0xffd83d:faction.includes('purple')?0xa668ff:faction.includes('moretti')?0xf5ead2:faction.includes('bellini')?0x596274:0xff4f68),bandColor=src.allied&&flag.primary?instanceColor.set(flag.primary).getHex():auraColor,x=npcVisualXs[i],z=npcVisualZs[i],phase=npcVisualPhases[i]||t*.008+i*.73,bob=src.walking?Math.abs(Math.sin(phase))*.13:Math.sin(t*.0018+i*1.7)*.018;
            rootMatrix.makeRotationY(npcFacingYaws[i]);rootMatrix.scale(npcScale);rootMatrix.setPosition(x,bob,z);setPart(npcParts.gangBand,i,rootMatrix,0,2.25,.405);npcParts.gangBand.setColorAt(i,instanceColor.setHex(bandColor));
            rootMatrix.makeRotationY(0);rootMatrix.setPosition(x,.11,z);setPart(npcParts.gangAura,i,rootMatrix,0,0,0,-Math.PI/2,src.allied?instanceScale.set(1.58,1.58,1.58):gangAuraScale);npcParts.gangAura.setColorAt(i,instanceColor.setHex(auraColor));
          }
          npcParts.gangAura.instanceMatrix.needsUpdate=true;npcParts.gangAura.instanceColor.needsUpdate=true;npcParts.gangBand.instanceMatrix.needsUpdate=true;npcParts.gangBand.instanceColor.needsUpdate=true;renderer.domElement.dataset.visibleGangs=String(visibleGangCount);
          const selectedIndex=performance.now()<selectedNpcUntil?dynamic.npcs.findIndex(n=>!n.dead&&String(n.sourceId||'')===selectedNpcSourceId):-1;
          if(selectedIndex>=0){selectedNpcRing.visible=true;selectedNpcRing.position.set(npcVisualXs[selectedIndex],.15,npcVisualZs[selectedIndex]);const pulse=1+Math.sin(t*.009)*.09;selectedNpcRing.scale.setScalar(pulse);selectedNpcRing.rotation.y=t*.0017;selectedNpcOuter.material.opacity=.82+Math.sin(t*.011)*.16;positionGangHirePrompt(npcVisualXs[selectedIndex],npcVisualZs[selectedIndex]);renderer.domElement.dataset.selectedGangNpc=selectedNpcSourceId;}
          else{selectedNpcRing.visible=false;showGangHirePrompt(null);if(selectedNpcSourceId&&performance.now()>=selectedNpcUntil){selectedNpcSourceId='';renderer.domElement.dataset.selectedGangNpc='none';}}
          ...
          for(let i=0;i<Math.min(NPC_CAP,dynamic.npcs.length);i++){
            ...
            const pick=(arr,salt=0)=>arr[(((seed>>>salt)^Math.imul(seed,31+salt))>>>0)%arr.length],role=String(src.role||'').toLowerCase(),police=src.visualRole==='police'||!!src.police||role.includes('police')||role.includes('cop'),gang=!!src.gang||role.includes('gang')||role.includes('boss')||role.includes('district_')||role.includes('occupier'),prisonStaff=!!src.prisonStaff||String(src.behavior||'')==='prison_patrol'||role.startsWith('prison_'),prisonGuard=prisonStaff&&role.includes('guard'),staffVariant=Math.abs(+src.staffVariant||0)%3,prisonGear=String(src.prisonGear||''),armed=gang||role.includes('guard')||police,look=src.look||{};
            ...
            const faction=String(src.faction||src.family||'').toLowerCase(),bodyColor=prisonStaff?(prisonGuard?[0x35483c,0x3e4937,0x2b4039][staffVariant]:[0x245f9b,0x31527d,0x1f6777][staffVariant]):police?0x245f9b:gang?(authoredSuit||(faction.includes('yellow')?0xf1e4c4:faction.includes('purple')?0x7447aa:pick([0xb84a5d,0x5f6f85,0xb48a49],3))):role.includes('guard')?0xb98738:authoredSuit||pick(npcShirts,1),legColor=prisonStaff?(prisonGuard?[0x1e2922,0x242a20,0x18251f][staffVariant]:[0x172536,0x202c3c,0x16313a][staffVariant]):police?0x172536:gang?(faction.includes('yellow')?0x5e523c:faction.includes('purple')?0x342a48:0x3b3740):pick(npcTrousers,5),skinColor=Number.isFinite(+look.skin)?npcSkins[Math.abs(+look.skin|0)%npcSkins.length]:pick(npcSkins,7),hairColor=pick(npcHairs,9),shoeColor=police||prisonStaff?0x101820:pick(npcShoes,11),hatColor=police?0x2869aa:gang?bodyColor:pick(npcHats,13),hairStyle=Number.isFinite(+look.hair)?Math.abs(+look.hair|0)%6:(seed>>>16)%6,accessory=(seed>>>20)%12,capOn=prisonStaff?prisonGear==='cap':police||armed||(Number.isFinite(+look.hat)&&+look.hat>0)||hairStyle===4||hairStyle===5,helmetOn=prisonStaff&&prisonGear.includes('helmet'),wearHat=capOn||helmetOn,glassesOn=!armed&&(accessory===0||accessory===4||role.includes('staff')),neckOn=armed||accessory%3===1,bagOn=!armed&&accessory%4===2,moustacheOn=!armed&&+look.gender!==1&&(accessory===3||accessory===9);
            ...
            npcParts.head.setColorAt(i,instanceColor.setHex(skinColor));npcParts.hair.setColorAt(i,instanceColor.setHex(hairColor));npcParts.hairBun.setColorAt(i,instanceColor.setHex(hairColor));npcParts.hairMohawk.setColorAt(i,instanceColor.setHex(hairColor));npcParts.hat.setColorAt(i,instanceColor.setHex(hatColor));npcParts.hatBrim.setColorAt(i,instanceColor.setHex(hatColor));npcParts.shoe.setColorAt(i*2,instanceColor.setHex(shoeColor));npcParts.shoe.setColorAt(i*2+1,instanceColor.setHex(shoeColor));npcParts.glasses.setColorAt(i,instanceColor.setHex(accessory%2?0x26343e:0x4b3028));npcParts.neckAccent.setColorAt(i,instanceColor.setHex(pick(npcAccents,15)));npcParts.bag.setColorAt(i,instanceColor.setHex(pick(npcBags,17)));npcParts.moustache.setColorAt(i,instanceColor.setHex(hairColor));
            npcParts.prisonHelmet.setColorAt(i,instanceColor.setHex(prisonGuard?[0x334437,0x3d4936,0x2d4037][staffVariant]:[0x244b70,0x2d4058,0x24505c][staffVariant]));npcParts.prisonHelmetBand.setColorAt(i,instanceColor.setHex(prisonGuard?0x161d19:0x152332));npcParts.prisonVest.setColorAt(i,instanceColor.setHex(prisonGuard?0x29372f:0x1b2c42));npcParts.riotShield.setColorAt(i,instanceColor.setHex([0x26323a,0x30383a,0x202e31][staffVariant]));
            ...
            if(prisonStaff)setPart(npcParts.prisonVest,i,rootMatrix,0,2.02-pose.crouch,.02);
            if(prisonStaff&&src.shield){setPart(npcParts.riotShield,i,rootMatrix,-.92,1.7-pose.crouch*.35,.62,0,unitScale,.12,-.08);setPart(npcParts.shieldViewport,i,rootMatrix,-.92,2.05-pose.crouch*.35,.72,0,unitScale,.12,-.08);}
          ...
          renderer.domElement.dataset.npcAppearanceVariants=String(npcAppearanceSignatures.size);renderer.domElement.dataset.npcAppearanceSystem='fresh-respawn-faces-hair-prison-tactical-v239';renderer.domElement.dataset.jailStaffGear='guards:military-helmet-riot-shield;police:cap-tactical-helmet-bare';renderer.domElement.dataset.jailStaffMaxHp='200';
          ...
          for(let i=0;i<Math.min(NPC_CAP,dynamic.npcs.length);i++){
            ...
            const role=String(src.role||'').toLowerCase(),police=src.visualRole==='police'||!!src.police||role.includes('police')||role.includes('cop'),armed=role.includes('gang')||role.includes('boss')||role.includes('guard')||police,prisonStaff=!!src.prisonStaff||String(src.behavior||'')==='prison_patrol'||role.startsWith('prison_'),prisonGear=String(src.prisonGear||''),capOn=prisonStaff?prisonGear==='cap':armed,helmetOn=prisonStaff&&prisonGear.includes('helmet'),hairStyle=Number.isFinite(+src.look?.hair)?Math.abs(+src.look.hair|0)%6:(seed>>>16)%6;
            hidePart(npcParts.hair,i);hidePart(npcParts.hairBun,i);hidePart(npcParts.hairMohawk,i);hidePart(npcParts.hat,i);hidePart(npcParts.hatBrim,i);hidePart(npcParts.glasses,i);hidePart(npcParts.neckAccent,i);hidePart(npcParts.bag,i);hidePart(npcParts.moustache,i);hidePart(npcParts.prisonHelmet,i);hidePart(npcParts.prisonHelmetBand,i);hidePart(npcParts.prisonVisor,i);hidePart(npcParts.prisonVest,i);hidePart(npcParts.riotShield,i);hidePart(npcParts.shieldViewport,i);
            ...
            if(prisonStaff)setPart(npcParts.prisonVest,i,rootMatrix,0,2.02,.02);
            if(prisonStaff&&src.shield){setPart(npcParts.riotShield,i,rootMatrix,-.92,1.7,.62,0,unitScale,.12,-.08);setPart(npcParts.shieldViewport,i,rootMatrix,-.92,2.05,.72,0,unitScale,.12,-.08);}
        ...
        let moving=false,playerArrestPhase='',playerArrestProgress=0,playerArrestCuffed=false,playerArrestHidden=false;
        if(bridge){
          bridge.updateDistrictEntry?.();
          const state=bridge.getPlayerState();
          playerArrestPhase=String(state.arrestPhase||'');playerArrestProgress=Math.max(0,Math.min(1,+state.arrestProgress||0));playerArrestCuffed=!!state.cuffed;playerArrestHidden=!!state.arrestHidden;if(playerArrestPhase)activeReloadProgress=0;
          ...
          activeReloadProgress=!playerArrestPhase&&state.reloading?Math.max(0,Math.min(1,+state.reloadProgress||0)):0;if(!playerArrestPhase&&state.reloading&&!reloadWasActive)spawnReloadDebris();reloadWasActive=!playerArrestPhase&&!!state.reloading;renderer.domElement.dataset.reloadAnimation=!playerArrestPhase&&state.reloading?`${currentWeaponFx.family}:${activeReloadProgress.toFixed(2)}`:'idle';
          ...
          const visualSig=`${state.role}:${state.family||''}:${state.customGangColor||''}:${state.look?.skin??0}:${state.weapon||''}:${state.holsteredWeapon||''}:${state.driving?1:0}:${state.vehicleEntry?.token||''}:${state.arrestPhase||''}`;if(visualSig!==playerVisualSig){playerVisualSig=visualSig;let customRoleColor=0;try{if(state.customGangColor)customRoleColor=new THREE.Color(state.customGangColor).getHex();}catch(_){}const roleColor=state.role==='police'?0x285c91:state.role==='mafia'?(state.family==='moretti'?0xe6e0d3:0x25272d):state.role==='custom_gang'&&customRoleColor?customRoleColor:0x7a3035;suitMat.color.set(roleColor);skinMat.color.set([0xf0c3a0,0xd8a07c,0xb87955,0x8b583e,0x633c2d][Math.max(0,Math.min(4,+state.look?.skin||0))]);configureWeaponVisual(state.weapon);gun.visible=gun.visible&&!state.driving&&!state.vehicleEntry&&!state.arrestPhase;holsteredPistol.visible=!!state.holsteredWeapon&&!state.weapon&&!state.driving&&!state.vehicleEntry&&!state.arrestPhase;renderer.domElement.dataset.holsteredWeapon=String(state.holsteredWeapon||'');}
          carriedMoneyBag.visible=!!state.carryingBag&&!state.driving&&!state.arrestPhase;carriedMoneyBag.rotation.z=-.18+(moving?Math.sin(walkPhase)*.07:0);playerHpPct=Math.max(0,Math.min(1,(+state.hp||0)/100));if(lastPlayerHp>=0&&(+state.hp||0)<lastPlayerHp){playerHitUntil=t+480;playerHitSide=playerHitSide>0?-1:1;}lastPlayerHp=+state.hp||0;playerHpBar.scale.x=3.9*Math.max(.02,playerHpPct);playerHpBar.material.color.set(playerHpPct>.55?0x55e778:playerHpPct>.25?0xffc94d:0xff4d55);
        ...
        const recoilBody=Math.min(.19,recoilKick*.045),entrySit=vehicleEntryState?Math.max(0,(entryProgress-.45)/.55):0,playerGaitRoll=playerLimping?(moving?.105+playerStep*.04:.09):(moving?playerStep*.026:0);body.scale.y=vehicleEntryState?1-entrySit*.18:(moving?1:1+idleBreath*.018);body.position.y=2.65+(moving?0:idleBreath*.025)-recoilBody-entrySit*.58-playerHitPulse*.08;body.rotation.x=vehicleEntryState?entrySit*.48:-recoilBody*.62-playerHitPulse*.14;body.rotation.z=vehicleEntryState?Math.sin(entryProgress*Math.PI)*-.14:playerGaitRoll+playerHitPulse*playerHitSide*.17+recoilSide*recoilBody*.12;head.rotation.z=playerHitPulse*playerHitSide*.11+(playerLimping?.035:0);player.rotation.x=playerCrawling?Math.PI/2:0;playerContactShadow.visible=!playerCrawling;player.position.y=playerFloorElevation+(playerCrawling?.62:.08+(moving?(playerLimping?Math.max(0,-playerStep)*.07:Math.abs(playerStep)*.095):0));if(!playerArrestPhase&&(triggerHeld||keys.has('Space'))&&['smg','tommy_gun','golden_tommy','rifle'].includes(currentWeaponId))shoot(t);updateWeather(dt,t);updateStreetLife(t);
        if(playerArrestPhase){
          ...
          arrestCuffs.visible=playerArrestCuffed;
          if(fallen){
        ...
        if(muzzleLife>0&&!playerArrestPhase){muzzleLife-=dt;const flashFade=Math.max(0,muzzleLife/.14);muzzle.intensity=96*flashFade;localMuzzleFlash.visible=true;localMuzzleFlash.scale.multiplyScalar(.91);for(const child of localMuzzleFlash.children)child.material.opacity=flashFade;}else{muzzle.intensity=0;localMuzzleFlash.visible=false;}
