...
#include <colorspace_fragment>
      ...
      const hideNpcVisual=i=>{for(const [key,mesh] of Object.entries(npcParts)){if(key==='eyeWhite'||key==='pupil'||key==='shoe'||key==='uniqueGunLimb'){hidePart(mesh,i*2);hidePart(mesh,i*2+1);}else if(key==='uniqueGunSpike'){for(let k=0;k<4;k++)hidePart(mesh,i*4+k);}else hidePart(mesh,i);}};
