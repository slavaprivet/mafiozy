...
#include <colorspace_fragment>
      ...
      if((location.hostname==='127.0.0.1'||location.hostname==='localhost')&&new URLSearchParams(location.search).has('previewallweaponsqa'))setTimeout(()=>{const qa=renderer.domElement.__animationQaWeapons?.();if(!qa)return;renderer.domElement.dataset.weaponQaResult=`${qa.weapons.length}:${qa.aliases.length}:${qa.weapons.reduce((sum,item)=>sum+item.faults.length,0)}`;renderer.domElement.dataset.weaponQaProjectileProfiles='pistol,nagan,revolver,pistol_heavy,pistol_gold,shotgun,smg,tommy_gun,golden_tommy,rifle,sniper,taser,rpg';renderer.domElement.dataset.weaponQaNonProjectileProfiles='grenade,molotov,c4,none';},900);
