...
#include <colorspace_fragment>
      ...
      const npcPrompt=document.createElement('div');npcPrompt.id='npcActionPrompt';npcPrompt.style.cssText='position:absolute;left:0;top:0;z-index:23;display:none;min-width:158px;padding:7px 11px 8px;border:1px solid #dfb75b;border-radius:11px;background:linear-gradient(145deg,#0a0f16,#2b180e);color:#fff0bd;font:900 13px system-ui;text-align:center;text-shadow:0 1px 4px #000;pointer-events:none;box-shadow:0 7px 20px #000c,0 0 16px #d49b3d55;transform:translate3d(-999px,-999px,0) translate(-50%,-100%);will-change:transform;animation:mfzNpcPromptIn .18s ease-out both';npcPrompt.innerHTML='<span style="display:inline-grid;place-items:center;min-width:22px;height:22px;margin-right:7px;border:1px solid #fff4b9;border-radius:7px;background:linear-gradient(#ffe27c,#b87519);color:#171008;font:1000 12px system-ui;animation:mfzBuildingKeyPulse 1.25s ease-in-out infinite">E</span><span>Действия</span>';stage.appendChild(npcPrompt);const npcPromptProjection=new THREE.Vector3();
