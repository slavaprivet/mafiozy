...
transformed.z+=cos(mfzWindTime*.82+mfzPhase*1.31+position.z*.42)*mfzGust*mfzWeight*${bendZ};`);mat.userData.shader=shader;};mat.customProgramCacheKey=()=>`mfz-wind-v3-${color}-${bendX}-${bendZ}`;windLeafMaterials.push(mat);return mat;};
...
#include <colorspace_fragment>
      ...
      if(worldSnapshot?.landmarks){
        ...
        if(jail){
          ...
          for(const bz of [intakeNorthZ+4.0,intakeSouthZ-4.0]){pbox(intakeCenterX-1.4,bz,11.6,1.2,.28,benchWood,1.18);for(const bx of [intakeCenterX-6.2,intakeCenterX+3.4])pbox(bx,bz,.42,.82,1.18,darkSteel,.68);pbox(intakeCenterX-1.4,bz,11.8,.24,1.55,darkSteel,1.36);}
          ...
          const searchZ=intakeNorthZ+4.1;for(const sx of [-1.75,1.75])pbox(intakeCenterX+sx,searchZ,.46,.46,3.8,darkSteel,.68);pbox(intakeCenterX,searchZ,4.0,.46,.38,darkSteel,4.1);pbox(intakeCenterX,searchZ+.04,3.25,.08,2.8,screenMat,1.18);pbox(intakeCenterX,searchZ+3.05,3.65,1.35,1.12,concreteDark,.68);
        ...
        const buildAuthoredBankExterior3D=bank=>{
          ...
          const sign=labelSprite(bank.size==='large'?'🏦 ЦЕНТРАЛЬНЫЙ БАНК':`🏦 ${bank.name}`,'#e4bd55');sign.position.set(0,bodyH+2.35*scale,front+porticoDepth*.95);sign.scale.multiplyScalar(bank.size==='small'?.78:bank.size==='large'?1.18:1);root.add(sign);
          ...
          const dockLabel=labelSprite('ИНКАССАЦИЯ','#8fdcff');dockLabel.position.set(serviceSide*(width*.5+.48),3.15,-depth*.18);dockLabel.scale.set(3.8,.78,1);root.add(dockLabel);
        ...
        for(const poi of snapshot?.pois||[])if(poi.id==='junkyard'){
          ...
          if(source!=='initial'){const label=labelSprite('🏗️ ГОРОДСКАЯ СВАЛКА','#e1a33a');label.position.set(toX(poi.c),10,toZ(poi.r));scene.add(label);}
      ...
      const defsFromSnapshot=snapshot=>(snapshot?.blocks||[]).flatMap(b=>(b.buildingParts?.length?b.buildingParts:(b.building?[b.building]:[])).flatMap((q,partIndex)=>{
        const candidates=[...(snapshot.pois||[]),...(snapshot.landmarks?.mafiaHq?[snapshot.landmarks.mafiaHq]:[])],containsPoi=p=>Number.isFinite(+q.minR)?+p.r>=+q.minR&&+p.r<=+q.maxR&&+p.c>=+q.minC&&+p.c<=+q.maxC:Math.abs((+p.r||0)-q.r)<=Math.max(.8,(+q.d||1)*.5)&&Math.abs((+p.c||0)-q.c)<=Math.max(.8,(+q.w||1)*.5),containedPoi=candidates.find(containsPoi)||null,nearMatch=candidates.map(p=>({p,d:Math.hypot((+p.r||0)-q.r,(+p.c||0)-q.c)})).filter(x=>x.d<6.2).sort((a,b)=>a.d-b.d)[0]||null,nearPoi=containedPoi||nearMatch?.p||null,architecturalKind=containedPoi?.id||(partIndex===0&&nearMatch?.d<1.8?nearPoi?.id:null)||null;
        ...
        return [[(q.c-originC)*WORLD_SCALE,(q.r-originR)*WORLD_SCALE,q.w*WORLD_SCALE,q.d*WORLD_SCALE,q.height,styleIndexes[b.styleId]??0,(nearPoi?.name||nearPoi?.label||'').toString().slice(0,14).toUpperCase(),b.styleId||'downtown',{r:q.r,c:q.c,entryR:Number.isFinite(+containedPoi?.entryR)?+containedPoi.entryR:q.entryR,entryC:Number.isFinite(+containedPoi?.entryC)?+containedPoi.entryC:q.entryC,w:q.w,d:q.d,minR:q.minR,maxR:q.maxR,minC:q.minC,maxC:q.maxC,tiles:q.tiles,primary:partIndex===0||!!containedPoi,architecturalKind}]];
      }));
      ...
      const factorySmokePuffs=[],blackmarketDoorActors=[],blackmarketGuardActors=[],cityDoorActors=[];
      const addThemedArchitecture=(kind,x,z,w,d,h,seed)=>{const front=z+d/2+.08,add=(xx,zz,ww,dd,hh,mat,yy=hh/2)=>{const q=box(xx,zz,ww,dd,hh,mat);q.position.y=yy;return q;};
        if(kind==='mafia_hq'){
          ...
          const marketBrick=new THREE.MeshStandardMaterial({color:0x241f24,roughness:.92,metalness:.06}),marketSteel=new THREE.MeshStandardMaterial({color:0x171d23,roughness:.34,metalness:.82}),marketTrim=new THREE.MeshStandardMaterial({color:0x6d737a,roughness:.42,metalness:.76}),marketGlow=new THREE.MeshBasicMaterial({color:accent,toneMapped:false}),marketGlass=new THREE.MeshPhysicalMaterial({color:0x292238,emissive:new THREE.Color(accent),emissiveIntensity:.28,roughness:.12,metalness:.18,transmission:.08,clearcoat:1}),marketDoorLeaf=new THREE.MeshStandardMaterial({color:0x111820,emissive:new THREE.Color(accent).multiplyScalar(.2),emissiveIntensity:.7,roughness:.3,metalness:.88,envMap:cityEnvironment,envMapIntensity:1.15}),marketDoorVoid=new THREE.MeshBasicMaterial({color:0x010204,toneMapped:false});
          ...
          add(x,entryZ,w*.27,.48,4.8,marketSteel,2.4);const doorVoid=add(x,entryZ+.18,doorW*2.08,.14,doorH,marketDoorVoid,doorH*.5);add(x,front+1.05,w*.36,1.85,.28,marketTrim,4.82);
          ...
          const leftDoor=new THREE.Mesh(new THREE.BoxGeometry(doorW,doorH,.24),marketDoorLeaf),rightDoor=new THREE.Mesh(new THREE.BoxGeometry(doorW,doorH,.24),marketDoorLeaf);leftDoor.position.x=doorW*.5;rightDoor.position.x=-doorW*.5;leftDoor.castShadow=rightDoor.castShadow=true;leftPivot.add(leftDoor);rightPivot.add(rightDoor);
          ...
          const doorGlowTop=add(x,entryZ+.4,doorW*2.2,.09,.1,marketGlow,doorH+.18),doorGlowLeft=add(x-doorW*1.08,entryZ+.4,.08,.09,doorH*.98,marketGlow,doorH*.5),doorGlowRight=add(x+doorW*1.08,entryZ+.4,.08,.09,doorH*.98,marketGlow,doorH*.5);
          blackmarketDoorActors.push({left:leftPivot,right:rightPivot,void:doorVoid,glow:[doorGlowTop,doorGlowLeft,doorGlowRight],x,z:front+1.25,kind,open:0});
          ...
          renderer.domElement.dataset.blackmarketArchitecture='armored-brick-neon-guarded-v2-dark-portal';renderer.domElement.dataset.blackmarketDoors=String(blackmarketDoorActors.length);renderer.domElement.dataset.blackmarketGuards=String(blackmarketGuardActors.length);
      ...
      const ensureMajorFactoryExterior=(snapshot,source='initial')=>{
        ...
        const factoryPoiLabel=labelSprite('INDUSTRIAL WORKS  ·  PROMZONA','#ffb54e');factoryPoiLabel.position.set(fx,13.4,fz+7.8);factoryPoiLabel.scale.set(13.5,2.25,1);scene.add(factoryPoiLabel);authoredLandmarkStaticDetails.push(...scene.children.slice(staticDetailStart));
      ...
      const cityDoorFrameMaterial=new THREE.MeshStandardMaterial({color:0x182129,roughness:.3,metalness:.86}),cityDoorStepMaterial=new THREE.MeshStandardMaterial({color:0x394149,roughness:.86}),cityDoorVoidMaterial=new THREE.MeshBasicMaterial({color:0x010204,toneMapped:false}),cityDoorMaterials=new Map(),cityDoorGlowMaterials=new Map();
      const cityDoorColorFor=(kind,district)=>({hospital:0x55dceb,hospital_east:0x55dceb,police:0x3c91e6,firestation:0xe34b45,mafia_hq:0xd7ac48,mansion:0xb88e52,casino:0xe1bd45,club:0xc35cff,bar:0xd94a82,gym:0x54a4d8,job_office:0xd1a455,coffee:0xd48a4b,pizza:0xe4543d,garage:0xe2a62c,carwash:0x43cddb,barbershop:0xe76370,warehouse:0xd29442,port:0xe47737,market:0x55c982,factory:0xcf7b35}[kind]||({downtown:0x55a9dc,nightlife:0xc85cff,rich:0xd0a95b,industrial:0xd17a37,poor:0xa85248,countryside:0x68a35a,coast:0x42bfb5,chinatown_poor:0xb54b42,chinatown_market:0xe0a541,chinatown_neon:0xd45cff,chinatown_rich:0x4ebba6,chinatown_docks:0xd27139}[district]||0x6ca7c9));
      const cityDoorMaterialFor=color=>{const key=String(color);if(!cityDoorMaterials.has(key))cityDoorMaterials.set(key,new THREE.MeshStandardMaterial({color,emissive:new THREE.Color(color).multiplyScalar(.22),emissiveIntensity:.3,roughness:.22,metalness:.72,envMap:cityEnvironment,envMapIntensity:1.1}));return cityDoorMaterials.get(key);};
      const cityDoorGlowFor=color=>{const key=String(color);if(!cityDoorGlowMaterials.has(key))cityDoorGlowMaterials.set(key,new THREE.MeshBasicMaterial({color,toneMapped:false}));return cityDoorGlowMaterials.get(key);};
      const addAnimatedCityDoor=({x,z,w,d,meta,kind,district,seed=0,main=null})=>{
        if(String(kind||'').startsWith('blackmarket'))return null;
        const centerR=+meta?.r||originR+z/WORLD_SCALE,centerC=+meta?.c||originC+x/WORLD_SCALE,entryR=+meta?.entryR,entryC=+meta?.entryC;
        let side='south';if(Number.isFinite(entryR)&&Number.isFinite(entryC)){const dr=entryR-centerR,dc=entryC-centerC;side=Math.abs(dc)>Math.abs(dr)?(dc>=0?'east':'west'):(dr>=0?'south':'north');}
        const entryX=Number.isFinite(entryC)?(entryC-originC)*WORLD_SCALE:x,entryZ=Number.isFinite(entryR)?(entryR-originR)*WORLD_SCALE:z,edgePad=1.6;
        let doorX=x,doorZ=z+d/2+.2,rotation=0,span=w;
        if(side==='north'){doorZ=z-d/2-.2;doorX=THREE.MathUtils.clamp(entryX,x-w/2+edgePad,x+w/2-edgePad);rotation=Math.PI;}
        else if(side==='east'){doorX=x+w/2+.2;doorZ=THREE.MathUtils.clamp(entryZ,z-d/2+edgePad,z+d/2-edgePad);rotation=Math.PI/2;span=d;}
        else if(side==='west'){doorX=x-w/2-.2;doorZ=THREE.MathUtils.clamp(entryZ,z-d/2+edgePad,z+d/2-edgePad);rotation=-Math.PI/2;span=d;}
        else doorX=THREE.MathUtils.clamp(entryX,x-w/2+edgePad,x+w/2-edgePad);
        const color=cityDoorColorFor(kind,district),doorMaterial=cityDoorMaterialFor(color),root=new THREE.Group(),leafW=Math.min(1.65,Math.max(.95,span*.085)),doorH=3.65;
        root.position.set(doorX,0,doorZ);root.rotation.y=rotation;root.visible=false;root.name=`animated-city-door:${kind||district||'building'}:${Math.round(centerR)},${Math.round(centerC)}`;scene.add(root);
        const voidPanel=new THREE.Mesh(new THREE.BoxGeometry(leafW*2.06,doorH*.98,.12),cityDoorVoidMaterial);voidPanel.position.set(0,doorH*.5,-.11);root.add(voidPanel);
        const leftPivot=new THREE.Group(),rightPivot=new THREE.Group();leftPivot.position.set(-leafW,doorH*.5,.05);rightPivot.position.set(leafW,doorH*.5,.05);root.add(leftPivot,rightPivot);
        const left=new THREE.Mesh(new THREE.BoxGeometry(leafW,doorH,.24),doorMaterial),right=new THREE.Mesh(new THREE.BoxGeometry(leafW,doorH,.24),doorMaterial);left.position.x=leafW*.5;right.position.x=-leafW*.5;leftPivot.add(left);rightPivot.add(right);
        const top=new THREE.Mesh(new THREE.BoxGeometry(leafW*2.35,.3,.4),cityDoorFrameMaterial);top.position.set(0,doorH+.13,0);root.add(top);for(const sx of [-leafW*1.12,leafW*1.12]){const post=new THREE.Mesh(new THREE.BoxGeometry(.3,doorH+.3,.4),cityDoorFrameMaterial);post.position.set(sx,(doorH+.3)/2,0);root.add(post);}
        const light=new THREE.Mesh(new THREE.BoxGeometry(leafW*1.72,.11,.12),cityDoorGlowFor(color));light.position.set(0,doorH+.15,.24);root.add(light);const step=new THREE.Mesh(new THREE.BoxGeometry(leafW*2.7,.16,1.15),cityDoorStepMaterial);step.position.set(0,.08,.68);root.add(step);
        const actor={root,left:leftPivot,right:rightPivot,void:voidPanel,x:doorX,z:doorZ,open:0,color,side,seed,main};cityDoorActors.push(actor);renderer.domElement.dataset.cityAnimatedDoors=String(cityDoorActors.length);renderer.domElement.dataset.cityDoorPortal='dark-recess-v1';return actor;
      };
      ...
      const createBuilding=(definition,bi)=>{
        ...
        const localRoof=roofMat.clone();localRoof.color.setHex(architectureFamily.roof);
        ...
        // Authored POIs must be complete even at the edge of the first streamed
        // snapshot. Otherwise their footprint key is cached as loaded while the
        // door/facade is skipped forever after the player approaches it.
        const detailed=buildingMeta.primary!==false&&(!!architecturalKind||detailDistanceTiles<=detailRadius),steppedTower=detailed&&!architecturalKind&&(districtStyle==='downtown'||districtStyle==='rich'||districtStyle==='chinatown_rich')&&h>24,lowerH=steppedTower?h*.64:h;
        ...
        addAnimatedCityDoor({x,z,w,d,meta:buildingMeta,kind:architecturalKind,district:districtStyle,seed:buildingSeed,main:mainBuilding});
      };
      ...
      const obstaclePools={
        ...
        carBody:makeObstaclePool(new THREE.BoxGeometry(3.5,.85,1.75),new THREE.MeshStandardMaterial({color:0xffffff,roughness:.48,metalness:.32})),
        carRoof:makeObstaclePool(new THREE.BoxGeometry(1.75,.65,1.48),new THREE.MeshPhysicalMaterial({color:0x506b78,roughness:.12,metalness:.18,clearcoat:1}))
      };
      ...
      const mapObstacleKeys=new Set(),addMapCollisionVisuals=snapshot=>{for(const o of snapshot?.obstacles||[]){const key=`${o.type}:${o.r}:${o.c}`;if(mapObstacleKeys.has(key))continue;mapObstacleKeys.add(key);const x=(o.c-originC)*WORLD_SCALE,z=(o.r-originR)*WORLD_SCALE,seed=+o.seed||0;
        ...
        else if(o.type===2){const yaw=(seed&1)?0:Math.PI/2;addObstacleInstance(obstaclePools.carBody,x,.62,z,yaw,1,1,1,obstacleCarColors[seed%obstacleCarColors.length]);addObstacleInstance(obstaclePools.carRoof,x,1.3,z,yaw);}
      ...
      const leftForearm=new THREE.Mesh(new THREE.BoxGeometry(.43,1.22,.46),suitMat),rightForearm=leftForearm.clone(),leftHand=new THREE.Mesh(new THREE.SphereGeometry(.27,12,9),skinMat),rightHand=leftHand.clone();
      ...
      const weaponExtraParts=[revolverCylinder,revolverBarrel,revolverHammer,heavySlide,heavyBarrel,heavyRearSight,goldTopRib,goldInlay,shotgunPump,tommyDrum,rifleForegrip,sniperLens,taserCoil,muzzleBrake,grenadeModel,molotovModel,c4Model,rpgRocket,reloadRound];
      ...
      const posePlayerLimb=(mesh,start,end,baseLength)=>{limbDir.subVectors(end,start);const len=Math.max(.01,limbDir.length());limbMid.addVectors(start,end).multiplyScalar(.5);mesh.position.copy(limbMid);mesh.quaternion.setFromUnitVectors(limbUp,limbDir.multiplyScalar(1/len));mesh.scale.set(1,len/baseLength,1);mesh.visible=true;};
      const hidePlayerGripParts=()=>{for(const part of [leftForearm,rightForearm,leftHand,rightHand])part.visible=false;leftArm.scale.set(1,1,1);rightArm.scale.set(1,1,1);};
      const poseTwoHandedGrip=(kick=0,reload=0)=>{
        ...
        rightShoulder.set(.76,3.38,.02-kick*.08);leftShoulder.set(-.76,3.38,.02-kick*.07);
        ...
        rightHand.position.copy(rightHandTarget);leftHand.position.copy(leftHandTarget);rightHand.scale.set(1,1,1);leftHand.scale.set(1,1,1);rightHand.visible=leftHand.visible=true;
      };
      ...
      const configureWeaponVisual=id=>{
        ...
        tommyDrum.position.set(0,-.43,.72);tommyDrum.rotation.set(0,0,Math.PI/2);revolverCylinder.position.x=0;rpgTube.position.z=.9;rpgTube.rotation.z=0;rpgRocket.position.set(0,.08,2.28);rpgRocket.rotation.set(0,0,0);
      ...
      const npcParts={
        body:makeInstances(new THREE.BoxGeometry(1.15,1.8,.75),new THREE.MeshStandardMaterial({color:0xffffff,vertexColors:true,roughness:.76,metalness:0,emissive:0x15181d,emissiveIntensity:.48}),NPC_CAP),
        head:makeInstances(new THREE.SphereGeometry(.44,12,9),new THREE.MeshStandardMaterial({color:0xffffff,vertexColors:true,roughness:.84,metalness:0,emissive:0x24160f,emissiveIntensity:.38}),NPC_CAP),
        leftLeg:makeInstances(new THREE.BoxGeometry(.38,1.25,.38),new THREE.MeshStandardMaterial({color:0xffffff,vertexColors:true,roughness:.8,emissive:0x15181d,emissiveIntensity:.46}),NPC_CAP),
        rightLeg:makeInstances(new THREE.BoxGeometry(.38,1.25,.38),new THREE.MeshStandardMaterial({color:0xffffff,vertexColors:true,roughness:.8,emissive:0x15181d,emissiveIntensity:.46}),NPC_CAP),
        leftArm:makeInstances(new THREE.BoxGeometry(.3,1.35,.38),new THREE.MeshStandardMaterial({color:0xffffff,vertexColors:true,roughness:.78,emissive:0x15181d,emissiveIntensity:.46}),NPC_CAP),
        rightArm:makeInstances(new THREE.BoxGeometry(.3,1.35,.38),new THREE.MeshStandardMaterial({color:0xffffff,vertexColors:true,roughness:.78,emissive:0x15181d,emissiveIntensity:.46}),NPC_CAP),
      ...
      const updateNpcLabel=(entry,src,x,y,z)=>{if(src.said){const sig=`said-v252:${src.hired?1:0}:${src.speech||''}`;if(sig!==entry.sig){entry.sig=sig;const c=entry.canvas.getContext('2d');c.clearRect(0,0,768,192);c.fillStyle='rgba(12,10,7,.98)';c.fillRect(36,8,696,176);c.strokeStyle='#e5bd55';c.lineWidth=11;c.strokeRect(42,14,684,164);fitOutlinedLabelText(c,'САИД · ВАЖНЫЙ NPC',384,57,620,54,'#ffd76b');fitOutlinedLabelText(c,src.hired?'Правая рука · нанят':'Поговорить · можно нанять',384,135,620,45,'#ffffff');entry.sprite.scale.set(17.2,4.3,1);entry.texture.needsUpdate=true;}entry.sprite.visible=true;entry.sprite.position.set(x,y+.35,z);return;}const role=String(src.role||'civilian').toLowerCase(),behavior=String(src.behavior||''),prisonStaff=behavior==='prison_patrol'||role.startsWith('prison_'),gang=!!src.gang||role.includes('gang')||role.includes('boss')||role.includes('district_')||role.includes('occupier'),police=!!src.police||role.includes('police')||role.includes('cop'),guard=role.includes('guard'),medic=role.includes('medic'),civilian=!police&&!gang&&!guard,title=police?'ПОЛИЦИЯ':gang?(behavior==='follows_player'?'ТВОЙ БОЕЦ':behavior==='guards_nest'?'ОХРАНА ГНЕЗДА':behavior==='guards_lair'?'ОХРАНА ЛОГОВА':behavior==='hostile_zone'?'ВРАЖЕСКАЯ БАНДА':'БОЕЦ БАНДЫ'):guard?'ОХРАНА':medic?'МЕДИК':'ЖИТЕЛЬ',accent=police?'#58b9ff':gang?'#ff5367':guard?'#ffc857':medic?'#64e7dd':'#7ee7a5',name=String(src.name||title).slice(0,18),hp=Math.max(0,Math.round(+src.hp||0)),maxHp=Math.max(1,Math.round(+src.maxHp||60)),level=gang?Math.max(1,Math.round(+src.level||1)):null,family=String(src.family||'').slice(0,14),rankText=gang?` · ${level} ур.`:'',subline=prisonStaff?`${police?'ПАТРУЛЬ':'КАРАУЛ'} · ${hp}/${maxHp}`:`${name} · HP ${hp}/${maxHp}`,sig=`v238:${title}:${subline}:${rankText}:${family}`;if(sig!==entry.sig){entry.sig=sig;const c=entry.canvas.getContext('2d');c.clearRect(0,0,768,192);if(civilian){c.fillStyle='rgba(5,10,17,.97)';c.fillRect(84,8,600,176);c.strokeStyle=accent;c.lineWidth=10;c.strokeRect(90,14,588,164);fitOutlinedLabelText(c,title,384,57,520,56,accent);fitOutlinedLabelText(c,subline,384,135,540,51,'#ffffff');entry.sprite.scale.set(15.5,3.88,1);}else{c.fillStyle='rgba(3,8,14,.98)';c.fillRect(8,8,752,176);c.strokeStyle=accent;c.lineWidth=11;c.strokeRect(14,14,740,164);c.fillStyle=accent;c.fillRect(30,88,708,5);fitOutlinedLabelText(c,`${title}${rankText}${family?` · ${family}`:''}`,384,55,690,prisonStaff?62:54,accent);fitOutlinedLabelText(c,subline,384,137,680,prisonStaff?60:51,'#ffffff');entry.sprite.scale.set(prisonStaff?17.8:17.2,prisonStaff?4.75:4.3,1);}entry.texture.needsUpdate=true;}entry.sprite.visible=true;entry.sprite.position.set(x,y,z);};
      const updateNpcSpeechLabel=(entry,src,x,y,z)=>{const speech=String(src.speech||'').trim();if(!speech){updateNpcLabel(entry,src,x,y,z);return;}const sig=`speech-v201:${speech}`;if(sig!==entry.sig){entry.sig=sig;const c=entry.canvas.getContext('2d');c.clearRect(0,0,768,192);c.fillStyle='rgba(255,250,226,.98)';c.fillRect(18,6,732,150);c.strokeStyle='#d2a947';c.lineWidth=10;c.strokeRect(24,12,720,138);c.fillStyle='rgba(255,250,226,.98)';c.beginPath();c.moveTo(326,156);c.lineTo(384,190);c.lineTo(442,156);c.closePath();c.fill();c.font='900 50px system-ui';c.textAlign='center';c.textBaseline='middle';c.fillStyle='#17130d';const words=speech.split(/\s+/),lines=[''];for(const word of words){const i=lines.length-1,test=(lines[i]+' '+word).trim();if(c.measureText(test).width>680&&lines.length<2)lines.push(word);else lines[i]=test;}if(lines.length===1)c.fillText(lines[0],384,83);else{c.fillText(lines[0],384,60);c.fillText(lines[1],384,116);}entry.sprite.scale.set(16.8,4.2,1);entry.texture.needsUpdate=true;}entry.sprite.visible=true;entry.sprite.position.set(x,y+.78,z);};
      ...
      const updateCustodyAwareNpcLabel=(entry,src,x,y,z)=>{
        ...
        // Реплики и Саид остаются крупнее, потому что появляются временно
        // или являются важным взаимодействием.
        if(!src?.said&&!String(src?.speech||'').trim()){
          const role=String(src?.role||'').toLowerCase(),prisonStaff=String(src?.behavior||'')==='prison_patrol'||role.startsWith('prison_');
          entry.sprite.scale.set(prisonStaff?11.8:10.6,prisonStaff?3.05:2.65,1);
        }
        if(custody&&escortOfficer){
          ...
          entry.sprite.scale.set(8.6,2.15,1);
      ...
      const npcLifeAnimationPose=(src,i,t)=>{
        ...
        const roll=(crawling?step*.016*gait:(limping?.105+step*.04*gait:step*.026*gait))+hit*hitSide*(.16+(1-Math.abs(hitForward))*.13);
        const crouch=cowering?.86:helping?.42:0,statePitch=cowering?.28:helping?.38:panicking?-.055:alerted?-.025:0,pitch=(crawling?1.22*crawlBlend:(walking?-(.025+pace*.035)*gait:0))+statePitch+hit*(.07+hitForward*.2);
      ...
      const decorateBankInterior=data=>{
        ...
        for(const material of [wall,wallDark]){material.polygonOffset=true;material.polygonOffsetFactor=1;material.polygonOffsetUnits=1;}
        const marbleCanvas=document.createElement('canvas');marbleCanvas.width=marbleCanvas.height=512;const marbleCtx=marbleCanvas.getContext('2d'),tileSize=128;marbleCtx.fillStyle='#c8bead';marbleCtx.fillRect(0,0,512,512);for(let row=0;row<4;row++)for(let col=0;col<4;col++){const shade=(row+col)%3===0?'#d9d0c1':(row+col)%3===1?'#cfc5b5':'#c4b9a8';marbleCtx.fillStyle=shade;marbleCtx.fillRect(col*tileSize+4,row*tileSize+4,tileSize-8,tileSize-8);marbleCtx.strokeStyle='rgba(105,91,74,.28)';marbleCtx.lineWidth=3;marbleCtx.strokeRect(col*tileSize+4,row*tileSize+4,tileSize-8,tileSize-8);for(let vein=0;vein<3;vein++){const phase=row*13+col*17+vein*23;marbleCtx.strokeStyle=vein===0?'rgba(115,102,90,.17)':'rgba(255,250,236,.28)';marbleCtx.lineWidth=vein===0?2:1;marbleCtx.beginPath();for(let step=0;step<=8;step++){const px=col*tileSize+8+step*(tileSize-16)/8,py=row*tileSize+18+vein*29+Math.sin((step+phase)*1.37)*11;if(step)marbleCtx.lineTo(px,py);else marbleCtx.moveTo(px,py);}marbleCtx.stroke();}}const marbleTexture=new THREE.CanvasTexture(marbleCanvas);marbleTexture.colorSpace=THREE.SRGBColorSpace;marbleTexture.wrapS=marbleTexture.wrapT=THREE.RepeatWrapping;marbleTexture.repeat.set(Math.max(4,W/4),Math.max(4,H/4));marbleTexture.anisotropy=Math.min(8,renderer.capabilities.getMaxAnisotropy());const marble=new THREE.MeshPhysicalMaterial({color:0xffffff,map:marbleTexture,roughness:.46,metalness:.03,clearcoat:.35,clearcoatRoughness:.42}),marbleDark=std(0x8e897f,.38,.18),carpet=std(0x273d59,.95),runner=std(0x711b2b,.88),runnerDark=std(0x3c101b,.92),loungeWood=std(0x745039,.72);
        ...
        const glass=new THREE.MeshPhysicalMaterial({color:0x96c9d7,transparent:true,opacity:.3,transmission:.38,roughness:.06,metalness:.08,side:THREE.DoubleSide,depthWrite:false});
        ...
        const floorZone=(c,r,w,d,material,y=.075)=>{const overlay=material.clone();overlay.polygonOffset=true;overlay.polygonOffsetFactor=-2;overlay.polygonOffsetUnits=-2;const floor=add(new THREE.PlaneGeometry(w*S,d*S),overlay,c,y,r);floor.rotation.x=-Math.PI/2;floor.receiveShadow=true;floor.castShadow=false;return floor;};
        const wallPost=(c,r,height,material)=>{const post=add(new THREE.CylinderGeometry(.16*S,.18*S,height,12),material,c,height/2,r);post.castShadow=true;return post;};
        const hWall=(c0,c1,r,height=4.35,material=wall)=>{if(c1-c0<.08)return;const inset=.035,len=Math.max(.08,(c1-c0-inset*2))*S,baseMat=material===wallDark?steelDark:woodDark,capMat=material===wallDark?steel:brass;add(new THREE.BoxGeometry(len,height,.3*S),material,(c0+c1)/2,height/2,r);add(new THREE.BoxGeometry(len-.04,.2,.35*S),baseMat,(c0+c1)/2,.105,r);add(new THREE.BoxGeometry(len-.06,.14,.35*S),capMat,(c0+c1)/2,height+.07,r).castShadow=false;wallPost(c0,r,height,capMat);wallPost(c1,r,height,capMat);};
        const vWall=(c,r0,r1,height=4.35,material=wall)=>{if(r1-r0<.08)return;const inset=.035,len=Math.max(.08,(r1-r0-inset*2))*S,baseMat=material===wallDark?steelDark:woodDark,capMat=material===wallDark?steel:brass;add(new THREE.BoxGeometry(.3*S,height,len),material,c,height/2,(r0+r1)/2);add(new THREE.BoxGeometry(.35*S,.2,len-.04),baseMat,c,.105,(r0+r1)/2);add(new THREE.BoxGeometry(.35*S,.14,len-.06),capMat,c,height+.07,(r0+r1)/2).castShadow=false;wallPost(c,r0,height,capMat);wallPost(c,r1,height,capMat);};
        const wallPanelH=(c0,c1,r,face=1)=>{const span=c1-c0;if(span<1)return;const panel=add(new THREE.BoxGeometry((span-.18)*S,1.02,.055*S),cream,(c0+c1)/2,.72,r+face*.19);panel.castShadow=false;panel.material.polygonOffset=true;panel.material.polygonOffsetFactor=-2;panel.material.polygonOffsetUnits=-2;add(new THREE.BoxGeometry((span-.08)*S,.08,.075*S),brass,(c0+c1)/2,1.28,r+face*.205).castShadow=false;for(let c=c0+.75;c<c1-.3;c+=1.55)add(new THREE.BoxGeometry(.055*S,.82,.08*S),woodDark,c,.72,r+face*.21).castShadow=false;};
        const wallPanelV=(c,r0,r1,face=1)=>{const span=r1-r0;if(span<1)return;const panel=add(new THREE.BoxGeometry(.055*S,1.02,(span-.18)*S),cream,c+face*.19,.72,(r0+r1)/2);panel.castShadow=false;panel.material.polygonOffset=true;panel.material.polygonOffsetFactor=-2;panel.material.polygonOffsetUnits=-2;add(new THREE.BoxGeometry(.075*S,.08,(span-.08)*S),brass,c+face*.205,1.28,(r0+r1)/2).castShadow=false;for(let r=r0+.75;r<r1-.3;r+=1.55)add(new THREE.BoxGeometry(.08*S,.82,.055*S),woodDark,c+face*.21,.72,r).castShadow=false;};
        const wallSconce=(c,r,turn=0)=>{const g=new THREE.Group();g.position.set(x(c),2.65,z(r));g.rotation.y=turn;g.layers.set(1);interiorGroup.add(g);addRaw(new THREE.CylinderGeometry(.08,.12,.62,10),brass,0,0,-.12,g).rotation.x=Math.PI/2;const shade=addRaw(new THREE.SphereGeometry(.25,14,8),new THREE.MeshBasicMaterial({color:0xffd79a,toneMapped:true}),0,.08,.22,g);shade.scale.set(.72,1,.5);shade.castShadow=false;};
        const signSprite=(text,color='#d4b35c')=>{const words=String(text).trim().split(/\s+/),lines=[];let line='';for(const word of words){const trial=line?`${line} ${word}`:word;if(trial.length>20&&line){lines.push(line);line=word;}else line=trial;}if(line)lines.push(line);if(lines.length>2)lines.splice(1,lines.length-1,lines.slice(1).join(' '));const canvas=document.createElement('canvas');canvas.width=768;canvas.height=lines.length>1?224:176;const ctx=canvas.getContext('2d');ctx.fillStyle='rgba(15,22,29,.96)';ctx.fillRect(5,5,canvas.width-10,canvas.height-10);ctx.strokeStyle=color;ctx.lineWidth=8;ctx.strokeRect(12,12,canvas.width-24,canvas.height-24);let font=64;do{ctx.font=`900 ${font}px system-ui`;font-=2;}while(font>34&&lines.some(value=>ctx.measureText(value).width>canvas.width-70));ctx.font=`900 ${font+2}px system-ui`;ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillStyle='#f6edd9';lines.forEach((value,i)=>ctx.fillText(value,canvas.width/2,canvas.height*(i+1)/(lines.length+1)));const texture=new THREE.CanvasTexture(canvas);texture.colorSpace=THREE.SRGBColorSpace;texture.anisotropy=Math.min(8,renderer.capabilities.getMaxAnisotropy());const sprite=new THREE.Sprite(new THREE.SpriteMaterial({map:texture,transparent:true,depthTest:true,depthWrite:false,toneMapped:false}));sprite.renderOrder=24;sprite.layers.set(1);sprite.userData.signLines=lines.length;return sprite;};
        const roomSign=(text,c,r,y=4.7,color='#d4b35c',scale=1)=>{const sprite=signSprite(text,color),width=THREE.MathUtils.clamp(5.8+String(text).length*.26,6.4,12.5)*Math.max(.88,scale);sprite.position.set(x(c),y,z(r));sprite.scale.set(width,sprite.userData.signLines>1?2.15:1.65,1);interiorGroup.add(sprite);return sprite;};
        ...
        if(data.room==='vault'){
          interiorFloor=floorZone(W/2,H/2,W,H,steelDark,.018);
          for(let r=.4;r<H;r+=2)for(let c=.4;c<W;c+=2){const plateMat=(Math.floor(r)+Math.floor(c))%2?std(0x232d35,.5,.58):std(0x2c3740,.45,.64),plate=add(new THREE.PlaneGeometry(1.98*S,1.98*S),plateMat,c,.09,r);plate.rotation.x=-Math.PI/2;plate.material.polygonOffset=true;plate.material.polygonOffsetFactor=-2;plate.material.polygonOffsetUnits=-2;for(const dc of [-.78,.78])for(const dr of [-.78,.78])add(new THREE.CylinderGeometry(.045,.045,.035,8),steel,c+dc,.13,r+dr);}
          floorZone(W/2,H-.72,2.9,1.05,runnerDark,.16);for(const side of [-1,1])floorZone(W/2+side*1.34,H-.72,.12,1.05,brass,.18);for(const side of [-1,1]){const lane=add(new THREE.BoxGeometry(.055*S,.035,(H-2.1)*S),brass,W/2+side*1.18,.18,H/2);lane.castShadow=false;}
          hWall(0,W,.12,5.35,wallDark);vWall(.12,0,H,5.35,wallDark);vWall(W-.12,0,H,1.45,wallDark);
          // Gameplay already treats the complete south side as the return to the lobby.
          // Keep it architecturally open and park the released door leaf beside the portal.
          bankDoor(W/2,H-.34,-1,true);for(const c of [1.2,W-1.2])wallPost(c,H-.18,1.5,steelDark);roomSign('БАНКОВСКОЕ ХРАНИЛИЩЕ',W/2,.42,6.45,'#f0d78a',1.42);
          ...
          for(const [label,c] of [['A',W*.22],['B',W*.5],['C',W*.78]])roomSign(`СЕКЦИЯ ${label}`,c,1.03,4.35,'#9ddff1',.48);
          ...
          if(W<=9){rack(1.72,H*.44);rack(W-1.72,H*.59);}else{rack(W*.38,H*.48);rack(W*.64,H*.48);}
          for(const c of [W*.26,W*.74]){add(new THREE.BoxGeometry(2.1*S,3.25,1.75*S),steelDark,c,1.63,3.35);add(new THREE.BoxGeometry(1.82*S,2.83,.14*S),steel,c,1.63,4.29);const dial=add(new THREE.TorusGeometry(.36,.085,8,20),brass,c,1.75,4.39);dial.rotation.x=Math.PI/2;for(let k=0;k<4;k++){const a=k/4*Math.PI*2,spoke=add(new THREE.BoxGeometry(.66,.045,.045),brass,c+Math.cos(a)*.16,1.75+Math.sin(a)*.16,4.43);spoke.rotation.z=a;spoke.castShadow=false;}}
          ...
          const countC=W*.5,countR=H*(W<9?.685:.72);
          ...
          const ledger=add(new THREE.BoxGeometry(.85*S,.055,.62*S),paper,countC+.72,1.42,countR-.38);ledger.rotation.y=-.18;ledger.castShadow=false;add(new THREE.BoxGeometry(.78*S,.018,.025*S),brass,countC+.72,1.455,countR-.38).rotation.y=-.18;
          add(new THREE.CylinderGeometry(.34*S,.4*S,.12,18),brass,countC-1.05,1.38,countR-.42);const scaleDial=add(new THREE.CylinderGeometry(.24*S,.24*S,.08,20),cream,countC-1.05,1.72,countR-.42);scaleDial.rotation.x=Math.PI/2;add(new THREE.CylinderGeometry(.035,.045,.72,8),steel,countC-1.05,1.62,countR-.42);const taskLamp=add(new THREE.ConeGeometry(.28*S,.42,16,1,true),new THREE.MeshBasicMaterial({color:0xffd28a,toneMapped:true}),countC+1.18,2.05,countR+.15);taskLamp.rotation.z=.42;taskLamp.castShadow=false;add(new THREE.CylinderGeometry(.04,.055,1.28,8),brass,countC+1.02,1.45,countR+.15).rotation.z=.22;
          ...
          const bioC=W*.5+1.75,bioR=H-.55;add(new THREE.BoxGeometry(.55*S,1.55,.48*S),steelDark,bioC,.78,bioR);const bioScreen=add(new THREE.PlaneGeometry(.4*S,.5),screenGreen,bioC,1.02,bioR-.29);bioScreen.castShadow=false;
          if(B.alarmTriggered)for(let beam=0;beam<4;beam++){const laser=add(new THREE.BoxGeometry(W*.42*S,.025,.025),new THREE.MeshBasicMaterial({color:0xff2333,transparent:true,opacity:.76,depthWrite:false,toneMapped:false}),W*.5,1.1+beam*.58,H*.32);laser.renderOrder=14;laser.castShadow=false;}
          renderer.domElement.dataset.bankInterior='vault-volumetric-v7';renderer.domElement.dataset.bankRooms='vault-deposit-walls-cash-racks-safe-bay-counting-table-service-bay-open-security-portals-biometric-airlock';renderer.domElement.dataset.bankVolumetricProps='oversized-vault-door-dark-threshold-brass-aisle-deposit-sections-a-b-c-drawers-number-slots-handles-safes-dials-cash-bundles-gold-ingots-racks-carts-large-bags-counting-table-ledger-scale-task-lamp-tools-cameras-alarm-pipes-open-security-portals-cash-sorter-biometric-console-ventilation-laser-grid';
        }else{
          const cr=Math.max(7,Math.min(H-4,+B.counterRow||Math.floor(H*.58))),plan=B.visualPlan||{},archR=+plan.archRow||Math.max(4.8,cr-4),staffEnd=+plan.staffEnd||Math.floor(W*.38),loungeStart=+plan.loungeStart||Math.floor(W*.62),staffGap=plan.staffOpening||[Math.floor(W*.1),Math.floor(W*.1)+3],centerGap=plan.centerOpening||[W/2-3,W/2+3],loungeGap=plan.loungeOpening||[Math.floor(W*.65),Math.floor(W*.65)+3];
          interiorFloor=floorZone(W/2,H/2,W,H,marble,.018);floorZone(staffEnd/2,archR/2,staffEnd-.35,archR-.35,carpet,.075);floorZone((loungeStart+W)/2,archR/2,W-loungeStart-.35,archR-.35,loungeWood,.075);for(let r=.65;r<archR;r+=.72){const seam=floorZone((loungeStart+W)/2,r,W-loungeStart-.5,.045,woodDark,.125);seam.castShadow=false;}floorZone(W/2,archR/2,loungeStart-staffEnd-.55,archR-.35,steelDark,.075);const runnerWidth=Math.max(5.2,Math.min(6.8,W*.18)),runnerLength=H-cr-.5,runnerCenterR=(cr+H-.5)/2,secureRunnerWidth=Math.max(3.2,runnerWidth*.62),secureRunnerLength=cr-.72,secureRunnerCenterR=(cr+.72)/2;floorZone(W/2,secureRunnerCenterR,secureRunnerWidth,secureRunnerLength,runnerDark,.115);for(const side of [-1,1])floorZone(W/2+side*(secureRunnerWidth/2-.1),secureRunnerCenterR,.14,secureRunnerLength,brass,.14);floorZone(W/2,runnerCenterR,runnerWidth,runnerLength,runner,.12);for(const side of [-1,1])floorZone(W/2+side*(runnerWidth/2-.12),runnerCenterR,.18,runnerLength,brass,.145);floorZone(W/2,cr+.42,runnerWidth+.45,.34,runnerDark,.15);for(const [c,r,w,d] of [[W/2,archR+.48,W-2.2,.16],[W/2,H-.82,W-3,.16],[.78,(archR+H)/2,.16,H-archR-1.55],[W-.78,(archR+H)/2,.16,H-archR-1.55]])floorZone(c,r,w,d,brass,.105);const vaultSeal=add(new THREE.RingGeometry(.72*S,1.02*S,36),gold,W/2,.16,2.05);vaultSeal.rotation.x=-Math.PI/2;for(let k=0;k<8;k++){const a=k/8*Math.PI*2,sealRay=add(new THREE.BoxGeometry(.08*S,.025,.38*S),brass,W/2+Math.sin(a)*.78,.175,2.05+Math.cos(a)*.78);sealRay.rotation.y=a;sealRay.castShadow=false;}
          hWall(0,W/2-2.9,.12,5.15);hWall(W/2+2.9,W,.12,5.15);vWall(.12,0,H,4.7);vWall(W-.12,0,H,1.25);hWall(0,W/2-2.1,H-.12,1.2);hWall(W/2+2.1,W,H-.12,1.2);vWall(staffEnd,.15,archR-.5,4.15);vWall(loungeStart,.15,archR-.5,4.15);
          wallPanelH(.3,W/2-3.15,.12,1);wallPanelH(W/2+3.15,W-.3,.12,1);wallPanelV(.12,archR+.35,H-.55,1);wallPanelV(W-.12,archR+.35,H-.55,-1);
          for(const c of [W*.18,W*.82])wallSconce(c,.35,0);for(const r of [archR+2.2,H-3.4]){wallSconce(.35,r,Math.PI/2);wallSconce(W-.35,r,-Math.PI/2);}
          const gaps=[staffGap,centerGap,loungeGap];let cursor=.15;for(const [g0,g1] of gaps){hWall(cursor,g0,archR,4.1);cursor=g1;}hWall(cursor,W-.15,archR,4.1);for(const [g0,g1] of gaps){for(const c of [g0,g1])add(new THREE.CylinderGeometry(.16,.21,4.25,12),brass,c,2.12,archR);add(new THREE.BoxGeometry((g1-g0)*S+.25,.24,.34*S),brass,(g0+g1)/2,4.1,archR);}
          bankDoor(W/2,.24,1,B.phase==='vault_open');for(const side of [-1,1])add(new THREE.CylinderGeometry(.34*S,.42*S,4.9,18),marbleDark,W/2+side*1.55,2.45,.38);add(new THREE.BoxGeometry(4.2*S,.28,.65*S),brass,W/2,4.92,.38).castShadow=false;roomSign('СЛУЖЕБНАЯ ЗОНА',staffEnd/2,archR-.05,4.7,'#8cc7e8',.78);roomSign('ХРАНИЛИЩЕ • VAULT',W/2,.58,6.45,'#f0d78a',1.3);roomSign('КОМНАТА ОТДЫХА',(loungeStart+W)/2,archR-.05,4.7,'#efc58c',.78);roomSign(B.name||'ГОРОДСКОЙ БАНК',W/2,cr-.65,5.2,'#d4b35c',1.22);
          const tellerCount=Math.max(4,Math.min(6,Math.round(W/7))),counterGap=runnerWidth/2+.72,wingInset=1.05,wingWidth=W/2-counterGap-wingInset;
          for(const side of [-1,1]){const wingC=W/2+side*(counterGap+wingWidth/2);add(new THREE.BoxGeometry(wingWidth*S,1.18,1.18*S),woodDark,wingC,.59,cr);add(new THREE.BoxGeometry((wingWidth+.18)*S,.18,1.34*S),marble,wingC,1.27,cr);for(let panel=0;panel<Math.max(2,Math.floor(wingWidth/2.3));panel++){const pc=wingC-wingWidth*.38+panel*(wingWidth*.76/Math.max(1,Math.floor(wingWidth/2.3)-1));add(new THREE.BoxGeometry(1.35*S,.54,.055*S),wood,pc,.58,cr+.61).castShadow=false;add(new THREE.BoxGeometry(.72*S,.055,.065*S),brass,pc,.58,cr+.65).castShadow=false;}}
          for(let tellerNo=1;tellerNo<=tellerCount;tellerNo++){const side=tellerNo<=Math.ceil(tellerCount/2)?-1:1,index=side<0?tellerNo-1:tellerNo-Math.ceil(tellerCount/2),count=side<0?Math.ceil(tellerCount/2):Math.floor(tellerCount/2),start=side<0?wingInset+1.3:W/2+counterGap+1.3,end=side<0?W/2-counterGap-1.3:W-wingInset-1.3,c=count===1?(start+end)/2:start+index*(end-start)/(count-1);add(new THREE.BoxGeometry(2.35*S,1.55,.055*S),glass,c,2.2,cr-.48);for(const dc of [-1.04,1.04])add(new THREE.CylinderGeometry(.035,.035,1.62,7),brass,c+dc,2.2,cr-.48);const speaker=add(new THREE.TorusGeometry(.16,.025,6,16),brass,c,2.25,cr-.42);speaker.rotation.x=Math.PI/2;speaker.castShadow=false;for(let hole=0;hole<5;hole++)add(new THREE.SphereGeometry(.016,5,4),black,c+(hole-2)*.06,2.25,cr-.35).castShadow=false;const tray=add(new THREE.BoxGeometry(.86*S,.06,.52*S),steel,c,1.42,cr+.58);tray.rotation.x=-.05;add(new THREE.BoxGeometry(.68*S,.035,.08*S),black,c,1.42,cr+.87).castShadow=false;monitor(c-.58,cr-.72,1.85,0,tellerNo%2?screenBlue:screenGreen);add(new THREE.CylinderGeometry(.055,.065,.42,9),brass,c+.58,1.57,cr+.16).rotation.z=-.32;for(let form=0;form<3;form++){const slip=add(new THREE.BoxGeometry(.42*S,.025,.3*S),paper,c+(form-1)*.22,1.39+form*.018,cr+.12);slip.rotation.y=(form-1)*.08;slip.castShadow=false;}const number=signSprite(`ОКНО ${tellerNo}`);number.position.set(x(c),3.48,z(cr-.52));number.scale.set(1.65,.52,1);interiorGroup.add(number);}
          for(const side of [-1,1]){const queueC=W/2+side*(runnerWidth/2+1.15);for(const r of [cr+1.8,cr+3.45,cr+5.1])add(new THREE.CylinderGeometry(.07,.1,1.1,10),brass,queueC,.55,r);for(const [r0,r1] of [[cr+1.8,cr+3.45],[cr+3.45,cr+5.1]]){const rope=add(new THREE.CylinderGeometry(.045,.045,(r1-r0)*S,8),red,queueC,.88,(r0+r1)/2);rope.rotation.x=Math.PI/2;rope.castShadow=false;}}
          const guardC=Math.min(3.35,W*.22),guardR=H-4.45;floorZone(guardC,guardR,5.2,3.4,std(0x26323d,.72,.15),.12);hWall(.65,Math.min(6,guardC+2.7),guardR-1.65,.82,wallDark);vWall(Math.min(6,guardC+2.7),guardR-1.65,guardR+1.55,.82,wallDark);add(new THREE.BoxGeometry(4.8*S,.62,.04*S),glass,guardC,1.12,guardR-1.48).castShadow=false;add(new THREE.BoxGeometry(.04*S,.62,2.75*S),glass,Math.min(5.82,guardC+2.48),1.12,guardR).castShadow=false;desk(guardC,guardR+.2,4.2,1.05);for(const dc of [-1.28,0,1.28])monitor(guardC+dc,guardR-.42,2.02,0,dc?screenBlue:screenGreen);chair(guardC,guardR+1.05,Math.PI,black);for(let key=0;key<10;key++)add(new THREE.SphereGeometry(.035,6,4),key%3?screenBlue:screenGreen,guardC-1+(key%5)*.5,1.22+Math.floor(key/5)*.16,guardR+.48).castShadow=false;roomSign('ОХРАНА',guardC,guardR-1.48,4.25,'#67d5ff',.82);cameraProp(.45,H-.65,4.2,-Math.PI*.25);cameraProp(W-.45,H-.65,4.2,Math.PI*.25);cameraProp(W-.45,archR+.25,4.2,Math.PI*.65);alarm(Math.min(5.7,guardC+2.4),guardR-1.48);
          ...
          desk(Math.min(3.2,W*.22),cr+2.15,3.4);monitor(Math.min(3.2,W*.22),cr+1.7,1.9,0,screenGreen);chair(Math.min(3.2,W*.22),cr+3.05,Math.PI,fabricWarm);roomSign('АДМИНИСТРАТОР',Math.min(3.2,W*.22),cr+1.55,3.55,'#d4b35c',.62);
          const waitingR=H-2.5;for(const [c,mat] of [[W*.32,fabricWarm],[W*.68,fabric]]){sofa(c,waitingR,Math.min(4.2,W*.19),Math.PI,mat);const tableC=c+(c<W/2?2.55:-2.55);add(new THREE.CylinderGeometry(.62*S,.62*S,.15,22),wood,tableC,.54,waitingR);add(new THREE.CylinderGeometry(.08,.12,.48,9),brass,tableC,.26,waitingR);for(let leaflet=0;leaflet<3;leaflet++){const sheet=add(new THREE.BoxGeometry(.42*S,.025,.3*S),leaflet%2?paper:screenBlue,tableC+(leaflet-1)*.18,.64+leaflet*.018,waitingR);sheet.rotation.y=(leaflet-1)*.12;sheet.castShadow=false;}}
          const ticketC=W*.82,ticketR=H-4.05;add(new THREE.BoxGeometry(.9*S,1.85,.72*S),steelDark,ticketC,.93,ticketR);const ticketScreen=add(new THREE.PlaneGeometry(.62*S,.48),screenGreen,ticketC,1.35,ticketR+.39);ticketScreen.castShadow=false;add(new THREE.BoxGeometry(.48*S,.08,.18*S),paper,ticketC,.72,ticketR+.42).castShadow=false;roomSign('ЭЛЕКТРОННАЯ ОЧЕРЕДЬ',ticketC,ticketR-.15,3.25,'#71d9a0',.54);for(const c of [1.2,W-1.2])plant(c,H-1.35,.95);
          ...
          const coolerC=loungeStart+.8,coolerR=Math.min(archR-1.2,loungeSplitR+1.1);add(new THREE.CylinderGeometry(.32*S,.38*S,1.45,14),cream,coolerC,.74,coolerR);const waterJug=add(new THREE.SphereGeometry(.38*S,14,10),new THREE.MeshPhysicalMaterial({color:0x7bd8ed,transparent:true,opacity:.48,depthWrite:false,roughness:.08}),coolerC,1.75,coolerR);waterJug.scale.y=1.25;waterJug.renderOrder=12;add(new THREE.BoxGeometry(.12*S,.1,.12*S),screenBlue,coolerC-.16,.96,coolerR+.36).castShadow=false;add(new THREE.BoxGeometry(.12*S,.1,.12*S),red,coolerC+.16,.96,coolerR+.36).castShadow=false;
          ...
          renderer.domElement.dataset.bankInterior='lobby-volumetric-v7';renderer.domElement.dataset.bankRooms='main-hall-teller-hall-waiting-lounge-security-booth-staff-office-manager-office-archive-break-room-conference-room-screening-corridor-cash-processing-vault-approach-atm-zone';renderer.domElement.dataset.bankVolumetricProps='patterned-marble-red-runner-gold-border-coherent-two-wing-counter-centered-teller-windows-short-queue-rails-side-waiting-lounges-ticket-terminal-paneled-walls-brass-dado-wall-sconces-oversized-vault-portal-teller-glass-speakers-trays-monitors-security-cctv-staff-desk-pods-files-printer-coffee-tv-atms-screening-gate-document-desk-carts-glass-partitions-office-doors-conference-table-cash-sorter-water-cooler-fire-extinguisher';
        }
        renderer.domElement.dataset.bankState=B.phase||'lobby';renderer.domElement.dataset.bankVisualVersion='7';renderer.domElement.dataset.bankDepthStability='polygon-offset-walls-separated-trims-transparent-depthwrite-off';renderer.domElement.dataset.bankCollisionArt='v7-heavy-furniture-matched-central-route-open';interiorGroup.visible=true;return true;
      };
      ...
      const decorateServiceInterior=data=>{
        ...
        renderer.domElement.dataset.serviceInterior=`${type}:authored-volumetric-v2`;return true;
      };
      ...
      const decorateVenueInterior=data=>{
        ...
        if(type==='gym'){
          ...
          for(const c of [2.2,4.4,6.6,8.8]){add(new THREE.CylinderGeometry(.48*S,.36*S,2.5,18),c%2>1?red:blue,c,2.35,H-3);add(new THREE.CylinderGeometry(.04*S,.04*S,2.4,7),steel,c,5.05,H-3);}
          ...
          for(const c of [2.1,4.25,8.55,10.7])chair(c,H-2.05,c%4>2?red:blue);
          for(const c of [3.1,5,7,8.9]){add(new THREE.CylinderGeometry(.07*S,.1*S,1.2,9),gold,c,.6,5.4);add(new THREE.SphereGeometry(.11*S,9,6),gold,c,1.22,5.4);}
          for(const c of [4.05,7.95])add(new THREE.BoxGeometry(1.9*S,.07,.07*S),red,c,.88,5.4).castShadow=false;
        ...
        const area=interiorGroup.children.find(o=>o.type==='RectAreaLight');if(area)area.intensity=type==='blackmarket'?2.2:type==='gym'?4:4.4;const venueLight=new THREE.PointLight(type==='gym'?0xb9ddff:type==='job'?0xffd6a0:0xb087ff,type==='blackmarket'?3.2:4.5,13,2);venueLight.position.set(x(type==='gym'?W*.3:type==='job'?W*.72:W*.38),4.6,z(type==='blackmarket'?3.4:H*.42));venueLight.layers.set(1);venueLight.castShadow=false;interiorGroup.add(venueLight);
        renderer.domElement.dataset.venueInterior=`${type}:${data.venueId||'default'}:authored-volumetric-v3`;renderer.domElement.dataset.venueNavigation='south-centre-clear-2.2-tiles';return true;
      };

      const help=document.createElement('div');help.textContent='3D: W / ↑ — вперёд · S / ↓ — назад · A/D — шаг вбок · E — здание · удерживать E — машина · ЛКМ — огонь / удержать и отпустить — бросок · ПКМ — лазерный прицел';
      ...
      const vehicleHoldPrompt=document.createElement('div');vehicleHoldPrompt.style.cssText='position:absolute;left:50%;bottom:112px;transform:translateX(-50%);z-index:24;display:none;width:min(330px,calc(100vw - 34px));padding:11px 14px;border:1px solid #ffe07a;border-radius:12px;background:rgba(13,18,27,.95);color:#fff4c2;font:900 13px system-ui;text-align:center;box-shadow:0 10px 32px #000c,0 0 22px #ffd24a3b;pointer-events:none';vehicleHoldPrompt.innerHTML='<div>🚗 Удерживайте E — сесть в машину</div><div style="height:7px;margin-top:8px;border-radius:6px;background:#2b3038;overflow:hidden"><i style="display:block;width:0;height:100%;border-radius:inherit;background:linear-gradient(90deg,#d18b25,#ffe67e);box-shadow:0 0 10px #ffd86b"></i></div>';stage.appendChild(vehicleHoldPrompt);const vehicleHoldFill=vehicleHoldPrompt.querySelector('i');
      ...
      const keys=new Set(),bullets=[],reloadDebris=[],vehicleHoldMs=170;let aimPoint=new THREE.Vector3(0,0,-10),lastShot=0,muzzleLife=0,recoilKick=0,recoilSide=1,recoilShotSeq=0,reloadWasActive=false,activeReloadProgress=0,triggerHeld=false,laserAimHeld=false,laserAimAllowed=true,laserShotPulseUntil=0,throwAimHeld=false,throwAimStartedAt=0,throwAimKind='',throwAimAngle=0,throwAimRange=1.6,throwReleaseUntil=0,animationActionLocked=false,eHoldStarted=0,eHoldTimer=0,eHoldTriggered=false,eHoldVehicleId='',eHoldHasBuilding=false;
      const muzzle=new THREE.PointLight(0xffb14a,0,9,2);scene.add(muzzle);
      const laserMaterial=new THREE.MeshBasicMaterial({color:0xff2038,transparent:true,opacity:.82,depthTest:false,depthWrite:false,toneMapped:false,blending:THREE.AdditiveBlending}),laserDotMaterial=new THREE.MeshBasicMaterial({color:0xff5261,transparent:true,opacity:.98,depthTest:false,depthWrite:false,toneMapped:false,blending:THREE.AdditiveBlending}),laserHaloMaterial=new THREE.MeshBasicMaterial({color:0xff142d,transparent:true,opacity:.46,side:THREE.DoubleSide,depthTest:false,depthWrite:false,toneMapped:false,blending:THREE.AdditiveBlending}),laserBeam=new THREE.Mesh(new THREE.CylinderGeometry(.018,.018,1,7,1,true),laserMaterial),laserDot=new THREE.Mesh(new THREE.SphereGeometry(.105,12,8),laserDotMaterial),laserHalo=new THREE.Mesh(new THREE.RingGeometry(.15,.31,24),laserHaloMaterial),laserAxis=new THREE.Vector3(0,1,0),laserOriginPoint=new THREE.Vector3(),laserTargetPoint=new THREE.Vector3(),laserDirection=new THREE.Vector3();laserBeam.renderOrder=76;laserDot.renderOrder=77;laserHalo.renderOrder=75;laserHalo.rotation.x=-Math.PI/2;for(const part of [laserBeam,laserDot,laserHalo]){part.visible=false;part.layers.enable(1);scene.add(part);}
      const laserReticle=document.createElement('div');laserReticle.style.cssText='position:absolute;z-index:31;display:none;width:54px;height:54px;transform:translate(-50%,-50%);pointer-events:none;filter:drop-shadow(0 3px 6px #000e);transition:transform 65ms ease-out';laserReticle.innerHTML='<span style="position:absolute;inset:7px;border:2px solid #d4ad4e;border-radius:50%;background:radial-gradient(circle,transparent 0 35%,#08090bd9 37% 50%,#5b0b18c9 52% 58%,transparent 60%);box-shadow:0 0 0 1px #13090b,0 0 9px #8e142b99,inset 0 0 7px #000"></span><span style="position:absolute;inset:13px;border:1px solid #7e6225;border-radius:50%;box-shadow:inset 0 0 5px #d5af5266"></span><i style="position:absolute;left:50%;top:0;width:2px;height:16px;transform:translateX(-50%);background:linear-gradient(#74101d 0 35%,#e4c05c 36% 72%,transparent 73%);box-shadow:0 0 3px #000"></i><i style="position:absolute;left:50%;bottom:0;width:2px;height:16px;transform:translateX(-50%) rotate(180deg);background:linear-gradient(#74101d 0 35%,#e4c05c 36% 72%,transparent 73%);box-shadow:0 0 3px #000"></i><i style="position:absolute;top:50%;left:0;width:16px;height:2px;transform:translateY(-50%);background:linear-gradient(90deg,#74101d 0 35%,#e4c05c 36% 72%,transparent 73%);box-shadow:0 0 3px #000"></i><i style="position:absolute;top:50%;right:0;width:16px;height:2px;transform:translateY(-50%) rotate(180deg);background:linear-gradient(90deg,#74101d 0 35%,#e4c05c 36% 72%,transparent 73%);box-shadow:0 0 3px #000"></i><b style="position:absolute;left:50%;top:50%;width:5px;height:5px;transform:translate(-50%,-50%);border-radius:50%;background:#fff0b0;border:1px solid #79101e;box-shadow:0 0 3px #fff,0 0 10px 3px #e31d38"></b><em style="position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);margin-top:12px;color:#d5af52;font:900 6px Georgia,serif;letter-spacing:.5px;font-style:normal;text-shadow:0 1px 2px #000">MF</em>';stage.appendChild(laserReticle);renderer.domElement.dataset.laserReticleProfile='famiglia-seal-burgundy-gold-v216';
      ...
      const canHoldLaserAim=()=>{const state=bridge?.getPlayerState?.();return !['','none','fists','unarmed','grenade','molotov','c4'].includes(currentWeaponId)&&currentWeaponFx.grip!=='throw'&&!state?.driving&&!state?.dead&&!state?.vehicleEntry;};
      const stopLaserAim=reason=>{laserAimHeld=false;laserBeam.visible=laserDot.visible=laserHalo.visible=false;laserReticle.style.display='none';renderer.domElement.style.cursor='crosshair';renderer.domElement.dataset.laserAim=`off:${reason||'released'}`;renderer.domElement.dataset.laserBeamLength='0';};
      const updateLaserAim=t=>{const active=laserAimHeld&&laserAimAllowed&&gun.visible;if(!active){laserBeam.visible=laserDot.visible=laserHalo.visible=false;laserReticle.style.display='none';return;}player.updateMatrixWorld(true);localMuzzleFlash.getWorldPosition(laserOriginPoint);laserTargetPoint.copy(aimPoint);laserTargetPoint.y=Math.max(playerFloorElevation+.16,.16);laserDirection.subVectors(laserTargetPoint,laserOriginPoint);let distance=laserDirection.length();const maxDistance=currentWeaponFx.family==='sniper'?52:currentWeaponFx.family==='rifle'?46:currentWeaponFx.family==='shotgun'?34:40;if(distance>maxDistance){laserDirection.multiplyScalar(maxDistance/Math.max(.001,distance));laserTargetPoint.copy(laserOriginPoint).add(laserDirection);distance=maxDistance;}if(distance<.3){laserBeam.visible=laserDot.visible=laserHalo.visible=false;return;}laserDirection.normalize();laserBeam.position.copy(laserOriginPoint).addScaledVector(laserDirection,distance*.5);laserBeam.scale.set(1,distance,1);laserBeam.quaternion.setFromUnitVectors(laserAxis,laserDirection);laserDot.position.copy(laserTargetPoint);laserHalo.position.copy(laserTargetPoint);const pulse=1+Math.sin(t*.018)*.13;laserDot.scale.setScalar(pulse);laserHalo.scale.setScalar(1+Math.sin(t*.012)*.08);laserBeam.visible=laserDot.visible=laserHalo.visible=true;const stageRect=stage.getBoundingClientRect(),reticleKick=Math.min(.18,recoilKick*.038),shotPulse=t<laserShotPulseUntil?.88:1,reticleScale=(1+reticleKick+Math.sin(t*.01)*.018)*shotPulse;laserReticle.style.left=`${mouseClientX-stageRect.left}px`;laserReticle.style.top=`${mouseClientY-stageRect.top}px`;laserReticle.style.transform=`translate(-50%,-50%) scale(${reticleScale.toFixed(3)})`;laserReticle.style.display='block';renderer.domElement.dataset.laserAim='held';renderer.domElement.dataset.laserReticleSpread=reticleScale.toFixed(3);renderer.domElement.dataset.laserBeamLength=distance.toFixed(2);};
      ...
      const releaseSpecialPrimary=()=>{
        if(!throwAimHeld)return false;updateThrowAim(performance.now());const kind=throwAimKind;throwAimHeld=false;throwAimLine.visible=throwLandingRing.visible=false;throwReleaseUntil=performance.now()+420;
      ...
      addEventListener('pointerdown',e=>{if(e.button!==2||e.pointerType==='touch'||(e.target.closest&&e.target.closest('button,input,textarea,select,#joyL,#joyR')))return;if(!canHoldLaserAim())return;e.preventDefault();e.stopImmediatePropagation();laserAimHeld=true;laserAimAllowed=true;mouseAimActive=true;mouseClientX=e.clientX;mouseClientY=e.clientY;renderer.domElement.dataset.laserAimPresses=String((+renderer.domElement.dataset.laserAimPresses||0)+1);renderer.domElement.style.cursor='none';syncMouseAim();updateLaserAim(performance.now());},true);
      ...
      addEventListener('mousedown',e=>{if(e.button!==0||!laserAimHeld||e.pointerType==='touch'||(e.target.closest&&e.target.closest('button,input,textarea,select,#joyL,#joyR')))return;e.preventDefault();e.stopImmediatePropagation();if(beginFirearmAtPointer(e,false)){renderer.domElement.dataset.laserFireChord='rmb-held-lmb-fired';renderer.domElement.dataset.laserFireChordCount=String((+renderer.domElement.dataset.laserFireChordCount||0)+1);}},true);
      addEventListener('pointerdown',e=>{if(e.button===0&&!(e.target.closest&&e.target.closest('button,input,textarea,select,#joyL,#joyR'))){if(laserAimHeld)return;e.preventDefault();e.stopImmediatePropagation();mouseAimActive=true;mouseClientX=e.clientX;mouseClientY=e.clientY;syncMouseAim();if(isSpecialPrimary()){triggerHeld=false;activeGangShotSourceId='';activeNpcShotId='';activeVehicleShotId='';throwAimHeld=true;throwAimStartedAt=performance.now();throwAimKind=currentWeaponId;renderer.domElement.dataset.throwPresses=String((+renderer.domElement.dataset.throwPresses||0)+1);renderer.domElement.dataset.primaryActionRoute=currentWeaponId==='c4'?'c4-place-hold':'throw-charge';updateThrowAim(throwAimStartedAt);return;}beginFirearmAtPointer(e,true);}},true);addEventListener('pointerup',e=>{if(e.button===0&&throwAimHeld)releaseSpecialPrimary();triggerHeld=false;activeGangShotSourceId='';activeNpcShotId='';activeVehicleShotId='';},true);addEventListener('pointercancel',e=>{if(e.button===0&&throwAimHeld){throwAimHeld=false;throwAimLine.visible=throwLandingRing.visible=false;renderer.domElement.dataset.throwAim='cancelled';}},true);addEventListener('blur',()=>{triggerHeld=false;throwAimHeld=false;throwAimLine.visible=false;throwLandingRing.visible=false;activeGangShotSourceId='';activeNpcShotId='';activeVehicleShotId='';stopLaserAim('blur');},true);
      function shoot(now){
        ...
        if(laserAimHeld){laserShotPulseUntil=now+115;renderer.domElement.dataset.laserConfirmedShots=String((+renderer.domElement.dataset.laserConfirmedShots||0)+1);}
      ...
      const decorateGrandCasinoInterior=data=>{
        ...
        floorZone(22,12,28,16,marble,.08);floorZone(22,15.5,5.3,25,rugMat,.14);floorZone(7.2,24.2,12,8,vipRug,.14);floorZone(40,15.5,6.4,30,serviceFloor,.08);
        for(const c of [19.35,24.65]){const trim=add(new THREE.BoxGeometry(.09,0.06,25*WORLD_SCALE),gold,xOf(c),.18,zOf(15.5));trim.castShadow=false;}
        const makeWall=w=>{const dx=(w.c1-w.c0)*WORLD_SCALE,dz=(w.r1-w.r0)*WORLD_SCALE,len=Math.max(.12,Math.hypot(dx,dz)),frontCutaway=Math.abs(w.r1-w.r0)<.01&&Math.min(w.r0,w.r1)>=19.5,height=w.kind==='vault'?5.1:w.kind==='curtain'?1.35:frontCutaway?1.5:4.35,mat=w.kind==='vault'?vaultWall:w.kind==='curtain'?velvet:wallMat,wall=add(new THREE.BoxGeometry(len,height,.38),mat,xOf((w.c0+w.c1)/2),height/2,zOf((w.r0+w.r1)/2));wall.rotation.y=-Math.atan2(dz,dx);const base=add(new THREE.BoxGeometry(len-.08,.28,.5),w.kind==='vault'?steel:darkWood,wall.position.x,.14,wall.position.z);base.rotation.y=wall.rotation.y;base.castShadow=false;const cap=add(new THREE.BoxGeometry(len-.1,.16,.5),gold,wall.position.x,height+.08,wall.position.z);cap.rotation.y=wall.rotation.y;cap.castShadow=false;if(w.kind==='curtain'){for(let i=0;i<=5;i++){const t=i/5-.5,post=add(new THREE.CylinderGeometry(.07,.1,2.7,9),brass,wall.position.x+t*dx,1.35,wall.position.z+t*dz);post.castShadow=false;}}return wall;};
        ...
        const galleryRug=floorZone(3.8,8.45,5.25,9.55,new THREE.MeshStandardMaterial({color:0x721d3e,roughness:.9}),galleryY+.27);
        for(let i=0;i<12;i++){const t=(i+.5)/12,r=18.8-t*5.04,h=(i+1)/12*galleryY,step=add(new THREE.BoxGeometry(3.2*WORLD_SCALE,h,.44*WORLD_SCALE),i%2?darkWood:velvet,xOf(3.8),h/2,zOf(r));step.castShadow=true;const nose=add(new THREE.BoxGeometry(3.22*WORLD_SCALE,.09,.08*WORLD_SCALE),gold,xOf(3.8),h+.045,zOf(r-.21));nose.castShadow=false;}
        ...
        const gamingTable=(p,label)=>{
          ...
          for(let k=0;k<5;k++){const a=Math.PI*(.08+.84*k/4);casinoChair(x+Math.sin(a)*2.65*WORLD_SCALE,z+Math.cos(a)*1.68*WORLD_SCALE,-a);}
        };
        ...
        const stageProp=(layout.props||[]).find(p=>p.id==='dance_stage');if(stageProp){const x=xOf(stageProp.c),z=zOf(stageProp.r),stageTop=add(new THREE.CylinderGeometry(3.0*WORLD_SCALE,3.0*WORLD_SCALE,.55,32),velvet,x,.28,z);stageTop.scale.z=.4;for(const sx of [-3,3]){add(new THREE.BoxGeometry(.22,5.4,.22),steel,x+sx*WORLD_SCALE,2.7,z);for(let h=0;h<3;h++)add(new THREE.SphereGeometry(.22,12,8),neonMats[(h+(sx>0?1:0))%4],x+sx*WORLD_SCALE,1.6+h*1.45,z+.2);}for(const sx of [-1.15,1.15]){add(new THREE.CylinderGeometry(.055,.055,5.25,12),gold,x+sx*WORLD_SCALE,2.9,z+.35*WORLD_SCALE);add(new THREE.CylinderGeometry(.38,.48,.16,18),brass,x+sx*WORLD_SCALE,.63,z+.35*WORLD_SCALE);}add(new THREE.BoxGeometry(6.4*WORLD_SCALE,5.1,.3),curtain,x,2.55,z-1.42*WORLD_SCALE);const show=casinoSignSprite('GRAND SHOW','#ff4ebc');show.position.set(x,5.45,z-1.28*WORLD_SCALE);show.scale.set(8.2,1.9,1);interiorGroup.add(show);}
        ...
        const office=(layout.props||[]).find(p=>p.id==='office');if(office){
          const x=xOf(office.c),z=zOf(office.r),top=add(roundedCasinoTop((office.w||4.4)*WORLD_SCALE,2.05*WORLD_SCALE,.28),darkWood,x,1.38,z);top.castShadow=true;
          for(const dx of [-1.65,1.65]){add(new THREE.CylinderGeometry(.16,.22,1.05,12),gold,x+dx*WORLD_SCALE,.54,z-.55*WORLD_SCALE);const drawers=add(new THREE.CapsuleGeometry(.34,1.05,7,12),darkWood,x+dx*WORLD_SCALE,.64,z+.45*WORLD_SCALE);drawers.rotation.z=Math.PI/2;drawers.scale.z=.55;for(const y of [.45,.76,.98])add(new THREE.TorusGeometry(.12,.025,5,10,Math.PI),gold,drawers.position.x,y,z+.85*WORLD_SCALE).rotation.z=Math.PI;}
        ...
        const security=(layout.props||[]).find(p=>p.id==='security');if(security){
          const x=xOf(security.c),z=zOf(security.r),consoleTop=add(roundedCasinoTop((security.w||3.8)*WORLD_SCALE,1.95*WORLD_SCALE,.26),black,x,1.28,z);consoleTop.castShadow=true;
          for(const dx of [-1.42,1.42])add(new THREE.CylinderGeometry(.16,.23,1.05,10),steel,x+dx*WORLD_SCALE,.54,z);
        ...
        const mirrorMat=new THREE.MeshPhysicalMaterial({color:0xb7d8df,metalness:.72,roughness:.08,clearcoat:1});
        ...
        if(stageProp){const x=xOf(stageProp.c),z=zOf(stageProp.r),stageRim=add(new THREE.TorusGeometry(3.0*WORLD_SCALE,.13,8,32),gold,x,.58,z);stageRim.scale.y=.4;stageRim.rotation.x=Math.PI/2;for(let k=0;k<9;k++){const a=Math.PI*(.08+.84*k/8),bulb=add(new THREE.SphereGeometry(.1,8,6),neonMats[k%4],x+Math.cos(a)*2.88*WORLD_SCALE,.72,z+Math.sin(a)*1.12*WORLD_SCALE);bulb.castShadow=false;}}
        ...
        renderer.domElement.dataset.casinoWalls=String((layout.walls||[]).length);renderer.domElement.dataset.casinoProps=String((layout.props||[]).length);renderer.domElement.dataset.casinoDoors='5:arched-open-panels-stair-aligned';renderer.domElement.dataset.casinoVolumetricProps='single-detail-slot-machines-roulette-wheel-betting-grid-card-suits-five-seat-tables-billiards-pockets-cues-aligned-vip-stairs-flattened-show-stage';renderer.domElement.dataset.casinoPremium='server-layout-v8-cutaway-rooms-clear-corridors';
      };
      const decorateApartmentInterior=data=>{
        ...
        const ownerText=apt.owned?'МОЯ КВАРТИРА':apt.ownerName?`КВАРТИРА · ${apt.ownerName}`:`КВАРТИРА · ${apt.district||'ГОРОД'}`,ownerColor=apt.owned?'#8dffae':apt.ownerName?'#7ed8ff':'#e5c66b',ownerSign=labelSprite(ownerText,ownerColor);ownerSign.position.set(cx,5.55,northZ+.65);ownerSign.scale.set(Math.max(6.4,Math.min(12,ownerText.length*.52)),1.55,1);ownerSign.layers.set(1);interiorGroup.add(ownerSign);
        renderer.domElement.dataset.apartmentInterior=`furnished-v7:${layoutVariant}:${layoutName}:theme-${theme}:${repair}:${+lvl.safe||0}:${+lvl.weaponRack||0}:${+lvl.cameras||0}:${+lvl.garage||0}`;
        renderer.domElement.dataset.apartmentLayout=`${layoutName}:topology-${topology}:theme-${theme}:seed-${seed.toString(16)}`;
        renderer.domElement.dataset.apartmentPassage='south-centre-clear-5.8';
        return true;
      };
      ...
      const updateDayNight=t=>{
        ...
        bulbMat.color.setRGB(.38+night*.62,.4+night*.42,.42+night*.18);streetLights.forEach(({light},i)=>{const electrical=.97+.03*Math.sin(t*.0021+i*2.17),flicker=(i===2&&Math.sin(t*.021)> .965)?.66:1;light.intensity=night*(i%2?17:25)*electrical*flicker;light.visible=light.userData.mfzBudgetVisible!==false&&night>.08;});renderer.domElement.dataset.environmentAnimationProfile='wind-water-wildlife-neon-electrical-v251';
      ...
      const animate = t => {
        ...
        let cityDoorsOpen=0,cityDoorsVisible=0,nearestCityDoor=Infinity;
        for(let i=cityDoorActors.length-1;i>=0;i--){const door=cityDoorActors[i];let parent=door.root;while(parent&&parent!==scene)parent=parent.parent;if(parent!==scene){cityDoorActors.splice(i,1);continue;}const distance=Math.hypot(player.position.x-door.x,player.position.z-door.z),fadeMaterials=door.main?.userData?.fadeMaterials,shellVisible=!door.main||(door.main.visible!==false&&(!fadeMaterials||fadeMaterials.some(material=>!material.transparent||material.opacity>.16))),visible=!interiorLightingActive&&shellVisible&&distance<72,near=!interiorLightingActive&&distance<13.2,target=near?1:0;nearestCityDoor=Math.min(nearestCityDoor,distance);door.root.visible=visible;if(visible)cityDoorsVisible++;door.open+=(target-door.open)*Math.min(1,dt*5.8);door.left.rotation.y=door.open*1.08;door.right.rotation.y=-door.open*1.08;if(door.open>.82)cityDoorsOpen++;}
        renderer.domElement.dataset.cityAnimatedDoors=String(cityDoorActors.length);renderer.domElement.dataset.cityDoorsVisible=String(cityDoorsVisible);renderer.domElement.dataset.cityDoorsOpen=String(cityDoorsOpen);renderer.domElement.dataset.nearestCityDoor=Number.isFinite(nearestCityDoor)?nearestCityDoor.toFixed(2):'none';renderer.domElement.dataset.cityDoorPalette=String(cityDoorMaterials.size);renderer.domElement.dataset.cityDoorLod='near-72-world-units';
        let nearestBlackmarketDoor=null,nearestBlackmarketDistance=Infinity;
        for(const door of blackmarketDoorActors){const distance=Math.hypot(player.position.x-door.x,player.position.z-door.z),selectedMarket=nearbyActionState?.type==='blackmarket'&&String(nearbyActionState?.id||'')===String(door.kind||''),near=!interiorLightingActive&&(selectedMarket||distance<12),target=near?1:0;door.open+=(target-door.open)*Math.min(1,dt*6.5);door.left.rotation.y=door.open*1.12;door.right.rotation.y=-door.open*1.12;door.left.visible=door.right.visible=door.void.visible=!interiorLightingActive;for(const glow of door.glow||[])glow.visible=!interiorLightingActive;if(distance<nearestBlackmarketDistance){nearestBlackmarketDistance=distance;nearestBlackmarketDoor=door;}}
        if(nearestBlackmarketDoor){renderer.domElement.dataset.blackmarketDoors=String(blackmarketDoorActors.length);renderer.domElement.dataset.blackmarketDoorDistance=nearestBlackmarketDistance.toFixed(2);renderer.domElement.dataset.blackmarketDoorCoordinates=`door:${nearestBlackmarketDoor.x.toFixed(1)},${nearestBlackmarketDoor.z.toFixed(1)}|player:${player.position.x.toFixed(1)},${player.position.z.toFixed(1)}`;renderer.domElement.dataset.blackmarketDoorState=nearestBlackmarketDoor.open>.82?'open':nearestBlackmarketDoor.open>.08?'moving':'closed';renderer.domElement.dataset.blackmarketDoorProfile='armored-visible-dark-portal-v2';}
        ...
        if(dynamic){
          ...
          for(const mesh of [worldBullets,worldBulletTrails,worldBulletGlows,worldBulletCores])mesh.count=activeProjectileInstances;
          ...
          renderer.domElement.dataset.policeArmoredVanTurrets=`visible:${visiblePoliceTurrets},active:${activePoliceTurrets}`;
          renderer.domElement.dataset.towLoadingAnimations=String(towLoadingAnimations);
          renderer.domElement.dataset.fireWaterProfile='compact-arched-hose-v230';
          renderer.domElement.dataset.persistentServiceFleetProfile='visible-parking-call-road-return-reuse-v230';
          for(const car of cars){const src=car.userData.source;if(!src?.serviceKind)continue;const active=src.emergencyLights===true;car.userData.siren.visible=active;car.userData.stripe.visible=true;for(let lamp=0;lamp<car.userData.siren.children.length;lamp++)car.userData.siren.children[lamp].visible=active&&(lamp===Math.floor(t/170)%car.userData.siren.children.length);}
          ...
          renderer.domElement.dataset.animatedWreckFx=String(animatedWreckFx);
          renderer.domElement.dataset.wreckRenderProfile='collapsed-charred-live-shell-v199';
          ...
          renderer.domElement.dataset.activeWreckLights=String(activeWreckLights);
          cars.forEach((car,i)=>{
          ...
          let crawlingNpcCount=0,limpingNpcCount=0,reactingNpcCount=0,bleedingNpcCount=0,panickingNpcCount=0,coweringNpcCount=0,surrenderingNpcCount=0,helpingNpcCount=0,socialNpcCount=0,alertedNpcCount=0,ambientNpcCount=0;
          citizenPool.forEach((npc,i)=>{
            ...
            armed?setPart(npcParts.hat,i,rootMatrix,0,3.77-pose.crouch,0):hidePart(npcParts.hat,i);const lifeGestureDisarms=pose.surrendering||pose.cowering||pose.helping||pose.panicking||pose.talking||!!pose.ambientKind;armed&&!lifeGestureDisarms&&!(severMask&2)?setPart(npcParts.gun,i,rootMatrix,.72,2-pose.crouch*.35,.52,pose.hit*.18):hidePart(npcParts.gun,i);
          ...
          renderer.domElement.dataset.ambientNpcAnimations=String(ambientNpcCount);renderer.domElement.dataset.npcAmbientProfile='phone-smoke-stretch-deterministic-lod-v251';
          for(let i=0;i<NPC_CAP;i++){const src=dynamic.npcs[i],label=npcLabels[i];if(!src){label.sprite.visible=false;continue;}const x=npcVisualXs[i],z=npcVisualZs[i],role=String(src.role||'').toLowerCase(),family=String(src.family||src.faction||'').toLowerCase(),prisonStaff=String(src.behavior||'')==='prison_patrol'||role.startsWith('prison_'),gang=src.visualRole==='gang'||!!src.gang||role.includes('gang')||role.includes('boss')||role.includes('district_')||role.includes('occupier'),police=src.visualRole==='police'||!!src.police||role.includes('police')||role.includes('cop'),guard=role.includes('guard'),medic=role.includes('medic'),gangSuit=family.includes('yellow')?0xf1e8cf:family.includes('purple')?0x7043a5:family.includes('moretti')?0xe6dfd1:family.includes('bellini')?0x3f4652:Math.max(1,+src.level||1)>=4?0x8f3044:0xb54859,bright=medic?0xe8f2f4:police?0x328fe2:gang?gangSuit:guard?0xc48a28:[0x3e9bd1,0xdb5c68,0x79a84f,0xd39b42,0x8a6dbe][i%5];instanceColor.set(bright);npcParts.body.setColorAt(i,instanceColor);npcParts.leftArm.setColorAt(i,instanceColor);npcParts.rightArm.setColorAt(i,instanceColor);npcParts.hat.setColorAt(i,instanceColor);updateCustodyAwareNpcLabel(label,src,x,7.3+(prisonStaff?(i%3)*.28:0),z);}for(const mesh of [npcParts.body,npcParts.leftArm,npcParts.rightArm,npcParts.hat])if(mesh.instanceColor)mesh.instanceColor.needsUpdate=true;const said3d=dynamic.npcs.find(n=>n.said);renderer.domElement.dataset.said3d=said3d?`visible:${said3d.hired?'hired':'available'}:important:${said3d.important?1:0}`:'out-of-range';
          ...
          for(let i=0;i<Math.min(NPC_CAP,dynamic.npcs.length);i++){
            ...
            const pick=(arr,salt=0)=>arr[(((seed>>>salt)^Math.imul(seed,31+salt))>>>0)%arr.length],role=String(src.role||'').toLowerCase(),said=!!src.said||role==='said',police=src.visualRole==='police'||!!src.police||role.includes('police')||role.includes('cop'),gang=!!src.gang||role.includes('gang')||role.includes('boss')||role.includes('district_')||role.includes('occupier'),prisonStaff=!!src.prisonStaff||String(src.behavior||'')==='prison_patrol'||role.startsWith('prison_'),responseTier=Math.max(0,Math.min(3,+src.responseTier||0)),tactical=responseTier>0,prisonGuard=prisonStaff&&role.includes('guard'),staffVariant=Math.abs(+src.staffVariant||0)%3,prisonGear=String(src.prisonGear||''),armed=gang||role.includes('guard')||police,look=src.look||{};
            let authoredSuit=0;try{if(look.suit||src.outfit)authoredSuit=instanceColor.set(look.suit||src.outfit).getHex();}catch(_){authoredSuit=0;}
            const tacticalBody=[0x245f9b,0x18334a,0x151b22,0x090c11][responseTier],tacticalLeg=[0x172536,0x102534,0x0d1218,0x05070a][responseTier],tacticalHat=[0x2869aa,0x234d69,0x1a222c,0x0d1015][responseTier];
            const faction=String(src.faction||src.family||'').toLowerCase(),bodyColor=said?0xf4f2e9:prisonStaff?(prisonGuard?[0x35483c,0x3e4937,0x2b4039][staffVariant]:[0x245f9b,0x31527d,0x1f6777][staffVariant]):police?tacticalBody:gang?(faction.includes('yellow')?0xf1e4c4:faction.includes('purple')?0x7447aa:pick([0xb84a5d,0x5f6f85,0xb48a49],3)):role.includes('guard')?0xb98738:authoredSuit||pick(npcShirts,1),legColor=said?0x17191c:prisonStaff?(prisonGuard?[0x1e2922,0x242a20,0x18251f][staffVariant]:[0x172536,0x202c3c,0x16313a][staffVariant]):police?tacticalLeg:gang?(faction.includes('yellow')?0x5e523c:faction.includes('purple')?0x342a48:0x3b3740):pick(npcTrousers,5),skinColor=Number.isFinite(+look.skin)?npcSkins[Math.abs(+look.skin|0)%npcSkins.length]:pick(npcSkins,7),hairColor=said?0x17191c:pick(npcHairs,9),shoeColor=said||police||prisonStaff?0x101820:pick(npcShoes,11),hatColor=police?tacticalHat:gang?bodyColor:pick(npcHats,13),hairStyle=Number.isFinite(+look.hair)?Math.abs(+look.hair|0)%6:(seed>>>16)%6,accessory=(seed>>>20)%12,capOn=said?false:prisonStaff?prisonGear==='cap':tactical?false:police||armed||(Number.isFinite(+look.hat)&&+look.hat>0)||hairStyle===4||hairStyle===5,helmetOn=!said&&(prisonStaff||tactical)&&prisonGear.includes('helmet'),wearHat=capOn||helmetOn,glassesOn=!said&&!armed&&(accessory===0||accessory===4||role.includes('staff')),neckOn=said||armed||accessory%3===1,bagOn=!said&&!armed&&accessory%4===2,moustacheOn=!said&&!armed&&+look.gender!==1&&(accessory===3||accessory===9);
            ...
            npcParts.head.setColorAt(i,instanceColor.setHex(skinColor));npcParts.hair.setColorAt(i,instanceColor.setHex(hairColor));npcParts.hairBun.setColorAt(i,instanceColor.setHex(hairColor));npcParts.hairMohawk.setColorAt(i,instanceColor.setHex(hairColor));npcParts.hat.setColorAt(i,instanceColor.setHex(hatColor));npcParts.hatBrim.setColorAt(i,instanceColor.setHex(hatColor));npcParts.shoe.setColorAt(i*2,instanceColor.setHex(shoeColor));npcParts.shoe.setColorAt(i*2+1,instanceColor.setHex(shoeColor));npcParts.glasses.setColorAt(i,instanceColor.setHex(accessory%2?0x26343e:0x4b3028));npcParts.neckAccent.setColorAt(i,instanceColor.setHex(said?0x090a0c:pick(npcAccents,15)));npcParts.bag.setColorAt(i,instanceColor.setHex(pick(npcBags,17)));npcParts.moustache.setColorAt(i,instanceColor.setHex(hairColor));
            npcParts.prisonHelmet.setColorAt(i,instanceColor.setHex(tactical?tacticalHat:prisonGuard?[0x334437,0x3d4936,0x2d4037][staffVariant]:[0x244b70,0x2d4058,0x24505c][staffVariant]));npcParts.prisonHelmetBand.setColorAt(i,instanceColor.setHex(responseTier>=3?0x8f202a:prisonGuard?0x161d19:0x152332));npcParts.prisonVest.setColorAt(i,instanceColor.setHex(tactical?tacticalLeg:prisonGuard?0x29372f:0x1b2c42));npcParts.riotShield.setColorAt(i,instanceColor.setHex(responseTier>=3?0x12161b:[0x26323a,0x30383a,0x202e31][staffVariant]));
            const x=npcVisualXs[i],z=npcVisualZs[i],pose=npcAnimationPose(src,i,t);
            setNpcRoot(pose,i,x,z);
            hidePart(npcParts.hair,i);hidePart(npcParts.hairBun,i);hidePart(npcParts.hairMohawk,i);hidePart(npcParts.hat,i);hidePart(npcParts.hatBrim,i);hidePart(npcParts.glasses,i);hidePart(npcParts.neckAccent,i);hidePart(npcParts.bag,i);hidePart(npcParts.moustache,i);hidePart(npcParts.prisonHelmet,i);hidePart(npcParts.prisonHelmetBand,i);hidePart(npcParts.prisonVisor,i);hidePart(npcParts.prisonVest,i);hidePart(npcParts.riotShield,i);hidePart(npcParts.shieldViewport,i);
            ...
            if(prisonStaff||tactical)setPart(npcParts.prisonVest,i,rootMatrix,0,2.02-pose.crouch,.02);
            ...
            npcAppearanceSignatures.add(`${bodyColor}:${legColor}:${skinColor}:${hairColor}:${hairStyle}:${prisonGear||'city'}:${glassesOn?1:0}:${neckOn?1:0}:${bagOn?1:0}:${moustacheOn?1:0}`);
          }
          renderer.domElement.dataset.npcAppearanceVariants=String(npcAppearanceSignatures.size);renderer.domElement.dataset.npcAppearanceSystem='fresh-respawn-faces-hair-police-escalation-v240';renderer.domElement.dataset.jailStaffGear='guards:military-helmet-riot-shield;police:cap-tactical-helmet-bare';renderer.domElement.dataset.policeResponseTiers=String(dynamic.npcs.reduce((m,n)=>Math.max(m,+n.responseTier||0),0));renderer.domElement.dataset.jailStaffMaxHp='200';
          ...
          let corpseBloodPoolCount=0;
          for(let i=0;i<activeNpcInstances;i++){
            const src=dynamic.npcs[i];if(!npcIsDead(src)){hidePart(corpseBloodDecals,i);continue;}
            ...
            instanceQuat.setFromEuler(instanceEuler.set(-Math.PI/2,0,seed));
            instanceMatrix.compose(instancePosition.set(x,.052,z),instanceQuat,instanceScale.set(.38+spread*1.12,.3+spread*.72,1));
            corpseBloodDecals.setMatrixAt(i,instanceMatrix);corpseBloodPoolCount++;
          }
          corpseBloodDecals.instanceMatrix.needsUpdate=true;renderer.domElement.dataset.corpseBloodPools=String(corpseBloodPoolCount);renderer.domElement.dataset.corpseMotion='frozen-at-death-v205';
          ...
          for(let i=0;i<BULLET_CAP;i++){const src=dynamic.projectiles[i];if(!src){hidePart(worldBullets,i);hidePart(worldBulletTrails,i);hidePart(worldBulletGlows,i);hidePart(worldBulletCores,i);continue;}const weapon=String(src.weapon||'pistol'),rocket=weapon==='rpg',water=weapon==='water_hose',pellet=weapon==='shotgun',scale=(rocket?2.7:src.thick?1.5:pellet?.65:1)*(+src.bulletScale||1),velocity=Math.hypot(+src.vr||0,+src.vc||0),speedStretch=THREE.MathUtils.clamp(velocity/20,.72,1.6),trail=(rocket?2.4:water?.55:weapon==='sniper'?3.2:pellet?.6:1.35)*(+src.trailScale||1)*speedStretch,dr=+src.vr||Math.sin(+src.ang||0),dc=+src.vc||Math.cos(+src.ang||0),progress=Math.max(0,Math.min(1,(+src.dist||0)/Math.max(.01,+src.maxDist||1))),elevation=THREE.MathUtils.lerp(+src.startZ||0,+src.endZ||0,progress)*WORLD_SCALE,y=(rocket?2.8:2.2)+elevation;instancePosition.set((src.c-originC)*WORLD_SCALE,y,(src.r-originR)*WORLD_SCALE);bulletDirection.set(dc,0,dr).normalize();instanceQuat.setFromUnitVectors(bulletUp,bulletDirection);instanceMatrix.compose(instancePosition,instanceQuat,instanceScale.set(scale,scale*(rocket?1.18:1),scale));worldBullets.setMatrixAt(i,instanceMatrix);trailPosition.copy(instancePosition).addScaledVector(bulletDirection,-.55*trail);instanceMatrix.compose(trailPosition,instanceQuat,instanceScale.set(scale,trail,scale));worldBulletTrails.setMatrixAt(i,instanceMatrix);const glowScale=scale*(rocket?2.2:1);instanceMatrix.compose(instancePosition,instanceQuat,instanceScale.set(glowScale,glowScale,glowScale));worldBulletGlows.setMatrixAt(i,instanceMatrix);const coreScale=scale*(rocket?1.15:water?.52:.68);instanceMatrix.compose(instancePosition,instanceQuat,instanceScale.set(coreScale,coreScale*(rocket?1.35:1),coreScale));worldBulletCores.setMatrixAt(i,instanceMatrix);try{instanceColor.set(src.color);}catch(_){instanceColor.set(0xffdc79);}worldBullets.setColorAt(i,instanceColor);worldBulletTrails.setColorAt(i,instanceColor);worldBulletGlows.setColorAt(i,rocket?instanceColor.set(0xff6a20):instanceColor);worldBulletCores.setColorAt(i,water?instanceColor.set(0xd9f8ff):rocket?instanceColor.set(0xffe07a):instanceColor.set(0xfff5d0));}
          for(const mesh of [worldBullets,worldBulletTrails,worldBulletGlows,worldBulletCores]){mesh.instanceMatrix.needsUpdate=true;if(mesh.instanceColor)mesh.instanceColor.needsUpdate=true;}
          renderer.domElement.dataset.trackedVehicleProjectiles=String(dynamic.projectiles.filter(x=>x?.tracked).length);
          for(let i=0;i<30;i++){const src=dynamic.shellFx?.[i];if(!src){hidePart(shellPool,i);continue;}const rot=+src.rot||0;instanceQuat.setFromEuler(instanceEuler.set(rot,rot*.7,rot*1.3));instanceMatrix.compose(instancePosition.set((src.c-originC)*WORLD_SCALE,.18+(+src.z||0)*WORLD_SCALE,(src.r-originR)*WORLD_SCALE),instanceQuat,unitScale);shellPool.setMatrixAt(i,instanceMatrix);}shellPool.instanceMatrix.needsUpdate=true;
          ...
          for(let i=0;i<32;i++){const src=dynamic.bulletHoleFx?.[i];if(!src){hidePart(bulletHoleDecals,i);continue;}const ang=+src.ang||0,size=(src.heavy?1.12:.68)*(+src.scale||1),fade=Math.max(.48,1-Math.max(0,+src.age||0)/120000),height=1.82+((+src.id||i)*.37%1)*.72;instanceQuat.setFromEuler(instanceEuler.set(0,-Math.PI/2-ang,+src.rot||0));instanceMatrix.compose(instancePosition.set((src.c-originC)*WORLD_SCALE-Math.cos(ang)*.055,height,(src.r-originR)*WORLD_SCALE-Math.sin(ang)*.055),instanceQuat,instanceScale.set(size,size,1));bulletHoleDecals.setMatrixAt(i,instanceMatrix);bulletHoleDecals.setColorAt(i,instanceColor.setRGB(.52*fade,.46*fade,.39*fade));}bulletHoleDecals.instanceMatrix.needsUpdate=true;if(bulletHoleDecals.instanceColor)bulletHoleDecals.instanceColor.needsUpdate=true;
          ...
          });renderer.domElement.dataset.explosionProfile='textured-fireball-pooled-v199';renderer.domElement.dataset.explosionParticles='10-flame-plumes-8-smoke-20-embers-10-debris-no-lights';
        ...
        if(bridge){
          ...
          if(prisonReleaseGate){
            ...
            renderer.domElement.dataset.jailReleaseGate=gateLocked?`locked:${Math.max(0,Math.ceil(+state.jailIn||0))}`:`open:${prisonReleaseGateVisual.toFixed(2)}`;renderer.domElement.dataset.jailGateProgress=prisonReleaseGateVisual.toFixed(3);
          }
          const prisonAlarmActive=!!state.prisonAlarm?.active;
          ...
          const reloadLayerActive=playerAnimationLayer==='reload';activeReloadProgress=reloadLayerActive?Math.max(0,Math.min(1,+state.reloadProgress||0)):0;if(reloadLayerActive&&!reloadWasActive)spawnReloadDebris();reloadWasActive=reloadLayerActive;renderer.domElement.dataset.reloadAnimation=reloadLayerActive?`${currentWeaponFx.family}:${activeReloadProgress.toFixed(2)}`:'idle';
          ...
          policeCapBand.visible=state.role==='police';policeBadge.visible=state.role==='police';shirtFront.visible=state.role==='mafia'&&!state.armor;tie.visible=state.role==='mafia'&&!state.armor;hatBrim.material.color.setHex(state.role==='police'?0x183a5c:state.role==='mafia'&&state.family==='moretti'?0xe6e0d3:0x151922);hatTop.material.color.copy(hatBrim.material.color);
          const visualSig=`${state.role}:${state.family||''}:${state.look?.skin??0}:${state.weapon||''}:${state.holsteredWeapon||''}:${state.armor||''}:${state.armorDurability||0}:${state.armorMax||0}:${state.driving?1:0}:${state.dead?1:0}:${state.vehicleEntry?.token||''}:${state.arrestPhase||''}`;if(visualSig!==playerVisualSig){playerVisualSig=visualSig;const roleColor=state.role==='police'?0x285c91:state.role==='mafia'?(state.family==='moretti'?0xe6e0d3:0x25272d):0x7a3035;suitMat.color.set(roleColor);skinMat.color.set([0xf0c3a0,0xd8a07c,0xb87955,0x8b583e,0x633c2d][Math.max(0,Math.min(4,+state.look?.skin||0))]);configureWeaponVisual(state.weapon);configureArmorVisual(state.armor,state.armorDurability,state.armorMax);gun.visible=gun.visible&&!state.driving&&!state.dead&&!state.vehicleEntry&&!state.arrestPhase;holsteredPistol.visible=!!state.holsteredWeapon&&!state.weapon&&!state.driving&&!state.dead&&!state.vehicleEntry&&!state.arrestPhase;renderer.domElement.dataset.holsteredWeapon=String(state.holsteredWeapon||'');}
          ...
          lastPlayerHp=+state.hp||0;playerHpBar.scale.x=3.9*Math.max(.02,playerHpPct);playerHpBar.material.color.set(playerHpPct>.55?0x55e778:playerHpPct>.25?0xffc94d:0xff4d55);
          ...
          if(showInterior){
            ...
            const layoutSig=interiorData.layout?`v${interiorData.layout.visualVersion||0}:${interiorData.layout.props?.map(p=>p.id).join(',')||''}:${interiorData.layout.safes?.map(s=>`${s.id}:${s.opened?1:0}`).join(',')||''}`:'',
