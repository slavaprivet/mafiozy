...
#include <colorspace_fragment>
      ...
      const npcPrompt=document.createElement('div');npcPrompt.style.cssText='position:absolute;left:0;top:0;z-index:23;display:none;min-width:190px;padding:9px 14px 10px;border:1px solid rgba(223,183,91,.96);border-radius:13px;background:linear-gradient(145deg,rgba(10,15,22,.985),rgba(43,24,14,.97));color:#fff0bd;font:900 13px system-ui;text-align:center;pointer-events:none;box-shadow:0 12px 32px #000e,0 0 25px #d49b3d44;transform:translate3d(-999px,-999px,0) translate(-50%,-100%);will-change:transform;animation:mfzNpcPromptIn .18s ease-out both';npcPrompt.innerHTML='<span style="display:inline-grid;place-items:center;min-width:23px;height:23px;margin-right:8px;border:1px solid #fff4b9;border-radius:7px;background:linear-gradient(#ffe27c,#b87519);color:#171008;font:1000 12px system-ui;animation:mfzBuildingKeyPulse 1.25s ease-in-out infinite">E</span><span>Действия</span>';stage.appendChild(npcPrompt);const npcPromptProjection=new THREE.Vector3();
      ...
      const showNearbyVehicle=(near,t)=>{if(!near){nearbyVehicleRing.visible=false;renderer.domElement.dataset.nearbyVehicle='none';return;}const x=(+near.c-originC)*WORLD_SCALE,z=(+near.r-originR)*WORLD_SCALE,pulse=1+Math.sin(t*.0065)*.018;nearbyVehicleMaterial.color.setHex(near.kind==='hijack'?0xe7b84f:0x8fd7a7);nearbyVehicleMaterial.opacity=.72+Math.sin(t*.007)*.08;if(!nearbyVehicleRing.visible)nearbyVehicleRing.position.set(x,.105,z);else{nearbyVehicleRing.position.x=THREE.MathUtils.lerp(nearbyVehicleRing.position.x,x,.3);nearbyVehicleRing.position.z=THREE.MathUtils.lerp(nearbyVehicleRing.position.z,z,.3);}nearbyVehicleRing.scale.setScalar(pulse);nearbyVehicleRing.visible=true;renderer.domElement.dataset.nearbyVehicle=`${near.id}:${near.kind}:${(+near.distance||0).toFixed(2)}`;};
      const updateNearbyNpcPromptPosition=near=>{if(!near||npcPrompt.style.display==='none')return;npcPromptProjection.set(nearbyNpcRing.position.x,5.45,nearbyNpcRing.position.z).project(camera);const anchorX=(npcPromptProjection.x*.5+.5)*lastW,anchorY=(-npcPromptProjection.y*.5+.5)*lastH,w=Math.min(220,lastW-24),h=50,pad=12,x=Math.max(pad+w/2,Math.min(lastW-pad-w/2,anchorX)),y=Math.max(pad+h,Math.min(lastH-pad,anchorY));npcPrompt.style.transform=`translate3d(${x.toFixed(2)}px,${y.toFixed(2)}px,0) translate(-50%,-100%)`;npcPrompt.dataset.anchor='frame-smooth-ring-projection-v304';};
      ...
      const animate = t => {
        ...
        camera.position.set(player.position.x+54,62+playerFloorElevation,player.position.z+54);camera.lookAt(player.position.x,1.6+playerFloorElevation,player.position.z);if(t-promptPositionAt>80){updateBuildingPromptPosition();promptPositionAt=t;}updateNearbyNpcPromptPosition(nearbyNpcState);
