...
if ((rendererParams.get('force3d') === '1' || rendererParams.get('render') !== 'canvas') && rendererConfig.threeEnabled !== false) {
  (async () => {
    ...
    try {
      ...
      const STATIC_SHADOW_LAYER=3;sun.shadow.camera.layers.enable(STATIC_SHADOW_LAYER);
...
transformed.z+=cos(mfzWindTime*.82+mfzPhase*1.31+position.z*.42)*mfzGust*mfzWeight*${bendZ};`);mat.userData.shader=shader;};mat.customProgramCacheKey=()=>`mfz-wind-v3-${color}-${bendX}-${bendZ}`;windLeafMaterials.push(mat);return mat;};
      ...
      const applyMaterialUvRepeat=(geometry,material,wallsOnly=false)=>{const repeat=material?.userData?.mfzUvRepeat,uv=geometry?.getAttribute?.('uv');if(!repeat||!uv)return geometry;for(let i=0;i<uv.count;i++){if(wallsOnly&&uv.count===24&&i>=8&&i<16)continue;uv.setXY(i,uv.getX(i)*repeat[0],uv.getY(i)*repeat[1]);}uv.needsUpdate=true;return geometry;};
      const box = (x, z, w, d, h, mat) => {
        const geometry=applyMaterialUvRepeat(new THREE.BoxGeometry(w,h,d),mat),mesh = new THREE.Mesh(geometry, mat);
      ...
      const buildingBox = (x, z, w, d, h, wallMat, roofMaterial) => {
        const geometry=applyMaterialUvRepeat(buildingBoxGeometry(w,h,d),wallMat,true),mesh = new THREE.Mesh(geometry, [wallMat, roofMaterial]);
        mesh.position.set(x, h / 2, z);
        mesh.castShadow = mesh.receiveShadow = true;
        scene.add(mesh);
        return mesh;
      ...
      // Large authored landmarks contain hundreds of immutable pieces. Keep
      // every vertex/material, but submit nearby opaque pieces sharing the same
      // render state as one mesh. Spatial chunks retain useful frustum culling
      // and excluded actors (doors, alarms, selectable masses) stay independent.
      const batchStaticRoot=(root,{exclude=[],chunkWorld=42,label='static-root'}={})=>{
        if(!root)return{sources:0,batches:0};
        root.updateMatrixWorld(true);
        const excluded=new Set(exclude.filter(Boolean)),rootInverse=new THREE.Matrix4().copy(root.matrixWorld).invert(),localMatrix=new THREE.Matrix4(),center=new THREE.Vector3(),buckets=new Map(),geometryUsage=new Map(),removedGeometryCounts=new Map();
        scene.traverse(object=>{if(object.geometry)geometryUsage.set(object.geometry,(geometryUsage.get(object.geometry)||0)+1);});
        const belongsToExcludedSubtree=object=>{for(let cursor=object;cursor&&cursor!==root;cursor=cursor.parent)if(excluded.has(cursor))return true;return false;};
        root.traverse(mesh=>{
          if(!mesh.isMesh||mesh.isInstancedMesh||mesh.isSkinnedMesh||mesh.children.length||!mesh.visible||Array.isArray(mesh.material)||!mesh.material||mesh.material.transparent||mesh.morphTargetInfluences||belongsToExcludedSubtree(mesh))return;
          const geometry=mesh.geometry,attributes=Object.keys(geometry?.attributes||{}).sort();
          if(!geometry||!attributes.includes('position')||attributes.some(name=>!geometry.getAttribute(name)?.array))return;
          mesh.getWorldPosition(center);center.applyMatrix4(rootInverse);
          const chunkX=Math.floor(center.x/chunkWorld),chunkZ=Math.floor(center.z/chunkWorld),attributeKey=attributes.map(name=>{const a=geometry.getAttribute(name);return`${name}:${a.itemSize}:${a.normalized?1:0}:${a.array.constructor.name}`;}).join(','),key=`${mesh.material.uuid}:${attributeKey}:${mesh.castShadow?1:0}:${mesh.receiveShadow?1:0}:${mesh.renderOrder}:${mesh.layers.mask}:${chunkX}:${chunkZ}`;
          if(!buckets.has(key))buckets.set(key,[]);buckets.get(key).push(mesh);
        });
        let sources=0,batches=0;
        for(const meshes of buckets.values()){
          if(meshes.length<3)continue;
          const geometries=meshes.map(mesh=>{mesh.updateMatrixWorld(true);localMatrix.multiplyMatrices(rootInverse,mesh.matrixWorld);const geometry=mesh.geometry.index?mesh.geometry.toNonIndexed():mesh.geometry.clone();geometry.applyMatrix4(localMatrix);return geometry;}),attributes=Object.keys(geometries[0].attributes).sort();
          if(!attributes.every(name=>geometries.every(geometry=>{const a=geometry.getAttribute(name),sample=geometries[0].getAttribute(name);return a&&a.itemSize===sample.itemSize&&a.normalized===sample.normalized&&a.array.constructor===sample.array.constructor;}))){geometries.forEach(geometry=>geometry.dispose());continue;}
          const merged=new THREE.BufferGeometry();
          for(const name of attributes){const sample=geometries[0].getAttribute(name),total=geometries.reduce((sum,geometry)=>sum+geometry.getAttribute(name).array.length,0),values=new sample.array.constructor(total);let offset=0;for(const geometry of geometries){const array=geometry.getAttribute(name).array;values.set(array,offset);offset+=array.length;}merged.setAttribute(name,new THREE.BufferAttribute(values,sample.itemSize,sample.normalized));}
          merged.computeBoundingBox();merged.computeBoundingSphere();
          const first=meshes[0],batch=new THREE.Mesh(merged,first.material);batch.name=`batched-${label}`;batch.castShadow=first.castShadow;batch.receiveShadow=first.receiveShadow;batch.renderOrder=first.renderOrder;batch.layers.mask=first.layers.mask;root.add(batch);
          for(const mesh of meshes){mesh.parent?.remove(mesh);removedGeometryCounts.set(mesh.geometry,(removedGeometryCounts.get(mesh.geometry)||0)+1);}geometries.forEach(geometry=>geometry.dispose());sources+=meshes.length;batches++;
        }
        for(const [geometry,count] of removedGeometryCounts)if(count===(geometryUsage.get(geometry)||0))geometry.dispose();
        return{sources,batches};
      };
      const staticShadowProxyMaterial=new THREE.MeshBasicMaterial({color:0x000000,colorWrite:false,depthWrite:false,depthTest:false,toneMapped:false});staticShadowProxyMaterial.userData.mfzPersistent=true;
      // Opaque static materials all use the same depth-only shader in the sun pass.
      // Merge their exact silhouettes independently of the colour batches: the main
      // camera still renders every authored material, while the 1536px shadow map
      // submits one spatial proxy instead of dozens of identical depth draw calls.
      const batchStaticShadowCasters=(root,{exclude=[],chunkWorld=42,label='static-root'}={})=>{
        if(!root)return{sources:0,batches:0};root.updateMatrixWorld(true);
        const excluded=new Set(exclude.filter(Boolean)),rootInverse=new THREE.Matrix4().copy(root.matrixWorld).invert(),localMatrix=new THREE.Matrix4(),center=new THREE.Vector3(),buckets=new Map();
        const belongsToExcludedSubtree=object=>{for(let cursor=object;cursor&&cursor!==root;cursor=cursor.parent)if(excluded.has(cursor))return true;return false;};
        root.traverse(mesh=>{const material=mesh.material,geometry=mesh.geometry;if(!mesh.isMesh||mesh.isInstancedMesh||mesh.isSkinnedMesh||mesh.children.length||!mesh.visible||!mesh.castShadow||Array.isArray(material)||!material||material.transparent||material.alphaTest>0||material.alphaMap||material.displacementMap||mesh.morphTargetInfluences||belongsToExcludedSubtree(mesh)||!geometry?.getAttribute?.('position'))return;mesh.getWorldPosition(center);center.applyMatrix4(rootInverse);const key=`${Math.floor(center.x/chunkWorld)}:${Math.floor(center.z/chunkWorld)}`;if(!buckets.has(key))buckets.set(key,[]);buckets.get(key).push(mesh);});
        let sources=0,batches=0;
        for(const meshes of buckets.values()){
          if(meshes.length<2)continue;
          const transformed=meshes.map(mesh=>{mesh.updateMatrixWorld(true);localMatrix.multiplyMatrices(rootInverse,mesh.matrixWorld);const geometry=mesh.geometry.index?mesh.geometry.toNonIndexed():mesh.geometry.clone();geometry.applyMatrix4(localMatrix);return geometry;}),total=transformed.reduce((sum,geometry)=>sum+geometry.getAttribute('position').count,0),positions=new Float32Array(total*3);let offset=0;
          for(const geometry of transformed){const position=geometry.getAttribute('position');for(let i=0;i<position.count;i++){positions[offset++]=position.getX(i);positions[offset++]=position.getY(i);positions[offset++]=position.getZ(i);}geometry.dispose();}
          const merged=new THREE.BufferGeometry();merged.setAttribute('position',new THREE.BufferAttribute(positions,3));merged.computeBoundingBox();merged.computeBoundingSphere();const proxy=new THREE.Mesh(merged,staticShadowProxyMaterial);proxy.name=`shadow-${label}`;proxy.castShadow=true;proxy.receiveShadow=false;proxy.layers.set(STATIC_SHADOW_LAYER);root.add(proxy);for(const mesh of meshes)mesh.castShadow=false;sources+=meshes.length;batches++;
        }
        return{sources,batches};
      };
...
#include <colorspace_fragment>
      ...
      if(worldSnapshot?.landmarks){
        ...
        if(jail){
          ...
          const prisonStaticExclude=[prisonReleaseGate,prisonReleaseGateLight,prisonVehicleLockGate,...prisonAlarmBeacons.map(beacon=>beacon.root)],prisonStaticBatch=batchStaticRoot(prison,{exclude:prisonStaticExclude,chunkWorld:36,label:'prison-static'}),prisonShadowBatch=batchStaticShadowCasters(prison,{exclude:prisonStaticExclude,chunkWorld:36,label:'prison-static'});renderer.domElement.dataset.prisonStaticBatch=`${prisonStaticBatch.sources}>${prisonStaticBatch.batches}`;renderer.domElement.dataset.prisonShadowBatch=`${prisonShadowBatch.sources}>${prisonShadowBatch.batches}`;
        ...
        const buildAuthoredBankExterior3D=bank=>{
          ...
          const bankStaticBatch=batchStaticRoot(root,{exclude:[bankBody],chunkWorld:32,label:`bank-${bank.id||bank.size}`}),bankShadowBatch=batchStaticShadowCasters(root,{exclude:[bankBody],chunkWorld:32,label:`bank-${bank.id||bank.size}`});root.userData.staticBatch=bankStaticBatch;root.userData.shadowBatch=bankShadowBatch;
        ...
        for(const poi of snapshot?.pois||[])if(poi.id==='junkyard'){
          ...
          const junkyardSceneStart=scene.children.length;
          ...
          const junkyardRoot=new THREE.Group();junkyardRoot.name='junkyard-streamed-visual';for(const child of scene.children.slice(junkyardSceneStart))junkyardRoot.add(child);scene.add(junkyardRoot);const junkyardStaticBatch=batchStaticRoot(junkyardRoot,{chunkWorld:34,label:'junkyard-static'}),junkyardShadowBatch=batchStaticShadowCasters(junkyardRoot,{chunkWorld:34,label:'junkyard-static'});renderer.domElement.dataset.junkyardStaticBatch=`${junkyardStaticBatch.sources}>${junkyardStaticBatch.batches}`;renderer.domElement.dataset.junkyardShadowBatch=`${junkyardShadowBatch.sources}>${junkyardShadowBatch.batches}`;
          if(source!=='initial'){junkyardRoot.visible=false;junkyardRoot.userData.mfzDeferredReveal=true;deferredRevealRoots.push(junkyardRoot);if(fullMaterialsReady)onIdle(warmDeferredSceneRoots);}
      ...
      const createBuilding=(definition,bi)=>{
        ...
        const familyFacades=architectureFacadeTextures.get(architectureFamily.id)||architectureFacadeTextures.get('concrete'),facade=familyFacades[buildingSeed%familyFacades.length],facadeRepeat=[Math.max(1,w/24),Math.max(1.35,h/16)];
        ...
        wall.userData.mfzUvRepeat=facadeRepeat;
      ...
      // Vehicle slots are a fixed render pool. Share immutable detail resources so
      // deferred slot creation does not allocate and upload identical GPU data again.
      const vehicleGeometryCache=new Map(),vehicleGeometry=(key,create)=>{let geometry=vehicleGeometryCache.get(key);if(!geometry){geometry=create();vehicleGeometryCache.set(key,geometry);}return geometry;};
      const sharedVehicleMaterials={
        glass:new THREE.MeshPhysicalMaterial({color:0x172b3e,metalness:.08,roughness:.07,transmission:.16,thickness:.22,clearcoat:1,clearcoatRoughness:.04,envMap:cityEnvironment,envMapIntensity:1.65}),
        bumper:new THREE.MeshStandardMaterial({color:0x727b82,metalness:.82,roughness:.22}),chrome:new THREE.MeshStandardMaterial({color:0xb8c2c9,metalness:.92,roughness:.18}),rubber:new THREE.MeshStandardMaterial({color:0x111419,roughness:.86}),wheelRubber:new THREE.MeshStandardMaterial({color:0x090a0c,roughness:.86}),
        plate:new THREE.MeshBasicMaterial({color:0xe9e3c9}),doorLine:new THREE.MeshBasicMaterial({color:0x14191d,transparent:true,opacity:.55}),taxi:new THREE.MeshBasicMaterial({color:0xffe34a}),bed:new THREE.MeshStandardMaterial({color:0x20252a,roughness:.78,metalness:.25}),seat:new THREE.MeshStandardMaterial({color:0x7d2b25,roughness:.75}),
        serviceSteel:new THREE.MeshStandardMaterial({color:0x59636b,metalness:.74,roughness:.3}),serviceWhite:new THREE.MeshStandardMaterial({color:0xe8edf0,metalness:.2,roughness:.48}),turretDark:new THREE.MeshStandardMaterial({color:0x111820,metalness:.7,roughness:.34}),turretUniform:new THREE.MeshStandardMaterial({color:0x17283a,roughness:.66}),turretSkin:new THREE.MeshStandardMaterial({color:0xc99773,roughness:.78}),turretMuzzle:new THREE.MeshBasicMaterial({color:0xffdf68,toneMapped:false}),medicalRed:new THREE.MeshStandardMaterial({color:0xc92934,roughness:.42,metalness:.18}),roadDust:new THREE.MeshStandardMaterial({color:0x493e35,roughness:1,transparent:true,opacity:.34}),rotor:new THREE.MeshStandardMaterial({color:0x232a30,metalness:.6,roughness:.35}),sirenRed:new THREE.MeshBasicMaterial({color:0xff2929}),sirenBlue:new THREE.MeshBasicMaterial({color:0x278cff})
      };
      const createVehicleSlot=({v,i})=>{
        ...
        const profile=utility?'utility':long?'long':'standard',base=new THREE.Mesh(vehicleGeometry(`base-${profile}`,()=>new THREE.BoxGeometry(L,H,utility?3.05:2.75)),paint);base.position.y=H*.5+.38;base.castShadow=true;car.add(base);
        const glass=sharedVehicleMaterials.glass;
        const cab=new THREE.Mesh(vehicleGeometry(`cab-${profile}`,()=>new THREE.BoxGeometry(utility?3.8:3.1,utility?1.35:1.05,utility?2.7:2.45)),glass);cab.position.set(utility?-.25:.15,utility?2.15:1.95,0);cab.castShadow=true;car.add(cab);
        const hood=new THREE.Mesh(vehicleGeometry(`hood-${profile}`,()=>new THREE.BoxGeometry(L*.27,.32,utility?2.82:2.5)),paint);hood.position.set(L*.34,H+1.02,0);hood.castShadow=true;car.add(hood);
        const bumperMat=sharedVehicleMaterials.bumper;
        for(const sx of [-1,1]){const bumper=new THREE.Mesh(vehicleGeometry(`bumper-${profile}`,()=>new THREE.BoxGeometry(.18,.24,utility?2.95:2.66)),bumperMat);bumper.position.set(sx*(L/2+.13),.58,0);car.add(bumper);}
        ...
        const siren=new THREE.Group();for(const [z,material] of [[-.36,sharedVehicleMaterials.sirenRed],[.36,sharedVehicleMaterials.sirenBlue]]){const lamp=new THREE.Mesh(vehicleGeometry('siren-lamp',()=>new THREE.BoxGeometry(.75,.22,.55)),material);lamp.position.set(0,2.82,z);siren.add(lamp);}siren.visible=false;car.add(siren);
        const plate=new THREE.Mesh(vehicleGeometry('plate',()=>new THREE.PlaneGeometry(1.15,.38)),sharedVehicleMaterials.plate);plate.rotation.y=-Math.PI/2;plate.position.set(-L/2-.11,.72,0);car.add(plate);
        const chrome=sharedVehicleMaterials.chrome,rubber=sharedVehicleMaterials.rubber;
        ...
        const wheelRubber=sharedVehicleMaterials.wheelRubber,wheelInstances=new THREE.InstancedMesh(vehicleGeometry('wheel',()=>new THREE.CylinderGeometry(.48,.48,.34,14)),wheelRubber,wheelPositions.length),hubInstances=new THREE.InstancedMesh(vehicleGeometry('wheel-hub',()=>new THREE.CylinderGeometry(.25,.25,.355,12)),chrome,wheelPositions.length),tireLipInstances=new THREE.InstancedMesh(vehicleGeometry('wheel-lip',()=>new THREE.TorusGeometry(.37,.075,7,16)),rubber,wheelPositions.length),wheelMatrix=new THREE.Matrix4(),wheelQuat=new THREE.Quaternion(),wheelUnit=new THREE.Vector3(1,1,1);
        ...
        for(const side of [-1,1])for(const dx of [-.75,1.05]){const doorLine=new THREE.Mesh(vehicleGeometry('door-line',()=>new THREE.PlaneGeometry(.035,1.05)),sharedVehicleMaterials.doorLine);doorLine.position.set(dx,1.62,side*(utility?1.536:1.386));doorLine.rotation.y=side<0?Math.PI:0;car.add(doorLine);}
        ...
        const taxiSign=new THREE.Mesh(vehicleGeometry('taxi-sign',()=>new THREE.BoxGeometry(1.35,.38,.58)),sharedVehicleMaterials.taxi);taxiSign.position.set(0,2.75,0);taxiSign.visible=false;car.add(taxiSign);
        ...
        const pickupBed=new THREE.Group(),bedMat=sharedVehicleMaterials.bed,bedFloor=new THREE.Mesh(vehicleGeometry('pickup-bed',()=>new THREE.BoxGeometry(2.35,.18,2.55)),bedMat);bedFloor.position.set(-1.55,1.15,0);pickupBed.add(bedFloor);for(const z of [-1.22,1.22]){const rail=new THREE.Mesh(vehicleGeometry('pickup-rail',()=>new THREE.BoxGeometry(2.45,.7,.16)),paint);rail.position.set(-1.55,1.45,z);pickupBed.add(rail);}pickupBed.visible=false;car.add(pickupBed);
        ...
        const cabrioSeats=new THREE.Group();for(const x of [-.55,.55]){const seat=new THREE.Mesh(vehicleGeometry('cabrio-seat',()=>new THREE.BoxGeometry(.65,.72,.82)),sharedVehicleMaterials.seat);seat.position.set(x,1.62,0);cabrioSeats.add(seat);}cabrioSeats.visible=false;car.add(cabrioSeats);
        ...
        const serviceSteel=sharedVehicleMaterials.serviceSteel,serviceWhite=sharedVehicleMaterials.serviceWhite,fireEquipment=new THREE.Group(),fireTank=new THREE.Mesh(vehicleGeometry('fire-tank',()=>new THREE.CylinderGeometry(.72,.72,2.75,18)),serviceWhite);fireTank.rotation.z=Math.PI/2;fireTank.position.set(-1.15,2.18,0);fireEquipment.add(fireTank);
        ...
        const ambulanceRearDoors=new THREE.Group(),ambulanceRearDoorPivots=[],medicalRed=sharedVehicleMaterials.medicalRed;
        for(const side of [-1,1]){
        ...
        const roadDustMat=sharedVehicleMaterials.roadDust;for(const z of [-1,1]){const grime=new THREE.Mesh(vehicleGeometry(`road-grime-${profile}`,()=>new THREE.PlaneGeometry(L*.72,.62)),roadDustMat);grime.rotation.y=z>0?0:Math.PI;grime.position.set(-.12,.72,z*(utility?1.535:1.385));car.add(grime);}
      ...
      let vehicleGeometryReuses=0;
      const shareVehicleGeometries=car=>{car.traverse(object=>{const geometry=object.geometry;if(!geometry?.parameters)return;let parameters;try{parameters=JSON.stringify(geometry.parameters);}catch(_){return;}const key=`shape:${geometry.type}:${parameters}`,shared=vehicleGeometryCache.get(key);if(shared&&shared!==geometry){object.geometry=shared;geometry.dispose();vehicleGeometryReuses++;}else if(!shared)vehicleGeometryCache.set(key,geometry);});renderer.domElement.dataset.vehicleSharedGeometries=String(vehicleGeometryReuses);renderer.domElement.dataset.vehicleGeometryPool=String(vehicleGeometryCache.size);};
      vehicleSlotDefs.slice(0,INITIAL_VEHICLE_SYNC_CAP).forEach(def=>{createVehicleSlot(def);const car=cars[cars.length-1];shareVehicleEffectMaterials(car);shareVehicleGeometries(car);});
      ...
      const pumpDeferredVehicleSlots=()=>{
        ...
        if(next){createVehicleSlot(next);const car=cars[cars.length-1];shareVehicleEffectMaterials(car);shareVehicleGeometries(car);}
      ...
      const nearbyNpcOuterMaterial=new THREE.MeshBasicMaterial({color:0xd9aa45,transparent:true,opacity:.9,side:THREE.DoubleSide,depthTest:false}),nearbyNpcInnerMaterial=new THREE.MeshBasicMaterial({color:0x78e3ad,transparent:true,opacity:.76,side:THREE.DoubleSide,depthTest:false}),nearbyNpcRing=new THREE.Group();for(const ring of [new THREE.Mesh(new THREE.RingGeometry(1.42,1.62,40),nearbyNpcOuterMaterial),new THREE.Mesh(new THREE.RingGeometry(.96,1.08,40),nearbyNpcInnerMaterial)]){ring.rotation.x=-Math.PI/2;ring.renderOrder=54;nearbyNpcRing.add(ring);}nearbyNpcRing.visible=false;scene.add(nearbyNpcRing);
      ...
      const updatePlayerSpeech=(entry,value,x,y,z)=>{const speech=String(value||'').trim();if(!speech){entry.sprite.visible=false;entry.sig='';return;}const sig=`player-chat-v269:${speech}`;if(sig!==entry.sig){entry.sig=sig;const c=entry.canvas.getContext('2d');c.clearRect(0,0,768,192);c.fillStyle='rgba(255,250,226,.98)';c.fillRect(18,6,732,150);c.strokeStyle='#d2a947';c.lineWidth=10;c.strokeRect(24,12,720,138);c.fillStyle='rgba(255,250,226,.98)';c.beginPath();c.moveTo(326,156);c.lineTo(384,190);c.lineTo(442,156);c.closePath();c.fill();let size=50;c.font=`900 ${size}px system-ui`;const words=speech.split(/\s+/),lines=[''];for(const word of words){const i=lines.length-1,test=(lines[i]+' '+word).trim();if(c.measureText(test).width>680&&lines.length<2)lines.push(word);else lines[i]=test;}while(size>30&&lines.some(line=>{c.font=`900 ${size}px system-ui`;return c.measureText(line).width>680;}))size-=2;c.font=`900 ${size}px system-ui`;c.textAlign='center';c.textBaseline='middle';c.fillStyle='#17130d';if(lines.length===1)c.fillText(lines[0],384,83);else{c.fillText(lines[0],384,60);c.fillText(lines[1],384,116);}entry.texture.needsUpdate=true;}entry.sprite.visible=true;entry.sprite.position.set(x,y,z);};
      ...
      const npcPrompt=document.createElement('div');npcPrompt.style.cssText='position:absolute;z-index:23;display:none;min-width:190px;padding:9px 14px 10px;border:1px solid rgba(223,183,91,.96);border-radius:13px;background:linear-gradient(145deg,rgba(10,15,22,.985),rgba(43,24,14,.97));color:#fff0bd;font:900 13px system-ui;text-align:center;pointer-events:none;box-shadow:0 12px 32px #000e,0 0 25px #d49b3d44;transform:translate(-50%,-100%);animation:mfzBuildingPromptIn .18s ease-out both';npcPrompt.innerHTML='<span style="display:inline-grid;place-items:center;min-width:23px;height:23px;margin-right:8px;border:1px solid #fff4b9;border-radius:7px;background:linear-gradient(#ffe27c,#b87519);color:#171008;font:1000 12px system-ui;animation:mfzBuildingKeyPulse 1.25s ease-in-out infinite">E</span><span>Действия</span>';stage.appendChild(npcPrompt);const npcPromptProjection=new THREE.Vector3(),promptViewport={stageW:size.W,stageH:size.H,canvasW:size.W,canvasH:size.H,canvasLeft:0,canvasTop:0},npcPromptSize={w:220,h:46},buildingPromptSize={w:230,h:52};
      const refreshPromptViewport=()=>{const stageRect=stage.getBoundingClientRect(),canvasRect=renderer.domElement.getBoundingClientRect();promptViewport.stageW=stageRect.width||size.W;promptViewport.stageH=stageRect.height||size.H;promptViewport.canvasW=canvasRect.width||promptViewport.stageW;promptViewport.canvasH=canvasRect.height||promptViewport.stageH;promptViewport.canvasLeft=canvasRect.left-stageRect.left;promptViewport.canvasTop=canvasRect.top-stageRect.top;};
      ...
      const keys=new Set(),bullets=[],reloadDebris=[],vehicleHoldMs=170;let aimPoint=new THREE.Vector3(0,0,-10),lastShot=0,muzzleLife=0,recoilKick=0,recoilSide=1,recoilShotSeq=0,reloadWasActive=false,activeReloadProgress=0,triggerHeld=false,laserAimHeld=false,laserAimAllowed=true,laserShotPulseUntil=0,throwAimHeld=false,throwAimSecondary=false,throwAimStartedAt=0,throwAimKind='',throwAimAngle=0,throwAimRange=1.6,throwReleaseUntil=0,animationActionLocked=false,eHoldStarted=0,eHoldTimer=0,eHoldTriggered=false,eHoldVehicleId='',eHoldHasBuilding=false,eHoldNpcKey='';
      ...
      let nearestNpcScanAt=0,nearestNpcCached=null,nearbyNpcVisualKey='';
      const nearestNpcInteraction=(force=false,t=performance.now())=>{if(!force&&t-nearestNpcScanAt<100)return nearestNpcCached;nearestNpcScanAt=t;const state=bridge?.getPlayerState?.();if(!dynamicState?.npcs?.length||state?.driving||state?.dead||state?.vehicleEntry)return nearestNpcCached=null;const pr=originR+player.position.z/WORLD_SCALE,pc=originC+player.position.x/WORLD_SCALE;let best=null,bestD2=3.05*3.05;for(let i=0;i<dynamicState.npcs.length&&i<NPC_CAP;i++){const src=dynamicState.npcs[i];if(!src||src.dead)continue;const r=+src.r,c=+src.c;if(!Number.isFinite(r)||!Number.isFinite(c))continue;const dr=r-pr,dc=c-pc,d2=dr*dr+dc*dc,key=String(src.id||src.sourceId||'');if(key&&d2<bestD2){bestD2=d2;best={src,index:i,key,distance:Math.sqrt(d2)};}}return nearestNpcCached=best;};
      const showNearbyNpc=(near,t)=>{if(!near){if(nearbyNpcVisualKey){npcPrompt.style.display='none';nearbyNpcRing.visible=false;renderer.domElement.dataset.nearbyNpc='none';nearbyNpcVisualKey='';}return;}const {src,key,distance}=near,x=(+src.c-originC)*WORLD_SCALE,z=(+src.r-originR)*WORLD_SCALE,role=String(src.role||'').toLowerCase(),police=!!src.police||role.includes('police')||role.includes('cop'),gang=!!src.gang||role.includes('gang')||role.includes('boss')||role.includes('guard'),kind=police?'police':gang?'gang':'civilian',accent=police?0x59b9ff:gang?0xff5d6c:0xd9aa45;if(key!==nearbyNpcVisualKey){nearbyNpcVisualKey=key;nearbyNpcOuterMaterial.color.setHex(accent);nearbyNpcInnerMaterial.color.setHex(police?0x93d8ff:gang?0xffa0aa:0x78e3ad);npcPrompt.style.display='block';npcPromptSize.w=npcPrompt.offsetWidth||npcPromptSize.w;npcPromptSize.h=npcPrompt.offsetHeight||npcPromptSize.h;renderer.domElement.dataset.npcActionPrompt='actions-reserved-v268';}else if(npcPrompt.style.display==='none')npcPrompt.style.display='block';nearbyNpcRing.position.set(x,.14,z);const pulse=1+Math.sin(t*.0085)*.07;nearbyNpcRing.scale.setScalar(pulse);nearbyNpcRing.rotation.y=t*.0014;nearbyNpcOuterMaterial.opacity=.76+Math.sin(t*.01)*.14;nearbyNpcRing.visible=true;npcPromptProjection.set(x,5.45,z).project(camera);const anchorX=promptViewport.canvasLeft+(npcPromptProjection.x*.5+.5)*promptViewport.canvasW,anchorY=promptViewport.canvasTop+(-npcPromptProjection.y*.5+.5)*promptViewport.canvasH,pad=12;npcPrompt.style.left=`${Math.max(pad+npcPromptSize.w/2,Math.min(promptViewport.stageW-pad-npcPromptSize.w/2,anchorX))}px`;npcPrompt.style.top=`${Math.max(pad+npcPromptSize.h,Math.min(promptViewport.stageH-pad,anchorY))}px`;buildingPrompt.style.display='none';renderer.domElement.dataset.nearbyNpc=`${key}:${distance.toFixed(2)}:${kind}`;};
      addEventListener('keydown',e=>{const typing=e.target?.matches?.('input,textarea,select,[contenteditable="true"]'),chatMode=document.documentElement.dataset.chatMode==='open';if(typing||chatMode){keys.clear();return;}keys.add(e.code);if(e.code==='KeyE'&&!e.repeat){const vehicle=bridge?.getNearbyVehicleInteraction?.(),building=bridge?.getNearbyBuildingInteraction?.(),npc=nearestNpcInteraction(true);eHoldStarted=performance.now();eHoldTriggered=false;eHoldVehicleId=String(vehicle?.id||'');eHoldHasBuilding=!!building;eHoldNpcKey=String(npc?.key||'');if(vehicle){vehicleHoldPrompt.firstElementChild.textContent=eHoldNpcKey?'Короткое E — действия NPC · удержание — машина':eHoldHasBuilding?'🚪 Короткое E — здание · удержание — машина':'🚗 Удерживайте E — сесть в машину';vehicleHoldPrompt.style.display='block';vehicleHoldFill.style.width='0';renderer.domElement.dataset.vehicleHold=`waiting:${eHoldVehicleId}`;renderer.domElement.dataset.interactionPriority=eHoldNpcKey?'tap-npc-hold-vehicle':eHoldHasBuilding?'tap-building-hold-vehicle':'hold-vehicle-only';eHoldTimer=setTimeout(()=>{if(keys.has('KeyE'))triggerVehicleEntry();},vehicleHoldMs);}else renderer.domElement.dataset.interactionPriority=eHoldNpcKey?'npc-actions':building?'building-only':'none';e.preventDefault();e.stopImmediatePropagation();}else if(e.code==='Escape'){bridge?.closeBuildingActions?.();}else if(e.code==='Space'){e.preventDefault();e.stopImmediatePropagation();if(currentWeaponId==='c4')bridge?.plantC4?.();else if(!isThrowablePrimary())shoot(performance.now());}},true);
      addEventListener('keyup',e=>{keys.delete(e.code);const typing=e.target?.matches?.('input,textarea,select,[contenteditable="true"]'),chatMode=document.documentElement.dataset.chatMode==='open';if(typing||chatMode){if(e.code==='KeyE'){clearTimeout(eHoldTimer);vehicleHoldPrompt.style.display='none';eHoldStarted=0;eHoldVehicleId='';eHoldHasBuilding=false;eHoldNpcKey='';}return;}if(e.code!=='KeyE')return;clearTimeout(eHoldTimer);vehicleHoldPrompt.style.display='none';if(!eHoldTriggered){if(eHoldNpcKey){renderer.domElement.dataset.npcAction=`reserved:${eHoldNpcKey}`;renderer.domElement.dataset.interactionPriority='npc-actions-reserved';}else{const bankResult=bridge?.interactBank?.(),result=bankResult?.ok?bankResult:bridge?.toggleNearbyBuildingActions?.(innerWidth/2,innerHeight*.58);renderer.domElement.dataset.buildingAction=result?.ok?(result.closed?'closed':`${result.kind}:${result.id||''}`):`rejected:${result?.reason||'none'}`;renderer.domElement.dataset.interactionPriority=eHoldHasBuilding?'building-tap-complete':eHoldVehicleId?'vehicle-short-ignored':'building-only-complete';}}if(eHoldTriggered)renderer.domElement.dataset.vehicleHold='complete';else renderer.domElement.dataset.vehicleHold=eHoldNpcKey?'short-npc-action':'short-building-action';eHoldStarted=0;eHoldVehicleId='';eHoldHasBuilding=false;eHoldNpcKey='';},true);
      ...
      const updateBuildingPromptPosition=()=>{if(!nearbyBuildingEntry||buildingPrompt.style.display==='none')return;if(!promptViewport.canvasW||!promptViewport.canvasH)return;const entry=nearbyBuildingEntry,building=buildingObjectForEntry(entry),anchorR=Number.isFinite(+entry.r)?+entry.r:originR,anchorC=Number.isFinite(+entry.c)?+entry.c:originC,buildingHeight=building?.geometry?.parameters?.height||6;if(building)promptProjection.set(building.position.x,building.position.y+buildingHeight*.55+1.2,building.position.z);else promptProjection.set((anchorC-originC)*WORLD_SCALE,5.2,(anchorR-originR)*WORLD_SCALE);promptProjection.project(camera);const anchorX=promptViewport.canvasLeft+(promptProjection.x*.5+.5)*promptViewport.canvasW,anchorY=promptViewport.canvasTop+(-promptProjection.y*.5+.5)*promptViewport.canvasH,pad=12,x=Math.max(pad+buildingPromptSize.w/2,Math.min(promptViewport.stageW-pad-buildingPromptSize.w/2,anchorX)),y=Math.max(pad+buildingPromptSize.h,Math.min(promptViewport.stageH-pad,anchorY-18));buildingPrompt.style.left=`${x}px`;buildingPrompt.style.top=`${y}px`;buildingPrompt.dataset.avoidsPlayer='fixed-building-anchor';};
      const showNearbyBuilding=entry=>{nearbyBuildingEntry=entry||null;if(!entry){if(nearbyBuildingVisualKey){buildingPrompt.style.display='none';entranceMarker.visible=false;nearbyBuildingVisualKey='';manualBuildingSelectionUntil=0;clearBuildingHighlight();renderer.domElement.dataset.nearbyBuilding='none';renderer.domElement.dataset.buildingPrompt='hidden-out-of-range';renderer.domElement.dataset.entranceMarker='hidden-out-of-range';}return;}const visualKey=`${entry.kind}:${entry.id}`;if(visualKey!==nearbyBuildingVisualKey){nearbyBuildingVisualKey=visualKey;buildingPromptText.textContent=`Действие: ${entry.name||'Здание'}`;buildingPromptHint.textContent=entry.kind==='bank'?'Войти в банк':entry.kind==='building'&&entry.type==='police_st'?'Войти в участок':entry.kind==='building'&&!entry.type?'Войти в здание':'Выбрать вход или действие';buildingPrompt.style.animation='none';void buildingPrompt.offsetWidth;buildingPrompt.style.animation='mfzBuildingPromptIn .28s cubic-bezier(.2,.9,.25,1) both';buildingPrompt.style.display='block';buildingPromptSize.w=buildingPrompt.offsetWidth||buildingPromptSize.w;buildingPromptSize.h=buildingPrompt.offsetHeight||buildingPromptSize.h;renderer.domElement.dataset.nearbyBuilding=visualKey;renderer.domElement.dataset.buildingPrompt='visible-in-range';renderer.domElement.dataset.businessActionMode='proximity-prompt-explicit-e';}else if(buildingPrompt.style.display==='none')buildingPrompt.style.display='block';const exterior=entry.kind==='business'?businessExteriorById.get(String(entry.id)):null,building=exterior?.main||buildingObjectForEntry(entry),markerR=Number.isFinite(+entry.entryR)?+entry.entryR:+entry.r+.5,markerC=Number.isFinite(+entry.entryC)?+entry.entryC:+entry.c+.5;entranceMarker.position.set((markerC-originC)*WORLD_SCALE,.24,(markerR-originR)*WORLD_SCALE);entranceMarker.visible=true;renderer.domElement.dataset.entranceMarker=`visible:${markerR.toFixed(2)},${markerC.toFixed(2)}`;if(building!==highlightedBuildingObject){if(building)highlightBuilding(building);else clearBuildingHighlight();}};
      ...
      const canHoldLaserAim=()=>{const state=bridge?.getPlayerState?.();return !['','none','fists','unarmed'].includes(currentWeaponId)&&!state?.driving&&!state?.dead&&!state?.vehicleEntry;};
      ...
      addEventListener('mousedown',e=>{if(e.button!==0||!laserAimHeld||e.pointerType==='touch'||(e.target.closest&&e.target.closest('button,input,textarea,select,#joyL,#joyR')))return;e.preventDefault();e.stopImmediatePropagation();if(isSpecialPrimary()){const accepted=releaseSpecialPrimary();renderer.domElement.dataset.secondaryThrowChord=accepted?(currentWeaponId==='c4'?'rmb-aim-lmb-place':'rmb-aim-lmb-throw'):`rejected:${currentWeaponId}`;if(accepted)renderer.domElement.dataset.secondaryThrowCount=String((+renderer.domElement.dataset.secondaryThrowCount||0)+1);return;}if(beginFirearmAtPointer(e,false)){renderer.domElement.dataset.laserFireChord='rmb-held-lmb-fired';renderer.domElement.dataset.laserFireChordCount=String((+renderer.domElement.dataset.laserFireChordCount||0)+1);}},true);
      ...
      let buildingPumpStarted=false,staticDetailFlushScheduled=false,initialCompileRunning=false,deferredWarmupRunning=false,deferredWarmupTarget=null;
      ...
      warmDeferredSceneRoots=()=>{
        ...
        for(const root of roots){root.userData.mfzWarmupInFlight=true;const clone=root.clone(true);clone.visible=true;clone.traverse?.(object=>{object.frustumCulled=false;});warmupScene.add(clone);}
        const warmupLights=[];scene.traverseVisible(object=>{if(object.isLight){const light=object.clone();warmupLights.push(light);warmupScene.add(light);}});
        ...
        const uploadSources=[];
        const collectUploadSources=(object,parentVisible=true,isRoot=false)=>{const visible=isRoot?true:parentVisible&&object.visible!==false;if(!visible)return;if(object.isMesh||object.isLine||object.isPoints)uploadSources.push(object);for(const child of object.children||[])collectUploadSources(child,visible,false);};
        for(const root of roots){root.updateMatrixWorld(true);collectUploadSources(root,true,true);}
        const uploadCompiledGeometry=()=>{
          warmupScene.clear();for(const light of warmupLights)warmupScene.add(light);if(!uploadSources.length){reveal();return;}
          deferredWarmupTarget||=new THREE.WebGLRenderTarget(2,2,{depthBuffer:true,stencilBuffer:false});let cursor=0;
          const pumpUploads=deadline=>{
            const sliceStartedAt=performance.now(),source=uploadSources[cursor++],proxy=source.clone(false);proxy.visible=true;proxy.frustumCulled=false;proxy.castShadow=false;proxy.layers.mask=camera.layers.mask;proxy.matrixAutoUpdate=false;proxy.matrix.copy(source.matrixWorld);warmupScene.add(proxy);const previousTarget=renderer.getRenderTarget(),previousShadowAutoUpdate=renderer.shadowMap.autoUpdate,previousShadowNeedsUpdate=renderer.shadowMap.needsUpdate;renderer.shadowMap.autoUpdate=false;renderer.shadowMap.needsUpdate=false;try{renderer.setRenderTarget(deferredWarmupTarget);renderer.clear();renderer.render(warmupScene,camera);}finally{renderer.setRenderTarget(previousTarget);renderer.shadowMap.autoUpdate=previousShadowAutoUpdate;renderer.shadowMap.needsUpdate=previousShadowNeedsUpdate;warmupScene.remove(proxy);}
            const sliceElapsed=performance.now()-sliceStartedAt,previousMax=+renderer.domElement.dataset.gpuWarmupSliceMaxMs||0;renderer.domElement.dataset.gpuWarmupSliceMs=sliceElapsed.toFixed(1);if(sliceElapsed>previousMax){renderer.domElement.dataset.gpuWarmupSliceMaxMs=sliceElapsed.toFixed(1);renderer.domElement.dataset.gpuWarmupSliceWorst=`${source.name||source.type}:${source.geometry?.getAttribute?.('position')?.count||0}:${source.material?.type||'material'}`;}renderer.domElement.dataset.gpuWarmupSources=`${cursor}/${uploadSources.length}`;if(cursor<uploadSources.length)onIdle(pumpUploads);else reveal();
          };
          onIdle(pumpUploads);
        };
        if(pendingCompile?.then)pendingCompile.then(uploadCompiledGeometry,error=>{console.warn('[ThreeStream] material warmup fallback',error);uploadCompiledGeometry();});
        else uploadCompiledGeometry();
      };
      ...
      const animate = t => {
        ...
        const s = viewSize();
        if (s.W !== lastW || s.H !== lastH) { lastW = s.W; lastH = s.H; camera.left = -cameraSpan * s.W / s.H; camera.right = cameraSpan * s.W / s.H; camera.updateProjectionMatrix(); renderer.setSize(s.W, s.H, false); refreshPromptViewport(); }
        ...
        if(bridge&&t-nearbyActionAt>120){nearbyActionState=bridge.getNearbyBuildingInteraction?.()||null;nearbyActionAt=t;}const nearbyNpcState=nearestNpcInteraction(false,t);if(document.getElementById('bizConfirmModal')?.classList.contains('show')){buildingPrompt.style.display='none';npcPrompt.style.display='none';nearbyNpcRing.visible=false;entranceMarker.visible=false;renderer.domElement.dataset.buildingPrompt='hidden-by-business-modal';}else{showNearbyBuilding(nearbyActionState);showNearbyNpc(nearbyNpcState,t);}
        ...
        if(bridge){
          ...
          policeCapBand.visible=state.role==='police';policeBadge.visible=state.role==='police';prisonerBadge.visible=state.role==='prisoner';shirtFront.visible=state.role==='mafia'&&!state.armor;tie.visible=state.role==='mafia'&&!state.armor;hatBrim.visible=hatTop.visible=state.role!=='prisoner';hatBrim.material.color.setHex(state.role==='police'?0x183a5c:state.role==='mafia'&&state.family==='moretti'?0xe6e0d3:0x151922);hatTop.material.color.copy(hatBrim.material.color);
          const visualSig=`${state.role}:${state.family||''}:${state.look?.skin??0}:${state.weapon||''}:${state.holsteredWeapon||''}:${state.armor||''}:${state.armorDurability||0}:${state.armorMax||0}:${state.driving?1:0}:${state.dead?1:0}:${state.vehicleEntry?.token||''}:${state.arrestPhase||''}`;if(visualSig!==playerVisualSig){playerVisualSig=visualSig;const roleColor=state.role==='prisoner'?0xf07818:state.role==='police'?0x285c91:state.role==='mafia'?(state.family==='moretti'?0xe6e0d3:0x25272d):0x7a3035;suitMat.color.set(roleColor);skinMat.color.set([0xf0c3a0,0xd8a07c,0xb87955,0x8b583e,0x633c2d][Math.max(0,Math.min(4,+state.look?.skin||0))]);configureWeaponVisual(state.weapon);configureArmorVisual(state.role==='prisoner'?null:state.armor,state.armorDurability,state.armorMax);gun.visible=gun.visible&&!state.driving&&!state.dead&&!state.vehicleEntry&&!state.arrestPhase&&state.role!=='prisoner';holsteredPistol.visible=!!state.holsteredWeapon&&!state.weapon&&!state.driving&&!state.dead&&!state.vehicleEntry&&!state.arrestPhase&&state.role!=='prisoner';renderer.domElement.dataset.holsteredWeapon=String(state.holsteredWeapon||'');renderer.domElement.dataset.playerUniform=state.role==='prisoner'?'orange-prisoner':'civilian-or-faction';}
