...
// 3D composite v254: twelve apartment plans, residential facade kits and mobile visual-FX LOD complete the current polish pass.
...
#include <colorspace_fragment>
      ...
      let prisonReleaseGate=null,prisonReleaseGateLight=null,prisonReleaseGateVisual=0;
      if(worldSnapshot?.landmarks){
        ...
        if(jail){
          ...
          // Two open sally ports: the west checkpoint leads to the old police
          // block and the north checkpoint meets the suspension bridge.
          const wallH=4.25,wallT=.82,westGateW=10.5,westGateOpen=6.8,northGateW=9.5,northGateOpen=6.5,southGateW=9.5,southGateOpen=6.8,sideWallD=(spanZ-westGateW)/2,sideWallW=(spanX-northGateW)/2,southSideWallW=(spanX-southGateW)/2;
          const perimeter=[
            ...
            pbox(-halfX,-(westGateW+sideWallD)/2,wallT,sideWallD,wallH,concrete,.68),pbox(-halfX,(westGateW+sideWallD)/2,wallT,sideWallD,wallH,concrete,.68),
          ...
          for(const side of [-1,1]){
            const westPost=pbox(-halfX,side*(westGateW/2+.48),1.3,1.05,5.5,concreteDark,.68);outline(westPost);
            ...
            pbox(-halfX,side*westGateOpen/2,.42,.42,4.75,darkSteel,.68);pbox(side*northGateOpen/2,-halfZ,.42,.42,4.75,darkSteel,.68);
            pbox(side*southGateOpen/2,halfZ,.42,.42,4.75,darkSteel,.68);
            for(let band=0;band<4;band++){pbox(-halfX-.67,side*(westGateW/2+.48),.08,.98,.28,band%2?warningMat:darkSteel,1.2+band*.62);pbox(side*(northGateW/2+.48),-halfZ-.67,.98,.08,.28,band%2?warningMat:darkSteel,1.2+band*.62);pbox(side*(southGateW/2+.48),halfZ+.67,.98,.08,.28,band%2?warningMat:darkSteel,1.2+band*.62);}
          }
          // Bright checkpoint arches make the openings legible from the
          // isometric camera even when the dark perimeter is in shadow.
          pbox(-halfX,0,.78,westGateW+.9,.32,darkSteel,5.2);pbox(-halfX-.43,0,.1,westGateW+.25,.18,warningMat,5.27);
          ...
          renderer.domElement.dataset.jailGateLeaves='north-west-open-south-1-personal-lift';renderer.domElement.dataset.jailGateStyle='personal-timed-lift-grille-v245';renderer.domElement.dataset.jailIntakeRoom='9x12-tiles-half-main-width-over-water';

          // Razor coils are instanced so the detailed perimeter remains cheap.
          const coilDefs=[],coilStep=2.35;for(let q=-halfX+2;q<=halfX-2;q+=coilStep){if(Math.abs(q)>northGateW/2+.6)coilDefs.push([q,5.35,-halfZ,0,Math.PI/2,0]);if(Math.abs(q)>southGateW/2+.6)coilDefs.push([q,5.35,halfZ,0,Math.PI/2,0]);}for(let q=-halfZ+2;q<=halfZ-2;q+=coilStep){if(Math.abs(q)>westGateW/2+.6)coilDefs.push([-halfX,5.35,q,0,0,0]);coilDefs.push([halfX,5.35,q,0,0,0]);}instance('prison-razor-wire',new THREE.TorusGeometry(.62,.038,5,12),galvanized,coilDefs,false);
      ...
      const addBusinessIdentityArchitecture=(kind,x,z,w,d,h,seed)=>{
        ...
        if(kind==='coffee'){
          ...
          businessExteriorFx.push({kind:'bar-neon',material:pink,phase:seed,baseX:x,baseZ:front});
        }else if(kind==='club'){
      ...
      const addProceduralBuildingIdentity=(x,z,w,d,h,seed,districtStyle,familyId='concrete')=>{
        const familyVariants={glass:[3,5],brick:[0,2],limestone:[4,1],concrete:[5,4],deco:[1,4],industrial:[2,5]},variantPool=familyVariants[familyId]||[0,5],front=z+d/2+.11,accent=identityAccents[(seed+variantPool.length)%identityAccents.length],variant=variantPool[(seed>>>3)%variantPool.length],floors=Math.max(1,Math.floor((h-4)/3.4)),add=(xx,zz,ww,dd,hh,mat,yy=hh/2)=>{const q=box(xx,zz,ww,dd,hh,mat);q.position.y=yy;return q;},addRound=(geo,mat,xx,yy,zz)=>{const q=new THREE.Mesh(geo,mat);q.position.set(xx,yy,zz);q.castShadow=false;q.receiveShadow=true;scene.add(q);return q;};
        ...
        // Ordinary homes between authored businesses need their own street-level
        // identity as well. Six lightweight residential kits are deterministic,
        // only exist inside the normal detail radius and preserve the doorway.
        const residentialDistrict=['downtown','rich','coast','countryside','chinatown_poor','chinatown_rich'].includes(districtStyle),homeKit=(seed>>>9)%6;
        if(residentialDistrict){
          if(homeKit===0){
            // Window boxes and a narrow planted ledge.
            for(const sx of [-.31,-.12,.12,.31]){add(x+sx*w,front+.34,w*.12,.5,.16,identityWood,2.05);for(const ox of [-.28,.28])add(x+sx*w+ox,front+.42,.34,.32,.28,identityAccents[3],2.27);}
          }else if(homeKit===1){
            // Visible air-conditioning cluster and service conduit.
            for(const [sx,y] of [[-.32,5.1],[.29,8.35]]){add(x+sx*w,front+.38,1.45,.62,.82,identityStone,y);for(let k=-1;k<=1;k++)add(x+sx*w+k*.32,front+.72,.08,.08,.58,identityDark,y);add(x+sx*w+.82,front+.25,.09,.14,Math.max(1.4,y-2.2),identityDark,y*.5+1.1);}
          }else if(homeKit===2){
            // Laundry balcony: unmistakably inhabited without animated cloth.
            const y=Math.min(h-2.1,7.1);add(x,front+.86,w*.52,1.45,.18,identityStone,y);for(const sx of [-.24,.24])add(x+sx*w,front+1.48,.08,.08,1.05,identityDark,y+.52);add(x,front+1.49,w*.48,.07,.07,identityDark,y+1.02);for(let k=-2;k<=2;k++)add(x+k*w*.09,front+1.47,w*.07,.05,.72,k%2?accent:identityGlass,y+.54);
          }else if(homeKit===3){
            // Older house: shallow entrance arch and paired wall lanterns.
            const arch=add(x,front+.52,Math.min(5.2,w*.44),.6,.38,identityStone,4.08);arch.rotation.z=.03;for(const sx of [-1,1]){const lantern=add(x+sx*Math.min(3.15,w*.28),front+.46,.34,.34,.7,identityGlow,3.15);lantern.castShadow=false;add(lantern.position.x,front+.3,.08,.12,1.25,identityDark,3.15);}
          }else if(homeKit===4){
            // Rooftop water tank creates a different skyline silhouette.
            const tank=addRound(new THREE.CylinderGeometry(1.25,1.35,2.25,14),identityDark,x-w*.24,h+1.5,z-d*.12);for(const sx of [-.72,.72])for(const sz of [-.56,.56])add(tank.position.x+sx,tank.position.z+sz,.12,.12,1.45,identityWood,h+.72);
          }else{
            // Ground-floor bicycle/service corner and house-number plaque.
            for(const sx of [-1.05,1.05]){const wheel=addRound(new THREE.TorusGeometry(.62,.09,8,18),identityDark,x+w*.31+sx*.45,1.02,front+.43);wheel.rotation.y=Math.PI/2;}const frame=add(x+w*.31,front+.45,1.5,.08,.1,accent,1.08);frame.rotation.z=.58;add(x-w*.31,front+.39,1.45,.16,.82,identityStone,2.65);
          }
        }
      };
      ...
      const interiorGroup=new THREE.Group(),interiorVisualFx=[];interiorGroup.layers.set(1);interiorGroup.visible=false;scene.add(interiorGroup);const interiorAmbient=new THREE.HemisphereLight(0xffead1,0x4a3542,3.05);interiorAmbient.layers.set(1);scene.add(interiorAmbient);let interiorSignature='',interiorFloor=null,interiorLightingActive=false,lastStableInteriorData=null,lastStableInteriorAt=0;
      ...
      const decorateBankInterior=data=>{
        ...
        const hWall=(c0,c1,r,height=4.35,material=wall)=>{if(c1-c0<.08)return;const len=(c1-c0)*S,baseMat=material===wallDark?steelDark:woodDark,capMat=material===wallDark?steel:brass;add(new THREE.BoxGeometry(len,height,.3*S),material,(c0+c1)/2,height/2,r);add(new THREE.BoxGeometry(len,.2,.34*S),baseMat,(c0+c1)/2,.1,r);add(new THREE.BoxGeometry(len,.14,.34*S),capMat,(c0+c1)/2,height-.07,r).castShadow=false;wallPost(c0,r,height,capMat);wallPost(c1,r,height,capMat);};
        const vWall=(c,r0,r1,height=4.35,material=wall)=>{if(r1-r0<.08)return;const len=(r1-r0)*S,baseMat=material===wallDark?steelDark:woodDark,capMat=material===wallDark?steel:brass;add(new THREE.BoxGeometry(.3*S,height,len),material,c,height/2,(r0+r1)/2);add(new THREE.BoxGeometry(.34*S,.2,len),baseMat,c,.1,(r0+r1)/2);add(new THREE.BoxGeometry(.34*S,.14,len),capMat,c,height-.07,(r0+r1)/2).castShadow=false;wallPost(c,r0,height,capMat);wallPost(c,r1,height,capMat);};
        ...
        const bankDoor=(c,r,face=1,open=false)=>{const pivot=new THREE.Group();pivot.position.set(x(c),0,z(r));pivot.layers.set(1);interiorGroup.add(pivot);const centerY=2.65,frame=addRaw(new THREE.TorusGeometry(2.62,.3,14,42),steelDark,0,centerY,0,pivot);frame.rotation.x=Math.PI/2;for(const side of [-1,1])addRaw(new THREE.CapsuleGeometry(.28,4.45,8,16),steelDark,side*2.48,2.35,0,pivot);const lintel=addRaw(new THREE.BoxGeometry(5.35,.42,.48),brass,0,5.08,0,pivot);lintel.castShadow=false;const g=new THREE.Group();g.position.x=open?-5.45:0;g.rotation.y=open?-.32*face:0;g.layers.set(1);pivot.add(g);const door=addRaw(new THREE.CylinderGeometry(2.32,2.32,.62,42),steel,0,centerY,.03*face,g);door.rotation.x=Math.PI/2;for(let k=0;k<14;k++){const a=k/14*Math.PI*2,bolt=addRaw(new THREE.CylinderGeometry(.1,.1,.2,9),brass,Math.cos(a)*1.78,centerY+Math.sin(a)*1.78,.39*face,g);bolt.rotation.x=Math.PI/2;}const wheel=addRaw(new THREE.TorusGeometry(.78,.12,10,28),brass,0,centerY,.43*face,g);wheel.rotation.x=Math.PI/2;for(let k=0;k<6;k++){const a=k/6*Math.PI*2,spoke=addRaw(new THREE.BoxGeometry(1.48,.07,.065),brass,Math.cos(a)*.37,centerY+Math.sin(a)*.37,.45*face,g);spoke.rotation.z=a;spoke.castShadow=false;}};
        ...
        const framedDoor=(c,r,turn=0,secure=false)=>{const g=new THREE.Group();g.position.set(x(c),0,z(r));g.rotation.y=turn;g.layers.set(1);interiorGroup.add(g);for(const side of [-1,1])addRaw(new THREE.CapsuleGeometry(.13*S,3.45,7,12),secure?steelDark:woodDark,side*1.12*S,1.88,0,g);addRaw(new THREE.BoxGeometry(2.42*S,.24,.24*S),secure?steel:brass,0,3.76,0,g);const leaf=addRaw(new THREE.BoxGeometry(1.86*S,3.3,.12*S),secure?steelDark:wood,.98*S,1.68,-.92*S,g);leaf.rotation.y=-Math.PI/2;addRaw(new THREE.SphereGeometry(.07,8,6),secure?screenGreen:brass,.92*S,1.72,-.12*S,g).castShadow=false;};
        ...
        const glassPartition=(c0,c1,r,doorC=null)=>{const postAt=c=>add(new THREE.CylinderGeometry(.07,.1,3.45,10),brass,c,1.73,r);postAt(c0);postAt(c1);if(doorC!==null){postAt(Math.max(c0,doorC-1.2));postAt(Math.min(c1,doorC+1.2));}add(new THREE.BoxGeometry((c1-c0)*S,.14,.16*S),brass,(c0+c1)/2,3.42,r).castShadow=false;for(let c=c0+.7;c<c1;c+=1.4){if(doorC!==null&&Math.abs(c-doorC)<1.35)continue;const pane=add(new THREE.BoxGeometry(1.15*S,.72,.04*S),glass,c,.48,r);pane.castShadow=false;}};
        // The vault has no matching gameplay collider for a closed wire fence.
        // Use an unmistakably open security portal instead of a fake solid wall.
        const wireCagePanel=(c,r,length=4,turn=0,doorGap=false)=>{const g=new THREE.Group();g.position.set(x(c),0,z(r));g.rotation.y=turn;g.layers.set(1);interiorGroup.add(g);const half=length*S*.5;for(const side of [-1,1])addRaw(new THREE.CapsuleGeometry(.1*S,3.35,7,12),steelDark,side*half,1.82,0,g);addRaw(new THREE.BoxGeometry(length*S,.16*S,.16*S),steelDark,0,3.62,0,g).castShadow=false;for(const side of [-1,1]){const brace=addRaw(new THREE.BoxGeometry(.9*S,.055*S,.055*S),steel,side*(half-.42*S),3.18,0,g);brace.rotation.z=side*.72;brace.castShadow=false;}if(doorGap)addRaw(new THREE.BoxGeometry(1.25*S,.12*S,.2*S),brass,0,3.42,0,g).castShadow=false;};
        ...
        if(data.room==='vault'){
          ...
          bankDoor(W/2,H-.34,-1,true);for(const c of [1.2,W-1.2])wallPost(c,H-.18,1.5,steelDark);roomSign('ХРАНИЛИЩЕ',W/2,.32,6.25,'#cde6ef',1.35);
      ...
      const decorateBusinessInterior=data=>{
        ...
        const floorZone=(q,material=std(q.color,.52,.08))=>{
          const overlay=material.clone();overlay.polygonOffset=true;overlay.polygonOffsetFactor=-2;overlay.polygonOffsetUnits=-2;const floor=add(new THREE.PlaneGeometry(q.w*S,q.d*S),overlay,q.c,.09,q.r);floor.rotation.x=-Math.PI/2;floor.receiveShadow=true;floor.castShadow=false;return floor;
        };
      ...
      const decorateServiceInterior=data=>{
        ...
        if(type==='hospital'){
          ...
          const cellC=W*.7;for(let cell=0;cell<2;cell++){const cc=cellC+(cell-.5)*3;for(const side of [-1,1])add(new THREE.CapsuleGeometry(.06*S,2.72,7,10),steel,cc+side*1.23,1.5,4.35);add(new THREE.BoxGeometry(2.55*S,.14,.16*S),steel,cc,2.96,4.35).castShadow=false;for(const side of [-1,1])add(new THREE.CylinderGeometry(.045*S,.045*S,3.1,7),steel,cc+side*1.25,1.55,5.2);add(new THREE.BoxGeometry(2.3*S,.28,.72*S),steelDark,cc,.32,6.15);}cabinet(1.1,3.2);cabinet(1.1,6.2);
      ...
      const decorateVenueInterior=data=>{
        ...
        if(type==='gym'){
          ...
          for(const c of [2.2,4.4,8.8,11.2]){add(new THREE.CylinderGeometry(.48*S,.36*S,2.5,18),c%2>1?red:blue,c,2.35,H-3);add(new THREE.CylinderGeometry(.04*S,.04*S,2.4,7),steel,c,5.05,H-3);}
          ...
          desk(W-2.25,H-2.05,3.3);monitor(W-2.25,H-2.47,'#73e0a2');for(let i=0;i<6;i++)add(new THREE.BoxGeometry(.85*S,2.3,.7*S),i%2?steel:dark,1.05+i*.9,1.15,1.15);
          add(new THREE.BoxGeometry(1.05*S,2.35,.85*S),cream,1.15,1.18,H-2);add(new THREE.CylinderGeometry(.28*S,.3*S,.55,14),glass,1.15,2.63,H-2).castShadow=false;for(let i=0;i<6;i++){const dumbbell=add(new THREE.CylinderGeometry(.07*S,.07*S,.72*S,8),steel,2.1+(i%3)*.78,.35+Math.floor(i/3)*.58,1.55);dumbbell.rotation.z=Math.PI/2;}
        }else if(type==='job'){
          ...
          const clock=add(new THREE.CylinderGeometry(.72*S,.72*S,.14,24),cream,W/2,3.45,.72);clock.rotation.x=Math.PI/2;for(const angle of [-.55,1.2]){const hand=add(new THREE.BoxGeometry(.035*S,.62,.035*S),dark,W/2,3.45,.81);hand.rotation.z=angle;}for(let i=0;i<7;i++){const paper=add(new THREE.PlaneGeometry(.62*S,.42),basic(i%3?'#e8dfc8':'#84b6c7'),2.4+(i%3)*3.6,1.16,3.05+(i%2)*.12);paper.rotation.x=-Math.PI/2;paper.rotation.z=(i-3)*.05;paper.castShadow=false;}add(new THREE.BoxGeometry(.9*S,1.05,.72*S),dark,W-1.15,.55,1.35);add(new THREE.PlaneGeometry(.55*S,.32),basic('#8dffae'),W-1.15,.82,1.72).castShadow=false;
        }else{
          ...
          for(const [c,r] of [[1.4,H-2],[4,H-2],[W-2.2,H-2]]){add(new THREE.BoxGeometry(2.05*S,.2,1.25*S),wood,c,.2,r);for(let crate=0;crate<4;crate++)add(new THREE.BoxGeometry(.68*S,.62,.58*S),crate%2?green:accent,c+(crate%2-.5)*.72,.52+Math.floor(crate/2)*.58,r+(crate%2-.5)*.48);}
          ...
          for(const r of [1.2,H-1.2])add(new THREE.CapsuleGeometry(.055*S,2.85,7,10),steel,W-.45,1.55,r);add(new THREE.BoxGeometry(.14*S,.15,(H-2.4)*S),steel,W-.45,2.98,H/2).castShadow=false;for(let i=0;i<6;i++){const tag=add(new THREE.PlaneGeometry(.42*S,.24),basic(i%2?'#e0b950':'#d7d3c5'),2.1+(i%3)*2.9,1.08,3.66);tag.rotation.x=-Math.PI/2;tag.castShadow=false;}
        }
        ...
        renderer.domElement.dataset.venueInterior=`${type}:${data.venueId||'default'}:authored-volumetric-v2`;renderer.domElement.dataset.venueNavigation='south-centre-clear-2.2-tiles';return true;
      };
      ...
      const decorateGrandCasinoInterior=data=>{
        ...
        const technicalGrid=interiorGroup.children.find(o=>o.type==='GridHelper');if(technicalGrid){interiorGroup.remove(technicalGrid);disposeTransientObjectTree(technicalGrid);}
        ...
        const casinoSignSprite=(text,color)=>{const canvas=document.createElement('canvas');canvas.width=768;canvas.height=180;const c=canvas.getContext('2d');c.fillStyle='rgba(8,5,12,.94)';c.fillRect(10,10,748,160);c.strokeStyle=color;c.lineWidth=10;c.strokeRect(15,15,738,150);c.font='900 72px system-ui';c.textAlign='center';c.textBaseline='middle';c.lineJoin='round';c.strokeStyle='#07070a';c.lineWidth=15;c.strokeText(text,384,92);c.fillStyle='#fff1c7';c.fillText(text,384,92);const texture=new THREE.CanvasTexture(canvas);texture.colorSpace=THREE.SRGBColorSpace;texture.anisotropy=Math.min(8,renderer.capabilities.getMaxAnisotropy());const sprite=new THREE.Sprite(new THREE.SpriteMaterial({map:texture,transparent:true,depthTest:true,depthWrite:false,toneMapped:false}));sprite.scale.set(Math.max(9.2,Math.min(15.5,text.length*.86)),3.1,1);sprite.renderOrder=24;sprite.layers.set(1);return sprite;};
        ...
        const floorZone=(c,r,w,d,mat,y=.08)=>{const overlay=mat.clone();overlay.polygonOffset=true;overlay.polygonOffsetFactor=-2;overlay.polygonOffsetUnits=-2;const floor=add(new THREE.PlaneGeometry(w*WORLD_SCALE,d*WORLD_SCALE),overlay,xOf(c),y,zOf(r));floor.rotation.x=-Math.PI/2;floor.receiveShadow=true;floor.castShadow=false;return floor;};
      ...
      const decorateApartmentInterior=data=>{
        ...
        // Twelve authored layouts: each has a different room emphasis and
        // furniture footprint, not a centimetre-sized random offset. Values are
        // fractions of the room, so the schemes remain valid at every apartment size.
        const layoutProfiles=[
          ['classic-lounge',-.02,.01,-.01,.01,0,0,0],['classic-dining',.055,-.015,.045,.055,-.025,.005,0],['classic-suite',-.055,.035,.02,-.045,.035,-.018,0],
          ['open-loft',.075,-.015,-.03,.075,-.055,.02,1],['loft-island',-.075,.02,.04,.035,.045,-.035,1],['loft-gallery',.02,.065,-.06,-.03,.06,.015,1],
          ['split-suite',-.045,-.055,.04,.04,-.035,.045,2],['split-office',.065,.045,-.055,-.05,.025,-.045,2],['split-family',-.075,-.02,.035,.075,.055,.035,2],
          ['courtyard',.045,-.065,-.065,.025,-.015,.055,3],['diagonal-studio',-.055,.065,.04,-.035,.035,-.055,3],['hotel-suite',.075,.025,-.035,-.07,-.06,-.01,3]
        ],profile=layoutProfiles[layoutVariant],[layoutName,livingDxF,livingDzF,diningDxF,diningDzF,bedDxF,bedDzF,topology]=profile,livingDx=livingDxF*W,livingDz=livingDzF*H,diningDx=diningDxF*W,diningDz=diningDzF*H,bedDx=bedDxF*W,bedDz=bedDzF*H;
        const add=(geo,mat,x,y,z,parent=interiorGroup)=>{const m=new THREE.Mesh(geo,mat);m.position.set(x,y,z);m.castShadow=m.receiveShadow=true;m.layers.set(1);parent.add(m);return m;},floorZone=(x,z,w,d,mat,y=.08)=>{const overlay=mat.clone();overlay.polygonOffset=true;overlay.polygonOffsetFactor=-2;overlay.polygonOffsetUnits=-2;const m=add(new THREE.PlaneGeometry(w,d),overlay,x,y,z);m.rotation.x=-Math.PI/2;m.receiveShadow=true;m.castShadow=false;return m;};
        ...
        for(const dx of [-2.65,2.65])add(new THREE.CapsuleGeometry(.17,3.86,7,12),trim,cx+dx,2.1,backZ-.45);add(new THREE.BoxGeometry(5.65,.32,.55),trim,cx,4.05,backZ-.45);floorZone(cx,backZ-1.15,5.2,2.25,new THREE.MeshStandardMaterial({color:0x39231f,roughness:1}),.14);
        // Room zoning: living room in front, kitchen left, bedroom and bath at the back.
        floorZone(cx-W*.18,cz+H*.12,W*.53,H*.42,new THREE.MeshStandardMaterial({color:rich?0x6b2430:0x49323a,roughness:1}),.08);
        floorZone(cx-W*.28,cz-H*.27,W*.4,H*.32,new THREE.MeshStandardMaterial({color:0x6a5037,roughness:.96}),.08);
        floorZone(cx+W*.3,cz-H*.28,W*.27,H*.3,tile,.08);
        ...
        for(const [x,z,w,d] of [[rugX,rugZ-rugD/2+.16,rugW-.18,.18],[rugX,rugZ+rugD/2-.16,rugW-.18,.18],[rugX-rugW/2+.16,rugZ,.18,rugD-.18],[rugX+rugW/2-.16,rugZ,.18,rugD-.18]])floorZone(x,z,w,d,rugEdge,.14);
        // Apartment partitions are visual zoning only. Render open rounded
        // portals so the player never appears to walk through a solid wall.
        const wallPart=(x,z,w,d,h=3.4)=>{const horizontal=w>=d,g=new THREE.Group();g.position.set(x,0,z);g.layers.set(1);interiorGroup.add(g);const span=Math.max(w,d),postDepth=Math.max(.32,Math.min(.6,horizontal?d:w)),postHeight=Math.max(1.7,h-.32);for(const side of [-1,1]){const px=horizontal?side*(span/2-postDepth*.5):0,pz=horizontal?0:side*(span/2-postDepth*.5);add(new THREE.CapsuleGeometry(postDepth*.42,postHeight,7,12),wall,px,postHeight/2+.12,pz,g);}const lintel=add(new THREE.BoxGeometry(horizontal?span:postDepth,.34,horizontal?postDepth:span),trim,0,h-.17,0,g);lintel.castShadow=false;return g;};
        // Four architectural families, three furniture arrangements each. All
        // partitions stop outside the 5.8-unit south-centre navigation lane.
        if(topology===0){wallPart(cx-W*.23,cz-H*.08,W*.3,.26);wallPart(cx+W*.3,cz-H*.08,W*.28,.26);wallPart(cx+W*.18,cz-H*.3,.26,H*.32);}
        else if(topology===1){wallPart(cx-W*.31,cz-H*.12,W*.24,.34,1.65);wallPart(cx+W*.35,cz-H*.2,.3,H*.38);add(new THREE.BoxGeometry(W*.16,.16,.72),trim,cx-W*.29,1.72,cz-H*.12).castShadow=false;}
        else if(topology===2){wallPart(cx-W*.24,cz-H*.28,.28,H*.22);wallPart(cx+W*.25,cz-H*.28,.28,H*.22);wallPart(cx-W*.34,cz+H*.04,W*.2,.28,2.25);wallPart(cx+W*.34,cz+H*.04,W*.2,.28,2.25);}
        else{wallPart(cx-W*.31,cz-H*.12,W*.18,.28,2.65);wallPart(cx+W*.31,cz-H*.12,W*.18,.28,2.65);wallPart(cx-W*.2,cz+H*.25,.28,H*.15,1.8);wallPart(cx+W*.2,cz+H*.25,.28,H*.15,1.8);}
        ...
        for(const [lampIndex,[x,z]] of [[cx-W*.22,cz+H*.14],[cx+W*.28,cz+H*.16],[cx-W*.22,cz-H*.28]].entries()){add(new THREE.CylinderGeometry(.045,.045,1.7,7),trim,x,6.5,z);const lamp=add(new THREE.SphereGeometry(.58,14,9),new THREE.MeshBasicMaterial({color:0xffd9a0,toneMapped:true}),x,5.55,z);lamp.scale.y=.45;if(lampIndex<2){const light=new THREE.PointLight(0xffc27a,rich?8:5.5,15,2);light.position.set(x,5.35,z);light.layers.set(1);interiorGroup.add(light);}}
        // Small lived-in evidence: paperwork, cable, dishes, shoes, tools and wear.
        const paperMat=new THREE.MeshBasicMaterial({color:0xe8dfca,toneMapped:true}),wearMat=new THREE.MeshBasicMaterial({color:0x4b4037,transparent:true,opacity:.28,depthWrite:false});
        for(let i=0;i<5;i++){const paper=add(new THREE.PlaneGeometry(1.05,.68),paperMat,livingCenterX-1.7+i*.78,1.115,livingCenterZ+(i%2?-.25:.18));paper.rotation.x=-Math.PI/2;paper.rotation.z=(i-2)*.08;paper.castShadow=false;}
        const tvCable=add(new THREE.BoxGeometry(.08,.07,2.4),black,tv.position.x+2.15,.22,tv.position.z+1.25);tvCable.rotation.y=.2;tvCable.castShadow=false;
        for(const dx of [-.62,.62]){const bowl=add(new THREE.TorusGeometry(.34,.08,7,16),cream,sink.position.x+dx,2.08,kitchenZ+.25);bowl.rotation.x=Math.PI/2;bowl.castShadow=false;}
        for(const [dx,dz,rot] of [[-1.1,0,.25],[1.0,.35,-.2]]){const shoe=add(new THREE.BoxGeometry(1.15,.28,.52),darkWood,cx+dx,.18,backZ-2.05+dz);shoe.rotation.y=rot;}
        for(let i=0;i<4;i++){const tool=add(new THREE.BoxGeometry(.12,.8,.1),i%2?steel:brass,cx-W*.39+i*.32,2.65,kitchenZ-.86);tool.rotation.z=(i-1.5)*.15;tool.castShadow=false;}
        for(const [x,z,s] of [[cx-W*.39,northZ+.405,1],[cx+W*.38,northZ+.41,.72]]){const scuff=add(new THREE.PlaneGeometry(2.5*s,1.1*s),wearMat,x,1.05,z);scuff.rotation.y=Math.PI;scuff.castShadow=false;}
        ...
        const theme=layoutVariant%6,themeX=cx+(layoutVariant%2?-.37:.37)*W,themeZ=cz+H*(layoutVariant%3===0?.22:.31);
        ...
        renderer.domElement.dataset.apartmentInterior=`furnished-v6:${layoutVariant}:${layoutName}:theme-${theme}:${repair}:${+lvl.safe||0}:${+lvl.weaponRack||0}:${+lvl.cameras||0}:${+lvl.garage||0}`;
      ...
      const interiorZoomByKey=new Map();
      ...
      renderer.domElement.addEventListener('wheel',e=>{const step=e.deltaY<0?.08:-.08;if(cameraZoomMode==='world'){e.preventDefault();worldZoom=THREE.MathUtils.clamp(worldZoom+step,.82,1.3);camera.zoom=worldZoom;camera.updateProjectionMatrix();renderer.domElement.dataset.worldZoom=worldZoom.toFixed(2);return;}if(cameraZoomMode==='interior'){e.preventDefault();camera.zoom=THREE.MathUtils.clamp(camera.zoom+step,.5,1.62);camera.updateProjectionMatrix();interiorZoomByKey.set(cameraZoomKey,camera.zoom);renderer.domElement.dataset.interiorZoom=camera.zoom.toFixed(2);}},{passive:false});
      ...
      const animate = t => {
        ...
        if(t-visualFxAt>=visualFxCadence){visualFxAt=t;visualFxVisible=0;factorySmokeVisible=0;
          ...
          for(let smokeIndex=0;smokeIndex<factorySmokePuffs.length;smokeIndex++){const smoke=factorySmokePuffs[smokeIndex],fx=smoke.userData.factorySmoke,near=(fx.baseX-player.position.x)**2+(fx.baseZ-player.position.z)**2<(lowVisualFps?82:135)**2,allowed=!interiorLightingActive&&near&&(!lowVisualFps||smokeIndex%2===0);smoke.visible=allowed;if(!allowed)continue;factorySmokeVisible++;const cycle=(t*.000045+fx.phase)%1,wobble=Math.sin(t*.00065+fx.phase*9);smoke.position.set(fx.baseX+wobble*(.55+cycle)*fx.drift,fx.baseY+cycle*8.5,fx.baseZ+Math.cos(t*.00048+fx.phase*7)*.42);smoke.scale.setScalar(.72+cycle*1.65);smoke.material.opacity=.04+Math.sin(cycle*Math.PI)*.24;}
          renderer.domElement.dataset.visualFxLod=lowVisualFps?'mobile-low':'distance-full';renderer.domElement.dataset.visualFxVisible=`${visualFxVisible}/${businessExteriorFx.length}`;renderer.domElement.dataset.factorySmokeVisible=`${factorySmokeVisible}/${factorySmokePuffs.length}`;
        }
        ...
        if(bridge){
          ...
          const freshInteriorData=state.interior?bridge.getInteriorState?.():null;if(freshInteriorData){lastStableInteriorData=freshInteriorData;lastStableInteriorAt=t;}else if(!state.interior){lastStableInteriorData=null;lastStableInteriorAt=0;}const interiorData=freshInteriorData||(state.interior&&lastStableInteriorData&&t-lastStableInteriorAt<1200?lastStableInteriorData:null),showInterior=!!(interiorData&&rendererConfig.interiorsEnabled!==false),showThree=!state.interior||showInterior;
          ...
          if(showInterior){
            ...
            const layoutSig=interiorData.layout?`${interiorData.layout.props?.map(p=>p.id).join(',')||''}:${interiorData.layout.safes?.map(s=>`${s.id}:${s.opened?1:0}`).join(',')||''}`:'',
              ...
              aptSig=interiorData.apartment?JSON.stringify({v:6,key:interiorData.apartment.key||'',owned:!!interiorData.apartment.owned,ownerUid:interiorData.apartment.ownerUid||'',ownerName:interiorData.apartment.ownerName||'',district:interiorData.apartment.district||'',levels:interiorData.apartment.levels||{}}):'',
            ...
            const bankRoom=interiorData.kind==='bank'?(interiorData.room||'lobby'):'',
              ...
              nextCameraKey=interiorData.bizId==='major_casino'?'casino':interiorData.kind==='bank'?`bank:${bankSize}:${bankRoom}`:`interior:${interiorData.type||'apartment'}:${interiorData.bizId||interiorData.apartment?.key||'room'}:${interiorData.width}x${interiorData.height}`;
            if(cameraZoomMode!=='interior'||cameraZoomKey!==nextCameraKey){
              cameraZoomMode='interior';cameraZoomKey=nextCameraKey;
              let fittedInteriorZoom;if(interiorData.bizId==='major_casino')fittedInteriorZoom=.68;
              else if(interiorData.kind==='bank'){
                const roomSpan=Math.max(+interiorData.width||1,+interiorData.height||1);
                fittedInteriorZoom=bankRoom==='vault'?THREE.MathUtils.clamp(14/roomSpan,.88,1.15):THREE.MathUtils.clamp(27/roomSpan,.58,.92);
              }else fittedInteriorZoom=THREE.MathUtils.clamp(18/Math.max(+interiorData.width||1,+interiorData.height||1),.58,1.12);camera.zoom=interiorZoomByKey.get(nextCameraKey)||fittedInteriorZoom;
              camera.updateProjectionMatrix();
              renderer.domElement.dataset.interiorZoom=camera.zoom.toFixed(2);
              renderer.domElement.dataset.interiorCamera=interiorData.bizId==='major_casino'?'architectural-fit-v2':interiorData.kind==='bank'?`bank-${bankRoom}-fit-v5-${bankSize}`:'room-size-fit-v2';
            }
          }
          else{
        ...
        if(playerArrestPhase){
          const fallen=playerArrestPhase==='downed'||playerArrestPhase==='cuffing',fall=playerArrestPhase==='downed'?(playerArrestProgress*playerArrestProgress*(3-2*playerArrestProgress)):1;
          gun.visible=false;holsteredPistol.visible=false;carriedMoneyBag.visible=false;localMuzzleFlash.visible=false;hidePlayerGripParts();
