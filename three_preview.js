...
#include <colorspace_fragment>
      ...
      const npcPrompt=document.createElement('div');npcPrompt.style.cssText='position:absolute;left:0;top:0;z-index:23;display:none;min-width:158px;padding:7px 11px 8px;border:1px solid rgba(223,183,91,.78);border-radius:11px;background:linear-gradient(145deg,rgba(10,15,22,.68),rgba(43,24,14,.58));color:#fff0bd;font:900 13px system-ui;text-align:center;text-shadow:0 1px 4px #000;pointer-events:none;box-shadow:0 7px 20px #0008,0 0 16px #d49b3d2b;transform:translate3d(-999px,-999px,0) translate(-50%,-100%);will-change:transform;animation:mfzNpcPromptIn .18s ease-out both';npcPrompt.innerHTML='<span style="display:inline-grid;place-items:center;min-width:22px;height:22px;margin-right:7px;border:1px solid rgba(255,244,185,.9);border-radius:7px;background:linear-gradient(rgba(255,226,124,.9),rgba(184,117,25,.88));color:#171008;font:1000 12px system-ui;animation:mfzBuildingKeyPulse 1.25s ease-in-out infinite">E</span><span>Действия</span>';stage.appendChild(npcPrompt);const npcPromptProjection=new THREE.Vector3();
      ...
      const updateNearbyNpcPromptPosition=near=>{if(!near||npcPrompt.style.display==='none')return;npcPromptProjection.set(nearbyNpcRing.position.x,6.15,nearbyNpcRing.position.z).project(camera);const anchorX=(npcPromptProjection.x*.5+.5)*lastW,anchorY=(-npcPromptProjection.y*.5+.5)*lastH,talking=!!String(near.src?.speech||'').trim(),speechSide=talking?(anchorX<lastW*.5?-150:150):0,w=Math.min(190,lastW-24),h=44,pad=12,x=Math.max(pad+w/2,Math.min(lastW-pad-w/2,anchorX+speechSide)),y=Math.max(pad+h,Math.min(lastH-pad,anchorY));npcPrompt.style.transform=`translate3d(${x.toFixed(2)}px,${y.toFixed(2)}px,0) translate(-50%,-100%)`;npcPrompt.dataset.anchor=talking?'speech-side-clearance-v307':'higher-translucent-npc-clearance-v306';};
