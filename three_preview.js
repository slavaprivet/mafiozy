...
if (rendererParams.get('render') === '3d' && rendererConfig.threeEnabled !== false) {
  (async () => {
    ...
    try {
      ...
      renderer.setPixelRatio(Math.min(2, Math.max(1.75, devicePixelRatio || 1)));
      ...
      const buildingDefs=worldSnapshot ? worldSnapshot.blocks.filter(b=>b.building).map(b=>{
        ...
        return [(q.c-originC)*WORLD_SCALE,(q.r-originR)*WORLD_SCALE,Math.max(7,q.w*WORLD_SCALE),Math.max(7,q.d*WORLD_SCALE),q.height,styleIndexes[b.styleId]??0,(nearPoi?.label||'').toString().slice(0,10).toUpperCase(),b.styleId||'downtown',{r:q.r,c:q.c,w:q.w,d:q.d}];
      }) : fallbackBuildingDefs;
      const occluders=[],buildingPickables=[],facadeMaterials=[],shopMaterials=[];
      ...
      for (let bi=0;bi<buildingDefs.length;bi++) {
        ...
        box(x,z,w+2,d+2,.65,curbMat);
        const facade=facades[style*3+(bi%3)];
        const wall = new THREE.MeshStandardMaterial({ map:facade, roughness:.74, metalness:.05, emissive:0xffb24c, emissiveMap:facade, emissiveIntensity:.075 });
        facadeMaterials.push(wall);
        const localRoof=roofMat.clone();
        const steppedTower=(districtStyle==='downtown'||districtStyle==='rich')&&h>24,lowerH=steppedTower?h*.64:h;
        const mainBuilding=box(x,z,w,d,lowerH,[wall,wall,localRoof,localRoof,wall,wall]);mainBuilding.userData.fadeMaterials=[wall,localRoof];mainBuilding.userData.building=buildingMeta;occluders.push(mainBuilding);buildingPickables.push(mainBuilding);outline(mainBuilding);
        if(steppedTower){const upperH=h-lowerH,upper=box(x,z,w*.72,d*.72,upperH,[wall,wall,localRoof,localRoof,wall,wall]);upper.position.y=lowerH+upperH/2;upper.userData.fadeMaterials=[wall,localRoof];upper.userData.building=buildingMeta;occluders.push(upper);buildingPickables.push(upper);outline(upper);const crownBand=box(x,z,w*.78,d*.78,.42,neonMats[bi%3]);crownBand.position.y=lowerH+.2;}
      ...
      lampSpots.forEach(([x,z],i) => {
        const post = new THREE.Mesh(new THREE.CylinderGeometry(.18, .24, 7, 8), lampMat); post.position.set(x, 3.5, z); scene.add(post);
        const bulb = new THREE.Mesh(new THREE.SphereGeometry(.48, 12, 8), bulbMat); bulb.position.set(x, 7, z); scene.add(bulb);
        const light = new THREE.PointLight(0xffb45e, 0, 25, 2); light.position.set(x, 6.7, z); light.castShadow = false; scene.add(light);streetLights.push({light,bulb,post});
      });
      ...
      Array.from({length:24},(_,i)=>[-25+i*3,-2,[0xe9a126,0x15202e,0x8b1825,0x376da3,0xd9d9d2][i%5]]).forEach((v, i) => {
        ...
        car.userData={paint,wheels,siren,cab,base,hood,standardParts,taxiSign,spoiler,pickupBed,roofRack,cabrioSeats,stripe,helicopter,rotor,lastPosition:new THREE.Vector3(v[0],0,v[1])};car.position.set(v[0], 0, v[1]);car.visible=false;scene.add(car);cars.push(car);
      });
      ...
      const help=document.createElement('div');help.textContent='3D: WASD / стрелки — бег · мышь — прицел · клик или пробел — огонь';
      ...
      addEventListener('keydown',e=>{keys.add(e.code);if(e.code==='Space'){e.preventDefault();e.stopImmediatePropagation();shoot(performance.now());}},true);
      ...
      let selectedBuildingHelper=null;const selectedBuildingRing=new THREE.Mesh(new THREE.RingGeometry(3.7,4.25,40),new THREE.MeshBasicMaterial({color:0xffd34f,transparent:true,opacity:.82,side:THREE.DoubleSide,depthTest:false}));selectedBuildingRing.rotation.x=-Math.PI/2;selectedBuildingRing.position.y=.2;selectedBuildingRing.renderOrder=48;selectedBuildingRing.visible=false;scene.add(selectedBuildingRing);
      const highlightBuilding=object=>{if(selectedBuildingHelper){scene.remove(selectedBuildingHelper);selectedBuildingHelper.geometry?.dispose?.();selectedBuildingHelper.material?.dispose?.();}selectedBuildingHelper=new THREE.BoxHelper(object,0xffd34f);selectedBuildingHelper.material.transparent=true;selectedBuildingHelper.material.opacity=.95;selectedBuildingHelper.material.depthTest=false;selectedBuildingHelper.renderOrder=49;scene.add(selectedBuildingHelper);selectedBuildingRing.position.set(object.position.x,.2,object.position.z);selectedBuildingRing.scale.set(Math.max(1,object.geometry?.parameters?.width/8||1),Math.max(1,object.geometry?.parameters?.depth/8||1),1);selectedBuildingRing.visible=true;};
      ...
      let lastW=size.W,lastH=size.H,lastT=performance.now(),walkPhase=0,lampAnchor='',fpsAt=performance.now(),fpsFrames=0,measuredFps=60,lastShadowAt=0,lastOcclusionAt=0,dynamicAt=0,dynamicState=null,cameraZoomMode='world',playerVisualSig='',fadedMaterials=[];
      ...
      const updateDayNight=t=>{
        ...
        skyLight.color.setRGB(.32+.42*daylight,.42+.36*daylight,.62+.27*daylight);skyLight.groundColor.set(daylight>.35?0x46392e:0x171a28);skyLight.intensity=.85+1.55*daylight;
        sun.color.set(sunset>.12?0xff8a4f:0xffd69a);sun.intensity=.3+3.15*daylight;sunOffsetVector.set(Math.cos(hour/24*Math.PI*2)*75,18+daylight*72,Math.sin(hour/24*Math.PI*2)*65);
        const night=1-daylight;renderer.toneMappingExposure=1.02+daylight*.24;
        facadeMaterials.forEach(m=>m.emissiveIntensity=.04+night*.42);shopMaterials.forEach(m=>m.opacity=.74+night*.26);
        streetLights.forEach(({light,bulb},i)=>{light.intensity=night*(i%2?17:25);bulb.material.color.setRGB(1,.55+daylight*.28,.2+daylight*.45);});
      ...
      const animate = t => {
        ...
        if(bridge&&t-dynamicAt>dynamicCadence){dynamicState=bridge.getDynamicEntities(38);dynamicAt=t;}const dynamic=dynamicState;
        if(dynamic){
        ...
        if(bridge){
          ...
          if(newAnchor!==lampAnchor){lampAnchor=newAnchor;const offsets=[[-4,-7],[4,-7],[-4,7],[4,7],[-14,-7],[14,-7],[-14,7],[14,7]];streetLights.forEach((lamp,i)=>{const rr=ar+offsets[i][0],cc=ac+offsets[i][1],visible=rr>1&&rr<Math.min(100,env.mapRows-1)&&cc>1&&cc<env.mapCols-1,x=(cc-originC)*WORLD_SCALE,z=(rr-originR)*WORLD_SCALE;lamp.post.visible=lamp.bulb.visible=lamp.light.visible=visible;lamp.post.position.set(x,3.5,z);lamp.bulb.position.set(x,7,z);lamp.light.position.set(x,6.7,z);});}
        ...
        if(moving){if(!bridge)walkPhase+=0;leftLeg.rotation.x=Math.sin(walkPhase)*.62;rightLeg.rotation.x=-Math.sin(walkPhase)*.62;leftArm.rotation.x=-Math.sin(walkPhase)*.38;rightArm.rotation.x=Math.sin(walkPhase)*.25;}else{leftLeg.rotation.x*=.72;rightLeg.rotation.x*=.72;leftArm.rotation.x*=.72;rightArm.rotation.x*=.72;}
        player.position.y=.08+Math.abs(Math.sin(walkPhase))*(moving?.08:0);
