...
#include <colorspace_fragment>
      ...
      const animate = t => {
        ...
        if(bridge){
          ...
          const visualSig=`${state.role}:${state.family||''}:${state.look?.skin??0}:${state.weapon||''}:${state.holsteredWeapon||''}:${state.driving?1:0}:${state.vehicleEntry?.token||''}:${state.arrestPhase||''}`;if(visualSig!==playerVisualSig){playerVisualSig=visualSig;const roleColor=state.role==='police'?0x285c91:state.role==='mafia'?(state.family==='moretti'?0xe6e0d3:0x25272d):0x7a3035;suitMat.color.set(roleColor);skinMat.color.set([0xf0c3a0,0xd8a07c,0xb87955,0x8b583e,0x633c2d][Math.max(0,Math.min(4,+state.look?.skin||0))]);configureWeaponVisual(state.weapon);gun.visible=gun.visible&&!state.driving&&!state.vehicleEntry&&!state.arrestPhase;holsteredPistol.visible=!!state.holsteredWeapon&&!state.weapon&&!state.driving&&!state.vehicleEntry&&!state.arrestPhase;renderer.domElement.dataset.holsteredWeapon=String(state.holsteredWeapon||'');}
