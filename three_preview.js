...
#include <colorspace_fragment>
      ...
      if(worldSnapshot?.landmarks){
        ...
        if(jail){
          ...
          // light slot per illuminated beacon so an alarm never changes the
          // global light count and recompiles the city during live combat.
      ...
      const nearbyVehicleOuterMaterial=new THREE.MeshBasicMaterial({color:0xffa735,transparent:true,opacity:.92,side:THREE.DoubleSide,depthTest:false}),nearbyVehicleInnerMaterial=new THREE.MeshBasicMaterial({color:0xff5448,transparent:true,opacity:.8,side:THREE.DoubleSide,depthTest:false}),nearbyVehicleRing=new THREE.Group();for(const ring of [new THREE.Mesh(new THREE.RingGeometry(2.05,2.34,48),nearbyVehicleOuterMaterial),new THREE.Mesh(new THREE.RingGeometry(1.55,1.72,40),nearbyVehicleInnerMaterial)]){ring.rotation.x=-Math.PI/2;ring.renderOrder=53;nearbyVehicleRing.add(ring);}nearbyVehicleRing.visible=false;scene.add(nearbyVehicleRing);renderer.domElement.dataset.vehicleProximityMarker='nearest-hijack-ring-v270';renderer.domElement.dataset.proximityScanCadence='140ms-shared-npc-vehicle-building-v270';renderer.domElement.dataset.promptLayoutReads='none-static-stage-metrics-v270';
      ...
      const updateCustodyAwareNpcLabel=(entry,src,x,y,z)=>{
        ...
        let layoutSig=src?.said?'said':speech?'speech':prisonStaff?'prison':'standard',layoutScale=src?.said?[17.2,4.3]:speech?[16.8,4.2]:prisonStaff?[11.8,3.05]:[10.6,2.65];
        if(custody&&escortOfficer){
          layoutSig='custody-escort';layoutScale=[7.6,1.9];
          entry.sprite.position.set(x+(name.includes('Приёмный')?-.48:.48),5.55+(name.includes('Приёмный')?0:.72),z);
        }else if(responseOfficer){
          layoutSig='response-officer';layoutScale=[8.6,2.15];
      ...
      const npcFramePoses=new Array(NPC_CAP),gangColorSignatures=new Array(NPC_CAP).fill(''),remoteColorSignatures=new Array(REMOTE_CAP).fill('');
      ...
      const npcLifeAnimationPose=(src,i,t)=>{
        ...
        const shotAge=Math.max(0,t-(+src._shotAt||+src.shotAt||0)),firing=!dead&&combatActor&&shotAge<360,recoil=firing?Math.sin(Math.min(1,shotAge/360)*Math.PI)*(.24+(String(src._shotWeapon||src.weapon||'').includes('rifle')?.12:0)):0;
        ...
        if(firing){armY=2.28;armZ=.43-recoil*.16;leftArmX=-.48;rightArmX=.5;leftArmPitch=-1.08-recoil*.2;rightArmPitch=-1.2-recoil*.28;leftArmRoll=-.2;rightArmRoll=.18;leftArmYaw=-.08;rightArmYaw=.08;}
      ...
      const nearestNpcInteraction=()=>{const state=bridge?.getPlayerState?.();if(!dynamicState?.npcs?.length||state?.driving||state?.dead||state?.vehicleEntry)return null;const pr=originR+player.position.z/WORLD_SCALE,pc=originC+player.position.x/WORLD_SCALE;let best=null,bestD=3.05;for(let i=0;i<dynamicState.npcs.length&&i<NPC_CAP;i++){const src=dynamicState.npcs[i];if(!src||src.dead)continue;const r=+src.r,c=+src.c;if(!Number.isFinite(r)||!Number.isFinite(c))continue;const d=Math.hypot(r-pr,c-pc),key=String(src.id||src.sourceId||'');if(key&&d<bestD){bestD=d;best={src,index:i,key,distance:d};}}return best;};
      const showNearbyNpc=(near,t)=>{if(!near){if(nearbyNpcRing.visible){npcPrompt.style.display='none';nearbyNpcRing.visible=false;renderer.domElement.dataset.nearbyNpc='none';}return;}const {src,key,distance}=near,x=(+src.c-originC)*WORLD_SCALE,z=(+src.r-originR)*WORLD_SCALE,role=String(src.role||'').toLowerCase(),police=!!src.police||role.includes('police')||role.includes('cop'),gang=!!src.gang||role.includes('gang')||role.includes('boss')||role.includes('guard'),accent=police?0x59b9ff:gang?0xff5d6c:0xd9aa45;nearbyNpcOuterMaterial.color.setHex(accent);nearbyNpcInnerMaterial.color.setHex(police?0x93d8ff:gang?0xffa0aa:0x78e3ad);if(!nearbyNpcRing.visible)nearbyNpcRing.position.set(x,.14,z);else{nearbyNpcRing.position.x=THREE.MathUtils.lerp(nearbyNpcRing.position.x,x,.34);nearbyNpcRing.position.z=THREE.MathUtils.lerp(nearbyNpcRing.position.z,z,.34);}const pulse=1+Math.sin(t*.0085)*.07;nearbyNpcRing.scale.setScalar(pulse);nearbyNpcRing.rotation.y=t*.0014;nearbyNpcOuterMaterial.opacity=.76+Math.sin(t*.01)*.14;nearbyNpcRing.visible=true;npcPrompt.style.display='block';buildingPrompt.style.display='none';renderer.domElement.dataset.nearbyNpc=`${key}:${distance.toFixed(2)}:${police?'police':gang?'gang':'civilian'}`;renderer.domElement.dataset.npcActionPrompt='actions-reserved-cadenced-v270';};
      const showNearbyVehicle=(near,t)=>{if(!near){nearbyVehicleRing.visible=false;renderer.domElement.dataset.nearbyVehicle='none';return;}const x=(+near.c-originC)*WORLD_SCALE,z=(+near.r-originR)*WORLD_SCALE,hijack=near.kind==='hijack',pulse=1+Math.sin(t*.0075)*.065;nearbyVehicleOuterMaterial.color.setHex(hijack?0xffa735:0xffd45a);nearbyVehicleInnerMaterial.color.setHex(hijack?0xff5448:0x72e6a4);nearbyVehicleOuterMaterial.opacity=.76+Math.sin(t*.009)*.14;if(!nearbyVehicleRing.visible)nearbyVehicleRing.position.set(x,.12,z);else{nearbyVehicleRing.position.x=THREE.MathUtils.lerp(nearbyVehicleRing.position.x,x,.3);nearbyVehicleRing.position.z=THREE.MathUtils.lerp(nearbyVehicleRing.position.z,z,.3);}nearbyVehicleRing.scale.setScalar(pulse);nearbyVehicleRing.rotation.y=-t*.00115;nearbyVehicleRing.visible=true;renderer.domElement.dataset.nearbyVehicle=`${near.id}:${near.kind}:${(+near.distance||0).toFixed(2)}`;};
      const updateNearbyNpcPromptPosition=near=>{if(!near||npcPrompt.style.display==='none')return;const x=(+near.src.c-originC)*WORLD_SCALE,z=(+near.src.r-originR)*WORLD_SCALE;npcPromptProjection.set(x,5.45,z).project(camera);const anchorX=(npcPromptProjection.x*.5+.5)*lastW,anchorY=(-npcPromptProjection.y*.5+.5)*lastH,w=Math.min(220,lastW-24),h=50,pad=12;npcPrompt.style.left=`${Math.max(pad+w/2,Math.min(lastW-pad-w/2,anchorX))}px`;npcPrompt.style.top=`${Math.max(pad+h,Math.min(lastH-pad,anchorY))}px`;};
      addEventListener('keydown',e=>{const typing=e.target?.matches?.('input,textarea,select,[contenteditable="true"]'),chatMode=document.documentElement.dataset.chatMode==='open';if(typing||chatMode){keys.clear();return;}keys.add(e.code);if(e.code==='KeyE'&&!e.repeat){const vehicle=nearbyVehicleState,building=nearbyActionState,npc=nearbyNpcState;eHoldStarted=performance.now();eHoldTriggered=false;eHoldVehicleId=String(vehicle?.id||'');eHoldHasBuilding=!!building;eHoldNpcKey=String(npc?.key||'');if(vehicle){vehicleHoldPrompt.firstElementChild.textContent=eHoldNpcKey?'Короткое E — действия NPC · удержание — машина':vehicle.kind==='hijack'?'🚗 Удерживайте E — угнать машину':'🚗 Удерживайте E — сесть в машину';vehicleHoldPrompt.style.display=eHoldHasBuilding?'none':'block';vehicleHoldFill.style.width='0';renderer.domElement.dataset.vehicleHold=`waiting:${eHoldVehicleId}`;renderer.domElement.dataset.interactionPriority=eHoldNpcKey?'tap-npc-hold-vehicle':eHoldHasBuilding?'tap-building-hold-vehicle':'hold-vehicle-only';eHoldTimer=setTimeout(()=>{if(keys.has('KeyE'))triggerVehicleEntry();},vehicleHoldMs);}else renderer.domElement.dataset.interactionPriority=eHoldNpcKey?'npc-actions':building?'building-only':'none';e.preventDefault();e.stopImmediatePropagation();}else if(e.code==='Escape'){bridge?.closeBuildingActions?.();}else if(e.code==='Space'){e.preventDefault();e.stopImmediatePropagation();if(currentWeaponId==='c4')bridge?.plantC4?.();else if(!isThrowablePrimary())shoot(performance.now());}},true);
      ...
      const updateBuildingPromptPosition=()=>{if(!nearbyBuildingEntry||buildingPrompt.style.display==='none')return;const entry=nearbyBuildingEntry,building=buildingObjectForEntry(entry),anchorR=Number.isFinite(+entry.r)?+entry.r:originR,anchorC=Number.isFinite(+entry.c)?+entry.c:originC,buildingHeight=building?.geometry?.parameters?.height||6;if(building)promptProjection.set(building.position.x,building.position.y+buildingHeight*.55+1.2,building.position.z);else promptProjection.set((anchorC-originC)*WORLD_SCALE,5.2,(anchorR-originR)*WORLD_SCALE);promptProjection.project(camera);const anchorX=(promptProjection.x*.5+.5)*lastW,anchorY=(-promptProjection.y*.5+.5)*lastH,w=Math.min(250,lastW-24),h=64,pad=12,x=Math.max(pad+w/2,Math.min(lastW-pad-w/2,anchorX)),y=Math.max(pad+h,Math.min(lastH-pad,anchorY-18));buildingPrompt.style.left=`${x}px`;buildingPrompt.style.top=`${y}px`;buildingPrompt.dataset.avoidsPlayer='fixed-building-anchor-no-layout-read-v270';};
      ...
      const isCombatSurfaceEvent=e=>e.target===renderer.domElement;
      ...
      renderer.domElement.dataset.combatInputProfile='canvas-only-pointerdown-rmb-lmb-v271';
      ...
      compileLoadedScene=()=>{
        ...
        try{
          ...
          if(prisonLightRoots.length){const prisonWarmStartedAt=performance.now(),previousShadowEnabled=renderer.shadowMap.enabled,prisonLightVisibility=prisonLightRoots.map(root=>root.visible),warmCarVisible=warmupCar?.visible,warmCarPosition=warmupCar?.position.clone(),shadowWarmX=sun.target.position.x,shadowWarmZ=sun.target.position.z;let mappedShadowSource=null;scene.traverse(object=>{if(mappedShadowSource||!object.isMesh||!object.castShadow)return;const material=Array.isArray(object.material)?object.material.find(candidate=>candidate?.map):object.material;if(material?.map)mappedShadowSource={geometry:object.geometry,material};});const mappedShadowProxy=mappedShadowSource?new THREE.Mesh(mappedShadowSource.geometry,mappedShadowSource.material):null;if(mappedShadowProxy){mappedShadowProxy.name='startup-mapped-shadow-proxy';mappedShadowProxy.position.set(shadowWarmX,1,shadowWarmZ);mappedShadowProxy.frustumCulled=false;mappedShadowProxy.castShadow=true;mappedShadowProxy.receiveShadow=false;mappedShadowProxy.layers.mask=camera.layers.mask;scene.add(mappedShadowProxy);}if(warmupCar){warmupCar.visible=true;warmupCar.position.set(shadowWarmX+4,0,shadowWarmZ+4);}try{renderer.shadowMap.enabled=realTimeShadows;renderer.compile(scene,camera);if(realTimeShadows)renderer.shadowMap.needsUpdate=true;renderer.render(scene,camera);prisonLightRoots.forEach(root=>{root.visible=false;});renderer.compile(scene,camera);if(realTimeShadows)renderer.shadowMap.needsUpdate=true;renderer.render(scene,camera);}finally{renderer.shadowMap.enabled=previousShadowEnabled;if(mappedShadowProxy)scene.remove(mappedShadowProxy);if(warmupCar){warmupCar.visible=warmCarVisible;warmupCar.position.copy(warmCarPosition);}prisonLightRoots.forEach((root,index)=>{root.visible=prisonLightVisibility[index];});}renderer.domElement.dataset.prisonCombatWarmup=`ready:${prisonLightRoots.length}:dual-car-${mappedShadowProxy?'mapped':'plain'}:${(performance.now()-prisonWarmStartedAt).toFixed(1)}ms`;}
      ...
      const animate = t => {
        ...
        if(dynamic){
          ...
          renderer.domElement.dataset.fireWaterProfile='continuous-volumetric-hose-splash-v273';
          ...
          let crawlingNpcCount=0,limpingNpcCount=0,reactingNpcCount=0,bleedingNpcCount=0,panickingNpcCount=0,coweringNpcCount=0,surrenderingNpcCount=0,helpingNpcCount=0,socialNpcCount=0,alertedNpcCount=0,ambientNpcCount=0,firingPoliceCount=0;
          citizenPool.forEach((npc,i)=>{
            ...
            armed?setPart(npcParts.hat,i,rootMatrix,0,3.77-pose.crouch,0):hidePart(npcParts.hat,i);const lifeGestureDisarms=pose.surrendering||pose.cowering||pose.helping||pose.panicking||pose.talking||!!pose.ambientKind;armed&&!lifeGestureDisarms&&!(severMask&2)?setPart(npcParts.gun,i,rootMatrix,pose.firing?.5:.72,pose.firing?2.28-pose.recoil*.08:2-pose.crouch*.35,pose.firing?.78:.52,pose.firing?-1.16-pose.recoil*.3:pose.hit*.18):hidePart(npcParts.gun,i);
          ...
          for(let i=Math.min(NPC_CAP,dynamic.npcs.length);i<NPC_CAP;i++)npcAppearanceSlotSignatures[i]='';if(npcColorBuffersDirty)for(const mesh of Object.values(npcParts))if(mesh.instanceColor)mesh.instanceColor.needsUpdate=true;
        ...
        if(bridge){
          ...
          const prisonAlarmActive=!!state.prisonAlarm?.active,prisonAlarmBatchRevision=Math.max(0,+state.prisonAlarm?.batchRevision||0);
        ...
        camera.position.set(player.position.x+54,62+playerFloorElevation,player.position.z+54);camera.lookAt(player.position.x,1.6+playerFloorElevation,player.position.z);if(t-promptPositionAt>80){updateBuildingPromptPosition();updateNearbyNpcPromptPosition(nearbyNpcState);promptPositionAt=t;}
