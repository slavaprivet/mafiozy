...
let refreshCustomGangHqs=()=>{},refreshNpcBusinessControls=()=>{},refreshNpcBusinessOperations=()=>{},casinoExteriorAnimation=()=>{};
      ...
      const npcLifeAnimationPose=(src,i,t)=>{
        ...
        const roll=(crawling?step*.016*gait:(limping?.105+step*.04*gait:step*.026*gait))+hit*hitSide*(.16+(1-Math.abs(hitForward))*.13)+(dodging?Math.sin(t*.012+i)*.18:0);
        const crouch=cowering?.86:helping?.42:gettingUp?.58:takingCover?.38:boarding?.24:0,statePitch=cowering?.28:helping?.38:gettingUp?.52:takingCover?.12:panicking?-.055:alerted?-.025:0,pitch=(crawling?1.22*crawlBlend:(walking?-(.025+pace*.035)*gait:0))+statePitch+hit*(.07+hitForward*.2);
      ...
      const animate = t => {
        ...
        if(dynamic){
          ...
          let crawlingNpcCount=0,limpingNpcCount=0,reactingNpcCount=0,bleedingNpcCount=0,panickingNpcCount=0,coweringNpcCount=0,surrenderingNpcCount=0,helpingNpcCount=0,socialNpcCount=0,alertedNpcCount=0,ambientNpcCount=0,reloadingNpcCount=0,coveringNpcCount=0,vehicleNpcCount=0;
          citizenPool.forEach((npc,i)=>{
            ...
            armed?setPart(npcParts.hat,i,rootMatrix,0,3.77-pose.crouch,0):hidePart(npcParts.hat,i);const lifeGestureDisarms=pose.surrendering||pose.cowering||pose.helping||pose.panicking||pose.talking||!!pose.ambientKind,combatPose=pose.aiming||pose.shooting,reloadPose=pose.reloading;armed&&!lifeGestureDisarms&&!(severMask&2)?setPart(npcParts.gun,i,rootMatrix,reloadPose?.42:combatPose?.58:.72,reloadPose?2.3:combatPose?2.34:2-pose.crouch*.35,reloadPose?.42:combatPose?.78:.52,reloadPose?-.82:pose.shooting?-.34:pose.aiming?-.2:pose.hit*.18):hidePart(npcParts.gun,i);
          ...
          const said3d=dynamic.npcs.find(n=>n.said);renderer.domElement.dataset.said3d=said3d?`visible:${said3d.hired?'hired':'available'}:important:${said3d.important?1:0}`:'out-of-range';
        ...
        if(bridge){
          ...
          const visualSig=`${state.role}:${state.family||''}:${state.customGangColor||''}:${state.look?.skin??0}:${state.weapon||''}:${state.holsteredWeapon||''}:${state.armor||''}:${state.armorDurability||0}:${state.armorMax||0}:${state.driving?1:0}:${state.dead?1:0}:${state.vehicleEntry?.token||''}:${state.arrestPhase||''}`;if(visualSig!==playerVisualSig){playerVisualSig=visualSig;let customRoleColor=0;try{if(state.customGangColor)customRoleColor=new THREE.Color(state.customGangColor).getHex();}catch(_){}const roleColor=state.role==='police'?0x285c91:state.role==='mafia'?(state.family==='moretti'?0xe6e0d3:0x25272d):state.role==='custom_gang'&&customRoleColor?customRoleColor:0x7a3035;suitMat.color.set(roleColor);skinMat.color.set([0xf0c3a0,0xd8a07c,0xb87955,0x8b583e,0x633c2d][Math.max(0,Math.min(4,+state.look?.skin||0))]);configureWeaponVisual(state.weapon);configureArmorVisual(state.armor,state.armorDurability,state.armorMax);gun.visible=gun.visible&&!state.driving&&!state.dead&&!state.vehicleEntry&&!state.arrestPhase;holsteredPistol.visible=!!state.holsteredWeapon&&!state.weapon&&!state.driving&&!state.dead&&!state.vehicleEntry&&!state.arrestPhase;renderer.domElement.dataset.holsteredWeapon=String(state.holsteredWeapon||'');}
          ...
          lastPlayerHp=+state.hp||0;carriedMoneyBag.rotation.z=-.18+(moving?Math.sin(walkPhase)*.07:0);playerHpBar.scale.x=3.9*Math.max(.02,playerHpPct);playerHpBar.material.color.set(playerHpPct>.55?0x55e778:playerHpPct>.25?0xffc94d:0xff4d55);
