...
if (rendererParams.get('render') === '3d' && rendererConfig.threeEnabled !== false) {
  (async () => {
    ...
    try {
      ...
      trafficSignalDefs.forEach(([x,z],i)=>{setTrafficPart(trafficPole,i,x,3.55,z);setTrafficPart(trafficBase,i,x,.16,z);setTrafficPart(trafficCollar,i,x,6.62,z);for(let direction=0;direction<2;direction++){const headIndex=i*2+direction,ry=direction*Math.PI/2,hx=x+(direction?0:1.82),hz=z+(direction?1.82:0);setTrafficPart(trafficArm,headIndex,x+(direction?0:.86),6.62,z+(direction?.86:0),ry);setTrafficPart(trafficBackboard,headIndex,hx,6.62,hz,ry);setTrafficPart(trafficCase,headIndex,hx,6.62,hz+(direction?0:.14),ry);for(let lamp=0;lamp<3;lamp++){const lampIndex=headIndex*3+lamp,ly=7.48-lamp*.86,faceX=hx+(direction?.42:0),faceZ=hz+(direction?0:.42),visorX=hx+(direction?.57:0),visorZ=hz+(direction?0:.57);setTrafficPart(trafficBezel,lampIndex,faceX,ly,faceZ,ry);setTrafficPart(trafficVisor,lampIndex,visorX,ly+.32,visorZ,ry);}const faceX=hx+(direction?.43:0),faceZ=hz+(direction?0:.43);setTrafficPart(trafficRed,headIndex,faceX,7.48,faceZ,ry);setTrafficPart(trafficYellow,headIndex,faceX,6.62,faceZ,ry);setTrafficPart(trafficGreen,headIndex,faceX,5.76,faceZ,ry);}});
      const trafficMeshes=[trafficPole,trafficBase,trafficCollar,trafficArm,trafficBackboard,trafficCase,trafficBezel,trafficVisor,trafficRed,trafficYellow,trafficGreen];for(const mesh of trafficMeshes){mesh.instanceMatrix.needsUpdate=true;mesh.frustumCulled=false;mesh.castShadow=!([trafficRed,trafficYellow,trafficGreen].includes(mesh));mesh.receiveShadow=mesh.castShadow;scene.add(mesh);}for(const mesh of [trafficRed,trafficYellow,trafficGreen]){mesh.renderOrder=24;for(let i=0;i<trafficHeadCount;i++)mesh.setColorAt(i,trafficColor.setHex(0x1a1a1a));mesh.instanceColor.needsUpdate=true;}
      let trafficSignalPhase='';const setTrafficSignals=phase=>{const ew=phase?.ew||'g',ns=phase?.ns||'r',sig=`${ew}:${ns}`;if(sig===trafficSignalPhase)return;trafficSignalPhase=sig;const bright={r:0xff3045,y:0xffcc2e,g:0x43f075},dim={r:0x45131a,y:0x493c13,g:0x123f27};for(let i=0;i<trafficCount;i++)for(let direction=0;direction<2;direction++){const index=i*2+direction,state=direction?ns:ew;trafficRed.setColorAt(index,trafficColor.setHex(state==='r'?bright.r:dim.r));trafficYellow.setColorAt(index,trafficColor.setHex(state==='y'?bright.y:dim.y));trafficGreen.setColorAt(index,trafficColor.setHex(state==='g'?bright.g:dim.g));}for(const mesh of [trafficRed,trafficYellow,trafficGreen])mesh.instanceColor.needsUpdate=true;renderer.domElement.dataset.trafficPhase=sig;};setTrafficSignals(envSnapshot?.traffic);
      ...
      const updateDayNight=t=>{
        ...
        skyColor.copy(nightSky).lerp(daySky,daylight).lerp(sunsetSky,sunset*.58);scene.background.copy(skyColor);scene.fog.color.copy(skyColor);skyUniforms.uTop.value.copy(nightSky).lerp(daySky,daylight);skyUniforms.uHorizon.value.set(daylight>.35?0xbfd8df:0x16263d);skyUniforms.uNight.value=1-daylight;skyUniforms.uSunset.value=sunset;
        ...
        const night=1-daylight;environmentNight=night;renderer.toneMappingExposure=1.15+daylight*.15;scene.environmentIntensity=.42+daylight*.38;lampGlowMat.opacity=.01+night*.72;starMaterial.opacity=Math.max(0,night-.18)*.92;postMaterial.uniforms.uBloom.value=.12+night*.12;postMaterial.uniforms.uWarmth.value=.08+sunset*.72;postMaterial.uniforms.uNight.value=night;
      ...
      const animate = t => {
        ...
        const dt=Math.min(.05,(t-lastT)/1000);lastT=t;const tt=t*.00035;updateDayNight(t);skyDome.position.copy(player.position);starField.position.copy(player.position);starField.rotation.y=t*.000004;windLeafMaterials.forEach(m=>{if(m.userData.shader)m.userData.shader.uniforms.mfzWindTime.value=t*.00115;});
