...
#include <colorspace_fragment>
      ...
      const npcEmpireWeaponColors={leila_mercy:0xd73b58,rustam_wrench:0xe58b32,marco_road:0xf0c85b,vera_verdict:0xb58cff,arsen_forge:0xff8b35,damir_oath:0x79d6a8,marat_wall:0x65a8dc,zara_dividend:0xffd46f,niko_whisper:0x9ad06f,alisa_signal:0x62c7ef,boris_tow:0xffe06d,inga_deed:0xffd0df,timur_express:0xffcf4d,emil_champion:0xf5e7d0,roman_plate:0xbcd0dc,sofia_headline:0xfff4a1,viktor_night:0xb28ad9,yana_frequency:0x6de0d7,musa_caravan:0xe6c56a},npcEmpireWeaponVariants=Object.fromEntries(Object.keys(npcEmpireWeaponColors).map((id,index)=>[id,index]));
      ...
      const updateNpcLabel=(entry,src,x,y,z)=>{
        if(src.empireBoss){const name=String(src.name||'БОСС').toUpperCase(),gang=String(src.empireGang||'КРИМИНАЛЬНАЯ СЕМЬЯ').toUpperCase(),sub=`ГЛАВА БАНДЫ · ${gang}`,sig=`empire-boss-v1:${name}:${sub}:${src.speech||''}`;if(sig!==entry.sig){entry.sig=sig;const c=entry.canvas.getContext('2d');c.clearRect(0,0,768,192);c.fillStyle='rgba(18,8,8,.98)';c.fillRect(24,8,720,176);c.strokeStyle='#efb94f';c.lineWidth=11;c.strokeRect(30,14,708,164);fitOutlinedLabelText(c,`♛ ${name} · БОСС`,384,57,660,50,'#ffd76b');fitOutlinedLabelText(c,sub,384,135,660,39,'#ffffff');entry.sprite.scale.set(18.4,4.6,1);entry.texture.needsUpdate=true;}entry.sprite.visible=true;entry.sprite.position.set(x,y+.35,z);return;}
        if(src.uniqueNpc||src.said){const name=String(src.name||'СПЕЦИАЛИСТ').toUpperCase(),title=String(src.specialistTitle||(src.said?'Правая рука':'Специалист')),salary=Math.max(0,+src.salary||0),sub=src.said?(src.hired?'Правая рука · нанят':'Правая рука · можно нанять'):`${title} · $${salary.toLocaleString('ru')}/день · услуги скоро`,sig=`unique-v1:${name}:${sub}:${src.speech||''}`;if(sig!==entry.sig){entry.sig=sig;const c=entry.canvas.getContext('2d');c.clearRect(0,0,768,192);c.fillStyle='rgba(12,10,7,.98)';c.fillRect(24,8,720,176);c.strokeStyle='#e5bd55';c.lineWidth=11;c.strokeRect(30,14,708,164);fitOutlinedLabelText(c,`${name} · УНИКАЛЬНЫЙ NPC`,384,57,660,50,'#ffd76b');fitOutlinedLabelText(c,sub,384,135,660,42,'#ffffff');entry.sprite.scale.set(18.4,4.6,1);entry.texture.needsUpdate=true;}entry.sprite.visible=true;entry.sprite.position.set(x,y+.35,z);return;}
      ...
      const updateCustodyAwareNpcLabel=(entry,src,x,y,z)=>{
        ...
        let layoutSig=(src?.uniqueNpc||src?.said)?'unique':speech?'speech':prisonStaff?'prison':'standard',layoutScale=(src?.uniqueNpc||src?.said)?[18.4,4.6]:speech?[16.8,4.2]:prisonStaff?[11.8,3.05]:[10.6,2.65];
      ...
      const npcFramePoses=new Array(NPC_CAP),gangColorSignatures=new Array(NPC_CAP).fill(''),remoteColorSignatures=new Array(REMOTE_CAP).fill(''),npcEmpireWeaponColorSignatures=new Array(NPC_CAP).fill('');
      ...
      const animate = t => {
        ...
        if(dynamic){
          ...
          let crawlingNpcCount=0,limpingNpcCount=0,reactingNpcCount=0,bleedingNpcCount=0,panickingNpcCount=0,coweringNpcCount=0,surrenderingNpcCount=0,helpingNpcCount=0,socialNpcCount=0,alertedNpcCount=0,ambientNpcCount=0,firingPoliceCount=0,firingGuardCount=0,walkingArmNpcCount=0,npcEmpireWeaponColorsDirty=false;
          citizenPool.forEach((npc,i)=>{
            ...
            armed?setPart(npcParts.hat,i,rootMatrix,0,3.77-pose.crouch,0):hidePart(npcParts.hat,i);const lifeGestureDisarms=pose.phoneCalling||pose.surrendering||pose.cowering||pose.helping||pose.panicking||(!empireBoss&&pose.talking)||!!pose.ambientKind,showGun=armed&&!lifeGestureDisarms&&!(severMask&2),gunX=pose.firing?.5:.72,gunY=pose.firing?2.28-pose.recoil*.08:empireBoss&&pose.talking?1.68:2-pose.crouch*.35,gunZ=pose.firing?.78:empireBoss&&pose.talking?.28:.52,gunPitch=pose.firing?-1.16-pose.recoil*.3:empireBoss&&pose.talking?.42:pose.hit*.18;showGun?setPart(npcParts.gun,i,rootMatrix,gunX,gunY,gunZ,gunPitch):hidePart(npcParts.gun,i);const uniqueWeaponId=String(src.uniqueWeaponId||''),uniqueWeaponColor=npcEmpireWeaponColors[uniqueWeaponId];if(showGun&&uniqueWeaponColor){const variant=npcEmpireWeaponVariants[uniqueWeaponId]??0,weaponFamily=String(src.weapon||''),longWeapon=weaponFamily.includes('rifle')||weaponFamily.includes('sniper')||weaponFamily.includes('shotgun')||weaponFamily.includes('tommy'),railScale=instanceScale.set((longWeapon?1.55:1.02)+(variant%3)*.2,.62+(variant%4)*.13,.78+(variant%5)*.1);setPart(npcParts.uniqueGunRail,i,rootMatrix,gunX,gunY+.16,gunZ+.08,gunPitch,railScale);setPart(npcParts.uniqueGunStock,i,rootMatrix,gunX,gunY-.03,gunZ-(longWeapon?.52:.31),gunPitch,instanceScale.set(.72+(variant%4)*.13,.72+(variant%3)*.16,longWeapon?1.15:.7));setPart(npcParts.uniqueGunMuzzle,i,rootMatrix,gunX,gunY+.03,gunZ+(longWeapon?1.12:.72),gunPitch+Math.PI/2,instanceScale.set(.72+(variant%2)*.25,1+(variant%5)*.15,.72+(variant%2)*.25));setPart(npcParts.uniqueGunCharm,i,rootMatrix,gunX+(variant%2?-.18:.18),gunY-.19,gunZ+.2,gunPitch+t*.0015,instanceScale.setScalar(.82+(variant%4)*.14));const colorSig=`${uniqueWeaponId}:${uniqueWeaponColor}`;if(npcEmpireWeaponColorSignatures[i]!==colorSig){npcEmpireWeaponColorSignatures[i]=colorSig;for(const mesh of [npcParts.uniqueGunRail,npcParts.uniqueGunCharm,npcParts.uniqueGunStock,npcParts.uniqueGunMuzzle])mesh.setColorAt(i,instanceColor.setHex(uniqueWeaponColor));npcEmpireWeaponColorsDirty=true;}}else{npcEmpireWeaponColorSignatures[i]='';hidePart(npcParts.uniqueGunRail,i);hidePart(npcParts.uniqueGunCharm,i);hidePart(npcParts.uniqueGunStock,i);hidePart(npcParts.uniqueGunMuzzle,i);}
          ...
          for(let i=0;i<Math.min(NPC_CAP,dynamic.npcs.length);i++){
            ...
            const pick=(arr,salt=0)=>arr[(((seed>>>salt)^Math.imul(seed,31+salt))>>>0)%arr.length],role=String(src.role||'').toLowerCase(),said=!!src.said||role==='said',uniqueNpc=!!src.uniqueNpc||said||role==='unique_npc',empireBoss=!!src.empireBoss,bossStyle=Math.max(0,+src.bossStyle||0)%19,police=src.visualRole==='police'||!!src.police||role.includes('police')||role.includes('cop'),gang=empireBoss||!!src.gang||role.includes('gang')||role.includes('boss')||role.includes('district_')||role.includes('occupier'),businessOwner=src.visualRole==='owner',ownerStyle=Math.abs(Number.isFinite(+src.ownerStyle)?+src.ownerStyle:+src.look?.ownerStyle||0)%10,prisonStaff=!!src.prisonStaff||String(src.behavior||'')==='prison_patrol'||role.startsWith('prison_'),securityGuard=!prisonStaff&&(src.visualRole==='guard'||role.includes('guard')),guardMajor=securityGuard&&String(src.guardClass||'')==='major',guardStyle=String(src.guardStyle||'generic'),securityUniform=SECURITY_UNIFORMS[guardMajor?guardStyle:'business']||SECURITY_UNIFORMS.generic,responseTier=Math.max(0,Math.min(3,+src.responseTier||0)),tactical=responseTier>0,prisonGuard=prisonStaff&&role.includes('guard'),staffVariant=Math.abs(+src.staffVariant||0)%3,prisonGear=String(src.prisonGear||''),armed=empireBoss||gang||securityGuard||role.includes('guard')||police,look=src.look||{};
            ...
            const faction=String(src.faction||src.family||'').toLowerCase(),bodyColor=said?0xf4f2e9:prisonStaff?(prisonGuard?[0x35483c,0x3e4937,0x2b4039][staffVariant]:[0x245f9b,0x31527d,0x1f6777][staffVariant]):police?tacticalBody:securityGuard?securityUniform[0]:empireBoss?(authoredSuit||pick([0x6b2730,0x263448,0x30234f],3)):gang?(faction.includes('yellow')?0xf1e4c4:faction.includes('purple')?0x7447aa:pick([0xb84a5d,0x5f6f85,0xb48a49],3)):authoredSuit||pick(npcShirts,1),legColor=said?0x17191c:prisonStaff?(prisonGuard?[0x1e2922,0x242a20,0x18251f][staffVariant]:[0x172536,0x202c3c,0x16313a][staffVariant]):police?tacticalLeg:securityGuard?securityUniform[1]:empireBoss?(authoredTrousers||0x12151a):gang?(faction.includes('yellow')?0x5e523c:faction.includes('purple')?0x342a48:0x3b3740):uniqueNpc?(authoredTrousers||pick(npcTrousers,5)):businessOwner?(authoredTrousers||0x111318):pick(npcTrousers,5),skinColor=Number.isFinite(+look.skin)?npcSkins[Math.abs(+look.skin|0)%npcSkins.length]:pick(npcSkins,7),hairColor=uniqueNpc?(authoredHair||pick(npcHairs,9)):businessOwner?(authoredHair||pick(npcHairs,9)):pick(npcHairs,9),shoeColor=uniqueNpc||police||prisonStaff||securityGuard||businessOwner?0x090b0e:pick(npcShoes,11),hatColor=police?tacticalHat:securityGuard?securityUniform[1]:empireBoss?(ownerAccent||bodyColor):gang?bodyColor:uniqueNpc?(ownerAccent||pick(npcHats,13)):pick(npcHats,13),hairStyle=businessOwner?ownerStyle:Number.isFinite(+look.hair)?Math.abs(+look.hair|0)%6:(seed>>>16)%6,accessory=(seed>>>20)%12,capOn=businessOwner?false:uniqueNpc?(Number.isFinite(+look.hat)&&+look.hat>0):prisonStaff?prisonGear==='cap':tactical?false:police||armed||(Number.isFinite(+look.hat)&&+look.hat>0)||hairStyle===4||hairStyle===5,helmetOn=!uniqueNpc&&(prisonStaff||tactical)&&prisonGear.includes('helmet'),wearHat=capOn||helmetOn,glassesOn=empireBoss?(bossStyle%5===0||bossStyle%7===3):!said&&!armed&&!businessOwner&&(accessory===0||accessory===4||role.includes('staff')),neckOn=businessOwner||uniqueNpc||armed||accessory%3===1,bagOn=!said&&!armed&&!businessOwner&&accessory%4===2,moustacheOn=empireBoss?+look.gender!==1&&bossStyle%3===1:!said&&!armed&&+look.gender!==1&&(businessOwner?ownerStyle===2||ownerStyle===5:accessory===3||accessory===9),variantSig=`${bodyColor}:${legColor}:${skinColor}:${hairColor}:${shoeColor}:${hatColor}:${hairStyle}:${prisonGear||'city'}:${responseTier}:${securityGuard?guardStyle:'none'}:${businessOwner?ownerStyle:'none'}:${empireBoss?bossStyle:'none'}:${glassesOn?1:0}:${neckOn?1:0}:${bagOn?1:0}:${moustacheOn?1:0}`,slotSig=`${key}:${variantSig}`;
          ...
          for(let i=Math.min(NPC_CAP,dynamic.npcs.length);i<NPC_CAP;i++){npcAppearanceSlotSignatures[i]='';npcEmpireWeaponColorSignatures[i]='';}if(npcColorBuffersDirty)for(const mesh of Object.values(npcParts))if(mesh.instanceColor)mesh.instanceColor.needsUpdate=true;
