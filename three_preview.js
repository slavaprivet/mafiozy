...
#include <colorspace_fragment>
      ...
      const playerName=new THREE.Sprite(new THREE.SpriteMaterial({map:playerNameTexture,transparent:true,depthTest:false}));playerName.position.y=6.75;playerName.scale.set(8.5,1.98,1);playerName.renderOrder=45;const playerHpBg=new THREE.Sprite(new THREE.SpriteMaterial({color:0x33151a,depthTest:false}));playerHpBg.position.y=5.92;playerHpBg.scale.set(4.15,.34,1);playerHpBg.renderOrder=44;const playerHpBar=new THREE.Sprite(new THREE.SpriteMaterial({color:0x55e778,depthTest:false}));playerHpBar.position.y=5.94;playerHpBar.scale.set(3.9,.21,1);playerHpBar.renderOrder=45;player.add(playerName,playerHpBg,playerHpBar);
      ...
      const animate = t => {
        ...
        if(dynamic){
          ...
          citizenPool.forEach((npc,i)=>{
            ...
            const pct=Math.max(.03,pose.hpPct),identityFocused=nearbyNpcRing.visible&&nearbyNpcState?.index===i&&nearbyNpcState.distance<=3.05,damaged=pose.hpPct<.999||pose.hit>0||!!src.bleeding;npc.hpGroup.visible=identityFocused||damaged;npc.hpGroup.position.set(x,pose.crawling?.24:pose.bob+.45-pose.crouch*.68,z);npc.hpBar.scale.x=1.7*pct;npc.hpBar.material.color.set(pct>.55?0x58e67c:pct>.25?0xffc94d:0xff5252);
          });
