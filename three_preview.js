...
#include <colorspace_fragment>
      ...
      const eyeScale=new THREE.Vector3(1,.78,.42),pupilScale=new THREE.Vector3(1,.78,.3),npcBodyScale=new THREE.Vector3(1,1,1),gangAuraScale=new THREE.Vector3(1.3,1.3,1.3),npcLimbOffset=new THREE.Vector3(),npcLimbQuat=new THREE.Quaternion();
      ...
      const npcParts={
        body:makeInstances(new THREE.CapsuleGeometry(.49,.82,6,14),new THREE.MeshStandardMaterial({color:0xffffff,vertexColors:true,roughness:.7,metalness:0,emissive:0x15181d,emissiveIntensity:.42}),NPC_CAP),
        ...
        shirtFront:makeInstances(new THREE.BoxGeometry(.46,.94,.045),new THREE.MeshStandardMaterial({color:0xffffff,vertexColors:true,roughness:.72}),NPC_CAP,false),
        ...
        leftArm:makeInstances(new THREE.CapsuleGeometry(.15,1.05,5,10),new THREE.MeshStandardMaterial({color:0xffffff,vertexColors:true,roughness:.76,emissive:0x15181d,emissiveIntensity:.42}),NPC_CAP),
        rightArm:makeInstances(new THREE.CapsuleGeometry(.15,1.05,5,10),new THREE.MeshStandardMaterial({color:0xffffff,vertexColors:true,roughness:.76,emissive:0x15181d,emissiveIntensity:.42}),NPC_CAP),
      ...
      const hideNpcVisual=i=>{for(const [key,mesh] of Object.entries(npcParts)){if(['eyeWhite','pupil','shoe','hand','uniqueGunLimb','brow','ear','ownerLapel','policePatch'].includes(key)){hidePart(mesh,i*2);hidePart(mesh,i*2+1);}else if(['uniqueGunSpike','hairCurls'].includes(key)){for(let k=0;k<4;k++)hidePart(mesh,i*4+k);}else hidePart(mesh,i);}};
      ...
      const animate = t => {
        ...
        if(dynamic){
          ...
          citizenPool.forEach((npc,i)=>{
            ...
            const bodyIndex=Math.abs(+look.body||0)%NPC_BODY_PROFILES.length,bodyProfile=NPC_BODY_PROFILES[bodyIndex],female=+look.gender===1,shoulderScale=female?.82:1,hipScale=female?1.08:1,depthScale=female?.94:1,shoulderX=.78*bodyProfile.shoulder*shoulderScale,armScale=bodyProfile.arm*(female?.9:1),legScale=bodyProfile.leg*(female?.92:1),hipWidth=bodyProfile.hip*hipScale;
            setNpcRoot(pose,i,x,z);npcBodyScale.set(bodyProfile.bodyX*shoulderScale,pose.cowering?.92:pose.walking?1:1+pose.idle*.018,bodyProfile.bodyZ*depthScale);
            ...
            setPart(npcParts.shirtFront,i,rootMatrix,0,2.18-pose.crouch-medicCrouch,.48*bodyProfile.bodyZ*depthScale,0,instanceScale.set(bodyProfile.bodyX*shoulderScale*(female?.78:1),1,1),pose.torsoTwist,pose.shoulderSway);
            ...
            const faceStyle=Math.abs(+look.face||0)%10,faceScale=instanceScale.set((female?.94:1)*(faceStyle===7?1.08:faceStyle===5?.94:1),faceStyle===1?1.08:faceStyle===6?.94:1,female?.95:1);
            ...
            for(const side of [-1,1]){const pitch=side<0?leftPitch:rightPitch,yaw=side<0?leftYaw:rightYaw,roll=side<0?leftRoll:rightRoll,ax=side<0?leftArmX:rightArmX,handIndex=i*2+(side>0?1:0);npcLimbQuat.setFromEuler(instanceEuler.set(pitch,yaw,roll));npcLimbOffset.set(0,-.68,0).applyQuaternion(npcLimbQuat);setPart(npcParts.hand,handIndex,rootMatrix,ax+npcLimbOffset.x,medicArmY+npcLimbOffset.y,medicArmZ+npcLimbOffset.z,0,instanceScale.set(armScale,armScale,armScale));}
            ...
            if(severMask&1){hidePart(npcParts.leftArm,i);hidePart(npcParts.hand,i*2);}
            if(severMask&2){hidePart(npcParts.rightArm,i);hidePart(npcParts.hand,i*2+1);}
          ...
          for(let i=0;i<Math.min(NPC_CAP,dynamic.npcs.length);i++){
            ...
            for(const part of ['shirtFront','nose','mouth','femaleLashes','femaleLips','femaleHips'])hidePart(npcParts[part],i);for(const part of ['hand','brow','ear']){hidePart(npcParts[part],i*2);hidePart(npcParts[part],i*2+1);}hidePart(npcParts.hair,i);hidePart(npcParts.hairBun,i);hidePart(npcParts.hairMohawk,i);for(let curl=0;curl<4;curl++)hidePart(npcParts.hairCurls,i*4+curl);hidePart(npcParts.ownerHairSide,i);hidePart(npcParts.ownerHairBack,i);hidePart(npcParts.ownerHairQuiff,i);hidePart(npcParts.ownerLapel,i*2);hidePart(npcParts.ownerLapel,i*2+1);hidePart(npcParts.ownerPocket,i);hidePart(npcParts.hat,i);hidePart(npcParts.hatBrim,i);hidePart(npcParts.glasses,i);hidePart(npcParts.eyePatch,i);hidePart(npcParts.chain,i);hidePart(npcParts.neckAccent,i);hidePart(npcParts.bag,i);hidePart(npcParts.moustache,i);hidePart(npcParts.prisonHelmet,i);hidePart(npcParts.prisonHelmetBand,i);hidePart(npcParts.prisonVisor,i);hidePart(npcParts.prisonVest,i);hidePart(npcParts.securityBadge,i);hidePart(npcParts.policeBelt,i);hidePart(npcParts.policeRadio,i);hidePart(npcParts.policePatch,i*2);hidePart(npcParts.policePatch,i*2+1);hidePart(npcParts.riotShield,i);hidePart(npcParts.shieldViewport,i);
