...
for(const item of plan.instances.filter(i=>i.role==='bank_shell'||(['pawnshop','gun_shop','bookmaker'].includes(i.assetId)&&/001$/.test(i.id)))){
 ...
 assert.equal(c.prompt(d.lower,[d]).end,'lower');assert(c.begin(d.lower,d),'up route clearance');for(let i=0;i<10000&&c.locked;i++)c.update(1/30);assert(!c.locked,'up completes');assert.equal(c.prompt(d.upper,[d]).end,'upper');assert(c.begin(d.upper,d),'down route clearance');for(let i=0;i<10000&&c.locked;i++)c.update(1/30);assert(!c.locked,'down completes');record.status='PASS';
...
assert.equal(results.length,6,'six selected roofs');assert(results.every(r=>r.status==='PASS'),'roof integration failures');
