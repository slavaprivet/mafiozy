...
Статус: corrective runtime 3eeeb995 production CPU READY. В LIVE6 Координатор20 проверил Kingswell с тремя пассажирами: боковой ракурс показывает оружие и кисть rear_left снаружи окна, оружие дальнего переднего пассажира также видно; обычный задний ракурс показывает пистолет левого бойца. Это ограниченная визуальная приёмка конкретной машины, после пользовательского FAIL позы834. Все автомобильные семейства проверены только CPU. Настоящий ответный огонь пока не принят: attempts/accepted0, изучается нападение штатного патрульного. Полная производительность общей сцены не принята. Это заменяет прежний вариант X/eliminate по выбранной цели.

## Текущий контракт

После LIVE6 root исправил четыре повреждённые русские строки диагностики;
текущий helper SHA256 `25b7ab80c747890d86f493f6725f2ab14d2657ca070eb8718273848c6ed6d5ad`.
Математика позы совпадает с проверенной в LIVE6 версией `3eeeb995`.
...
## История reload5: CPU PASS, визуальный FAIL

LIVE reload5 после прямого отзыва пользователя выявил незакрытый дефект: головы вышли, а receiver и большая часть оружия остались в салоне. Проверки головы, дула и положения относительно груди оказались недостаточны. Эта версия не принята визуально. Предыдущий read-only ready audit (голова примерно 40 см внутри) описывал ещё более ранний helper. Runtime SHA256 `834C0A76EB75394204950D5D8F0DA3EACFFD99A89F9F607D9B7587B562947E6E` заменяет загруженную у координатора WIP-версию `4c7fd996`.
...
## Коррекция после отзыва «только головы торчат»

Узкий следующий patch затрагивает только mercenary_vehicle_fire.mjs, production lean regression и этот handoff. Actor, общая походка/посадка, Walk, source authority и git index не менялись.

Readiness теперь направляет оружие наружу и немного вперед (локальное направление ±0.8,0,0.6). План задаёт receiver за собственной оконной плоскостью с учетом настоящей длины текущего оружия; раньше он задавал лишь дуло и оставлял рукоять внутри. Опорные точки кистей остаются реальными authored grips. Общие reach/head/hull/ray проверки не ослаблены. Пассажирский двухкостный IK сохраняет длины рук, но ведёт локти выше нижней рамки. Вынос рукояти заканчивается после наклона корпуса; длинное цевьё проходит позади передней рамки окна. Время входа .18 с сохранено. На выключении в течение .22 с сначала убираются кисти с оружием над sill, затем корпус. Для возвращаемых контактов используются actual palm sockets, а не wrist positions.

Новый обязательный regression отдельно проверяет receiver >2.5 см за оконной плоскостью, trigger grip >1.2 см, muzzle >16 см, supporting grip длинного оружия >2.5 см; более 55% реальных видимых вершин оружия должны находиться снаружи. Полная геометрия оружия, пересекающая плоскость окна, должна проходить через проём. Прежние проверки тела/рук/нижнего скелета и source negatives сохранены. Подстановка сохранённого runtime834 в усиленный тест воспроизводимо падает уже на compact/male/front_right/nagan/ready: receiver внутри. Это ожидаемый regression FAIL прежней версии, а не ослабление требований.

Actual-production `test_mercenary_vehicle_lean23.mjs`: 4632 PASS /156 cases /0 skin-or-frame failures. Минимумы readiness: receiver 2.662 см, trigger grip 2.039 см, barrel/muzzle 34.826 см за плоскостью окна; минимум 93.155% вершин оружия снаружи. Контакт стрелковой/опорной кисти не хуже 7.55e-8/1.09e-7 м. Для отдельного настоящего Kingswell TT/Uzi/AK receiver выступает на 10.276–18 см. Голова/плечо остаются снаружи, pelvis/ноги/root неизменны. Проверка включает ready и actual firing, все13families, оба пола, обе машины, все3места и переходы.

Actual-production `test_mercenary_vehicle_fire23.mjs`: 2039 PASS, 72 geometry /22 other-weapon geometry /43 negative cases. Moving source bridge/поворачивающая машина и движущийся атакующий, friendly/hero/стена/чужая сторона/устаревшая поза/сменаоружия/online, fairness/cooldown/HP authority проходят без ослаблений. .11 с возврата уже уменьшает наклон, .22 с возвращает исходную посадку.

Runtime SHA256 `3EEEB995D66B4D52E375B89BCE1389AD6AC8F69CB544B6CF80A855604FB5C4FD`. Последний isolated CPU (12 warmup +40): seated idle p50/p95 0.346/0.392 мс; active pose 0.902/1.012 мс; physical admission 15.116/15.666 мс. Нет нового цикла подбора позы: сохранились прежние shared mount/ray проверки и source budget один probe/tick. Это CPU одного NPC, не производительность общего города. Все CPU процессы завершены перед передачей.

В checkpoint добавить только новый diff helper +lean regression +этот handoff. Временные `.git/.../lean6-*`, `test_mercenary_vehicle_lean6_audit.mjs`, старые audit/candidate файлы исключить. Новых runtime imports нет. LIVE новой версии, видимость оружия снаружи и accepted incoming fire остаются задачей координатора в единственной игре.
