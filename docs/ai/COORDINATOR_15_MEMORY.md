...
## Лимиты: отложить дальнейшую рассылку — 10 сентября

Пользователь ввёл обязательное правило: каждый автор сразу оптимизирует свои доработки и проверяет их влияние на общую игровую производительность. Правило записано в AGENTS.md. Затем уточнил: «передашь потом. пока зафиксируй просто в памяти уже лимитов не хвтит». Новые запуски, проверки и дальнейшую рассылку остановить; продолжить при следующем поручении.

До уточнения уже отправлены 8 сообщений: Художник 16, Автомобили — продолжение архитектора, Добавить прыжок и перелезание, оптимизация (слабая), Здания рисовка внутри, художник 15, Архитектор — магазины и отель, Архитектор — штабы и жилые дома. Повторно не рассылать. Собственный subagent decor_perf остановлен во время опционального расширения vehicle_render_batches на двери; возможные незавершённые изменения проверить при возобновлении, не включать опцию без тестов. Действующий interior batching ранее проверен.

GitHub сохранение завершено: main 3442bf0d84dd66e787005aa2bbb456caf298b9ca, удалённый refs/heads/main совпал. Изменения после снимка (включая новое правило, расширение profiler matrix/shadow и чужие активные правки) пока локальные. Profiler test после расширения PASS, LIVE ещё не запускался. Пользовательская вкладка не закрывалась.

## Согласованный план следующего запуска — 10 сентября 2026

Пользователь одобрил план и явно отложил начало: «с этого и начнем в следующий раз». Сейчас не запускать новых агентов, рассылку, замеры или реализацию.

Предложенный пользователем состав следующей команды: 5 основных агентов, включая координатора, каждому по 3 субагента. При возобновлении проверить доступную параллельность: в текущем дереве лимит 4 слота всего; не обещать 20 одновременно работающих агентов. Использовать существующие задачи владельцев, согласовать независимые области и запускать в пределах реальных лимитов. GPU-прогоны по очереди, исходники общие. Число агентов само по себе не гарантирует ускорения.

Согласованный порядок:
1. Убрать лаги города, машин и NPC. Проверить ходьбу, вращение камеры, поездки и взрывы. Это ближайший приоритет.
2. Довести уже подключённых NPC: скачки, столкновения, маршруты, посадка в машины, переходы анимаций и смерть всех типов.
3. Доделать бой и HP: пули/взрывы, смерть/восстановление, полиция, оставшиеся серверные случаи гранат/RPG.
4. Проверить весь город: логичные знаки/светофоры/разметка/повороты/въезды, парковки у домов и больницы, фактическое соблюдение правил автомобилями.
5. Довести взаимодействия: двери, интерьеры, лестницы, крыши, прыжки, укрытия, выход из воды, отсутствие застреваний и конфликтов управления.
6. Добавлять игровой смысл: небольшие задания, покупки, поездки, столкновения с бандами и погони как законченные сценарии существующего города.

Каждый автор сразу оптимизирует свои доработки: функциональная проверка и сопоставимая стоимость до/после, устранение собственных регрессий, сохранение в GitHub, затем пользовательский тест. Следующий запуск начинать с чтения этого плана и фактического состояния незавершённого perf-подэтапа выше, не пересоздавать уже подключённых NPC и не повторять завершённые исправления миникарты/знаков.

## Согласовано: осмысленное поведение NPC из world

Пользователь согласился развивать существующих NPC из world, сохраняя роли, ID, состояние и игровые связи. Это дополнение к этапу 2 после устранения лагов, а не поручение начать сейчас: прежнее «в следующий раз» сохраняется.

Первый сквозной сценарий: житель идёт в магазин → замечает стрельбу с учётом видимости/слышимости → ищет доступное укрытие → сообщает полиции → после исчезновения угрозы постепенно возвращается к делам. Проверить поведение, связь с source world и стоимость до/после, затем распространять на роли.

Направления: цели и распорядок; восприятие с расстоянием/препятствиями; разные решения гражданских/вооружённых/полиции; короткая память угрозы и последней видимой позиции без всеведения; передача сведений и совместные действия; ожидание/обход/смена цели при недоступном маршруте. Для базовых решений достаточно целей и состояний без нейросети на каждого NPC. Ближние обновляются подробнее, дальние реже, поиск маршрутов распределяется по кадрам; сохранять авторитетную симуляцию и действующие механики world. Не добавлять массовую нагрузку без сопоставимого замера.


## Укрытия / C / стрельба из окна — уточнения пользователя, 10.09

В задаче укрытий исправляются последние скриншоты: оружие у головы, слишком далёкий присед у кузова, стрельба/выглядывание. Пользователь явно подтвердил также стрельбу из окна.

- Подключено окно собственного места для всех четырёх мест парка /walk: Q выбор, ПКМ вынос, ЛКМ огонь; движение/удержание E сохранены. Нет патрона при закрытом стволе. LIVE водитель TT12→11, задний правый TT11→10. Не смешивать с исходным myDrivingCarId: в 2D source отдельный запрет ещё сохранён.
- Настоящая причина неработающего угла машины: детальный collisionHull получил скосы шин, cover принимал их за концы кузова. Новый vehicle_cover_edges.mjs строит только для укрытия четыре непрерывных грани, driving hull не меняет. LIVE red_sedan aimed shotgun5→4, blind4→3.
- Обычный C восстановлен с умеренным наклоном; физическая высота1.69/глаза1.40. CPU6480кадров всехоружий/обоихGLB PASS, maxskin1.680883. Укрытие имеет отдельное пригибание.
- hero_weapon_clearance.mjs: безопасное размещение оружия относительно головы/шеи в пределах обеих рук. Финальная коррекция blindduck учитывает оба GLB. Пока завершается строгая проверка finalpose; последние LIVE были до финальной поправки duck.
- Новые windowhooks оптимизированы неактивным fastpath, сохранено появившееся параллельно DOM-кеширование setVehicleWindowPrompt/updateVehicleWindowPrompt (авторство не установлено, координатор не добавлял). test_window_walk_input адаптирован и PASS.
- По правилу оптимизации: общийGPUзамер согласован запросом, затем откоординатора получена просьба экономитьлимиты/фиксироватьпамять без новыхпрогонов. Производительностьобщейсцены НЕпроверена. ТекущийCPUbench выявил contactquery≈14мс на10Гц, идёт одно точное broadphase-исправление в своём helper; итоговыецифры будутв HERO_COVER_HANDOFF.md.

Общий handoff: docs/city-rebuild/HERO_COVER_HANDOFF.md. Scoped владельцы: root walk_preview hooks/coverhost, агентcover_geometry finalpose+tests, агентcover_car_contact contact/clearancehelper. Сохранять параллельные воду/health/NPC/vehicle edits. Новыхветок/коммитов/рестартов этойзадачей не делалось.
