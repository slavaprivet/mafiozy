...
## Two-handed stock follow-up — final
User screenshot exposed chest penetration missed by old back-plane check. weaponPose now uses a slight .25rad upright chest turn with neck counterturn, retaining aim gaze and accepted prone/dive. Long prop forward offsets at1.9m: .11m common, .13m sawn_off, .14m RPG; original lateral mount preserved. Both wrist reach constraints apply to all long props.
Shared test_weapon_torso_clearance.mjs runs360 cases:9two-handed weapons x2sex x2posture x5pitch x2recoil. Actual deformed jacket triangles against stock/rear surface samples AND weapon triangle edges: zero intersections, maximum palm error <1e-6m. It uses the same vendor runtime as existing tests; female reference asset is from accepted D demo (env override supported). Root ran this on actual shared final source. Existing full walker and14-weapon posture/reload/jump suites pass. Live final shotgun standing inspected; earlier candidate crouch also inspected. Only weaponPose changed, coordinator jump edits preserved.
