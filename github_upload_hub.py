"""
Загружает hub.html на GitHub + ВЕРИФИЦИРУЕТ.
...
print()
print("=" * 64)
print(" hub.html  ->  GitHub  (с верификацией)")
print("=" * 64)
...
markers = [
    (b"async function actTake",    "actTake (HTTP fetch)"),
    (b"async function actAbandon", "actAbandon (HTTP fetch)"),
    (b"'/job/' + coopMyUidStr()",  "HTTP-маршрут /job/.../take"),
]
...
if found:
    ...
    print(f"                Похоже, у тебя hub.html старой версии.")
...
for attempt in range(1, 7):
    try:
        ...
        if remote_sha == local_sha:
            print(f"      Попытка {attempt}: SHA совпали ({remote_sha[:16]}...)   ✓")
            verified = True
            break
