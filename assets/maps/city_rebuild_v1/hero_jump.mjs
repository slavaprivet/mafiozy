...
export const JUMP=Object.freeze({flight:.8,recovery:.45,height:1.05,diveHeight:.8,speed:10.5,radius:.36});
...
export function tryDiveJump(state,direction,now){
 ...
 return {...state,mode:'dive',diveStartElapsed:state.elapsed,diveBlend:0,dx:direction.x/n,dz:direction.z/n,directional:true};
}
export function stepJump(state,dt,allowed){
 ...
 const blend=Math.max(0,Math.min(1,(elapsed-(state.diveStartElapsed??elapsed))/DIVE_BLEND_SECONDS));
 const diveBlend=state.mode==='dive'?blend*blend*(3-2*blend):0;
 const height=JUMP.height+(JUMP.diveHeight-JUMP.height)*diveBlend;
 return {...state,x:result.x,z:result.z,y:4*height*p*(1-p),diveBlend,elapsed,progress:elapsed/(JUMP.flight+JUMP.recovery),done:elapsed>=JUMP.flight+JUMP.recovery,blocked:state.blocked||(state.directional&&travel>0&&Math.hypot(result.x-state.x,result.z-state.z)<travel-.00001)};
}
