...
function geometryComponents(T,geometry,matrix){
 const p=geometry?.attributes?.position,index=geometry?.index;if(!p)return[];const parents=new Map(),keys=Array.from({length:p.count},(_,i)=>`${Math.round(p.getX(i)*10000)},${Math.round(p.getY(i)*10000)},${Math.round(p.getZ(i)*10000)}`);
...
function withoutFaces(T,source,removed){
 ...
 const count=source.index?.count??source.attributes.position.count,output=Object.fromEntries(attributes.map(([name])=>[name,[]])),groups=[];let emitted=0;
 const materialAt=offset=>source.groups.find(group=>offset>=group.start&&offset<group.start+group.count)?.materialIndex??0;
...
function subtractBoxesFromGeometry(T,source,toReference,boxes){
 ...
 const output=Object.fromEntries(attributes.map(([name])=>[name,[]])),groups=[],count=source.index?.count??position.count;let emitted=0;
 const planes=box=>[[0,box.min.x,1],[0,box.max.x,-1],[1,box.min.y,1],[1,box.max.y,-1],[2,box.min.z,1],[2,box.max.z,-1]],materialAt=offset=>source.groups.find(group=>offset>=group.start&&offset<group.start+group.count)?.materialIndex??0;
...
export function applyResidentialWindows({THREE:T,visual,instance}){
 ...
 const bundle=acquireBundle(T),{dark,reveal,brass,curtain,glass}=bundle.materials,batches=new Map();
 ...
 for(const [material,transforms]of batches){const mesh=new T.InstancedMesh(bundle.geometry,material,transforms.length);mesh.name=material===glass?'ResidentialWindow_DeepGlass':material===dark?'ResidentialWindow_DeepRecess':'ResidentialWindow_Frame_And_Interior';mesh.userData.breakableGlass=material===glass;for(let i=0;i<transforms.length;i++)mesh.setMatrixAt(i,transforms[i]);mesh.receiveShadow=true;mesh.castShadow=material!==glass;group.add(mesh)}visual.add(group);
