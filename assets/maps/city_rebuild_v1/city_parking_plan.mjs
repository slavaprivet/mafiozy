...
const M=4.1,EPS=1e-7,MAX_WALK_DISTANCE=125;
...
function computeCityParkingWalks({lots,raw,targets,topology,keepouts,railPlan,colliders,m,extraBodies=[]}){
 ...
 for(const lot of lots){lot.servedBuildingIds=[];const start=point(lot.origin,lot.dx,lot.dz,0,lot.layout==='parallel'?2.2:5),goals=targets.map(b=>({building:b,anchor:(()=>{const a=b.entry?.anchorRC,p=b.entry?.roadProbeRC;if(!a||!p)return null;const x=(p.c-a.c)*m,z=(p.r-a.r)*m,d=Math.hypot(x,z);return {x:a.c*m+x/d*.7,z:a.r*m+z/d*.7}})()})).filter(g=>g.anchor&&Math.hypot(start.x-g.anchor.x,start.z-g.anchor.z)<=MAX_WALK_DISTANCE);if(!goals.length)continue;
 ...
 return {coverage,walkingRoutes:chosenRoutes,walkingAlternatives:walkingRoutes.map(({points,...route})=>route)};
}
...
export function replanCityParkingWalks(plan,{topology,instances=[],keepouts=[],railPlan=null,extraBodies=[]}={}){
 ...
 const {coverage,walkingRoutes,walkingAlternatives}=computeCityParkingWalks({lots,raw,targets,topology,keepouts,railPlan,colliders:plan.colliders,m:plan.metresPerCell,extraBodies});
 return {...plan,lots,coverage,walkingRoutes,walkingAlternatives,stats:{...plan.stats,servedBuildings:coverage.filter(c=>c.status==='served').length,uncoveredBuildings:coverage.filter(c=>c.status==='uncovered').length}};
}
