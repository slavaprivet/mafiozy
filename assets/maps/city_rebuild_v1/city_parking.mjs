...
export function createCityParking({THREE,plan,chunkSize=128}={}){
 ...
 for(const [key,parts]of buckets){const mesh=new THREE.InstancedMesh(geometry[key.split(':').at(-1)],material,parts.length);parts.forEach((p,i)=>{mesh.setMatrixAt(i,p.matrix);mesh.setColorAt(i,tint.setHex(p.color))});mesh.name='ParkingBatch:'+key;mesh.instanceMatrix.needsUpdate=true;mesh.instanceColor.needsUpdate=true;mesh.computeBoundingBox();mesh.computeBoundingSphere();mesh.castShadow=false;mesh.receiveShadow=true;object.add(mesh);meshes.push(mesh)}
 ...
 const stats={...plan.stats,totalDraws:meshes.length,triangles:meshes.reduce((sum,m)=>sum+m.geometry.index.count/3*m.count,0),get visibleDraws(){return meshes.filter(m=>m.visible).length}};
