...
export function stepTyres(state,dt,distance){
 ...
 return state.map(t=>{if(!t.punctured||t.detached)return t;const pressure=Math.max(0,t.pressure-.7*dt),travel=t.distance+(pressure<.2?Math.abs(distance):0),detached=travel>=60;if(pressure===t.pressure&&travel===t.distance&&detached===t.detached)return t;return{...t,pressure,distance:travel,detached}});
}
...
export function createTyreDamage(T,car,{groundHeight=()=>0,onDetach=()=>{}}={}){
 let state=createTyreState(),disposed=false,sparkCursor=0,idleUpdates=0;const emitted=new Set(),wheelRest=new Map();
 ...
 for(const wheel of car.wheels){const radius=wheel.nominalRollingRadius||wheel.rollingRadius||.4,rimRadius=wheel.nominalRimRadius||radius*.60;wheelRest.set(wheel.id,{y:wheel.pivot.position.y,scale:wheel.tire.scale.clone(),visible:wheel.tire.visible,radius,rimRadius});const tube=radius*.225,mesh=new T.Mesh(new T.TorusGeometry(radius-tube,tube,8,32),rubber.clone());mesh.visible=false;mesh.raycast=()=>{};object.add(mesh);loose.set(wheel.id,{mesh,radius,age:0,origin:new T.Vector3(),velocity:new T.Vector3()})}
 ...
 const particles=Array.from({length:64},()=>({life:0,active:false,p:new T.Vector3(),v:new T.Vector3()})),activeParticleIndices=[],matrix=new T.Matrix4(),q=new T.Quaternion(),scale=new T.Vector3(),zero=new T.Matrix4().makeScale(0,0,0),inverse=new T.Matrix4(),sparkForward=new T.Vector3(0,0,1),sparkDirection=new T.Vector3(),worldContact=new T.Vector3(),loosePosition=new T.Vector3();for(let i=0;i<64;i++)sparks.setMatrixAt(i,zero);
 ...
 function update(carState,dt){
  // Pristine wheels have no visual simulation: their authored transforms and
  // all spark slots already represent this state. Do not walk the car graph
  // and rewrite 64 zero matrices every rendered frame before the first hit.
  if(disposed||state.every(t=>!t.punctured&&!t.detached))return;const before=state;state=stepTyres(state,dt,carState?.distance||0);
  const changed=state.some((t,i)=>t!==before[i]),stationary=Math.abs(carState?.distance||0)<=.002,looseActive=[...loose.values()].some(item=>item.mesh.visible);
  ...
  for(let slot=activeParticleIndices.length-1;slot>=0;slot--){const i=activeParticleIndices[slot],particle=particles[i];particle.life=Math.max(0,particle.life-dt);if(!particle.life){particle.active=false;activeParticleIndices[slot]=activeParticleIndices[activeParticleIndices.length-1];activeParticleIndices.pop();sparks.setMatrixAt(i,zero);continue}particle.v.y-=9.8*dt;particle.p.addScaledVector(particle.v,dt);particle.p.y=Math.max(groundHeight(particle.p.x,particle.p.z)+.012,particle.p.y);scale.setScalar(Math.min(1,particle.life/.1));q.setFromUnitVectors(sparkForward,sparkDirection.copy(particle.v).normalize());matrix.compose(particle.p,q,scale).premultiply(inverse);sparks.setMatrixAt(i,matrix)}sparks.instanceMatrix.needsUpdate=true;
 }
 function reset(){state=createTyreState();emitted.clear();sparkCursor=0;idleUpdates=0;for(const wheel of car.wheels){const rest=wheelRest.get(wheel.id);wheel.pivot.position.y=rest.y;wheel.rollingRadius=rest.radius;wheel.tire.scale.copy(rest.scale);wheel.tire.visible=rest.visible}activeParticleIndices.length=0;for(const item of loose.values())item.mesh.visible=false;for(let i=0;i<64;i++){particles[i].life=0;particles[i].active=false;sparks.setMatrixAt(i,zero)}sparks.instanceMatrix.needsUpdate=true}
 ...
 return{object,hit,update,reset,dispose,get state(){return state},get effects(){return tyreDriveEffects(state)},stats:()=>({wheels:state.map(t=>({...t})),sparks:activeParticleIndices.length,looseTyres:[...loose.values()].filter(t=>t.mesh.visible).length,idleUpdates})};
}
