...
const geometryCache=new WeakMap(),cacheRecords=new Set();let cacheHits=0,cacheMisses=0;
function signature(matrix,boxes,suffix){
  const n=value=>Math.round(value*1e5);
  return `${suffix}|${matrix.elements.map(n).join(',')}|${boxes.map(box=>[box.min.x,box.min.y,box.min.z,box.max.x,box.max.y,box.max.z].map(n).join(',')).join(';')}`;
}
function acquireGeometry(source,key,create){
  let variants=geometryCache.get(source);if(!variants)geometryCache.set(source,variants=new Map());
  let record=variants.get(key);if(record){record.refs++;cacheHits++;return record}
  record={source,key,geometry:create(),refs:1,variants};variants.set(key,record);cacheRecords.add(record);cacheMisses++;return record;
}
...
export function buildingEntryProfileCacheStats(){return{entries:cacheRecords.size,refs:[...cacheRecords].reduce((sum,r)=>sum+r.refs,0),hits:cacheHits,misses:cacheMisses}}
...
export function createAdditionalBuildingEntry({THREE,visual,instance,metresPerCell=4.1}){
  ...
  const originalGeometry=[],originalTransforms=[],ownedGeometry=extracted.map(n=>n.geometry),ownedMaterial=[],acquiredGeometry=[];
  const restoreGeometry=(node,key,create)=>{
    const record=acquireGeometry(node.geometry,key,create);
    originalGeometry.push([node,node.geometry]);acquiredGeometry.push(record);node.geometry=record.geometry;
  ...
  if(profile.floor!==undefined)for(const node of doorNodes){
    const b=boxOf(node);if(b.min.y<-.005){
      ...
      const transform=referenceMatrix(node);
      restoreGeometry(node,signature(transform,[below],`floor:${instance.assetId}:${node.name}`),()=>subtractBoxFromGeometry(THREE,node.geometry,transform,below));
    }
  }
  ...
  for(const node of nodes){
    ...
    const transform=referenceMatrix(node),boxes=[];
    if(b.intersectsBox(roomBox))boxes.push(roomBox);
    if(b.intersectsBox(doorBox))boxes.push(doorBox);
    restoreGeometry(node,signature(transform,boxes,`entry:${instance.assetId}:${node.name}`),()=>{
      let geometry=node.geometry,temporary=null;
      if(boxes.includes(roomBox)){geometry=subtractBoxFromGeometry(THREE,geometry,transform,roomBox);temporary=geometry}
      if(boxes.includes(doorBox)){geometry=subtractBoxFromGeometry(THREE,geometry,transform,doorBox);temporary?.dispose()}
      return geometry;
    });clipped++;
  }
  ...
  if(profile.split){
    ...
    const original=leaf.geometry,transform=referenceMatrix(leaf);
    restoreGeometry(leaf,signature(transform,[leftRemoval],`split-left:${instance.assetId}:${leaf.name}`),()=>subtractBoxFromGeometry(THREE,original,transform,leftRemoval));
    const rightRecord=acquireGeometry(original,signature(transform,[rightRemoval],`split-right:${instance.assetId}:${leaf.name}`),()=>subtractBoxFromGeometry(THREE,original,transform,rightRemoval));
    acquiredGeometry.push(rightRecord);rightLeaf.geometry=rightRecord.geometry;
  }
  ...
  const api={visual,object:space,instance,report,contentRoot,sampleBounds,get needsUpdate(){return fraction!==target},
