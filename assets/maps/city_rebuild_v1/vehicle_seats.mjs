...
export const VEHICLE_SEATS=Object.freeze([
 ...
 Object.freeze({id:'front_right',doorId:'front_right',label:'Передний пассажир',side:-1,canDrive:false,anchor:Object.freeze({side:-.43,front:-.15,y:.22})}),
 Object.freeze({id:'rear_left',doorId:'rear_left',label:'Задний левый пассажир',side:1,canDrive:false,anchor:Object.freeze({side:.43,front:-1.15,y:.22})}),
 Object.freeze({id:'rear_right',doorId:'rear_right',label:'Задний правый пассажир',side:-1,canDrive:false,anchor:Object.freeze({side:-.43,front:-1.15,y:.22})}),
]);
export function vehicleSeat(id){const seat=VEHICLE_SEATS.find(item=>item.id===id);if(!seat)throw Error('Unknown vehicle seat '+id);return seat}
...
export function vehicleSeatPoint(car,seatId){const a=vehicleSeat(seatId).anchor;return local(car,a.side,a.front,a.y)}
export function vehicleDoorPoint(car,seatId,distance=1.9){const seat=vehicleSeat(seatId);return local(car,seat.side*distance,seat.anchor.front)}
...
export function vehicleEntryPoint(car,seatId,progress,outside=vehicleDoorPoint(car,seatId)){
 const seat=vehicleSeat(seatId),target=vehicleSeatPoint(car,seatId),pose={...entryPose(progress),cabinSlide:0,side:seat.side,driver:seat.canDrive};
...
export function vehicleDeparturePoint(car,seatId,distance,progress){
 const seat=vehicleSeat(seatId),target=vehicleSeatPoint(car,seatId),outside=vehicleDoorPoint(car,seatId,distance),pose={...entryPose(progress,true),cabinSlide:0,side:seat.side,driver:seat.canDrive};
