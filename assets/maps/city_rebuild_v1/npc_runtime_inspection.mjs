...
import {createInspectionExport} from './npc_inspection_export.mjs';
...
export function createNpcRuntimeInspection({document:doc=globalThis.document,parent=doc?.body,getActors=()=>[],getActorState=()=>null,focusActor=()=>{},getFocus=()=>null,search=doc?.defaultView?.location?.search||''}={}){
 ...
 const exportButton=doc.createElement('button'),exportText=doc.createElement('textarea');
 exportButton.type='button';exportButton.textContent='Скачать снимок NPC (JSON)';exportText.hidden=true;exportText.readOnly=true;exportText.setAttribute('aria-label','Снимок NPC JSON — резервная копия');exportText.style.cssText='width:100%;height:100px';body.append(exportButton,exportText);
 let downloadUrl=null;const urlApi=doc.defaultView?.URL;
 function releaseDownload(){if(downloadUrl){urlApi?.revokeObjectURL(downloadUrl);downloadUrl=null;}}
 const datasetJson=value=>{if(typeof value!=='string'||value.length>2000000)return null;try{return JSON.parse(value)}catch{return null}};
 exportButton.addEventListener('click',()=>{
  if(disposed)return;
  try{
   const data=createInspectionExport({rows:getActors()||[],getActorState,time:lastRefresh,routeReplay:datasetJson(doc.documentElement?.dataset?.npcRouteReplay),npcWorld:datasetJson(doc.body?.dataset?.npcWorld)});
   exportText.value=JSON.stringify(data);exportText.hidden=false;releaseDownload();
   if(urlApi?.createObjectURL&&doc.defaultView?.Blob){downloadUrl=urlApi.createObjectURL(new doc.defaultView.Blob([exportText.value],{type:'application/json'}));const link=doc.createElement('a');link.href=downloadUrl;link.download='npc-inspection-'+Date.now()+'.json';body.append(link);try{link.click()}finally{link.remove()}}
   status.textContent='Снимок NPC: '+data.actors.length+'. Если скачивание недоступно, JSON находится в поле ниже.';
  }catch(error){status.textContent='Не удалось экспортировать NPC: '+String(error?.message||error);}
 });
 ...
 update();return {enabled:true,panel,update,getSelection:selection,isFollowing:()=>following,dispose(){if(disposed)return;disposed=true;releaseDownload();rows=[];samples.clear();panel.remove()}};
}
