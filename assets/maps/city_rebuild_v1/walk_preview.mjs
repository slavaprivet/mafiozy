...
import {buildingShadowCullingEnabled,createVehicleShadowCulling,vehicleShadowCullingEnabled} from './vehicle_shadow_culling.mjs';
...
const entryLightViewCulling=new URLSearchParams(location.search).get('entrylightfrustum')==='1';
...
let vehicleShadowCulling=null,vehicleShadowQaButton=null,buildingShadowQaButton=null;
...
addEventListener('pagehide',()=>{vehicleShadowCulling?.dispose();vehicleShadowQaButton?.remove();buildingShadowQaButton?.remove();performanceProbe?.dispose();},{once:true});
...
if(vehicleShadowCulling&&renderFreezeQa&&vehicleShadowEnabled){
...
if(vehicleShadowCulling&&renderFreezeQa&&buildingShadowEnabled){
 buildingShadowQaButton=document.createElement('button');buildingShadowQaButton.id='building-shadow-qa';buildingShadowQaButton.type='button';buildingShadowQaButton.style.cssText='position:fixed;top:274px;left:50%;transform:translateX(-50%);z-index:10000;padding:8px;max-width:90vw';document.body.append(buildingShadowQaButton);resetVehicleShadowQa();
 buildingShadowQaButton.onclick=()=>{if(!renderFreezeQa.active){buildingShadowQaButton.textContent='Сначала зафиксируйте 3D-сцену';return;}const enabled=!vehicleShadowCulling.stats.buildingEnabled;vehicleShadowCulling.setBuildingEnabled(enabled);buildingShadowQaButton.textContent=enabled?'Тени зданий: отсечение невидимых · сравнить':'Тени зданий: прежняя отрисовка · сравнить';performanceProbe?.reset();};
}
...
let carRollover=null;let car=null,carState=null,carDamage=null,carQa=null,occupiedSeat=null,driveAllowed=()=>false,transition=null,entryHeld=0,entryArmed=true,pointerHeld=false,lastDoorSide=1,followCarCamera=true,carDriveDiagnosticsAt=-Infinity,walkVehicleExplosionOccupants=null;
...
const npcVehicleIncomingCapability=createLocalVehicleIncomingCapability();
let npcVehicleIncoming=null;
let npcBlastParts=null,npcBlastPresentation=null,npcBloodMarks=null,npcShotEffects=null,npcLogicalVehicles=null,npcInspection=null,worldTrafficPresentation=null,npcPopulation=null,npcBridge=window.Mafiozi3DBridge||null,worldWalkCombat=null,npcGallery=null,worldWalkMelee=null,npcDiagnosticsAt=-Infinity;
// Register before asynchronous NPC loading. This cheap live closure remains
// available even while the precise car/glass resolver is not ready or fails.
npcBridge?.registerWalkVehicleOccupancyResolver?.(()=>!!occupiedSeat||!!transition||sourceVehicleActive());
addEventListener('pagehide',()=>npcBridge?.registerWalkVehicleOccupancyResolver?.(null),{once:true});
...
async function initNpcPopulation(){
 ...
 npcVehicleIncoming=createNpcVehicleIncomingBridge({THREE,getTarget:()=>{
  if(sourceVehicleActive())return {unavailable:true,source:true,reason:'source-pose-stamp-unavailable'};
  if(!occupiedSeat&&!transition)return null;
  if(!hero||!car||!occupiedSeat||transition||worldHealthFrame?.snapshot.dead||heroCustodyActive)return {unavailable:true,reason:'vehicle-transition-or-life'};
  const capability=npcVehicleIncomingCapability.check(car,occupiedSeat);if(!capability.supported)return {unavailable:true,reason:capability.reason};
  const windows=[];for(const door of car.doors?.values()||[]){windows.push(door.rotation.x,door.rotation.y,door.rotation.z);door.traverse(n=>{if(n.isMesh&&n.material?.userData?.breakableGlass)windows.push(n.visible?1:0,n.geometry.index?.version||0);});}
  return {id:'player',phase:'drive',seatId:occupiedSeat,car,actor:hero,r:hero.object.position.z/M,c:hero.object.position.x/M,visibilityRevision:[glass.stats().brokenPanels,carDamage?.state?.hp,...windows].join(',')};
 },getActors:()=>[...(npcPopulation?.getActors()||[]),...(hero?[{id:'player',object:hero.object}]:[])],obstacles:shotObstacles,getVehicles:()=>[...(fleet?.records||[]),...(worldTrafficPresentation?.getActors()||[])],bodiesAt:(c,r)=>walkCollisionIndex?.(c,r)||[],groundHeight,ready:()=>!!walkCollisionIndex,local:()=>window.MafioziMercenaries?.canUseLocalEffects?.()===true,onImpact:payload=>{for(const record of fleet?.records||[]){record.damage?.impact(payload);record.tyres?.hit(payload);}if(payload.hit)glass.hit(payload.hit,{direction:payload.direction,impulse:payload.damage,weaponId:payload.weaponId});}});
 npcBridge?.registerWalkNpcVehicleIncomingResolver?.(npcVehicleIncoming.resolve);
 ...
 const vehicleSurfaceAccess=createNpcVehicleSurfaceAccess({worldScale:M,getParkingPlan:()=>environmentVisuals?.parkingPlan,getRoadPlan:()=>environmentVisuals?.roadPlan,verifyLaneSegment:request=>cityRoadNavigation?.verifyLaneSegment(request)??{allowed:false,reason:'route_expired'}});
 npcVehicleNavigation=createNpcVehicleNavigation({worldScale:M,inspectRoutes:!!performanceProbe,ready:()=>!!walkCollisionIndex&&!!driveAllowed,poseAllowed:(x,z,yaw,shape)=>carFits(x,z,yaw,driveAllowed,shape),isRoad:(x,z)=>!!topology?.roadMask?.[Math.floor(z/M)]?.[Math.floor(x/M)],vehicleAccess:vehicleSurfaceAccess.query,waterAt,groundHeight,getVehicle:id=>npcLogicalVehicles.getActor(id),getVehicles:()=>[...(fleet?.records||[]),...npcLogicalVehicles.getVehicles()]});
 const playerVehiclePhysical=createPlayerVehiclePhysicalResolver({getVehicleState:()=>npcBridge?.getWalkVehicleState?.(),queryPhysical:npcVehicleNavigation.queryPhysical});
 ...
 npcBloodMarks=createWorldBloodMarks({THREE,scene,groundHeight,worldScale:M,allowGround:(x,z)=>!waterAt(x,z)&&!sampledBuildingEntries(x,z).some(entry=>entry.floorHeight(x,z,0)!==null)});
...
function updateNpcPopulation(dt){
 ...
 npcNativePerception?.beginFrame();npcVehicleIncoming?.beginFrame();
 ...
 const sourceClock=npcBridge?.getWorldClock?.(),now=(Number.isFinite(sourceClock)?sourceClock:sourceClock?.now??performance.now())/1000;npcPopulation.update(dt,now);npcBlastPresentation?.update(now);npcInspection?.update(performance.now()/1000);
 npcShotEffects?.update(performance.now()/1000);
 npcBloodMarks?.update(performance.now()/1000);
 ...
 const diagnosticNow=performance.now();if(diagnosticNow-npcDiagnosticsAt>=250){npcDiagnosticsAt=diagnosticNow;document.body.dataset.npcWorld=JSON.stringify({...npcPopulation.diagnostics(),blast:{...npcBlastPresentation?.diagnostics(),...npcBlastParts?.stats()},traffic:worldTrafficPresentation?.diagnostics(),perception:npcNativePerception?.diagnostics(),navigation:{...npcNativeNavigation?.diagnostics(),support:npcSupportCache?.diagnostics(),source:npcBridge?.getWalkNpcNavigationDiagnostics?.()}});}
}
...
const updateEntryLightsForRenderCamera=finalCamera=>stableEntryLights?.updateView?.(finalCamera);
...
const fireStates=new Map(),effects=createWeaponEffects(THREE,scene,{groundHeight,onImpact(payload){
 if(payload.nativeRpgImpact)npcBridge?.impactWalkRpg?.({shotId:payload.shotId,point:{x:payload.point.x,y:payload.point.y,z:payload.point.z}});
 for(const record of fleet?.records||[]){record.damage?.impact(payload);record.tyres?.hit(payload)}
 if(payload.explosive)blastResponse?.enqueue({point:payload.point,power:1.4,radius:11});
 if(payload.hit)glass.hit(payload.hit,{direction:payload.direction,impulse:payload.damage,weaponId:payload.weaponId});
...
blastResponse=createBlastResponse(THREE,scene,{getHero:()=>hero,getVehicles:()=>fleet?.records||[],getOcclusionVehicles:()=>[...(fleet?.records||[]),...sourceVehicleDriveControllerSet,...(worldTrafficPresentation?.getActors?.()||[])],getRoots:()=>[content],getGlass:()=>glass,groundHeight:heroGroundHeight,staticBatchSurfaceFix:staticRenderBatching,onBlast(event,heroExposure){
 ...
 if(!sourceVehicleId||!npcBridge?.applyWalkVehicleBlast)return;
 const receipt=npcBridge.applyWalkVehicleBlast({eventId:`walk-fleet:${walkBlastSession}:${event.id}`,kind:'vehicle_explosion',r:event.point.z/M,c:event.point.x/M,y:event.point.y,sourceVehicleId,...(heroExposure||{})});
 document.body.dataset.walkBlastDamage=JSON.stringify(receipt);
...
if(npcBridge?.registerWalkBlastExposureResolver){
 npcBridge.registerWalkBlastExposureResolver(request=>{
 const r=Number(request?.r),c=Number(request?.c),y=Number(request?.y),targets=Array.isArray(request?.targets)?request.targets:[];
  if(!Number.isFinite(r)||!Number.isFinite(c))return {exposures:[]};
  const x=c*M,z=r*M,point={x,y:Number.isFinite(y)?y:groundHeight(x,z)+.8,z};
  const physicalTargets=[];
  for(let i=0;i<targets.length&&i<512;i++){
   const target=targets[i],tr=Number(target?.r),tc=Number(target?.c),height=Number(target?.height),id=String(target?.id??'');
   if(!id||!Number.isFinite(tr)||!Number.isFinite(tc))continue;
   const tx=tc*M,tz=tr*M;physicalTargets.push({id,point:{x:tx,y:groundHeight(tx,tz)+(Number.isFinite(height)?height:1.1),z:tz}});
  }
  return blastResponse.resolveExposures({point,sourceVehicleId:request.sourceVehicleId,targets:physicalTargets});
 });
 addEventListener('pagehide',()=>npcBridge?.registerWalkBlastExposureResolver?.(null),{once:true});
}
...
function emitCombatShots(combat){
 ...
 for(const shot of combat.result.shots)effects.shoot(shot,transforms,shot.worldTarget||coverTarget,shotObstacles,shot.nativeRpgImpact?{resolveContact:worldWalkCombat?.projectileContact}:{});
}
...
let vehicleFleetQaSelect=null;
function refreshVehicleFleetQa(){return refreshVehicleFleetQaSelect(vehicleFleetQaSelect,fleet);}
function installVehicleFleetQa(){
 ...
 const select=document.createElement('select');select.id='fleet-qa-select';select.setAttribute('aria-label','QA: модель автомобиля');carQa.panel.append(select);vehicleFleetQaSelect=select;
...
function initCar(){
 ...
 walkVehicleExplosionOccupants??=createWalkVehicleExplosionLocalAuthority({
  sessionId:walkBlastSession,
  canUseLocalEffects:()=>window.MafioziMercenaries?.canUseLocalEffects?.()===true,
  getHeroState(record){const active=fleet?.active===record,source=npcBridge?.getPlayerState?.(),snapshot=worldHealthFrame?.snapshot;return {vehicleId:`fleet:${record.id}`,occupiedSeat:active?occupiedSeat:null,transition:active&&transition?{exiting:transition.exiting===true,phase:transition.phase,seatId:transition.seatId}:null,hp:Number(snapshot?.hp??source?.hp),dead:snapshot?.dead===true||source?.healthDead===true,lifeGeneration:npcBridge?.getWalkLocalVehicleOccupantLifeGeneration?.()??0};},
  getCrewState:vehicleId=>window.MafioziMercenaries?.snapshotVehicleExplosionOccupants?.(vehicleId)||[],
  applyHeroReceipt:receipt=>npcBridge?.applyWalkLocalVehicleOccupantLethal?.(receipt)||{accepted:false,reason:'owner-missing'},
  applyCrewReceipt:receipt=>window.MafioziMercenaries?.applyVehicleExplosionOccupantReceipt?.(receipt)||{accepted:false,reason:'owner-missing'},
 });
 fleet=createVehicleFleet(THREE,{scene,wheelRenderOptimization,detailOptimization:renderer.extensions.has('WEBGL_multi_draw'),RoundedBox:RoundedBoxGeometry,world:()=>driveAllowed,groundHeight,pose:(vehicle,state,angle,dt)=>poseWalkVehicle(vehicle,state,angle,dt),getHero:()=>hero,onDamageReset:event=>walkVehicleExplosionOccupants?.noteReset(event.record),onExplosion(event){const occupantResult=walkVehicleExplosionOccupants?.handle(event);document.body.dataset.walkVehicleExplosionOccupants=JSON.stringify({accepted:occupantResult?.accepted===true,duplicate:occupantResult?.duplicate===true,eventId:occupantResult?.eventId||'',targets:occupantResult?.targets||0,delivered:occupantResult?.delivered||0});const vehicle=event.vehicle;blastResponse?.enqueue({point:vehicle.object.localToWorld(new THREE.Vector3(0,1,0)),power:1,radius:10,source:vehicle});glass.shatterAll(vehicle.object,{impulse:80,weaponId:'vehicle_explosion'});if(vehicle===car){exitNotice='Машина уничтожена';exitNoticeUntil=performance.now()+2500;if(occupiedSeat&&!transition)beginCarTransition()}}});
...
async function refresh(){if(renderFreezeQa?.active||busy||jump||insideBuilding()||buildingQaMove)return;const startedAt=performance.now();busy=true;$('status').textContent='Читаю свежую расстановку…';
 ...
 waterEffects=createWaterInteractionEffects({THREE,waterAt,groundHeight});content.add(waterEffects.object);waterVapor=createVehicleWaterVapor({THREE});content.add(waterVapor.object);
  buildingSampleIndex=createWalkCollisionIndex([buildingEntries.map(entry=>{const b=entry.sampleBounds;return {entry,polygonCR:[[b.min.x/M,b.min.z/M],[b.max.x/M,b.min.z/M],[b.max.x/M,b.max.z/M],[b.min.x/M,b.max.z/M]]}})]);stableEntryLights=createStableEntryLights(THREE,buildingEntries,scene,{maxLights:32,getFocus:()=>hero?.object.position||controls.target,viewCull:entryLightViewCulling});
 if(performanceProbe)for(const group of instances){const item=group.userData.instance;group.userData.shadowCensusCategory=/^lamp_/.test(item?.assetId||'')?'lamps':buildingItems.has(item)?'buildingsInteriors':'decor';}
 const staticRenderBatchStarted=performance.now();
 if(staticRenderBatching)try{staticRenderBatches=createStaticRenderBatches({THREE,root:content,instances,minInstances:3,maxDistance:220,localMatrixOptimization:staticMatrixOptimization,shadowCensus:performanceProbe?.tagShadowBatch,multiDraw:renderer.extensions.has('WEBGL_multi_draw')});document.body.dataset.staticRenderBatches=JSON.stringify(staticRenderBatches.stats())}catch(error){console.warn('Static render batching disabled',error);staticRenderBatches=null;document.body.dataset.staticRenderBatches=JSON.stringify({disabled:true,error:error.message})}
...
if(['127.0.0.1','localhost'].includes(location.hostname)&&new URLSearchParams(location.search).get('npccombatqa')==='1'){
 window.addEventListener('npccombatqa:input',event=>{
  ...
  if(action!=='aim'&&action!=='fire')return;
  ...
  // Clicking a diagnostic button releases held controls. Re-establish the same
  // visible aim before firing; the normal source still resolves spread/contact.
  if(action==='fire')triggerPressed=true;
 });
}
