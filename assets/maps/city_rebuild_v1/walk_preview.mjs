import {createWorldTrafficPresentation,npcVehicleBlocks} from './world_traffic_presentation.mjs';
...
import {createExplorationMinimap,normalizeNativeInstances,mapKind} from './exploration_minimap.mjs';
...
addEventListener('pagehide',()=>performanceProbe?.dispose(),{once:true});
...
const hudInputBlocked=()=>worldHealthFrame?.inputsBlocked===true||walkPlayerHud?.isBlocked()===true;
...
let entryBodyVersions=[],walkCollisionIndex=null,buildingSampleIndex=null;
...
let worldWalkHealth=null,worldWalkHealthBridge=null,worldHealthFrame=null,healthDiagnosticsAt=-Infinity;
...
function updateWorldWalkHealth(){
 ...
 const source=npcBridge.getPlayerState(),now=performance.now();
...
function artistAllowed(){return jump?.mode!=='traversal'&&!hudInputBlocked()&&!!hero&&walking&&!occupiedSeat&&!transition&&!busy&&!arsenalOpen()&&$('scene-menu').hidden&&!heroBlast&&!verticalNavigation.active&&!artistBusy()&&!artistSwimming();}
...
let npcInspection=null,worldTrafficPresentation=null,npcPopulation=null,npcBridge=window.Mafiozi3DBridge||null,worldWalkCombat=null,npcGallery=null,worldWalkMelee=null,npcDiagnosticsAt=-Infinity;
let npcNativeNavigation=null;
async function initNpcPopulation(){
 worldTrafficPresentation=createWorldTrafficPresentation({THREE,RoundedBox:RoundedBoxGeometry,loader,scene,groundHeight:(x,z)=>groundHeight(x,z),worldScale:M});
 ...
 npcNativeNavigation=createNpcNativeNavigation({worldScale:M,waterAt,groundHeight,bodiesAt:(c,r)=>walkCollisionIndex?.(c,r)||[],containsBody:(body,r,c)=>inPolygon(r,c,body.polygonCR),terrainAllows:(x,z)=>!landscape?.contains(x,z)||landscape.canWalk(x,z),blocksDynamic:(x,z,body)=>!!explorationRailway?.blocks(x,z,0)||!!fleet?.records.some(record=>npcVehicleBlocks(record.car,x,z,0,body))||!!worldTrafficPresentation?.blocks?.(x,z,0,body)});
 ...
 npcPopulation=await createNpcPopulation({THREE,scene,loader,cloneSkeleton:cloneNpcSkeleton,bridge:npcBridge,groundHeight:(x,z)=>groundHeight(x,z),waterAt:(x,z)=>waterAt(x,z),worldScale:M,getFocus:()=>hero?.object.position,creationBudget:1,profile:new URLSearchParams(location.search).get('perfqa')==='1',getInspectId:()=>npcInspection?.getSelection()?.id,getVehicle:getSourceVehicle,onSnapshot:snapshot=>worldTrafficPresentation.sync(snapshot?.cars||[]),onBeforePose:({dt})=>worldTrafficPresentation.update(dt)});
...
function updateNpcPopulation(dt){
 ...
 if(npcBridge?.syncWalkPlayer){const receipt=npcBridge.syncWalkPlayer({r:hero.object.position.z/M,c:hero.object.position.x/M,ang:Math.PI/2-hero.object.rotation.y,walking:[...keys].some(key=>['KeyW','KeyA','KeyS','KeyD'].includes(key)),stance:heroPosture.target});
  if(receipt?.locked&&receipt.state){const s=receipt.state;releaseControls();hero.object.position.set(s.c*M,groundHeight(s.c*M,s.r*M),s.r*M);}
 }
 ...
 const diagnosticNow=performance.now();if(diagnosticNow-npcDiagnosticsAt>=250){npcDiagnosticsAt=diagnosticNow;document.body.dataset.npcWorld=JSON.stringify({...npcPopulation.diagnostics(),traffic:worldTrafficPresentation?.diagnostics(),navigation:{...npcNativeNavigation?.diagnostics(),source:npcBridge?.getWalkNpcNavigationDiagnostics?.()}});}
}
...
function artistUpdate(dt,moved,running){artistSurfaceFrameDt=dt;if(verticalNavigation.active)return;
 ...
 artistSurfaceState=artistSurface.update(dt,{time:performance.now()/1000,waterLevel:water?.level??null,inWater:!!water&&water.depth>0,moving:moved,fast:running,chestWorldY:floor+3.08*unit,groundWorldY:floor,blocked:!!occupiedSeat||(!!transition&&(!transition.exiting||transition.phase!=='body'))||!!jump||!!heroBlast||verticalNavigation.active});
...
function combatAllowed(){return jump?.mode!=='traversal'&&!hudInputBlocked()&&!artistSwimming()&&!artistBusy()&&!!hero&&walking&&(!occupiedSeat||!carDamage?.disabled&&!carRollover?.unstable)&&!transition&&!heroBlast&&!verticalNavigation.active&&!busy&&!arsenalOpen()&&$('scene-menu').hidden&&currentWeapon.id!=='none'}
function releaseWeapon(){resetVehicleWindowFire();pendingCoverShotUntil=0;triggerHeld=false;triggerPressed=false;reloadPressed=false;aiming=false}
function resetVehicleWindowFire(){
 vehicleWindowPrompt.hidden=true;
...
function updateVehicleWindowFire(dt,{basePose=true}={}){
 ...
 vehicleWindowPrompt.hidden=true;if(occupiedSeat&&aiming&&vehicleWindowFire.result){const r=vehicleWindowFire.result;vehicleWindowPrompt.hidden=false;vehicleWindowPrompt.textContent=r.canFire?'ПКМ — прицел из окна · ЛКМ — огонь · R — перезарядить':r.reason;}
...
function shotObstacles(origin,direction,distance){const staticRoots=shotRaycastIndex?.query(origin,direction,distance)??[content],cars=fleet?.records.map(record=>record.car.object)||[];return cars.length?[...staticRoots,...cars]:staticRoots}
function emitCombatShots(combat){
 ...
 const obstacles=shotObstacles(camera.position,combat.forward,100),hit=aimRay.intersectObjects(obstacles,true).find(hit=>{for(let node=hit.object;node;node=node.parent)if(!node.visible)return false;return true}),target=hit?hit.point:aimRay.ray.at(100,new THREE.Vector3()),transforms=resolveWeaponShotTransforms(THREE,weaponModel);
...
function updateAimCamera(dt){
 ...
 if(savedCameraOffset){
  ...
  const target=occupiedSeat?vehicleWindowCameraTarget(forward):hero.object.position.clone().add(new THREE.Vector3(0,postureEyeHeight(),0)).add(heroCover.active?heroCover.offset:new THREE.Vector3()).addScaledVector(right,heroCover.active||scoped?0:.75),behind=forward.clone().multiplyScalar(scoped&&!occupiedSeat?-.12:-4.2);
...
function pedestrianAllowed(x,z){return canWalk(x,z)&&(fleet?!fleet.overlaps(x,z,0):!carState||!pointInPolygon(x,z,carCorners(carState.x,carState.z,carState.yaw,carState.vehicleProfile)))}
function exitWalkAllowed(x,z){return canWalk(x,z)&&(!fleet||!fleet.overlaps(x,z,0,fleet.activeId))}
...
function entrySpot({fresh=false}={}){
 ...
 for(const record of fleet?.nearby(hero.object.position)||[]){
  if(record.damage?.disabled||record.roll?.unstable||record.state?.waterState?.flooded)continue;
...
function beginCarTransition(){
 ...
 if(occupiedSeat){
  ...
  const candidate=entrySpot();activateVehicle(candidate.record);lastDoorSide=candidate.side;transition={exiting:false,elapsed:0,...candidate,from:hero.object.position.clone(),outside:new THREE.Vector3(candidate.outside.x,candidate.outside.y,candidate.outside.z)};carState.speed=0;carState.distance=0;
 }
...
function updateCarPrompt(){
 ...
 if(!car||!hero||occupiedSeat||transition||(!selected&&!service)){prompt.hidden=true;return}
 let anchor;
 ...
 prompt.querySelector('small').textContent=service?(candidate.action==='repair'?'R — ремонт двигателя':candidate.kind==='hood'&&(candidate.repairAvailable||candidate.record.hood?.stats().open)?'E — закрыть · R — ремонт двигателя':'E — нажать'):'Удерживайте E · 0,3 с';
...
const verticalNavigation=createBuildingVerticalNavigation({THREE,getHero:()=>hero,getEntries:()=>buildingEntries,getWeapon:()=>weaponModel,camera,controls,canOccupy(p,radius,height){for(const[dx,dz]of[[0,0],[radius,0],[-radius,0],[0,radius],[0,-radius]]){const x=p.x+dx,z=p.z+dz,c=x/M,r=z/M;if(ceilingHeight(x,z,p.y)<p.y+height-.03)return false;for(const body of walkCollisionIndex?.(c,r)||[]){if((body.maxYM??Infinity)<p.y+.08||(body.minYM??-Infinity)>p.y+height-.03)continue;if(inPolygon(r,c,body.polygonCR))return false}}return true},onBegin(){jump=null;artistInput.cancel();releaseControls();releaseWeapon();resetHeroPosture(heroPosture);hero.reset()},onEnd(){releaseControls();surfaceMotion.reset(hero.object.position);hero.reset()}});
...
function interactWithBuilding(){
 const candidate=nearestInteraction({fresh:true});if(candidate?.kind==='ladder'){buildingKeyConsumed=true;entryHeld=0;pointerHeld=false;if(!verticalNavigation.begin(candidate.ladder)){exitNotice='Проход к лестнице закрыт';exitNoticeUntil=performance.now()+1800}return true}if(candidate?.kind!=='building')return false;
...
function updateBuildingEntries(dt){
 ...
 for(let i=0;i<buildingEntries.length;i++){const entry=buildingEntries[i];if(!entry.needsUpdate&&entryBodyVersions[i]!==undefined)continue;entry.update(dt,hero?.object.position);const next=entry.getCollisionBodies();if(entryBodyVersions[i]!==next){entryBodyVersions[i]=next;changed=true}}
...
function updateBuildingPrompt(){
 ...
 if(candidate?.kind!=='building'&&candidate?.kind!=='ladder'){prompt.hidden=true;return}
 ...
 if(!transition&&!occupiedSeat&&!jump&&performance.now()>=exitNoticeUntil)$('drive-status').textContent=candidate.kind==='ladder'?(candidate.ladder.end==='upper'?'Ctrl — быстрый спуск':'E — подъём · Ctrl — скользить вниз'):'E — дверь · WASD — пройти';
...
function clampBuildingCamera(){
 if(verticalNavigation.active||!hero||occupiedSeat||transition){indoorCamera.reset();return;}
 ...
 indoorCameraFrame=indoorCamera.solve({feet:hero.object.position,eyeHeight:postureEyeHeight(),desired:camera.position,target:controls.target,objects:relevant.map(entry=>entry.visual),ceilingY:current?.ceilingHeight(hero.object.position)??Infinity,inside:!!current,aiming});
...
function initBuildingQa(){
 ...
 const available=()=>hero&&!occupiedSeat&&!transition&&!jump&&!heroBlast&&!verticalNavigation.active&&!busy;
...
async function loadVehicleFleet(origin){
 ...
 try{vehicles=await loadArtistFleetModels({THREE,loader,RoundedBox:RoundedBoxGeometry,includeTaxi:true,baseUrl:root+'models/artist_vehicle_pack/',onProgress:progress=>{document.body.dataset.vehicleFleetLoading=JSON.stringify(progress)}})}catch(error){vehicles=error.vehicles||[];fail('Автопарк: '+error.message)}
...
function installVehicleFleetQa(){
 ...
 const near=(kind)=>{if(occupiedSeat||transition||jump||heroBlast)return;const record=fleet.records.find(r=>r.id===select.value);if(!record)return;activateVehicle(record);let p;
...
function explorationNpcMarkers(){const now=performance.now();if(now-npcMapCacheAt>250){npcMapCacheAt=now;npcMapCache=(window.MafioziWalkNpcs?.getActors?.()||[]).filter(a=>a.object?.visible!==false&&a.object?.position).map(a=>({id:a.id,name:a.name,kind:a.source?.police?'police':a.source?.empireBoss?'boss':a.role||'civilian',position:a.object.position,active:!a.source?.carried&&!a.source?.evacuated}))}return npcMapCache}
...
async function refresh(){if(busy||jump||insideBuilding()||buildingQaMove)return;const startedAt=performance.now();busy=true;$('status').textContent='Читаю свежую расстановку…';
 ...
 if(staticRenderBatching)try{staticRenderBatches=createStaticRenderBatches({THREE,root:content,instances,minInstances:3,maxDistance:220});document.body.dataset.staticRenderBatches=JSON.stringify(staticRenderBatches.stats())}catch(error){console.warn('Static render batching disabled',error);staticRenderBatches=null;document.body.dataset.staticRenderBatches=JSON.stringify({disabled:true,error:error.message})}
...
function setWalking(enabled){walking=enabled;$('walk').setAttribute('aria-pressed',String(walking));controls.enablePan=!walking;$('cross').style.display='none';if(walking&&hero){const shift=hero.object.position.clone().add(new THREE.Vector3(0,postureEyeHeight(),0)).sub(controls.target);controls.target.add(shift);camera.position.add(shift)}}
 ...
 $('walk').onclick=()=>{if(!occupiedSeat&&!transition&&!jump&&!heroBlast&&!verticalNavigation.active)setWalking(!walking)};
...
function requestNetworkWeaponTransfer(action,candidate=null){
 ...
 Promise.resolve(request).then(result=>{
  if(result?.ok){if(action==='drop')lastWeaponId=null;syncWorldWeapon();updateWeaponUi();exitNotice=action==='drop'?'Оружие выброшено · исчезнет через 5 минут':'Оружие добавлено в инвентарь';}
...
function pickupNearbyWeapon(){
 ...
 // Collecting is not equipping: preserve the active model, reload and trigger.
 for(const id of weaponInventory.getOwnedIds())fireStates.set(id,weaponInventory.getFireState(id));
 updateWeaponUi();syncGroundWeapons();return true;
}
function updateGroundWeaponInteraction(dt){
 expireGroundWeapons();
 groundWeapons?.update(dt);
 const candidate=nearbyGroundWeapon();weaponPickupPrompt.hidden=!candidate;
 if(candidate){weaponPickupPrompt.textContent=`E · Подобрать ${ARSENAL.find(item=>item.id===candidate.drop.weaponId)?.label||'оружие'} · ${candidate.drop.fireState.magazine} / ${candidate.drop.fireState.reserveAmmo}`;$('car-prompt').hidden=true;$('building-prompt').hidden=true;}
...
const traversalWorld=createTraversalWorld({groundHeight,ceilingHeight,bodiesAt:(x,z)=>walkCollisionIndex?.(x/M,z/M)||[],contains:traversalMapContains,walkable:traversalMapWalkable,waterAt,blocksDynamic:(x,z)=>!!explorationRailway?.blocks(x,z,.36)||(fleet?fleet.overlaps(x,z,0):!!carState&&pointInPolygon(x,z,carCorners(carState.x,carState.z,carState.yaw,carState.vehicleProfile))),inBody:(body,x,z)=>inPolygon(z/M,x/M,body.polygonCR)});
// Cover reuses live collision faces. Moving doors and moving cars are excluded.
function vehicleCoverHeight(record,position,polygon,bounds){
 let closest=null;for(let i=0;i<polygon.length;i++){const a=polygon[i],b=polygon[(i+1)%polygon.length],dx=b.x-a.x,dz=b.z-a.z,l2=dx*dx+dz*dz,t=THREE.MathUtils.clamp(((position.x-a.x)*dx+(position.z-a.z)*dz)/l2,0,1),x=a.x+dx*t,z=a.z+dz*t,d=Math.hypot(position.x-x,position.z-z);if(!closest||d<closest.d)closest={x,z,d};}
 const center=new THREE.Vector3(record.state.x,0,record.state.z),point=new THREE.Vector3(closest.x,0,closest.z);point.addScaledVector(center.sub(point).normalize(),.35);point.y=bounds.max.y+.2;
 const probe=new THREE.Raycaster(point,new THREE.Vector3(0,-1,0),0,bounds.max.y-bounds.min.y+.5),hit=probe.intersectObject(record.car.object,true).find(h=>{for(let n=h.object;n;n=n.parent)if(!n.visible)return false;const mats=Array.isArray(h.object.material)?h.object.material:[h.object.material];return mats.some(m=>(m?.transmission||0)<.2&&(!m?.transparent||m.opacity>.65));});
 return hit?.point.y??bounds.max.y;
}
function coverBodies(position){
 ...
 const result=[...found].filter(b=>!b.movingDoor&&b.polygonCR?.length>=3).map((body,index)=>({id:body.id||body.node||`${body.buildingEntryId||'wall'}:${index}`,polygon:body.polygonCR.map(([c,r])=>({x:c*M,z:r*M})),minY:body.minYM??position.y,maxY:body.maxYM??position.y+3,source:body,valid:()=>bodies.includes(body)||buildingBodies.includes(body)}));
 for(const record of fleet?.nearby(position,8)||[]){if(Math.abs(record.state.speed)>.25||record.roll?.unstable)continue;
  ...
  result.push({id:`vehicle:${record.id}`,polygon,standOff:.37,minY:bounds.min.y,maxY:vehicleCoverHeight(record,position,polygon,bounds),heightAt:p=>vehicleCoverHeight(record,p,polygon,bounds),vehicle:true,source:record,valid:()=>Math.abs(record.state.speed)<.35&&Math.hypot(record.state.x-x,record.state.z-z)<.12&&Math.abs(record.state.yaw-yaw)<.05&&!record.roll?.unstable});
 }return result;
}
...
const heroCover=createHeroCover({THREE,document,getHero:()=>hero,getWeapon:()=>weaponModel,getBodies:coverBodies,canOccupy:coverCanOccupy,
 allowed:()=>!!hero&&walking&&!occupiedSeat&&!transition&&!jump&&!heroBlast&&!verticalNavigation.active&&!busy&&!artistSwimming()&&!artistBusy()&&artistAction.action.type==='none'&&surfaceMotion.state?.grounded!==false&&!hudInputBlocked()&&!arsenalOpen()&&$('scene-menu').hidden,
 getPosture:()=>heroPosture,requestPosture:target=>requestHeroPosture(heroPosture,target,{canOccupyHeight:height=>canOccupyPostureHeight(height)}),
...
function initCoverQa(){installCoverQa({document,enabled:new URLSearchParams(location.search).get('coverqa')==='1',
 ...
 toggle:toggleHeroCover,crouch(){heroCover.leave();setHeroPosture(heroPosture.target==='crouch'?'stand':'crouch');},fire(aim,held){camera.position.y=controls.target.y;camera.lookAt(controls.target);aiming=aim;triggerHeld=held;triggerPressed=held;},move(amount){if(heroCover.state)heroCover.move(new THREE.Vector3(heroCover.state.tangent.x,0,heroCover.state.tangent.z).multiplyScalar(amount));},finish(){heroCover.leave();releaseControls();}});}
function toggleHeroCover(){return heroCover.toggle(camera.getWorldDirection(new THREE.Vector3()));}
...
addEventListener('keydown',e=>{
 ...
 if(e.code==='KeyC'&&!e.repeat){e.preventDefault();heroCover.leave();setHeroPosture((jump?.queuedPosture??heroPosture.target)==='crouch'?'stand':'crouch');return}
...
renderer.domElement.addEventListener('click',()=>{if(hudInputBlocked())return;setFreeMouse(true);if(!document.pointerLockElement&&renderer.domElement.requestPointerLock){try{renderer.domElement.requestPointerLock()?.catch(()=>setFreeMouse(true))}catch{setFreeMouse(true)}}});
...
document.addEventListener('mousemove',e=>{const locked=document.pointerLockElement===renderer.domElement;if(hudInputBlocked()||!freeMouseLook||(!locked&&e.target!==renderer.domElement))return;const rect=renderer.domElement.getBoundingClientRect();edgeTurn=locked?0:e.clientX<rect.left+22?1:e.clientX>rect.right-22?-1:0;turnCamera(THREE.MathUtils.clamp(e.movementX,-80,80)*.004,THREE.MathUtils.clamp(e.movementY,-80,80)*.003)});
...
const clock=new THREE.Clock();let lastCull=0,lastStats=0,frameCount=0,frameElapsed=0,frameLongest=0;


function moveHeroOnFoot(delta){
 const covered=heroCover.move(heroCover.active?delta.clone().multiplyScalar(Math.min(1,1.25/postureMotion.maxSpeed)):delta);if(covered!==null)return covered;


 const before=hero.object.position.clone(),swimming=artistSwimming()||artistSurfaceState?.swim.blend>.1,grounded=surfaceMotion.state?.grounded!==false;let movementStartFloor,movementStartFloorReady=false;const allowed=(x,z)=>{if(!pedestrianAllowed(x,z))return false;if(swimming)return heroGroundHeight(x,z)<=before.y+.28;if(!grounded)return true;if(!movementStartFloorReady){movementStartFloor=heroGroundHeight(before.x,before.z);movementStartFloorReady=true;}return surfaceMotion.canMove(before,{x,z},heroGroundHeight,movementStartFloor)};const result=movePedestrian(before,delta,allowed);
...
function frame(){requestAnimationFrame(frame);frameInteraction=frameEntrySpot=undefined;const rawDt=clock.getDelta();frameCount++;frameElapsed+=rawDt;frameLongest=Math.max(frameLongest,rawDt);const dt=Math.min(rawDt,.04)*(exitQaMode?(exitQaPaused?0:.25):1)*(vehicleVisualQa?.timeScale||1);if(document.hidden||waterImpactCapture.paused)return;carQa?.update(dt,carState);
 ...
 if(hero&&!occupiedSeat&&!transition&&!jump&&!heroBlast&&!verticalNavigation.active)postureMotion=stepHeroPosture(heroPosture,dt,{canOccupyHeight:height=>canOccupyPostureHeight(height)});
 ...
 heroCover.update(dt,{direction:camera.getWorldDirection(new THREE.Vector3()),aiming,firing:triggerHeld||triggerPressed});
 if(heroCover.active){const forward=camera.getWorldDirection(new THREE.Vector3()),aimYaw=Math.atan2(forward.x,forward.z),aimPitch=Math.asin(THREE.MathUtils.clamp(forward.y,-.958,.958));hero.update(0,false,false,currentWeapon.id==='none'?null:currentWeapon,heroCover.mode==='aimed'?{aimYaw,aimPitch}:{aimYaw:hero.object.rotation.y,aimPitch:0},{posture:heroPosture});heroCover.pose({aimYaw,aimPitch});}
 controls.update();updateAimCamera(dt);clampLandscapeView();clampBuildingCamera();syncWorldWeapon();updateVehicleWindowFire(dt);const combat=updateCombat(dt,moved,running);
 if(hero&&!transition){
  ...
  else {if(combat.active&&!heroCover.active)hero.object.rotation.y=combat.aim.aimYaw;hero.update(dt,moved,running,currentWeapon.id==='none'?null:currentWeapon,heroCover.active&&heroCover.mode!=='aimed'?{aimYaw:hero.object.rotation.y,aimPitch:0}:combat.active?combat.aim:{}, {posture:heroPosture,slowWalking,motionSpeed:heroCover.active?1.25:undefined,reloadProgress:combat.reloadProgress,action:artistAction.action})}
 }
 if(heroCover.active)heroCover.pose(combat.aim);performanceProbe?.mark('heroCombat');
 if(occupiedSeat)updateVehicleWindowFire(0,{basePose:false});
 artistUpdate(dt,moved,running);performanceProbe?.mark("heroSurface");updateNpcPopulation(dt);performanceProbe?.mark("npc");worldWalkMelee?.update(artistAction);npcGallery?.update(dt);emitCombatShots(combat);glass.update(dt);const now=performance.now();if(now-lastCull>250){for(const node of instances)node.visible=node.position.distanceToSquared(controls.target)<220*220;staticRenderBatches?.update({focus:controls.target,maxDistance:220});explorationDecor?.update?.({focus:controls.target,maxDistance:420});sun.position.copy(controls.target).add(new THREE.Vector3(-75,130,90));sun.target.position.copy(controls.target);stableEntryLights?.update();lastCull=now}streetLighting.update({focus:hero?.object.position||controls.target,night:environmentNight});performanceProbe?.mark("cullLights");explorationMap?.setVisible($('scene-menu').hidden||explorationMap.expanded);if(hero)explorationMap?.update({position:hero.object.position,yaw:hero.object.rotation.y,vehicles:explorationMinimapVehicles(),trains:explorationMinimapTrains(),actors:explorationNpcMarkers()});updateWaterInteractions(dt);waterVapor?.update(dt,fleet?.records||[],hero?.object.position);captureWaterInspection(dt);environmentVisuals?.update({dt,time:now*.001,focus:hero?.object.position||controls.target,camera,...grassHeroContact(),actorRadius:.65});performanceProbe?.mark("mapEnvironment");const indoorVisibility=indoorHeroVisibility.update(!!indoorCameraFrame?.hideHead);if(now-indoorCameraDiagnosticsAt>=100){indoorCameraDiagnosticsAt=now;document.body.dataset.indoorCamera=JSON.stringify({...indoorVisibility,mode:indoorCameraFrame?.mode||'orbit',position:camera.position.toArray(),feet:hero?.object.position.toArray(),eyeHeight:postureEyeHeight()});}vehicleVisualQa?.applyCamera(camera,controls);vehicleImpactView.render(camera,car,occupiedSeat&&!transition?fleet?.active?.impactReaction?.sample():null,()=>{renderWeaponView(renderer,scene,camera,combat.allowed?combat.recoil:null);});updateWaypointDistance();frameInteraction=undefined;updateCarPrompt();updateBuildingPrompt();
 ...
 if(now-lastStats>1000){
  ...
  if(hero){
   ...
   if($('building-qa-status'))$('building-qa-status').textContent=`${current?'Внутри комнаты':'Снаружи'} · пол ${heroGroundHeight(hero.object.position.x,hero.object.position.z).toFixed(2)} · герой ${hero.object.position.y.toFixed(2)} · ${buildingQaMove?'идёт':'стоит'} · камера ${cameraBeforeBuildingClamp?'перед стеной':'свободна'}`;
...
function createHudHeroSurface(target){return createArtist14Surface({THREE,context:target.artistContext(),scene,applySwim:(sample,ctx)=>{if(worldHealthFrame?.snapshot.dead)return;if(transition?.waterExit&&transition.phase==='body'){artistPose.swim(sample,ctx);return;}if(occupiedSeat||transition||jump||heroBlast)return;if(sample.blend>0||exitSwimHandoff)ctx.object.position.y=heroSwimHeight(ctx.object.position.x,ctx.object.position.z,sample);artistPose.swim(sample,ctx);},applyReaction:(sample,ctx)=>{if(!occupiedSeat&&!transition&&!heroBlast&&!verticalNavigation.active)artistPose.reaction(sample,ctx);}})}
function canApplyHudHero(){return !!hero&&!busy&&!occupiedSeat&&!transition&&!jump&&!heroBlast&&!verticalNavigation.active&&!artistBusy()&&!artistSwimming()&&artistAction.action.type==='none'&&!triggerHeld&&!aiming}
...
function initPlayerDossier(){
 if(walkPlayerHud)return;walkPlayerHud=createWalkHudController({THREE,loader,cloneSkeleton:cloneNpcSkeleton,document,window,getBridge:()=>npcBridge,getHero:()=>hero,getNpcActor:id=>npcPopulation?.getActor(id)||npcPopulation?.getActor('npc_'+id)||npcPopulation?.getActor('npc_unique_'+id),canSwapHero:canApplyHudHero,swapHero:applyHudHero,onInputLock(){releaseControls();setArsenalOpen(false);setFreeMouse(false);if(document.pointerLockElement)document.exitPointerLock()},onActionError:reason=>{document.body.dataset.walkHudActionError=reason}});
}
