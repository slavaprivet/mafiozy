...
async function refresh(){if(renderFreezeQa?.active||busy||jump||insideBuilding()||buildingQaMove)return;const startedAt=performance.now();busy=true;$('status').textContent='Читаю свежую расстановку…';
 ...
 if(staticRenderBatching)try{staticRenderBatches=createStaticRenderBatches({THREE,root:content,instances,minInstances:3,maxDistance:220,localMatrixOptimization:staticMatrixOptimization,shadowCensus:performanceProbe?.tagShadowBatch});document.body.dataset.staticRenderBatches=JSON.stringify(staticRenderBatches.stats())}catch(error){console.warn('Static render batching disabled',error);staticRenderBatches=null;document.body.dataset.staticRenderBatches=JSON.stringify({disabled:true,error:error.message})}
