...
export function resolveBuildingCameraPosition({THREE, from, desired, objects, padding = .28, ceilingY = Infinity}) {
  ...
  const direction = delta.divideScalar(distance), up = new THREE.Vector3(0, 1, 0);
  const right = new THREE.Vector3().crossVectors(direction, up).normalize();
  const cameraUp = new THREE.Vector3().crossVectors(right, direction).normalize();
  let allowed = distance;
  for (const offset of [new THREE.Vector3(), right.clone().multiplyScalar(.16), right.clone().multiplyScalar(-.16), cameraUp.clone().multiplyScalar(.16), cameraUp.clone().multiplyScalar(-.16)]) {
    const ray = new THREE.Raycaster(from.clone().add(offset), direction, .015, distance + padding);
...
export function createBuildingEntry({THREE, visual, instance, metresPerCell = 4.1}) {
  applyBuildingSizeTransform(visual,instance,THREE);
  ...
  const api = {
    report, object: generated, visual, instance,contentRoot,
