...
tyres.update({speed:8,distance:61,yaw:0},.1);assert(!wheel.tire.visible&&wheel.hub.visible);assert.equal(detached,1);assert.equal(wheel.rollingRadius,.24);assert(tyres.stats().sparks>0&&tyres.stats().sparks<=64);
...
tyres.reset();assert(wheel.tire.visible&&wheel.tire.scale.x===1&&wheel.pivot.position.y===.43);assert.equal(tyres.effects.speedFactor,1);tyres.dispose();assert.equal(tyres.object.parent,null);
