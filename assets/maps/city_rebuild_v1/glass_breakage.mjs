...
export function createGlassBreakage(THREE,scene,options={}){
 ...
 const registry=new Map(),decorations=[],shards=[],group=new T.Group();group.name='World_Glass_Shards';group.userData.breakableGlass=false;scene.add(group);
 ...
 function decorate(record,target,panel,localPoint,normal,key,random){
  ...
  const entry={root,cracks,edges,geometries:[cg,rg],age:0,target,panel,key,record,fractured:false,surface:s};decorations.push(entry);while(decorations.length>limits.panels){const old=decorations.shift();fracture(old);removeDecoration(old)}return entry;
 }
 ...
 function update(dt){
  ...
  for(const d of decorations){d.age+=dt;if(d.age>=.085)fracture(d)}
  group.updateWorldMatrix(true,false);sceneInverse.copy(group.matrixWorld).invert();for(let i=0;i<limits.shards;i++){
   const s=shards[i];if(!s||s.life<=0){particles.setMatrixAt(i,zero);continue}s.life-=dt;s.velocity.y-=9.81*dt;s.position.addScaledVector(s.velocity,dt);const floor=typeof options.groundHeight==='function'?options.groundHeight(s.position.x,s.position.z):options.groundY??.035;
   if(s.position.y<floor+s.size*.1){s.position.y=floor+s.size*.1;if(Math.abs(s.velocity.y)>.4)s.velocity.y=-s.velocity.y*.24;else s.velocity.y=0;s.velocity.x*=Math.exp(-dt*8);s.velocity.z*=Math.exp(-dt*8);s.spin.multiplyScalar(Math.exp(-dt*9))}
 ...
 function reset(root=null){
  ...
  for(let i=decorations.length-1;i>=0;i--)if(selected.has(decorations[i].record.mesh)){removeDecoration(decorations[i]);decorations.splice(i,1)}
  ...
  for(let i=0;i<limits.shards;i++)if(shards[i]&&selected.has(shards[i].source)){shards[i]=null;particles.setMatrixAt(i,zero)}particles.instanceMatrix.needsUpdate=true;totalBroken=[...registry.values()].reduce((sum,r)=>sum+r.broken.size,0);
 }
 ...
 function stats(){return {registeredMeshes:registry.size,brokenPanels:totalBroken,decoratedPanels:decorations.length,activeShards:shards.filter(s=>s&&s.life>0).length,limits:{...limits}}}
