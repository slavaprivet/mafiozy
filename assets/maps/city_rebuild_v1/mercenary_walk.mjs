...
export function createMercenaryWalk({THREE,document,parent=document.body,camera,scene,getHost,getFleet,getTraffic,getNpcs,getBuildings,getRoots,getPickRoots,getFocus=()=>null,getHero=()=>null,canMove,groundHeight=()=>0,isBlocked=()=>false,onOpenChange=()=>{},onFollowGesture=()=>{},onExplosion=()=>{},createDamage=createVehicleDamage}){
 ...
 const pickingProbe=createMercenaryPickingProbe({enabled:pickingQaParams.get('perfqa')==='1'&&pickingQaParams.get('mercenarypickqa')==='1',document});
 ...
 const targets=createMercenaryTargets({THREE,camera,getRoots,getPickRoots,getFleet,getTraffic,getNpcs,getBuildings,pickingProbe,getVehicleLock:id=>globalThis.MafioziMercenaryVehicleLocks?.get(id),
 ...
 function updateAimSelection(){
  ...
  const pickStarted=performance.now();pickingProbe?.begin(pickStarted);let blocked=false,failed=true;
  try{
  ...
  }finally{const ended=nativeSample?performance.now():0;pickingProbe?.finish({blocked,failed});nativePickingQa?.finish(nativeSample,{started:pickStarted,ended,blocked,failed});}
 }
