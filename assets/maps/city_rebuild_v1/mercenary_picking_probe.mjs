...
export function createMercenaryPickingProbe({THREE,enabled=false,document:doc,now=()=>performance.now(),intervalMs=1000,capacity=120}={}){
 ...
 function begin(time){
  ...
  active={version:2,kind:'natural-hover-qa',sampleId:++samples,timings:{},roots:[],candidateCount:0,intersections:0,raycastCalls:0,methodCalls:{raycast:0,_computeIntersections:0},instrumentationComplete:true};
 ...
 // Nested stages are inclusive components of resolveMs, not additional work.
 function measure(name,fn){if(!active)return fn();const sample=active,start=now();try{return fn();}finally{sample.timings[name]=(sample.timings[name]||0)+Math.max(0,now()-start);}}
 function count(name){if(active){active.counts??={};active.counts[name]=(active.counts[name]||0)+1;}}
 ...
 function intersect(raycaster,roots,getKind=()=>null){
  ...
  function wrap(node,record){
   // Exact triangle indexes guard the identity of Mesh.raycast. Wrapping that
   // entry point would force their slow fallback only in measured samples.
   // Time their installed narrowphase instead, leaving the guard untouched.
   const indexedMesh=THREE?.Mesh&&node.raycast===THREE.Mesh.prototype.raycast&&
    Object.hasOwn(node,'_computeIntersections')&&typeof node._computeIntersections==='function';
   const key=indexedMesh?'_computeIntersections':'raycast';
   const original=node[key];if(typeof original!=='function')return;
   ...
   function wrapped(...args){
    ...
    finally{const elapsed=Math.max(0,now()-start);stack.pop();if(stack.length)stack.at(-1).childMs+=elapsed;record.raycastMs+=Math.max(0,elapsed-frame.childMs);record.calls++;record.methodCalls[key]++;sample.raycastCalls++;sample.methodCalls[key]++;}
   }
   try{Object.defineProperty(node,key,descriptor?{...descriptor,value:wrapped}:{value:wrapped,writable:true,configurable:true,enumerable:true});restores.push({node,key,descriptor,wrapped});}
   catch{sample.instrumentationComplete=false;}
  }
  function restore(){
   const start=now();
   for(let i=restores.length-1;i>=0;i--){const {node,key,descriptor,wrapped}=restores[i];
    ...
    if(node[key]!==wrapped){sample.instrumentationComplete=false;continue;}
    if(descriptor)Object.defineProperty(node,key,descriptor);else delete node[key];
   }
   sample.timings.restoreMs=Math.max(0,now()-start);sample.cursor=now();
  }
  try{
   for(const root of roots){const record={uuid:root?.uuid??null,name:root?.name||'',category:mercenaryPickingRootCategory(root,getKind(root)),raycastMs:0,calls:0,methodCalls:{raycast:0,_computeIntersections:0}};records.push(record);
   ...
   try{const hits=raycaster.intersectObjects(roots,true);sample.intersections=hits.length;return hits;}
    ...
    sample.timingScope='mixed: indexed Mesh narrowphase; other raycast methods; bounds/traversal/sort/probe overhead in residual';
   }
  }finally{restore();}
 }
 ...
 return {begin,mark,measure,count,selected,intersect,finish,get active(){return !!active;},dispose(){disposed=true;active=null;history.length=0;if(doc?.documentElement?.dataset)delete doc.documentElement.dataset.mercenaryPickingProfile;}};
}
