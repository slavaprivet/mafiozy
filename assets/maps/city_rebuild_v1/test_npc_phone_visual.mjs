...
test('shares one geometry/material, follows the right-hand socket and has no pick or shadow cost',()=>{
 ...
 const size=one.geometry.boundingBox.getSize(new THREE.Vector3());assert(Math.abs(size.x-.105)<1e-6);assert(Math.abs(size.y-.285)<1e-6);assert(size.z<.04);assert.equal(one.material.vertexColors,true);assert.equal(one.material.color.getHex(),0xffffff);assert.equal(one.material.toneMapped,false);assert.equal(one.material.transparent,false);
