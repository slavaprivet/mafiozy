import {createShadowSubmissionCensus,tagShadowBatch} from './shadow_submission_census.mjs';
...
export function createWalkPerformanceProbe({renderer,scene,document,getStartup,now=()=>performance.now()}){
 const shadowCensus=createShadowSubmissionCensus();
 const clearShadowCensus=()=>{shadowCensus.reset();delete document.body.dataset.walkShadowSubmissions;};
 ...
 function reset(){clearShadowCensus();samples.length=0;gpuSamples.length=0;previous=0;frameNumber=0;publishAt=0;capture=false;census={};renderGroups={};materialGroups={};clearQueries();}
 ...
 renderer.renderBufferDirect=wrappedDirect=function(camera,renderScene,geometry,material,object,group){
  ...
  try{return originalDirect.call(this,camera,renderScene,geometry,material,object,group)}finally{
   if(directProfiling){const submitElapsed=now()-begin;if(renderDepth)addTime(shadowDepth?'renderShadowSubmit':'renderMainSubmit',submitElapsed);
    // Three's WebGLShadowMap deliberately passes null as renderScene. The
    // active shadow/render depths are the ownership proof for this scene.
    if(shadowDepth&&renderDepth)shadowCensus.add(object,renderer.info.render.calls-before,renderer.info.render.triangles-beforeTris);
 ...
 return {snapshot,tagShadowBatch,begin(){shadowCensus.reset();start=last=now();for(const key of Object.keys(frameDraws))frameDraws[key]=0;for(const key of Object.keys(stages))delete stages[key];},mark(name){const end=now();stages[name]=(stages[name]||0)+end-last;last=end;},end(){
  ...
  if(!disposed)document.body.dataset.walkShadowSubmissions=JSON.stringify(shadowCensus.snapshot({frame:frameNumber,enabled:directProfiling,ready,expectedCalls:frameDraws.shadow,expectedTriangles:frameDraws.shadowTriangles}));
 },reset,getDrawProfilingEnabled(){return directProfiling&&!disposed},setDrawProfilingEnabled(enabled){if(disposed)return false;enabled=!!enabled;if(enabled!==directProfiling){directProfiling=enabled;delete stages.renderMainSubmit;delete stages.renderShadowSubmit;reset();}return directProfiling;},getGpuTimingEnabled(){return gpuTimingEnabled&&!disposed},setGpuTimingEnabled(enabled){if(disposed)return false;enabled=!!enabled&&!!timer;if(enabled!==gpuTimingEnabled){gpuTimingEnabled=enabled;gpuTimingError=null;reset();}return gpuTimingEnabled;},measure(name,run){const begin=now();try{return run()}finally{stages[name]=(stages[name]||0)+now()-begin;}},dispose(){if(disposed)return;disposed=true;clearShadowCensus();gpuTimingEnabled=false;if(renderer.renderBufferDirect===wrappedDirect)renderer.renderBufferDirect=originalDirect;if(renderer.render===wrappedRender)renderer.render=originalRender;if(wrappedMatrix&&scene.updateMatrixWorld===wrappedMatrix)scene.updateMatrixWorld=originalMatrix;if(wrappedShadow&&renderer.shadowMap.render===wrappedShadow)renderer.shadowMap.render=originalShadow;clearQueries();}};
}
