...
let obstacleQueries=0;assert.equal(effects.shoot(pistolShot,{origin,ejectionOrigin:new THREE.Vector3(.2,1,0)},target,()=>{obstacleQueries++;return [obstacle]}),true);
...
for(let i=0;i<20;i++)effects.update(.02);assert.ok(obstacleQueries>0,'projectiles may query a conservative broadphase each frame');assert.equal(effects.stats().activeProjectiles,0);assert(effects.stats().totalImpacts>=1,'moving projectile must sweep into scene geometry');
