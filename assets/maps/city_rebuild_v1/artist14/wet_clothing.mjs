import {surfaceMeshRecord,validateSurfaceMesh,encodeWetBytes,decodeWetBytes} from '../npc_surface_state.mjs';
...
export function createWetClothing(THREE,{drySeconds=WET_DRY_SECONDS,sampleInterval=.08}={}) {
  ...
  function snapshot(model){attach(model);const record=records.get(model),root=model.root||model;return {version:1,entries:record.meshes.map(e=>({...surfaceMeshRecord(root,e.mesh),count:e.wet.count,data:encodeWetBytes(e.wet.array)}))};}
  function validateSnapshot(model,data){
    attach(model);const record=records.get(model),root=model.root||model;
    if(data?.version!==1||!Array.isArray(data.entries)||data.entries.length!==record.meshes.length)throw Error('Invalid wet state version/mesh count');
    const seen=new Set();return data.entries.map(item=>{const mesh=validateSurfaceMesh(root,item),entry=record.meshes.find(e=>e.mesh===mesh);if(!entry||seen.has(mesh)||item.count!==entry.wet.count)throw Error('Wet mesh/count mismatch');seen.add(mesh);return {entry,bytes:decodeWetBytes(item.data,item.count)};});
  }
  function restore(model,data,{elapsedSeconds=0}={}){
