// Camera-owned exact triangle index for large, immutable plain building meshes.
...
export function installBuildingCameraTriangleIndex({THREE,mesh,minTriangles=1024}={}){
 const prototype=THREE?.Mesh?.prototype,geometry=mesh?.geometry;
 const plain=()=>mesh?.isMesh&&!mesh.isInstancedMesh&&!mesh.isSkinnedMesh&&!mesh.isBatchedMesh&&
 ...
 if(!Number.isInteger(minTriangles)||minTriangles<1||!plain()||mesh._computeIntersections!==prototype._computeIntersections||
 ...
 if(total/3<minTriangles||own&&(!own.writable||own.get||own.set)||!own&&!Object.isExtensible(mesh))return null;
 ...
 const stats={triangles:triangleCount,nodes:nodes.length,queries:0,fallbacks:0,disabledQueries:0,enabled:true,disposed:false,lastCandidates:0,lastBoxTests:0,candidateTests:0,boxTests:0};
 ...
 function accelerated(raycaster,hits,ray){
  ...
  candidates.sort((a,b)=>a-b);stats.lastCandidates=candidates.length;stats.candidateTests+=candidates.length;stats.boxTests+=stats.lastBoxTests;
 ...
 function dispose(){
  if(stats.disposed)return;geometry.removeEventListener('dispose',dispose);
  ...
  stats.enabled=false;stats.disposed=true;nodes=null;order.length=0;stack.length=0;candidates.length=0;
  facade.geometry.attributes=null;facade.geometry.index=null;facade.geometry.groups=null;facade.material=null;facade.matrixWorld=null;facade.getVertexPosition=null;
 }
