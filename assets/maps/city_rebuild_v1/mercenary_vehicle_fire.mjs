...
export function createMercenaryVehicleFirePose({THREE,walker}={}){
 ...
 const handReturn=['l','r'].map(side=>({side,p:new THREE.Vector3(),q:new THREE.Quaternion()}));let lastPoseBlend=0,lastTwoHanded=false;
 // Lift the arms before crossing the sill; reverse that order on return.
 ...
 const poseContext={...context,rotateAdd(name,...args){context.rotateAdd(name,...args.map(v=>v*poseBlend/Math.max(.001,armBlend)));if(name==='head')upperLean();}};
 function braceFreeHand(weapon){
  ...
  target.lerp(context.worldPosition('hand_'+hand),1-riseBlend);weapon?.getWorldPosition(gunPosition);weapon?.getWorldQuaternion(gunRotation);
 ...
 function reset(){if(car&&seatId)setVehicleWindowOpen(car,seatId,false);car=seatId=frame=result=intentKey=null;blend=poseBlend=riseBlend=armBlend=lastPoseBlend=0;hasMuzzle=false;}
 function apply(dt,{binding,weapon,intent,readiness,allowed=true}={}){
  ...
  if(!presentation&&allowed&&car===nextCar&&seatId===binding?.seatId&&binding.phase==='drive'&&blend>0){
   setBlend(blend-Math.max(0,dt)/.22);result={canFire:false,reason:'Возвращаемся в салон'};
   if(blend>0){const c=context,side=frame.side;c.rotateAdd('chest',-.10*poseBlend,side*.92*poseBlend,-side*.46*poseBlend);c.rotateAdd('neck',.08*poseBlend,side*.18*poseBlend,side*.12*poseBlend);c.rotateAdd('head',.02*poseBlend,side*.12*poseBlend,side*.15*poseBlend);upperLean();for(const hand of handReturn){position.copy(hand.p);car.object.localToWorld(position);position.lerp(c.worldPosition('hand_'+hand.side),1-Math.min(1,riseBlend/Math.max(.001,lastPoseBlend)));car.object.getWorldQuaternion(rotation).multiply(hand.q);c.reachPalm(hand.side,position,rotation);}return result;}reset();return null;
  }
  ...
  if(readyOnly)direction.set((frame?.side||0)*Math.cos(binding.yaw),0,-(frame?.side||0)*Math.sin(binding.yaw));
  else direction.set(intent.targetPosition.x,intent.targetPosition.y,intent.targetPosition.z).sub(seatPoint);
  const planShot=()=>{const p=planVehicleWindowShot({vehicleState:{yaw:binding.yaw},seatId,aimDirection:direction,weaponId,frame});if(p.allowed){const long=!!(weapon.twoHanded??weapon.userData?.twoHanded);p.localMuzzle.z=Math.min(frame.max[2]-.24,p.localMuzzle.z+(long?.3:.18))-(long?.12*(1-poseBlend):0);}return p;};
  let plan=planShot();
  ...
  for(const hand of handReturn){hand.p.copy(context.worldPosition('hand_'+hand.side));car.object.worldToLocal(hand.p);context.bones['hand_'+hand.side].getWorldQuaternion(hand.q);hand.q.premultiply(car.object.getWorldQuaternion(rotation).invert());}lastPoseBlend=riseBlend;
