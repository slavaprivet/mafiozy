...
for(const assetId of [...new Set(placement.instances.map(i=>i.assetId))]){
 ...
 const r=createGlassBreakage(T,world,{maxShards:16,maxDecoratedPanels:2});r.prepare(visual);let count=0,hitNode=null,panel=null,glassMeshes=0;
 visual.traverse(n=>{if(n.isMesh){const mats=Array.isArray(n.material)?n.material:[n.material];if(mats.some(m=>isBreakableGlass(n,m)))glassMeshes++;const a=discoverGlassPanels(n);count+=a.panels.length;if(!hitNode&&a.panels.length){hitNode=n;panel=a.panels[0]}}});
 ...
 }if(glassMeshes)assert.ok(count>0,`${assetId} glass must be discovered with runtime windows`);else assert.equal(count,0,`${assetId} has no authored breakable glass and must not gain a false panel`);rows.push({assetId,panels:count,glassMeshes});r.dispose();windows?.dispose();
}
