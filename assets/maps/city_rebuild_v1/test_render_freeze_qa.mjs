...
test('shadow comparison button and hold hooks restore on timeout and error',()=>{
 const events=[],f=fixture({hooks:{onHoldStart:()=>events.push('start'),onHoldEnd:()=>events.push('end')}}),button=f.document.createElement('button');button.id='vehicle-shadow-qa';f.document.body.append(button);let enabled=true;button.onclick=()=>{enabled=!enabled;};
 f.state.snapshot={npc:{pending:1}};f.start();assert.deepEqual(events,[]);f.state.snapshot={};f.start();assert.deepEqual(events,['start']);f.document.fire('click',button);assert.equal(enabled,false);assert.equal(f.document.fire('keydown',button,{code:'KeyW'}).stopped,true);
...
test('actual static matrix walk setup remains opt-in and updates exposed counts only on comparison',()=>{
 ...
 assert(source.includes('staticRenderBatches=createStaticRenderBatches({THREE,root:content,instances,minInstances:3,maxDistance:220,localMatrixOptimization:staticMatrixOptimization})'));
});
...
const events=[],context={requestAnimationFrame(){},frameInteraction:null,frameEntrySpot:null,clock:{getDelta:()=>.016},frameCount:0,frameElapsed:0,frameLongest:0,exitQaMode:false,vehicleVisualQa:null,document:{hidden:false},waterImpactCapture:{paused:false},renderIsolationQa:null,renderFreezeQa:{render(fn){events.push('freeze');if(context.held){fn();return true}return false}},renderWeaponView(){events.push('render')},renderer:{},scene:{},camera:{},carQa:{update(){events.push('car')}},carState:{},performanceProbe:{begin(){events.push('begin')},mark(){events.push('mark')},measure(name,fn){return fn()}},updateWorldWalkHealth(){events.push('health')},walkPlayerHud:{update(){events.push('hud')}},performance:{now:()=>0},updateMercenaries(){events.push('mercenaries')},hudInputBlocked:()=>false,releaseControls(){events.push('release')},held:true};
 Object.assign(context,{coverElapsedTime,coverMovementScale:1});
 vm.createContext(context);vm.runInContext(prefix+'\n}',context);vm.runInContext('frame()',context);assert.deepEqual(events,['freeze','render']);
