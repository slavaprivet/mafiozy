...
test('real source immutable, deterministic island placement has four ecological regions',()=>{assert.equal(JSON.stringify({topology,buildings,existingDecor}),original);assert.deepEqual(plan,planEnvironmentGrass(options));assert.ok(plan.tufts.length>=25000&&plan.tufts.length<=GRASS_LIMITS.maxTufts);assert.ok(plan.patches.length>700);for(const count of Object.values(plan.stats.counts))assert.ok(count>600);assert.ok(plan.tufts.some(t=>t.height>.75));assert.ok(plan.tufts.some(t=>t.height<.25));});
const rects=[];
...
test('solid 7/9 blade meshes and rounded shrubs, finite normals and shaders use pinned root coordinates',()=>{
  ...
  assert.equal(geometries.size,3);for(const g of geometries.values()){assert.ok(g.attributes.normal.array.every(Number.isFinite));assert.ok(g.boundingBox.max.z-g.boundingBox.min.z>.1);assert.ok(g.index.count/3<=171);assert.equal(g.userData.solidGeometry,true);}
...
test('dense worst case caps actual triangles and fades before the selection cutoff',()=>{
  ...
  dense.update({time:0,focus:{x:15,z:12},force:true});assert.equal(dense.stats.visibleTufts,Math.floor(GRASS_LIMITS.maxVisibleTriangles/108));assert.ok(dense.stats.visibleTriangles<=90000);assert.ok(dense.uniforms.uGrassFadeFar.value<105);assert.equal(grassDistanceFade(dense.uniforms.uGrassFadeFar.value,dense.uniforms.uGrassFadeNear.value,dense.uniforms.uGrassFadeFar.value),0);dense.dispose();
});
test('GPU contact anchors roots, compresses tips and recovers completely',()=>{
  ...
  assert.ok(a.x>.45&&a.y<-.5);assert.ok(b.x>0&&b.x<a.x);assert.equal(Math.hypot(c.x,c.y,c.z),0);
...
test('pressed blades retain visible middle/tip and underground egress without a tall arch',()=>{
  ...
  for(const style of ['grass','reed']){
    ...
    for(const height of [.23,.65,1.01])for(const distance of [0,.2,.45,.75])for(let i=0;i<positions.count;i++){
      ...
      assert.ok(pressed>=-1e-8&&pressed<=.030001,'flat blade remains at most 15mm above terrain');
      if(progress>.54)assert.ok(pressed-.015>=.009,'laid middle/tip must stay visible above terrain');
      if(progress<.23)assert.ok(pressed-.015<-.014,'base egress stays under terrain');
...
test('combined hero pressure and wind fit the actual bent geometry envelopes',()=>{
  ...
  for(const chunk of render.object.children)for(const mesh of chunk.children)if(mesh.visible)for(let i=0;i<Math.min(12,mesh.count);i++){
    ...
    for(let j=0;j<p.count;j++)for(const time of[0,1,3]){
      v.fromBufferAttribute(p,j).applyMatrix4(matrix);const wind=grassWindOffset({x:t.x,z:t.z,time,height:t.height,weight:p.getY(j)}),contact=grassContactOffset({x:t.x,z:t.z,time,height:t.height,weight:p.getY(j),flex,trail:[{x:t.x-.001,z:t.z,time,radius:1.05}]});
      v.x+=wind.x*flex+contact.x;v.z+=wind.z*flex+contact.z;v.y+=contact.y;
