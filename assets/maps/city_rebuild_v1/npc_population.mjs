...
export function normalizeNpcSnapshot(src,{time,sourceNowMs,worldScale=4.1,originR=0,originC=0,groundHeight=()=>0,waterAt=()=>null}={}){
 ...
 const weapon=npcWeaponId(src.weapon),unarmed=weapon==='none',activeMelee=shotAge>=0&&shotAge<duration&&Number(src._shotAt)>0;
...
export async function createNpcPopulation({THREE,scene,loader,cloneSkeleton,bridge=null,groundHeight=()=>0,waterAt=()=>null,worldScale=4.1,originR=0,originC=0,maxActors=72,cacheSeconds=8,maxCachedActors=maxActors*2,interpolationSeconds=.1,getFocus=null,renderLOD=true,renderDistance=100,shadowDistance=35,getInspectId=()=>null,creationBudget=Infinity,getVehicle=null,onSnapshot=null,onBeforePose=null,profile=false}={}){
 ...
 const cpu={samples:0,creationFrames:0,last:null,totals:{}};
 ...
 function make(src,sourceNowMs=sourceClock()){
  ...
  return {shadowNodes,shadowNear:null,gpuVisible:true,gaitSample:{x:initial.position.x,z:initial.position.z},gaitDistance:0,lastGaitDistance:0,motionSample:{position:{...initial.position},time:lastTime},motionSpeed:0,motionValid:false,actor,descriptor,source:src,lastSeen:lastTime,inSnapshot:true,poseElapsed:0,lastPoseAt:lastTime,visualYOffset:actor.object.position.y-initial.position.y,weaponId:null,deathKey:saved?.deathKey||null,renderTransform:{position:{...initial.position},yaw:initial.yaw},motion:{from:{...initial.position},to:{...initial.position},yawFrom:initial.yaw,yawTo:initial.yaw,at:lastTime}};
 }
 ...
 function configure(record,row,time,sourceNowMs=sourceClock()){
   record.source=row;record.lastSeen=time;const weapon=npcWeaponId(row.weapon);if(record.weaponId!==weapon){record.actor.mountWeapon(weapon==='none'?null:template(weapon));record.weaponId=weapon;const previous=new Map(record.shadowNodes.map(item=>[item.node,item]));record.shadowNodes=[];record.actor.object.traverse(node=>{if(node.isMesh)record.shadowNodes.push(previous.get(node)||{node,cast:node.castShadow,receive:node.receiveShadow})});record.shadowNear=null;}
 ...
 function sync(rows,time=lastTime,sourceNowMs=sourceClock(time)){
  ...
  for(const row of latest){
   ...
   if(!record){if(creationBudget!==Infinity)continue;record=make(row,sourceNowMs);actors.set(row.id,record);}else{
    if(!record.inSnapshot){const state=record.actor.saveSurfaceState();record.actor.restoreSurfaceState(state,{time,elapsedSeconds:Math.max(0,time-state.surface.time)});}
    const current=renderTransform(record,time),next=normalize(row,time,sourceNowMs),teleport=Math.hypot(next.position.x-current.position.x,next.position.y-current.position.y,next.position.z-current.position.z)>8;
    if(!record.inSnapshot){record.gaitSample={x:current.position.x,z:current.position.z};record.gaitDistance=0;}
    const measured=measureNpcMotion(record.motionSample,next.position,time,{reentry:!record.inSnapshot,teleport});record.motionSample=measured.sample;record.motionSpeed=measured.speed;record.motionValid=measured.valid;const measuredSourceNow=sourceNowMs,previousSourceNow=record.motionMeasurement?.sourceNowMs;record.motionMeasurement={dt:measured.dt,distance:measured.distance,sourceNowMs:measuredSourceNow,sourceDt:Number.isFinite(previousSourceNow)?(measuredSourceNow-previousSourceNow)/1000:null};
    ...
    record.motion={from:teleport?{...next.position}:{...current.position},to:{...next.position},yawFrom:teleport?next.yaw:current.yaw,yawTo:next.yaw,at:time,duration};
 ...
 function update(dt,time){
  ...
  const clock=profile?()=>performance.now():()=>0,start=clock(),timings=profile?{source:0,snapshot:0,creation:0,traffic:0,pose:0,cache:0,total:0}:null;let stage=start,createdBefore=actors.size;
  ...
  for(const src of latest){
   ...
   lodCounts[tier]++;record.poseElapsed=Math.min(.25,record.poseElapsed+dt);const gpuVisible=!focus||distance<=renderDistance||src.id===inspectId,visible=gpuVisible&&!src.carried&&!src.evacuated;
   ...
   if(visible&&(interval===0||record.poseElapsed+1e-9>=interval)){
    ...
    if(gap>stepDt+.001&&gap>.25){const saved=record.actor.saveSurfaceState();record.actor.restoreSurfaceState(saved,{time:time-stepDt,elapsedSeconds:Math.max(0,time-stepDt-saved.surface.time)});}
    const normalized=normalize(src,time,sourceNow());normalized.motionSpeed=time-record.motionSample.time<=.5?record.motionSpeed:0;normalized.gaitDistance=record.gaitDistance;normalized.moving=record.gaitDistance>.00001;normalized.running=normalized.moving&&normalized.motionSpeed>HERO_POSTURES.stand.walkSpeed*1.15;normalized.slowWalking=normalized.moving&&!normalized.running&&normalized.motionSpeed<=HERO_POSTURES.stand.slowWalkSpeed*1.25;normalized.position=motion.position;normalized.yaw=motion.yaw;normalized.aim.aimYaw=motion.yaw;
    record.actor.update(stepDt,normalized);record.lastGaitDistance=record.gaitDistance;record.gaitDistance=0;record.visualYOffset=record.actor.object.position.y-motion.position.y;record.poseElapsed=0;record.lastPoseAt=time;lodCounts.posed++;
   }else{
  ...
  if(profile){timings.pose=clock()-stage;stage=clock();}
 ...
 function motionDiagnostics(){
  ...
  for(const row of latest){const record=actors.get(row.id);if(!record?.motionValid){if(record&&invalidSamples.length<5)invalidSamples.push({id:row.id,dt:record.motionMeasurement?.dt??null,distance:record.motionMeasurement?.distance??null,sourceDt:record.motionMeasurement?.sourceDt??null});continue;}const speed=lastTime-record.motionSample.time<=.5?record.motionSpeed:0;
   ...
   maxMetresPerSecond=Math.max(maxMetresPerSecond,speed);if(speed>.04){moving++;if(speed>HERO_POSTURES.stand.walkSpeed*1.15)running++;else walking++;}
 ...
 return {sync,update,receive,dispose,attach(next){bridge=next;lastSnapshotAt=-Infinity;},getActors:()=>latest.filter(src=>actors.has(src.id)).map(src=>({id:src.id,name:src.name,role:src.role,object:actors.get(src.id)?.actor.object,source:src})),getActor:id=>actors.get(String(id))?.actor,diagnostics:()=>({visible:latest.filter(row=>actors.get(row.id)?.actor.object.visible).length,seen:latest.filter(row=>actors.has(row.id)).length,pending:latest.filter(row=>!actors.has(row.id)).length,requested:latest.length,creationBudget,cached:actors.size,serialized:savedActors.size,source:bridge?'world-runtime':'external-snapshot',maxActors,maxCachedActors,cacheSeconds,renderLOD,renderDistance,shadowDistance,motion:motionDiagnostics(),lod:{...lodCounts},...(profile?{cpu:{samples:cpu.samples,creationFrames:cpu.creationFrames,last:cpu.last,meanMs:Object.fromEntries(Object.entries(cpu.totals).map(([key,value])=>[key,Math.round(value/Math.max(1,cpu.samples)*100)/100]))}}:{})})};
}
