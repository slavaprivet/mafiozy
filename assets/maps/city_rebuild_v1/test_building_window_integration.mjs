...
assert.equal(active.length,43);assert.equal(count,675,'room integration must retain every planned window');assert.ok(interiorDoorDraws<interiorDoors*2,'real room doors are material-batched instead of two draw calls each');const finishCache=interiorFinishCacheStats(),storeyResources=buildingStoreyResourceCacheStats();assert.equal(finishCache.refs,active.length*2,'every room keeps wall and floor finish leases until teardown');assert.ok(finishCache.entries<finishCache.refs&&finishCache.hits>0,'repeated palettes share immutable interior finish materials');assert.equal(storeyResources.refs,active.length,'every multi-storey entry leases the same immutable storey resources');assert.equal(storeyResources.entries,1,'all storeys share one material and unit-box resource bundle');const furnishingSources=[];for(const a of active)a.visual.traverse(node=>{if(/^Interior_Furnishings_/.test(node.name))furnishingSources.push(node)});assert.equal(furnishingSources.length,interiorFurnitureDraws);const staticBatches=createStaticRenderBatches({THREE:T,root:scene,instances:active.map(a=>a.group),minInstances:3}),staticStats=staticBatches.stats(),batchedFurnishingSources=furnishingSources.filter(mesh=>mesh.material.visible===false).length;assert.ok(staticStats.members>=interiorFurniture,'global batch includes every static furnishing instance');assert.equal(batchedFurnishingSources,furnishingSources.length,'every real furnishing source is rendered exactly once through the global batch');for(const mesh of furnishingSources)assert.equal(mesh.material.visible,false,'batched furnishing source stays raycastable but is not drawn twice');staticBatches.dispose();for(const mesh of furnishingSources)assert.equal(mesh.material.visible,true,'batch disposal restores original furnishing material');const cache=residentialWindowCacheStats();assert.ok(cache.refs>0&&cache.hits>0,'shared source windows must reuse cache');
