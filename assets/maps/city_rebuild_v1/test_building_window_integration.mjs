...
const placement=JSON.parse(fs.readFileSync(new URL('buildings_placement.v1.json',import.meta.url))),scene=new T.Scene(),glass=createGlassBreakage(T,scene),active=[];let count=0;
for(const id of Object.keys(WINDOW_PROFILES)){
 ...
 for(const item of items){
  ...
  const applied=createWindowedBuildingEntry({THREE:T,visual,instance:item});assert.ok(applied.windows&&applied.entry);assert.equal(applied.windows.group.visible,true);assert.equal(applied.entry.contentRoot.children.length,0);count+=applied.windows.report.windows;glass.prepare(visual);
...
assert.equal(active.length,43);assert.equal(count,675,'room integration must retain every planned window');const cache=residentialWindowCacheStats();assert.ok(cache.refs>0&&cache.hits>0,'shared source windows must reuse cache');
...
console.log('PASS 43 full rooms + 675 panes; individual glass hit, shared cache reuse, ordered restoration/refcount zero',cache);
