...
(function(){
 ...
 let core,professions,routeFactory,longFollowFactory,vehicleExplosionOwner=null,targets={},lastCandidates=-Infinity,lastSave=-Infinity,dismissing=false,ready=false,followAngle=null;
 ...
 function persist(saveGang=false){if(!local()||!core||restoreBlocked)return;try{const rows=core.getRoster().map(r=>{const m=raw(r.id);return m?{id:r.id,sourceBotId:m.sourceBotId,name:m.name,look:m.look,r:m.r,c:m.c,hp:m.hp,max_hp:m.max_hp,weapon:m.weapon,faction:m.faction,gangName:m.gangName,baseHp:m._mercenaryBaseHp,_mercenaryLifeGeneration:Number.isInteger(m._mercenaryLifeGeneration)&&m._mercenaryLifeGeneration>=0?m._mercenaryLifeGeneration:0,...(m._mercenaryVehicleExplosionFatal?{_mercenaryVehicleExplosionFatal:true,dead:true,deathConfirmed:true,deadAt:Number(m.deadAt)||0,_lifeState:'dead'}:{}),_mercenaryOrder:m._mercenaryOrder||'follow',_mercenaryRally:m._mercenaryRally||null,...(qaSupported()&&Number.isFinite(m._mercenaryQaRescueUntil)&&m._mercenaryQaRescueUntil>now()?{_mercenaryQaRescueUntil:Math.min(now()+600,m._mercenaryQaRescueUntil)}:{})}:null;}).filter(Boolean);const text=JSON.stringify({core:core.snapshot(),rows});localStorage.setItem(KEY,text);persistenceDiagnostics.save={ok:true,at:now(),ids:rows.map(r=>r.id),bytes:text.length};if(saveGang&&typeof _saveGang==='function')_saveGang();}catch(e){persistenceDiagnostics.save={ok:false,at:now(),error:String(e.message||e)};console.warn('Mercenary save failed',e);}lastSave=now();reportPersistence();}
 ...
 function detachSquadVehicleSeat(m){
  if(!m)return false;const transport=targets.squadTransport,claims=[],seen=new Set();
  for(const [vehicleId,seatId]of [[m._mercenaryVehicleId,m._mercenaryVehicleSeat],[m._mercenaryVehicleReservedId,m._mercenaryVehicleReservedSeat]]){
   const key=vehicleId&&seatId?String(vehicleId)+'\0'+String(seatId):'';if(!key||seen.has(key))continue;seen.add(key);claims.push({vehicleId,seatId});
  }
  for(const claim of claims)transport?.access?.({carId:claim.vehicleId,seatId:claim.seatId,phase:'release'});
  const keys=['_mercenaryVehicleSeat','_mercenaryVehicleId','_mercenaryVehicleChaseAt','_mercenaryVehiclePhase','_mercenaryVehicleProgress','_mercenaryVehicleReservedId','_mercenaryVehicleReservedSeat','_mercenaryVehicleChase','_mercenaryVehicleMove','_mercenaryVehicleCatchup','_mercenaryVehicleCatchupUntil','_mercenaryVehicleExitPending','_mercenaryVehicleDropState','_mercenaryVehicleExitPlan'];
  const changed=claims.length>0||keys.some(key=>Object.hasOwn(m,key));for(const key of keys)delete m[key];return changed;
 }
 ...
 function squadVehicleTick(roster,elapsed=0){
  ...
  for(const {m}of members)if(m._mercenaryHospital)detachSquadVehicleSeat(m);
 ...
 function effect(e){if(!local())return false;const armed=core.getAction(e.memberId);if(e.kind==='plant_bomb'&&armed?.armed&&armed.id===e.actionId)return targets.performEffect?.(e)||false;const m=raw(e.memberId);if(!m||!record(e.memberId))return false;if(e.kind==='hospitalize'){m.hp=0;m._mercenaryHospital=true;m._mercenaryPath=null;m._mercenaryMove=null;detachSquadVehicleSeat(m);return true;}if(e.kind==='discharge'){const door=typeof _hospitalDoor==='function'?_hospitalDoor():null;if(!door||!_npcBodyPassable(door.r,door.c))return false;detachSquadVehicleSeat(m);m.r=door.r;m.c=door.c;m.hp=m.max_hp;m._mercenaryDownAt=0;m.dead=false;m._mercenaryHospital=false;m._mercenaryAction=null;vehicleExplosionOwner?.beginNewLife(m.id);return true;}if(e.kind==='revive'){if(e.targetId==='player'){return revivePlayer(e);}if(vehicleExplosionOwner?.isReviveBlocked(e.targetId))return false;const ally=raw(e.targetId);if(!ally||!record(ally.id)||record(ally.id).status==='hospital'||ally.hp>0)return false;ally.hp=Math.max(1,Math.round(ally.max_hp*(e.stats?.reviveFraction||.35)));ally.dead=false;delete ally._mercenaryQaRescueUntil;ally._mercenaryDownAt=0;ally._mercenaryHospital=false;vehicleExplosionOwner?.beginNewLife(ally.id);return true;}if(e.kind==='intimidate'){const n=findNpc(e.targetId);if(!n||n.dead||n.hp<=0||n._empireBoss||n._guard||n._invulnerable)return false;const t=performance.now();if(typeof _npcApplyIntimidation==='function'){if(!_npcApplyIntimidation(n,m.r,m.c,t))return false;}else{n._intimidatedUntil=t+60000;n.snitching=false;n.snitchUntil=0;n._witnessCallUntil=0;n.panicUntil=t+8000;n.panicSrcR=m.r;n.panicSrcC=m.c;n._hostile=false;n._fighting=false;n._fightingMelee=false;n._panicStyle='flee';}n.cryText='Не трогай меня!';n.cryUntil=t+3000;return true;}return targets.performEffect?.(e)||false;}
 function restore(){restoredOnce=true;try{
  ...
  const result=core.restore(save.core);if(!result.ok)throw Error(result.reason||'core_restore_rejected');
   for(const row of save.rows){const rec=record(row.id);if(!rec)continue;let m=gang().find(g=>g.id===row.id||(String(g.sourceBotId)===String(row.sourceBotId)&&row.sourceBotId!=null));if(!m){if(gang().length>=Math.max(5,typeof GANG_MAX==='number'?GANG_MAX:5))continue;m={};gang().push(m);}Object.assign(m,row,{id:row.id,weapon:rec.weapon?.id||row.weapon||'pistol',walkPhase:0,ang:0,_nextShootAt:0,_threatUntil:0,_mercenaryBaseHp:row.baseHp||80,_mercenaryHospital:rec.status==='hospital',_mercenaryQaRescueUntil:qaSupported()&&Number.isFinite(row._mercenaryQaRescueUntil)?Math.min(now()+600,Math.max(0,row._mercenaryQaRescueUntil)):0});m._mercenaryLifeGeneration=Number.isInteger(row._mercenaryLifeGeneration)&&row._mercenaryLifeGeneration>=0?row._mercenaryLifeGeneration:0;if(row._mercenaryVehicleExplosionFatal===true){m.hp=0;m.dead=true;m.deathConfirmed=true;m.deadAt=Number(row.deadAt)||0;m._lifeState='dead';m._mercenaryVehicleExplosionFatal=true;}records.set(row.id,rec);const index=npcs().findIndex(n=>String(n.id)===String(row.sourceBotId));if(index>=0){recruitedResidents.set(row.id,npcs()[index]);npcs().splice(index,1);}if(typeof _hiredBotIds!=='undefined')_hiredBotIds.add(String(row.sourceBotId));}
 ...
 function qaPlacePatient({position}={}){
  ...
  vehicleExplosionOwner?.beginNewLife(m.id);m.r=point.r;m.c=point.c;m.hp=0;m.dead=false;m._mercenaryDownAt=performance.now();m._mercenaryQaRescueUntil=now()+600;m._mercenaryHospital=false;m._mercenaryMove=null;m._followSpeed=0;m._threatUntil=0;m._mercenaryDefending=false;persist();
