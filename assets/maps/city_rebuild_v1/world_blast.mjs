...
export function applyWorldBlast(T,{point,radius,power=120,roots=[],glass,onSurfaceHit,maxMeshes=96,maxPanels=96,maxSurfaceHits=24}={}){
 const stats={broken:0,surfaceHits:0,candidates:0,panelsTested:0,truncated:false};
 ...
 const origin=new T.Vector3(point.x,point.y,point.z),candidates=[],seen=new Set(),matrix=new T.Matrix4(),local=new T.Matrix4(),bounds=new T.Box3();
 for(const root of Array.isArray(roots)?roots:[roots]){
  root?.updateWorldMatrix?.(true,true);root?.traverse?.(mesh=>{
   ...
   for(let id=0;id<(mesh.isInstancedMesh?mesh.count:1);id++){
    ...
    candidates.push({mesh,instanceId:mesh.isInstancedMesh?id:undefined,matrix:matrix.clone(),bounds:bounds.clone(),distance,breakable});
   }
  });
 }
 candidates.sort((a,b)=>a.distance-b.distance);stats.candidates=candidates.length;if(candidates.length>maxMeshes)stats.truncated=true;const selected=candidates.slice(0,maxMeshes);
