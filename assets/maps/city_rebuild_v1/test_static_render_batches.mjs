...
for(const multiDraw of [false,true]){
 ...
 changed.layers.set(1);assert.equal(guardBatches.update().visible,2);changed.position.x=7;changed.layers.set(0);assert.equal(guardBatches.update().visible,2,'a source changed while failed open stays exact until explicit validation');guardBatches.setOptimizationEnabled(false);guardBatches.setOptimizationEnabled(true);assert.equal(guardBatches.stats().visible,2,'architecture toggle rejects a stale copied matrix after a fail-open mutation');assert.equal(changed.material,guardMaterial,'the mutated source remains rendered exactly');
 const disabledMutation=guardGroups[1].children[0];guardBatches.setOptimizationEnabled(false);disabledMutation.position.x=9;guardBatches.setOptimizationEnabled(true);assert.equal(guardBatches.stats().visible,1,'architecture toggle validates runtime mutations made while disabled');assert.equal(disabledMutation.material,guardMaterial,'runtime-mutated source remains rendered exactly after re-enable');
 const materialMutation=guardGroups[2].children[0],replacementMaterial=new T.MeshStandardMaterial({color:'#bb3311'});materialMutation.material=replacementMaterial;assert.equal(guardBatches.update().visible,0,'external material ownership immediately fails open');materialMutation.position.x=11;materialMutation.material=guardMaterial;assert.equal(guardBatches.update().visible,0,'returning the original material does not bypass validation');guardBatches.setOptimizationEnabled(false);guardBatches.setOptimizationEnabled(true);assert.equal(guardBatches.stats().visible,0,'toggle rejects a stale copied matrix after material ownership returns');assert.equal(materialMutation.material,guardMaterial);replacementMaterial.dispose();
