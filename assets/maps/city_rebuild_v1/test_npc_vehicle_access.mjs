...
// Threat cancellation reverses the current boarding fold instead of snapping
// to a fully seated exit pose on its first frame.
npc.r=outside.r;npc.c=outside.c;npc._threat=false;const cancelled={car,npc,carId:'car1',phase:'approach',door:outside,access:{outside,seat},index:0,plan:{goal:{door:{r:9,c:10}},lots:[]}};api.set(cancelled);api.tick(npc,.05,now);assert.equal(cancelled.phase,'board');
for(let i=0;i<2;i++){now+=50;api.tick(npc,.05,now);}const boardingFold=cancelled.progress,startDistance=Math.hypot(npc.r-outside.r,npc.c-outside.c);assert(boardingFold>0&&boardingFold<1);npc._threat=true;now+=50;api.tick(npc,.05,now);assert.equal(cancelled.phase,'exit');const physical=1-Math.min(1,Math.hypot(npc.r-outside.r,npc.c-outside.c)/(startDistance||.85)),expectedProgress=1-boardingFold+boardingFold*physical;assert(Math.abs(cancelled.progress-expectedProgress)<1e-12,'cancelled exit preserves current boarding fold while reversing physically');
for(let i=0;i<100&&api.trip();i++){now+=50;api.tick(npc,.05,now);}assert.equal(api.trip(),null);assert.equal(npc.r,outside.r);npc._threat=false;
