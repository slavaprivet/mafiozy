...
export function createVehicleRollover(T,car){
 const halfWidth=1.28,halfHeight=1.08,inertia=(halfWidth**2+halfHeight**2)/3;
 ...
 function impact(contact,yaw=car.object.rotation.y){
  ...
  const speed=contact.impactSpeed,lever=.75;
 ...
 return{impact,update,reset,get unstable(){return active||Math.abs(Math.sin(angle/2))>.08},stats:()=>({angle,angularVelocity:omega,active,overturned:Math.cos(angle)<.5,lastContact})};
}
