...
const currentHelper=fs.readFileSync(new URL('./npc_native_directed_route_source.js',import.meta.url),'utf8');
const oldHelper=currentHelper.replace(/const heuristic=\(r,c\)=>\{[^\n]+?\};/, 'const heuristic=(r,c)=>Math.max(0,Math.hypot(r+.5-goalR,c+.5-goalC)-goalRadius);');assert.notEqual(oldHelper,currentHelper);
vm.runInContext(oldHelper,b);
...
const results={};
for(const [label,code]of [['euclidean',candidate],['cardinal',candidate]]){
 if(label==='cardinal')vm.runInContext(currentHelper,b);
...
fs.writeFileSync(new URL('../../../outputs/npc_cardinal_route_queue_20260919.json',import.meta.url),JSON.stringify(results,null,2));
assert.equal(results.cardinal.pending,0);assert(results.cardinal.reached>=results.euclidean.reached,'at least equal route completion');
assert(results.cardinal.nativeQueries<results.euclidean.nativeQueries*.9,'fewer exact native queries at equal reachability');
