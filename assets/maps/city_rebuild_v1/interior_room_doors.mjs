...
export function createInteriorRoomDoors(T,{root,entry,metresPerCell=4.1}){
 ...
 function create({x,z,y,axis='x',width=1.3,inward=1,name='Комната',color='#665043'}){
  if(disposed)throw Error('Interior doors disposed');if(!Number.isFinite(width)||width<1.2||width>2.4)throw Error('Invalid interior door width');const half=(width-.1)/2,leafEnd=width-.11;if(![x,y,z].every(Number.isFinite)||!['x','z'].includes(axis)||![1,-1].includes(inward))throw Error('Invalid interior door');
  ...
  const pivot=new T.Group();pivot.position.x=-half;hinge.add(pivot);root.add(hinge);
  ...
  const d={hinge,pivot,x,z,y,axis,width,half,leafEnd,inward,name,fraction:0,target:0,baseYaw,parts:[]};doors.push(d);const identity=new T.Quaternion();
  ...
  add([width-.12,2.06,.065],[half,1.04,0],material);
  for(const yy of[.58,1.49])for(const side of[-1,1])add([width-.38,.7,.022],[half,yy,side*.043],material);
  for(const side of[-1,1]){add([.045,.19,.025],[leafEnd-.14,1.04,side*.062],brass);add([.13,.036,.045],[leafEnd-.185,1.08,side*.084],brass)}
 ...
 function near(point){const x=point.x,y=point.y,z=point.z;if(!doors.length)return null;const p=root.worldToLocal(new T.Vector3(x,y,z));let best=null;for(const d of doors){const distance=Math.hypot(p.x-d.x,p.z-d.z);if(Math.abs(p.y-d.y)<.65&&distance<1.65&&(!best||distance<best.distance))best={door:d,distance,anchor:root.localToWorld(new T.Vector3(d.x,d.y+1.65,d.z)),action:(d.target?'Закрыть · ':'Открыть · ')+d.name,opening:!!d.target,fraction:d.fraction,instanceId:entry.instance.id}}return best}
 ...
 function sweepContains(d,point,from,to){if(!point)return false;const p=d.hinge.worldToLocal(new T.Vector3(point.x,point.y,point.z));if(p.y>=2.08||p.y+1.9<=0)return false;
  ...
  for(let i=0;i<=steps;i++){const t=a+(b-a)*i/steps,c=Math.cos(t),s=Math.sin(t),x=c*(p.x+d.half)-s*p.z,z=s*(p.x+d.half)+c*p.z,dx=Math.max(.01-x,0,x-d.leafEnd),dz=Math.max(0,Math.abs(z)-.11);if(Math.hypot(dx,dz)<.36)return true;}return false;
 }
 ...
 function bodies(){if(bodiesCache.version===version)return bodiesCache.value;bodiesCache.version=version;bodiesCache.value=doors.map(d=>{const points=[[.01,-.05],[d.leafEnd,-.05],[d.leafEnd,.05],[.01,.05]].map(([x,z])=>d.pivot.localToWorld(new T.Vector3(x,0,z)));return{polygonCR:points.map(p=>[p.x/metresPerCell,p.z/metresPerCell]),minYM:points[0].y,maxYM:points[0].y+2.08,buildingEntryId:entry.instance.id,movingDoor:true,interiorDoor:true}});return bodiesCache.value}
 return{create,finalize,near,interact,update,bodies,doors,get active(){return active},stats:()=>({doors:doors.length,draws:batchMeshes.length,active,idleUpdates}),dispose(){if(disposed)return;disposed=true;for(const mesh of batchMeshes){mesh.removeFromParent();mesh.dispose();}batchMeshes.length=0;for(const d of doors)d.hinge.removeFromParent();doors.length=0;version++;geo.dispose();brass.dispose();for(const m of materials.values())m.dispose();materials.clear();partBuckets.clear()}};
}
