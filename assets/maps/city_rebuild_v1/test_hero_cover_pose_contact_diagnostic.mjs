let auditPrint=0;const auditFailures=new Map();function report(message){const key=message.replace(/[0-9]+(?:\.[0-9]+)?/g,'N');auditFailures.set(key,(auditFailures.get(key)||0)+1);if(auditPrint++<10)console.log('AUDIT_FAILURE '+message);}function auditOk(...a){try{assert.ok(...a)}catch(e){report(e.message)}}function auditEqual(...a){try{assert.equal(...a)}catch(e){report(e.message)}}
...
for(const model of ['player_male.8130dfb1f7eb.glb','player_female.298d50e6244a.glb']){
  ...
  function skin(){
    ...
    for(const mesh of meshes){mesh.skeleton.update();for(let i=0;i<mesh.geometry.attributes.position.count;i++){
      ...
      auditOk([v.x,v.y,v.z].every(Number.isFinite));minFloor=Math.min(minFloor,v.y-hero.object.position.y);out.push(v.x,v.y,v.z);
    }}return out;
  }
  ...
  for(const id of ['tt_pistol','uzi','ak74','rpg']){
    ...
    for(const low of [false,true])for(const yaw of [0,.71,-2])for(const side of [-1,1])for(const mode of ['hidden','blind','blocked','aimed']){
      ...
      for(const blend of [0,.125,.25,.375,.5,.625,.75,.875,1]){
        ...
        auditEqual(output.applied,true);auditOk(hero.object.position.distanceTo(root)<1e-9);auditOk(hero.object.quaternion.angleTo(q)<1e-7);
        for(const [i,name]of ['foot_l','foot_r'].entries())auditOk(c.worldPosition(name).distanceTo(feet[i])<1e-8,'support feet unchanged');
        for(const [name,bone]of Object.entries(c.bones)){
          ...
          auditOk(p.distanceTo(baseBones[name].p)<1e-8&&s.distanceTo(baseBones[name].s)<1e-6,`bones cannot stretch or move: ${name}`);
        }
        const grips=[['r',[0,-.13,-.02]],...(weapon.userData.twoHanded?[['l',weapon.userData.supportGrip]]:[])];
        for(const [s,point]of grips){const error=c.worldPosition('socket_hand_'+s).distanceTo(weapon.localToWorld(new THREE.Vector3(...point)));maxGripError=Math.max(maxGripError,error);auditOk(error<.002,`${model} ${id} ${mode} ${blend} ${s} grip ${error}`);}
        const current=skin();
        if(previous){let step=0;for(let i=0;i<current.length;i+=3)step=Math.max(step,Math.hypot(current[i]-previous[i],current[i+1]-previous[i+1],current[i+2]-previous[i+2]));maxStep=Math.max(maxStep,step);auditOk(step<.18,`${model} low=${low} yaw=${yaw} side=${side} mode=${mode} id=${id} blend=${blend} skinStep=${step}`);}
    ...
    auditOk(over.requestedReachable,`${model} ${id} blind over-cover reach: ${over.reachCorrection}`);
    auditOk(Math.abs(c.visualPivot.rotation.y)<1e-8,'concealed torso keeps back against wall');
    const actualMuzzle=weapon.localToWorld(new THREE.Vector3(...weapon.userData.muzzle));
    auditOk(actualMuzzle.y>1.25,'real muzzle remains over the solid cover top');
    for(const [s,p]of [['r',[0,-.13,-.02]],...(weapon.userData.twoHanded?[['l',weapon.userData.supportGrip]]:[])]){
      auditOk(c.worldPosition('socket_hand_'+s).distanceTo(weapon.localToWorld(new THREE.Vector3(...p)))<.002,'raised gun retains both authored grips');
    }
    skin();
    ...
    auditEqual(far.requestedReachable,false,'unreachable muzzle cannot authorize firing');auditOk(far.reachCorrection>100);
    for(const fps of []){
      ...
      for(const mode of ['hidden','blind','aimed','blind','hidden']){
        for(let i=0;i<=Math.ceil(fps*.20);i++){
          ...
          if(i===0)auditEqual(output.transitionReady,false,'new mode waits for transition');
          if(i/fps>=.16)auditEqual(output.transitionReady,true,'transition finishes within 160 ms');
          if(previous){const step=origin.distanceTo(previous);maxTransitionGunStep=Math.max(maxTransitionGunStep,step);auditOk(step<.5*30/fps,`weapon mode transition step ${id} ${fps} ${mode} ${i}: ${step}`);if(i===0)auditOk(step<.015,'mode change starts at previous gun position');}
          previous=origin;
          for(const [s,p]of [['r',[0,-.13,-.02]],...(weapon.userData.twoHanded?[['l',weapon.userData.supportGrip]]:[])])auditOk(c.worldPosition('socket_hand_'+s).distanceTo(weapon.localToWorld(new THREE.Vector3(...p)))<.003,`${id} transition grips remain exact`);
          ...
          auditEqual(repeated.transitionReady,output.transitionReady,'zero dt does not advance readiness');
          auditOk(weapon.getWorldPosition(new THREE.Vector3()).distanceTo(before)<1e-6,'zero dt render pass is idempotent');
          transitionFrames++;
        }
      }
      const restarted=applyCoverPose(THREE,c,{mode:'hidden',low:true,blend:1,weapon,attachmentId:attachmentId+'/new',dt:0});
      auditEqual(restarted.transitionReady,false,'new attachment has independent transition');
    }
  }
}
auditOk(maxCrouchSkinY<=HERO_POSTURES.crouch.height,`hidden crouch fits its capsule: ${maxCrouchSkinY}`);
auditOk(maxBaseCrouchSkinY<=HERO_POSTURES.crouch.height,`ordinary C crouch fits its capsule: ${maxBaseCrouchSkinY}`);
auditOk(maxBlindHeadY<=1.1,`blind-fire head stays behind 1.1 m cover: ${maxBlindHeadY}`);
auditOk(minFloor>-.012,`skin cannot sink under floor: ${minFloor}`);
auditEqual(applyCoverPose(THREE,null,{}).applied,false);
...
console.log('AUDIT_FAILURE_COUNTS '+JSON.stringify([...auditFailures]));
