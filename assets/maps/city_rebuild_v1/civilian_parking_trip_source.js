...
function _nativeParkingAdmissionTick(now){
 ...
 while(state.queue.length&&checks<2&&iterations++<16&&performance.now()<deadline){
  ...
  if(!occupied){
   ...
   checks++;state.checked++;const result=_walkTrafficNavigationResolver({carId:_threeVehicleEntityId(car),from:pose,to:pose,halfLength:shape.halfLength,halfWidth:shape.halfWidth,roadsOnly:false,lotId:slot.lotId,parkingSlotId:slot.id});
...
function _civilianTripParkingCurve(car,start,slot){
 ...
 let previous=start;for(const point of points){if(!_civilianTripSweep(car,previous,point,[],false,{lotId:slot.lotId,parkingSlotId:slot.id}))return null;previous=point;}return points;
}
...
function _civilianTripSweep(car,a,b,lots,dynamic=true,authority=null){
 if(_civilianTripNative()){
  const trip=_civilianTripForCar(car),slot=trip?.plan?.goal?.slot||car._civilianNativePlan?.parkingSlot,result=typeof _walkTrafficNavigationResolver==='function'?_walkTrafficNavigationResolver({carId:_threeVehicleEntityId(car),from:a,to:b,roadsOnly:false,accessRouteIds:authority?.accessRouteIds||trip?.plan?.accessRouteIds||[],accessLotIds:authority?.accessLotIds||trip?.plan?.accessLotIds||[],lotId:authority?.lotId||slot?.lotId,parkingSlotId:authority?.parkingSlotId||slot?.id,laneRouteToken:authority?.laneRouteToken}):null;car._civilianBlockReason=result?.clear===true?'':result?.reason||'navigation-not-ready';car._civilianBlockerId=result?.clear===true?'':result?.blockerId||'';if(result?.clear!==true)return false;
...
function _civilianTripTickCar(car,dt){
 ...
 if(!_civilianTripSweep(car,current,next,t.plan.lots,true,{laneRouteToken:t.plan.laneRouteToken})){car.vr=car.vc=0;car.braking=true;t.driveSpeed=Math.max(0,t.driveSpeed-brake*Math.min(.1,dt));const now=performance.now();if(t.plan.native){t.blockedAt=t.blockedAt||now;if(now-t.blockedAt>(['vehicle','pedestrian'].includes(car._civilianBlockReason)?12000:1500)){const slot=t.plan.goal?.slot;if(slot&&_nativeParkingAdmission.reservations.get(slot.id)===car)_nativeParkingAdmission.reservations.delete(slot.id);t.destinationDoor=t.plan.goal.door;_civilianTripCancelLane(car._civilianNativePlan);delete car._civilianNativePlan;t.phase='planning';t.blockedAt=0;}}return true;}t.blockedAt=0;
