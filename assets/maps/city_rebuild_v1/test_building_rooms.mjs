...
for(const assetId of Object.keys(BUILDING_ROOM_PROFILES)){
 ...
 for(const item of items){
  ...
  const firstBodies=entry.getCollisionBodies();assert.equal(entry.getCollisionBodies(),firstBodies,'cache stable at rest');const approach=entry.approachPoint(),sample=entry.sampleBounds,nearX=Math.max(sample.min.x-approach.x,0,approach.x-sample.max.x),nearZ=Math.max(sample.min.z-approach.z,0,approach.z-sample.max.z);assert.ok(nearX*nearX+nearZ*nearZ<=9,'3 m interaction broadphase reaches every authored entrance');assert.equal(entry.interact(approach).accepted,true);
