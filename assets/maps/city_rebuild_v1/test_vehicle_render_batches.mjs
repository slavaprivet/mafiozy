...
for(const [includeDoors,includeBody]of [[false,false],[true,false],[true,true]])for(const profile of ARTIST_VEHICLE_PROFILES){
 ...
 const helper=createVehicleRenderBatches({THREE:T,root:car.object,includeDoors,includeBody}),batches=[];car.object.traverse(n=>{if(n.userData.vehicleRenderBatch)batches.push(n)});
 ...
 for(const pose of [{x:0,y:0,z:0,yaw:0},{x:84,y:2.4,z:-19,yaw:1.17}]){
  ...
  for(const batch of batches){
   assert.ok(batch.parent===car.interior.object||includeDoors&&[...car.doors.values()].includes(batch.parent)||includeBody&&batch.parent===car.object,'batch remains within moving owner');assert.equal(batch.raycast(),undefined);
...
for(const kind of ['crash','explosion']){
 ...
 const damage=createVehicleDamage(T,car,{scene}),helper=createVehicleRenderBatches({THREE:T,root:car.object,includeDoors:true,includeBody:true}),door=car.doors.get('front_left');car.setDoorById(.6,'front_left');helper.update();
