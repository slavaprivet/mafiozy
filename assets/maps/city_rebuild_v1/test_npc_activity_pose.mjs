...
for(const sex of ['male','female'])for(const height of [1.65,2.05]){
 ...
 tick(1.6,{seat,phoneCalling:true});assert(c.bones.hand_r.getObjectByName('NPC_Phone').visible,'phone actual call visible');
 tick(1.65,{seat,state:'calling'});assert(!c.bones.hand_r.getObjectByName('NPC_Phone').visible,'state label alone no phone');
 for(let i=0;i<11;i++)tick(1.7+i*.05,{});assert(c.visualPivot.position.length()<.001,'getup clears visual offset');
 ...
 tick(2.3,{phoneCalling:true,cowering:true});assert(!c.bones.hand_r.getObjectByName('NPC_Phone').visible,'fear hides phone');
 tick(2.4,{phoneCalling:true},{stun:{active:true,age:1}});assert(!c.bones.hand_r.getObjectByName('NPC_Phone').visible,'stun hides phone');
 a.dispose();
}
