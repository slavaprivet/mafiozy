...
WORLD_VIEW_R   = 50   # тайлы, в радиусе которых шлём остальных игроков
                      ...
                      # весь город даже на больших картах)
...
def _world_is_wall(r: int, c: int) -> bool:
    ...
    if rm <= 3 or cm <= 3:
        return False
    # Бордюр — mod ∈ {4, BLOCK-1}
    if rm == 4 or rm == BLOCK - 1 or cm == 4 or cm == BLOCK - 1:
        return False
...
class WorldSim:
    ...
    # Координаты совпадают с клиентским MICHAEL_POS в world.html (r=183, c=42).
    # Это нос корабля, лицом к пирсу — игрок подходит сюда чтобы поговорить.
    MICHAEL_X    = 42.0
    MICHAEL_Y    = 183.0
    ...
    # Серверные beachgoers ОТКЛЮЧЕНЫ (0): пляж теперь заселяют обычные
    # клиентские NPC (NPCS) в купальниках — они ходят и стреляемы, как в
    # городе (единая система). См. spawnNpc/npcPassable в world.html.
    BEACHGOER_MAX           = 0       # одновременно на пляже (0 = выкл)
    ...
    def add_or_update(self, uid: str, name: str, look: dict,
        ...
        # попадать в радиус рендера VIS_R=12 на клиенте и видеть друг друга.
        # Раньше я раскидывал спавн по 8 точкам периметра — но если хост
        # в (24,24), а друг в (40,40), между ними 22 тайла, и в видимой
        # области друга не было. Создавалось ощущение «друг застрял».
        if not self.players:
            sy, sx = 32.0, 32.0
        else:
            # Выбираем «опорного» игрока — чаще всего это хост (первый
            # появившийся, MIN по last_seen-старту). Если их много —
            # тот, кто прислал последний инпут (актуально активный).
            anchor = max(self.players.values(),
                         key=lambda p: p.get('_input_t', 0) or p.get('last_seen', 0))
            ax, ay = anchor['x'], anchor['y']
            # Пробуем 8 точек на 3-тайловом «кресте» вокруг — все
            # детерминированно на дорожной сетке (col%8 в {0,1}).
            ...
            for (dy, dx) in offsets:
                ny = ay + dy; nx = ax + dx
                # Внутри карты + не на той же клетке что чужой игрок
                if 2 <= nx <= WORLD_MAP_COLS - 2 and 2 <= ny <= WORLD_MAP_ROWS - 2:
                    too_close = False
                    for p in self.players.values():
                        if (p['x']-nx)**2 + (p['y']-ny)**2 < 1.0:
                            too_close = True; break
                    if not too_close:
                        sy, sx = ny, nx
                        break
            # Если все слоты заняты — садимся в +1.5 диагонально (всё равно
            # рядом, чуть-чуть может задеть, но в радиусе видимости).
            else:
                sy = ay + 1.5; sx = ax + 1.5
        ...
        # Выбираем случайную из JAIL_SPAWNS — чтобы несколько сидящих
        # не наслаивались в одну клетку.
    ...
    def tick_cops(self, dt: float) -> list:
        ...
        # Бой-копы (2★/3★) копируют поведение боссов demo_isometric: держим
        ...
        for cop in list(self.cops):
            ...
            # ПАТРУЛЬ (1★): подходит на разговорное расстояние и предупреждает
            if is_patrol:
                desired = 3.0
                speed   = self.COP_PATROL_SPEED
                if dist > desired + 0.2:
                    step = speed * dt
                ...
                continue
            # БОЙ (2★/3★): boss-like AI со стрейфом
            ...
            # Стрельба: SWAT быстрее и больнее
        ...
        if action == 'battle_won':
            ...
            return web.json_response({
                ...
                'state': {
                    ...
                    'cash':  updated['cash'],
                    'exp':   updated['exp'],
                    'exp_need': exp_for_level(int(updated.get('level', 1) or 1)),
